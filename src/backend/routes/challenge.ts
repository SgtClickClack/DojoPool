...
>>>>>>> be60d962e0a003c332590651959395f4b8c61239
  });
...
>>>>>>> be60d962e0a003c332590651959395f4b8c61239
   ...
   * Create a new challenge (API endpoint for ChallengeService)
   */
  const leaderboardService = new VenueLeaderboardService();

  router.post('/challenge/create', async (req: Request, res: Response) => {
    try {
      const { defenderId, dojoId, type } = req.body;
      const challengerId = req.user?.id || 'anonymous'; // TODO: Get from auth
      ...
      if (type === 'pilgrimage') {
        // Must have defeated at least 2 Top Ten players of the target dojo
        const leaderboard = leaderboardService.getLeaderboard().find(l => l.venueId === dojoId);
        if (!leaderboard) {
          return res.status(404).json({
            success: false,
            error: 'Dojo leaderboard not found'
          });
        }
        // Simulate: Assume we have a function getDefeatedTopTenCount(challengerId, dojoId)
        // For now, just check if challenger is in topPlayers and has at least 2 wins
        const topTen = leaderboard.topPlayers;
        const defeatedCount = topTen.filter(p => p.playerId !== challengerId && p.matchesWon >= 1).length;
        if (defeatedCount < 2) {
          return res.status(403).json({
            success: false,
            error: 'You have not met the requirements to issue this challenge. Defeat at least 2 Top Ten players of this dojo.'
          });
        }
      }
      if (type === 'gauntlet') {
        // Must have defeated all players ranked below the Dojo Master
        const leaderboard = leaderboardService.getLeaderboard().find(l => l.venueId === dojoId);
        if (!leaderboard) {
          return res.status(404).json({
            success: false,
        ...
        const dojoMaster = leaderboard.dojoMaster;
        if (!dojoMaster) {
          return res.status(403).json({
            success: false,
            error: 'No Dojo Master found for this dojo.'
          });
        }
        // Simulate: Assume we have a function hasDefeatedAllBelowMaster(challengerId, dojoId)
        // For now, just check if challenger is in topPlayers and has more wins than all below master
        const topTen = leaderboard.topPlayers;
        const masterIndex = topTen.findIndex(p => p.playerId === dojoMaster.masterId);
        if (masterIndex === -1) {
          return res.status(403).json({
            success: false,
            error: 'Dojo Master not found in leaderboard.'
          });
        }
        const belowMaster = topTen.slice(masterIndex + 1);
        const hasDefeatedAll = belowMaster.every(p => p.matchesWon >= 1);
        if (!hasDefeatedAll) {
          return res.status(403).json({
            success: false,
            error: 'You have not met the requirements to issue this challenge. Defeat all players ranked below the Dojo Master.'
          });
        }
      }
      // --- End Challenge Requirements Validation ---
      ...
      // --- WebSocket Notification: New Challenge ---
      const defenderSocketId = userSocketMap.get(defenderId);
      if (defenderSocketId) io.to(defenderSocketId).emit('new_challenge', challenge);
      const challengerSocketId = userSocketMap.get(challengerId);
      if (challengerSocketId) io.to(challengerSocketId).emit('new_challenge', challenge);
  ...
  router.post('/challenge/:id/respond', async (req: Request, res: Response) => {
    try {
      ...
      // --- WebSocket Notification: Challenge Response ---
      const challengerSocketId = userSocketMap.get(challenge.challengerId);
      if (challengerSocketId) io.to(challengerSocketId).emit('challenge_response', challenge);
