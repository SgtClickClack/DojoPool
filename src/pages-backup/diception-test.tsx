...
const DiceptionTest: React.FC = () => {
  ...
  const fetchStatus = async () => {
    try {
      const response = await fetch(
        'http://localhost:3002/api/diception/status'
      );
  ...
  const startTracking = async () => {
    try {
      const response = await fetch(
        'http://localhost:3002/api/diception/start',
        {
          method: 'POST',
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8 p-6 rounded-lg bg-gray-800">
          ...
          <div className="flex items-center space-x-4">
            ...
            <span className={isConnected ? 'text-green-400' : 'text-red-400'}>
              {isConnected
                ? 'Connected to Diception AI Server'
                : 'Disconnected'}
            </span>
          ...
          <div className="mb-8 p-6 rounded-lg bg-gray-800">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p>
                  <strong>Detection Method:</strong> {status.detection_method}
                </p>
                <p>
        ...
        {demoData && (
          <div className="mb-8 p-6 rounded-lg bg-gray-800">
            <h2 className="text-2xl font-semibold mb-4">
              Demo Ball Detection Data
            </h2>
            <div className="space-y-4">
              <p>
                <strong>Timestamp:</strong> {demoData.timestamp}
              </p>
              <p>
                <strong>Detection Mode:</strong> {demoData.detection}
              </p>
              <div>
                <p>
                  <strong>Detected Balls:</strong>
                </p>
                {demoData.balls.map((ball, index) => (
                  <div
                    key={index}
                    className="ml-4 p-3 bg-gray-700 rounded mt-2"
                  >
                    <p>
                      Ball {ball.id}: Position ({ball.position[0]},{' '}
                      {ball.position[1]})
                    </p>
                    <p>
                      Radius: {ball.radius}px, Confidence: {ball.confidence}%
                    </p>
                  </div>
                ))}
              </div>
        ...
        <div className="p-6 rounded-lg bg-gray-800">
          ...
          <div className="space-y-2 text-sm">
            <p>
              <strong>GET</strong> /health - Server health check
            </p>
            <p>
              <strong>GET</strong> /api/diception/status - System status
            </p>
            <p>
              <strong>GET</strong> /api/diception/demo - Demo detection data
            </p>
            <p>
              <strong>GET</strong> /api/diception/live - Live camera detection
            </p>
            <p>
              <strong>POST</strong> /api/diception/start - Start camera tracking
            </p>
            <p>
              <strong>POST</strong> /api/diception/stop - Stop camera tracking
            </p>
            <p>
              <strong>GET</strong> /api/diception/match_state - Complete match
              state
            </p>
          </div>
        </div>
        ...
        <div className="mt-8 p-6 rounded-lg bg-blue-900 border border-blue-700">
          ...
          <ol className="list-decimal list-inside space-y-1 text-sm">
            ...
            <li>Click "Refresh Connection" to check server status</li>
            <li>Use "Start Tracking" to begin ball detection</li>
            <li>Monitor the demo data for real-time ball positions</li>
            <li>Use "Stop Tracking" to end the session</li>
          </ol>
        </div>
      </div>
    </div>
