...
import { MonitoringConfig, AlertThreshold } from './config';
...
export declare interface AlertManager {
  on<K extends keyof AlertManagerEvents>(event: K, listener: AlertManagerEvents[K]): this;
  emit<K extends keyof AlertManagerEvents>(event: K, ...args: Parameters<AlertManagerEvents[K]>): boolean;
}
...
export class AlertManager extends EventEmitter {
  ...
  public checkPerformanceMetrics(metrics: PerformanceMetrics): void {
    ...
    if (metrics.queueLength > this.config.performanceThresholds.maxQueueLength) {
      this.createAlert({
        id: 'queue-length',
        type: 'Queue length',
        severity: 'warning',
  ...
  public checkNodeMetrics(metrics: NodeMetrics): void {
    ...
    if (metrics.pendingOperations > this.config.nodeThresholds.maxPendingOperations) {
      this.createAlert({
