...
import { GameState, Shot, Foul, GameStatus } from '../types/game';
...
export const useGameState = ({ gameId, playerId }: UseGameStateProps) => {
  ...
  const reportFoul = useCallback(async (foul: Foul) => {
    try {
  ...
  return {
    ...
    canTakeShot: gameState?.gameStatus === GameStatus.ACTIVE && gameState?.currentPlayer === playerId,
  };
}; 