...
import { realTimeAICommentaryService, AICommentaryEvent, PoolGod } from '../services/ai/RealTimeAICommentaryService';
...
export const useAICommentary = (matchId?: string): UseAICommentaryReturn => {
  ...
  const [currentMatchEvents, setCurrentMatchEvents] = useState<AICommentaryEvent[]>([]);
  ...
  useEffect(() => {
    ...
    realTimeAICommentaryService.on('commentaryGenerated', handleCommentaryGenerated);
    ...
    return () => {
      ...
      realTimeAICommentaryService.off('commentaryGenerated', handleCommentaryGenerated);
    };
  }, []);
  ...
  const joinMatch = useCallback((newMatchId: string) => {
    ...
    const matchEvents = realTimeAICommentaryService.getEventsByMatch(newMatchId);
    setCurrentMatchEvents(matchEvents);
  }, []);
  ...
  const onCommentaryGenerated = useCallback((callback: (event: AICommentaryEvent) => void) => {
