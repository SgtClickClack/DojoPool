...
interface UseAdvancedAnalyticsReturn {
  ...
  getTournamentPrediction: (tournamentId: string) => Promise<TournamentPrediction | null>;
...
export const useAdvancedAnalytics = (): UseAdvancedAnalyticsReturn => {
  ...
  const fetchAnalytics = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/advanced-analytics/dashboard`);
      if (!response.ok) {
  ...
  const getPlayerInsight = useCallback(async (playerId: string): Promise<PlayerInsights | null> => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/advanced-analytics/player-insight/${playerId}`);
      if (!response.ok) {
  ...
  const getVenueAnalytic = useCallback(async (venueId: string): Promise<VenueAnalytics | null> => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/advanced-analytics/venue-analytic/${venueId}`);
      if (!response.ok) {
        if (response.status === 404) return null;
  ...
  const getTournamentPrediction = useCallback(async (tournamentId: string): Promise<TournamentPrediction | null> => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/advanced-analytics/tournament-prediction/${tournamentId}`);
      if (!response.ok) {
        if (response.status === 404) return null;
  ...
  const getVenueOptimization = useCallback(async (venueId: string): Promise<VenueOptimization | null> => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/advanced-analytics/venue-optimization/${venueId}`);
      if (!response.ok) {
        if (response.status === 404) return null;
  ...
  const getRevenueForecast = useCallback(async (venueId: string): Promise<RevenueForecast | null> => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/advanced-analytics/revenue-forecast/${venueId}`);
      if (!response.ok) {
        if (response.status === 404) return null;
