...
interface UseAdvancedAIRefereeRuleEnforcementReturn {
  ...
  // Violation methods
  detectViolation: (violationData: Omit<RuleViolation, 'id' | 'timestamp' | 'status'>) => Promise<RuleViolation>;
  getViolations: (matchId?: string) => RuleViolation[];
  appealDecision: (violationId: string, appealReason: string) => Promise<boolean>;
  ...
  analyzeStrategy: (analysisData: Omit<StrategyAnalysis, 'id' | 'timestamp'>) => Promise<StrategyAnalysis>;
  getStrategyAnalyses: (matchId?: string) => StrategyAnalysis[];
  
  // Performance assessment methods
  assessPerformance: (assessmentData: Omit<PerformanceAssessment, 'id' | 'timestamp'>) => Promise<PerformanceAssessment>;
...
export const useAdvancedAIRefereeRuleEnforcement = (): UseAdvancedAIRefereeRuleEnforcementReturn => {
  const [isConnected, setIsConnected] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  
  const [violations, setViolations] = useState<RuleViolation[]>([]);
  const [ruleInterpretations, setRuleInterpretations] = useState<RuleInterpretation[]>([]);
  const [strategyAnalyses, setStrategyAnalyses] = useState<StrategyAnalysis[]>([]);
  ...
  const serviceRef = useRef<AdvancedAIRefereeRuleEnforcementService | null>(null);
  ...
  useEffect(() => {
    const initializeService = () => {
      try {
        ...
        service.on('ruleInterpreted', (interpretation: RuleInterpretation) => {
        ...
        service.on('performanceAssessed', (assessment: PerformanceAssessment) => {
        ...
        setError(err instanceof Error ? err.message : 'Failed to initialize AI Referee service');
      }
    };
  ...
  // Violation methods
  const detectViolation = useCallback(async (violationData: Omit<RuleViolation, 'id' | 'timestamp' | 'status'>): Promise<RuleViolation> => {
    if (!serviceRef.current) {
    ...
    try {
      ...
      const violation = await serviceRef.current.detectViolation(violationData);
      return violation;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to detect violation';
  ...
  const appealDecision = useCallback(async (violationId: string, appealReason: string): Promise<boolean> => {
    if (!serviceRef.current) {
      throw new Error('Service not initialized');
    }
    ...
    try {
      ...
      const success = await serviceRef.current.appealDecision(violationId, appealReason);
      return success;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to appeal decision';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setIsLoading(false);
  ...
  // Rule interpretation methods
  const interpretRule = useCallback(async (ruleCode: string): Promise<RuleInterpretation> => {
    if (!serviceRef.current) {
      throw new Error('Service not initialized');
    }
    ...
    try {
      ...
      const interpretation = await serviceRef.current.interpretRule(ruleCode);
      return interpretation;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to interpret rule';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setIsLoading(false);
  ...
  // Strategy analysis methods
  const analyzeStrategy = useCallback(async (analysisData: Omit<StrategyAnalysis, 'id' | 'timestamp'>): Promise<StrategyAnalysis> => {
    if (!serviceRef.current) {
      throw new Error('Service not initialized');
    }
    ...
    try {
      ...
      const analysis = await serviceRef.current.analyzeStrategy(analysisData);
      return analysis;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to analyze strategy';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setIsLoading(false);
  ...
  const getStrategyAnalyses = useCallback((matchId?: string): StrategyAnalysis[] => {
    if (!serviceRef.current) return [];
    return serviceRef.current.getStrategyAnalyses(matchId);
  }, []);

  // Performance assessment methods
  const assessPerformance = useCallback(async (assessmentData: Omit<PerformanceAssessment, 'id' | 'timestamp'>): Promise<PerformanceAssessment> => {
    if (!serviceRef.current) {
      throw new Error('Service not initialized');
    }
    ...
    try {
      ...
      const assessment = await serviceRef.current.assessPerformance(assessmentData);
      return assessment;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to assess performance';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setIsLoading(false);
  ...
  const getPerformanceAssessments = useCallback((matchId?: string): PerformanceAssessment[] => {
    if (!serviceRef.current) return [];
    return serviceRef.current.getPerformanceAssessments(matchId);
  }, []);

  // Configuration methods
  const updateConfig = useCallback(async (newConfig: Partial<AIRefereeConfig>): Promise<void> => {
    if (!serviceRef.current) {
      throw new Error('Service not initialized');
    }
