...
export const useAdvancedPlayerAnalytics = () => {
  ...
  const updateConfig = useCallback(async (newConfig: Partial<PlayerAnalyticsConfig>) => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/advanced-player-analytics/config', {
        method: 'PUT',
  ...
  const getAllPerformances = useCallback(async () => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/advanced-player-analytics/performances');
      const data = await response.json();
  ...
  const getPlayerPerformance = useCallback(async (playerId: string) => {
    try {
      setIsLoading(true);
      const response = await fetch(`/api/advanced-player-analytics/performances/${playerId}`);
      const data = await response.json();
  ...
  const updatePlayerPerformance = useCallback(async (playerId: string, matchData: any) => {
    try {
      setIsLoading(true);
      const response = await fetch(`/api/advanced-player-analytics/performances/${playerId}`, {
        method: 'POST',
  ...
  const getTopPerformers = useCallback(async (limit: number = 10) => {
    try {
      setIsLoading(true);
      const response = await fetch(`/api/advanced-player-analytics/performances/top/${limit}`);
      const data = await response.json();
  ...
  const getMostImproved = useCallback(async (limit: number = 10) => {
    try {
      setIsLoading(true);
      const response = await fetch(`/api/advanced-player-analytics/performances/improved/${limit}`);
      const data = await response.json();
  ...
  // Performance Metrics
  const addPerformanceMetric = useCallback(async (metric: Omit<PerformanceMetric, 'id' | 'timestamp'>) => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/advanced-player-analytics/metrics', {
        method: 'POST',
  ...
  const getPerformanceMetrics = useCallback(async (playerId: string, metricType?: string) => {
    try {
      setIsLoading(true);
  ...
  // Skill Progression
  const updateSkillProgression = useCallback(async (
    playerId: string,
    skillArea: string,
    improvement: number,
    reason: string,
    ...
    try {
      setIsLoading(true);
      const response = await fetch(`/api/advanced-player-analytics/skills/${playerId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const analyzeMatch = useCallback(async (matchData: any) => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/advanced-player-analytics/matches/analyze', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ matchData })
      });
  ...
  const getMatchAnalyses = useCallback(async (playerId: string) => {
    try {
      setIsLoading(true);
      const response = await fetch(`/api/advanced-player-analytics/matches/${playerId}`);
      const data = await response.json();
  ...
  const generatePlayerInsights = useCallback(async (playerId: string) => {
    try {
      setIsLoading(true);
      const response = await fetch(`/api/advanced-player-analytics/insights/${playerId}`, {
        method: 'POST'
      });
      const data = await response.json();
  ...
  // Player Comparison
  const comparePlayers = useCallback(async (playerId: string, comparisonPlayerId: string) => {
    try {
      setIsLoading(true);
  ...
  const getPlayerComparisons = useCallback(async (playerId: string) => {
    try {
      setIsLoading(true);
      const response = await fetch(`/api/advanced-player-analytics/compare/${playerId}`);
      const data = await response.json();
  ...
  useEffect(() => {
    ...
    getTopPerformers();
    getMostImproved();
  }, [getConfig, getMetrics, getAllPerformances, getTopPerformers, getMostImproved]);
