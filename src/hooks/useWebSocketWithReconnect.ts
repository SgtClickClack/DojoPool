...
type WebSocketMessage = BaseWebSocketMessage | SubscriptionMessage | UnsubscribeMessage;
...
export function useWebSocketWithReconnect(options: WebSocketOptions) {
  ...
  }, [reconnectAttempt, reconnectConfig.initialDelay, reconnectConfig.maxDelay]);
  ...
  const send = useCallback((message: WebSocketMessage): UnsubscribeFunction | void => {
  ...
  const subscribe = useCallback((type: WebSocketMessageType, callback: (data: any) => void) => {
  ...
  useEffect(() => {
    const handleConnection = (data: { status: string }) => {
      ...
      if (isNowConnected) {
        ...
        clearReconnectTimeout();
      } else if (!isNowConnected && reconnectAttempt < reconnectConfig.maxAttempts) {
        attemptReconnect();
      }
    };
