...
export interface UseTerritoryGameplayReturn {
  ...
  // Events
  onChallengeCreated: (callback: (challenge: TerritoryChallenge) => void) => void;
  onChallengeUpdated: (callback: (challenge: TerritoryChallenge) => void) => void;
  onChallengeAccepted: (callback: (challenge: TerritoryChallenge) => void) => void;
  onChallengeDeclined: (callback: (challenge: TerritoryChallenge) => void) => void;
  onChallengeCompleted: (callback: (challenge: TerritoryChallenge) => void) => void;
  
  onTerritoryOwnershipUpdated: (callback: (update: TerritoryOwnershipUpdate) => void) => void;
...
export const useTerritoryGameplay = (userId?: string): UseTerritoryGameplayReturn => {
  const [isConnected, setIsConnected] = useState(false);
  const [activeChallenges, setActiveChallenges] = useState<TerritoryChallenge[]>([]);
  const [pendingChallenges, setPendingChallenges] = useState<TerritoryChallenge[]>([]);
  ...
  useEffect(() => {
    ...
    setPendingChallenges(
      territoryGameplayService.getAllActiveChallenges().filter(c => c.status === 'pending')
    );
    ...
    return () => {
      ...
      territoryGameplayService.off('challengeAccepted', handleChallengeAccepted);
      territoryGameplayService.off('challengeDeclined', handleChallengeDeclined);
      territoryGameplayService.off('challengeCompleted', handleChallengeCompleted);
    };
  }, []);
  ...
  useEffect(() => {
    if (userId) {
      ...
      setPendingChallenges(
        territoryGameplayService.getActiveChallenges(userId).filter(c => c.status === 'pending')
      );
    }
  }, [userId]);
  ...
  const acceptChallenge = useCallback(async (challengeId: string, defenderId: string) => {
    await territoryGameplayService.acceptChallenge(challengeId, defenderId);
  }, []);

  const declineChallenge = useCallback(async (challengeId: string, defenderId: string) => {
    await territoryGameplayService.declineChallenge(challengeId, defenderId);
  }, []);

  // Event registration functions
  const onChallengeCreated = useCallback((callback: (challenge: TerritoryChallenge) => void) => {
    territoryGameplayService.on('challengeCreated', callback);
  }, []);

  const onChallengeUpdated = useCallback((callback: (challenge: TerritoryChallenge) => void) => {
    territoryGameplayService.on('challengeUpdated', callback);
  }, []);

  const onChallengeAccepted = useCallback((callback: (challenge: TerritoryChallenge) => void) => {
    territoryGameplayService.on('challengeAccepted', callback);
  }, []);

  const onChallengeDeclined = useCallback((callback: (challenge: TerritoryChallenge) => void) => {
    territoryGameplayService.on('challengeDeclined', callback);
  }, []);

  const onChallengeCompleted = useCallback((callback: (challenge: TerritoryChallenge) => void) => {
    territoryGameplayService.on('challengeCompleted', callback);
  }, []);

  const onTerritoryOwnershipUpdated = useCallback((callback: (update: TerritoryOwnershipUpdate) => void) => {
    territoryGameplayService.on('territoryOwnershipUpdated', callback);
  }, []);

  const onMatchResult = useCallback((callback: (result: MatchResult) => void) => {
    territoryGameplayService.on('matchResult', callback);
  }, []);

  const onMatchResultProcessed = useCallback((callback: (result: MatchResult) => void) => {
