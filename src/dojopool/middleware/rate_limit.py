...
class RateLimitMiddleware:
    ...
    def _check_rate_limit(self) -> Optional[Response]:
        ...
        if request.endpoint in current_app.config.get('RATE_LIMIT_EXCLUDED_ENDPOINTS', set()):
            return None
            
        # Determine rate limit tier
        tier = self._get_limit_tier()
        limits = self.default_limits[tier]
    ...
    def _get_limit_tier(self) -> str:
        ...
        if any(pattern in endpoint for pattern in ['auth', 'password', 'login', 'register']):
            return 'strict'
    ...
    def get_rate_limit_headers(self, key: str) -> Dict[str, str]:
        ...
        tier = self._get_limit_tier()
        limit = self.default_limits[tier]['rate']
