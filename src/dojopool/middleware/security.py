...
class SecurityMiddleware:
    ...
    def register_middleware(self) -> None:
        ...
        def add_cache_headers(response: Response):
            ...
            if request.endpoint not in self.app.config.get(
                "CACHEABLE_ENDPOINTS", set()
            ):
        ...
        def decorator(f: Callable):
            ...
            def decorated(*args: Any, **kwargs: Any):
                if current_app.config.get("CSRF_ENABLED", True):
                    ...
                    if not token or token != g.csrf_token:
    ...
    @staticmethod
    def rate_limit(requests: int, period: int, by: Optional[str] = None) -> Callable:
        ...
        def decorator(f: Callable):
            ...
            def decorated(*args: Any, **kwargs: Any):
                # Get rate limit key
                if by == "user" and hasattr(g, "token_payload"):
