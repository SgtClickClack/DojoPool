...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const dateInputs: any = document.querySelectorAll('input[type="datetime-local"]');
  ...
  const createTournamentForm: any = document.getElementById('createTournamentForm');
  if (createTournamentForm) {
    createTournamentForm.addEventListener('submit', function (e) {
      e.preventDefault();

      const startDate: any = new Date(document.getElementById('startDate').value);
  ...
  const registerForms: any = document.querySelectorAll('form[action*="register"]');
