...
class PerformanceMetrics {
    ...
    initObservers() {
        ...
        if ('PerformanceObserver' in window) {
            const loadObserver: any = new PerformanceObserver(list => {
    ...
    trackLoadTime(entry) {
        const duration: any = entry.duration;
        const name: any = entry.name;
        const type: any = entry.entryType;
        const size: any = entry.transferSize || 0;
        ...
        if (size > 0 && duration > 0) {
            const bandwidthMbps: any = (size * 8) / (duration * 1000); // Convert to Mbps
    ...
    updateBandwidth() {
        if (navigator.connection) {
            const downlink: any = navigator.connection.downlink;
            if (downlink) {
    ...
    updateBandwidthSample(value) {
        ...
        this.metrics.bandwidth.average = this.metrics.bandwidth.samples.reduce((a, b) => a + b, 0) /
...
const performanceMetrics: any = new PerformanceMetrics();
