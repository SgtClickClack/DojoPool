...
export class MemoryReporter {
  ...
  private startMonitoring(): void {
    ...
    const sample = () => {
      ...
      this.samplingTimeout = self.setTimeout(
        sample,
        MemoryReporter.SAMPLING_INTERVAL
      );
    };
  ...
  private updateMemoryHistory(stats: MemoryStats): void {
    ...
    this.memoryHistory = Array.from(newHistory).map((heapUsed, i) => ({
      ...
      timestamp:
        stats.timestamp -
        (newHistory.length - 1 - i) * MemoryReporter.SAMPLING_INTERVAL,
    }));
  ...
  private checkMemoryTrend(): MemoryTrend {
    ...
    const growthRates = this.memoryHistory
      .slice(1)
      .map(
        (stats, i) =>
          (stats.heapUsed - this.memoryHistory[i].heapUsed) /
          this.memoryHistory[i].heapUsed
      );
    ...
    const isStable =
      Math.abs(avgGrowthRate) <= MemoryReporter.STABLE_GROWTH_RATE;
  ...
  private notifyPressureCallbacks(): void {
    this.pressureCallbacks.forEach((callback) => {
      try {
        ...
        console.error(
          'Memory pressure callback failed:',
