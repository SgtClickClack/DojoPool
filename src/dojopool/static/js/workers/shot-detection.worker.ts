...
type WorkerIncomingMessage = ProcessFrameMessage | SetThresholdMessage | CleanupMessage;
...
class ShotDetectionWorker {
    ...
    private metricsBuffer: { processingTime: number; detectionTime: number; confidence: number }[] = [];
    ...
    private setupMemoryChecks(): void {
        this.memoryCheckerId = self.setInterval(() => {
            try {
                ...
                console.error('Memory check failed:', error instanceof Error ? error.message : 'Unknown error');
            }
    ...
    private setupMessageHandler(): void {
        self.addEventListener('message', (event: MessageEvent<WorkerIncomingMessage>) => {
            try {
                ...
                console.error('Message handling failed:', error instanceof Error ? error.message : 'Unknown error');
            }
        });
    }
    ...
    private async handleMessage(message: WorkerIncomingMessage): Promise<void> {
        switch (message.type) {
            ...
            default: {
                ...
                throw new Error(`Unhandled message type: ${(message as BaseWorkerMessage).type}`);
            }
        }
    }
    ...
    private async processNextFrame(): Promise<void> {
        ...
        if (timeSinceLastProcess < MIN_FRAME_INTERVAL) {
            await new Promise(resolve => setTimeout(resolve, MIN_FRAME_INTERVAL - timeSinceLastProcess));
        }
    ...
    private async detectShot(frame: ImageData): Promise<{ detected: boolean; confidence: number; detectionTime: number }> {
        const startTime = performance.now();
        ...
        if (USE_SIMD) {
            ...
            for (let offset = 0; offset < currentData.length; offset += CHUNK_SIZE) {
                ...
                const current32 = new Uint32Array(currentData.buffer, offset, (end - offset) / 4);
                const last32 = new Uint32Array(lastData.buffer, offset, (end - offset) / 4);
        ...
        const averageDifference = significantPixels > 0 ? totalDifference / significantPixels : 0;
    ...
    private async processSIMD(currentData: Uint8ClampedArray, lastData: Uint8ClampedArray): Promise<{ totalDifference: number; significantPixels: number }> {
    ...
    private getFrameFromPool(frame: ImageData): ImageData {
        ...
        if (!pooledFrame || pooledFrame.width !== frame.width || pooledFrame.height !== frame.height) {
