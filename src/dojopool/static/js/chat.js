...
function getRoomName(room) {
    if (room.type === 'direct') {
        const otherParticipant = room.participants.find(p => p.user_id !== currentUser.id);
...
async function loadMessages(roomId, page = 1) {
    try {
        const response = await fetch(`/api/v1/chat/room/${roomId}/messages?page=${page}`);
        const data = await response.json();
...
function appendMessage(message) {
    ...
    const shouldScroll = messagesList.scrollHeight - messagesList.scrollTop === messagesList.clientHeight;
    if (shouldScroll) {
        messagesList.scrollTop = messagesList.scrollHeight;
    }
}
