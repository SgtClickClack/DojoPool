...
class MetricsDashboard {
    ...
    initCharts() {
        // Load time chart
        this.charts.set('load-times', this.createChart('load-times-chart', {
            type: 'line',
            data: {
                labels: [],
                datasets: [{
        ...
        // Bandwidth chart
        this.charts.set('bandwidth', this.createChart('bandwidth-chart', {
            type: 'line',
            data: {
                labels: [],
                datasets: [{
    ...
    updateStats(elementId, stats) {
        ...
        container.innerHTML = Object.entries(stats)
            .map(([key, value]) => `<div class="stat-item"><span>${key}:</span> ${value}</div>`)
            .join('');
    }
    ...
    checkAlerts() {
        ...
        // Check memory alerts
        const memoryUsagePercent: any = (stats.memory.current / stats.memory.limit) * 100;
