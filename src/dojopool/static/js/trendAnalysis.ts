...
export class TrendAnalysis {
    ...
    private addDataPoint(type: string, data: MetricsUpdateEvent['detail']['data']): void {
    ...
    private calculateTrendDirection(data: DataPoint[]): 'stable' | 'increasing' | 'decreasing' {
        ...
        const firstMean = this.calculateMean(firstHalf.map(v => ({ timestamp: 0, value: v })));
        const secondMean = this.calculateMean(secondHalf.map(v => ({ timestamp: 0, value: v })));
    ...
    private notifySignificantChanges(type: string, analysis: TrendAnalysisResult): void {
        const trend = this.trends[type];
    ...
    private isMetricBad(type: string, direction: 'increase' | 'decrease'): boolean {
        switch (type) {
            case 'loadTime':
    ...
    public getTrendData(type: string, timeRange: number = 3600000): {
        data: DataPoint[];
        baseline: Baseline | null;
        anomalies: DataPoint[];
    } {
    ...
    public getMetricHealth(type: string): {
        ...
        if (anomalyCount > 5) {
            status = 'critical';
        } else if (anomalyCount > 2 || (trendDirection === 'increasing' && this.isMetricBad(type, 'increase'))) {
            status = 'warning';
        }
