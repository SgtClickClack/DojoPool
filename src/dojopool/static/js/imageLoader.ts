...
// Type imports
// TODO: Import types from ./bandwidthTracker.js
...
class ImageLoader {
    ...
    preloadNextImages() {
        document.querySelectorAll('img[data-src]:not(.loaded):not(.loading)').forEach(img => {
            const rect: any = img.getBoundingClientRect();
    ...
    getNetworkQuality() {
        ...
        const bandwidthCondition: any = bandwidthTracker.getNetworkCondition();
    ...
    getAppropriateSize(availableSizes) {
        ...
        const quality: any = this.networkQuality;
        const avgLoadTime: any = bandwidthTracker.getAverageLoadTime();

        // Sort sizes from smallest to largest
        const sizes: any = Object.entries(availableSizes)
            .sort((a, b) => parseInt(a[1]) - parseInt(b[1]));

        // Find the first size that's larger than the viewport
        const appropriateSize: any = sizes.find(([_, width]) => width >= viewportWidth);

        // If load times are too high, reduce quality
        if (avgLoadTime > 3000) {
            const index: any = sizes.indexOf(appropriateSize);
        ...
        switch (quality) {
            ...
            case 'high':
                const nextIndex: any = sizes.indexOf(appropriateSize) + 1;
    ...
    loadImage(img) {
        const src: any = img.dataset.src;
        const webpSrc: any = img.dataset.webp;
        const sizes: any = JSON.parse(img.dataset.sizes || '{}');

        // Get appropriate size based on network conditions
        const appropriateSize: any = sizes ? this.getAppropriateSize(sizes) : null;
        ...
        const handleLoad: any = () => {
            ...
            const size: any = parseInt(img.dataset.size || '0');
    ...
    updateLoadedImages() {
        document.querySelectorAll('img.loaded').forEach(img => {
            const originalSrc: any = img.dataset.originalSrc;
            const originalWebp: any = img.dataset.originalWebp;
            const sizes: any = JSON.parse(img.dataset.originalSizes || '{}');

            if (originalSrc && sizes) {
                const appropriateSize: any = this.getAppropriateSize(sizes);
                const newSrc: any = this.supportsWebP() && originalWebp ?
...
const imageLoader: any = new ImageLoader();
