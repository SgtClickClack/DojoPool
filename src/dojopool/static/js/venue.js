...
if (venueSearch && venueFilter) {
    ...
    const performSearch = debounceSearch(async () => {
        ...
        try {
            const response = await fetch(`/api/v1/venues/search?q=${searchQuery}&filter=${filterValue}`);
            const data = await response.json();
...
async function handleCheckInOut(event) {
    ...
    try {
        ...
        if (response.ok) {
            showToast(`Successfully ${isCheckIn ? 'checked in to' : 'checked out from'} venue`, 'success');
            // Toggle button state
            ...
            showToast(data.message || `Failed to ${isCheckIn ? 'check in' : 'check out'}`);
        }
    } catch (error) {
        showToast(`An error occurred while ${isCheckIn ? 'checking in' : 'checking out'}`);
...
if (loadMoreBtn) {
    loadMoreBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            const response = await fetch(`/api/v1/venues?page=${currentPage}&q=${searchQuery}&filter=${filterValue}`);
            const data = await response.json();

            if (response.ok) {
                // Append new venues to the grid
                const newVenues = data.venues.map(venue => createVenueCard(venue)).join('');
