...
const SUPPORTS_TRANSFER_CONTROL = 'transferControlToOffscreen' in HTMLCanvasElement.prototype;
...
class GenericMemoryPool<T> implements MemoryPool<T> {
    ...
    gc(): void {
        ...
        this.items = this.items.filter(item => item !== null && item !== undefined);
...
class PerformanceProfiler {
    ...
    takeSnapshot(frameTime: number, gpuTime: number): void {
        ...
        const snapshot: PerformanceSnapshot = {
            ...
            workerUtilization: frameWorker ? frameWorker.activeTaskCount / frameWorker.maxTasks : 0
        };
...
function initializeDOMElements(): void {
    domElements.gameVideo = document.getElementById('game-video') as HTMLVideoElement;
