...
export class NetworkProfiler extends BaseManager<NetworkProfiler> {
    ...
    private interceptXHR(): void {
        ...
        window.XMLHttpRequest = function () {
            ...
            xhr.addEventListener('loadend', () => {
                ...
                requestInfo.size = parseInt(xhr.getResponseHeader('content-length') || '0', 10);
    ...
    public getMetrics(): NetworkMetrics {
        const completedRequests = this.history.filter(r => r.endTime !== undefined);
        ...
        const stats: NetworkStats = {
            ...
            errorRate: completedRequests.length > 0 ?
                (completedRequests.length - successfulRequests.length) / completedRequests.length : 0
        };
        ...
        completedRequests.forEach(request => {
            ...
            current.count++;
            current.averageLatency = (current.averageLatency * (current.count - 1) + request.duration!) / current.count;
            current.errorRate = (current.count - 1) * current.errorRate / current.count +
