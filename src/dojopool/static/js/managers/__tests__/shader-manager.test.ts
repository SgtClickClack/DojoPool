...
const mockWebGLContext = {
    createShader: jest.fn<WebGLShader, [number]>().mockReturnValue({} as WebGLShader),
    shaderSource: jest.fn<void, [WebGLShader, string]>(),
    compileShader: jest.fn<void, [WebGLShader]>(),
    getShaderParameter: jest.fn<boolean, [WebGLShader, number]>().mockReturnValue(true),
    getShaderInfoLog: jest.fn<string, [WebGLShader]>().mockReturnValue(''),
    deleteShader: jest.fn<void, [WebGLShader]>(),
    createProgram: jest.fn<WebGLProgram, []>().mockReturnValue({} as WebGLProgram),
    attachShader: jest.fn<void, [WebGLProgram, WebGLShader]>(),
    linkProgram: jest.fn<void, [WebGLProgram]>(),
    getProgramParameter: jest.fn<boolean, [WebGLProgram, number]>().mockReturnValue(true),
    getProgramInfoLog: jest.fn<string, [WebGLProgram]>().mockReturnValue(''),
    deleteProgram: jest.fn<void, [WebGLProgram]>(),
    getActiveAttrib: jest.fn<{ name: string }, [WebGLProgram, number]>().mockReturnValue({ name: 'position' }),
    getActiveUniform: jest.fn<{ name: string }, [WebGLProgram, number]>().mockReturnValue({ name: 'modelViewMatrix' }),
    getAttribLocation: jest.fn<number, [WebGLProgram, string]>().mockReturnValue(0),
    getUniformLocation: jest.fn<WebGLUniformLocation | null, [WebGLProgram, string]>().mockReturnValue({} as WebGLUniformLocation),
...
const mockContextManager = {
    getContext: jest.fn<WebGLRenderingContext | WebGL2RenderingContext | null, []>().mockReturnValue(mockWebGLContext),
...
describe('ShaderManager', () => {
    ...
    describe('Shader Registration', () => {
        ...
        it('should handle shader compilation failure', () => {
            ...
            (mockWebGLContext.getShaderParameter as jest.Mock).mockReturnValueOnce(false);
            (mockWebGLContext.getShaderInfoLog as jest.Mock).mockReturnValueOnce('Compilation error');
        ...
        it('should handle program linking failure', () => {
            const consoleSpy = jest.spyOn(console, 'error').mockImplementation();
            (mockWebGLContext.getProgramParameter as jest.Mock).mockReturnValueOnce(false);
            (mockWebGLContext.getProgramInfoLog as jest.Mock).mockReturnValueOnce('Linking error');
    ...
    describe('Context Management', () => {
        it('should handle context restoration', () => {
            ...
            const contextListener = (mockContextManager.addContextListener as jest.Mock).mock.calls[0][0];
    ...
    describe('Context Management', () => {
        it('should handle context loss and restoration', () => {
            ...
            mockContextManager.addContextListener.mock.calls[0][0](contextRestoredEvent);
