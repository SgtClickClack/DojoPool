...
describe("ErrorRecoveryManager", () => {
  ...
  describe("Strategy Registration", () => {
    ...
    it("should throw when registering duplicate strategy", () => {
      recoveryManager.registerStrategy(mockStrategy);
      expect(() => recoveryManager.registerStrategy(mockStrategy)).toThrow(
        "Strategy testStrategy already exists",
      );
    });
  });
  ...
  describe("Error Handling", () => {
    ...
    it("should respect max attempts", async () => {
      ...
      const handled = await recoveryManager.handleError(error);
      expect(handled).toBe(false);
      expect(mockStrategy.handler).toHaveBeenCalledTimes(
        mockStrategy.maxAttempts!,
      );
    });
    ...
    it("should handle strategy execution errors", async () => {
      ...
      await expect(recoveryManager.handleError(error)).rejects.toThrow(
        "Recovery strategy failingStrategy failed",
      );
    });
  });
