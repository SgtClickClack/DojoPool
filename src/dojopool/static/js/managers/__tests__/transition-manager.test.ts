...
describe("TransitionManager", () => {
  ...
  describe("Transition Events", () => {
    it("should dispatch progress events during transition", () => {
      ...
      expect(progressHandler.mock.calls[0][0].detail.progress).toBeGreaterThan(
        0,
      );
  ...
  describe("Progress Calculation", () => {
    it("should calculate eased progress correctly", () => {
      const progress = 0.5;
      const easedProgress = (transitionManager as any).calculateEasedProgress(
        progress,
      );
    ...
    it("should interpolate quality levels correctly", () => {
      ...
      const end = 3;
      const progress = 0.5;
      const interpolated = (transitionManager as any).interpolateQuality(
        start,
        end,
        progress,
      );
