...
describe("TextureManager", () => {
  ...
  describe("Texture Format Detection", () => {
    it("should detect compressed texture formats", () => {
      ...
      (mockWebGLContext.getExtension as jest.Mock).mockImplementation(
        (name) => {
          switch (name) {
            case "WEBGL_compressed_texture_s3tc":
              return {
                COMPRESSED_RGBA_S3TC_DXT5_EXT:
                  mockWebGLContext.COMPRESSED_RGBA_S3TC_DXT5_EXT,
              };
            case "WEBGL_compressed_texture_etc":
              return {
                COMPRESSED_RGBA8_ETC2_EAC:
                  mockWebGLContext.COMPRESSED_RGBA8_ETC2_EAC,
              };
            case "WEBGL_compressed_texture_astc":
              return {
                COMPRESSED_RGBA_ASTC_4x4_KHR:
                  mockWebGLContext.COMPRESSED_RGBA_ASTC_4x4_KHR,
              };
            default:
              return null;
          }
    ...
    it("should handle compressed texture data", () => {
      ...
      (mockDeviceProfileManager.isMobileDevice as jest.Mock).mockReturnValue(
        true,
      );
  ...
  describe("Texture Memory Management", () => {
    ...
    it("should track active textures", () => {
      const texture = webglManager.createTexture(256, 256);
      expect(
        (webglManager as any).contextState.activeTextures.has(texture),
      ).toBe(true);
  ...
  describe("Streaming Textures", () => {
    it("should request streaming texture", () => {
      ...
      expect((webglManager as any).contextState.streamingQueue).toContain(
        options,
      );
    });
    ...
    it("should update last used time for existing streaming textures", () => {
      ...
      (webglManager as any).contextState.streamingTextures.set(
        options.url,
        streamingTexture,
      );
      webglManager.requestStreamingTexture(options);
  ...
  describe("Progressive Loading", () => {
    it("should handle progressive texture loading", async () => {
      ...
      const mockTexture = {};
      (mockWebGLContext.createTexture as jest.Mock).mockReturnValue(
        mockTexture,
      );
      ...
      const streamingTexture = (
        webglManager as any
      ).contextState.streamingTextures.get(options.url);
    ...
    it("should handle progressive loading failure", async () => {
      ...
      await expect(
        (webglManager as any).loadStreamingTexture(options),
      ).rejects.toThrow();
  ...
  describe("Cleanup", () => {
    ...
    it("should clean up streaming textures", () => {
      ...
      expect(mockWebGLContext.deleteTexture).toHaveBeenCalledWith(
