...
describe('BufferManager', () => {
    ...
    describe('Buffer Creation', () => {
        it('should create a new buffer', () => {
            ...
            const buffer = webglManager.createBuffer(data, mockWebGLContext.ARRAY_BUFFER);
            expect(buffer).toBeDefined();
            expect(mockWebGLContext.createBuffer).toHaveBeenCalled();
            expect(mockWebGLContext.bindBuffer).toHaveBeenCalledWith(mockWebGLContext.ARRAY_BUFFER, buffer);
        ...
        it('should create buffer with specified usage', () => {
            const data = new Float32Array([1, 2, 3, 4]);
            const buffer = webglManager.createBuffer(data, mockWebGLContext.ARRAY_BUFFER, mockWebGLContext.DYNAMIC_DRAW);
            expect(buffer).toBeDefined();
            expect(mockWebGLContext.bufferData).toHaveBeenCalledWith(
                mockWebGLContext.ARRAY_BUFFER,
                data,
        ...
        it('should reuse buffer from pool when available', () => {
            const poolBuffer = {};
            (webglManager as any).contextState.bufferPool.set(mockWebGLContext.ARRAY_BUFFER, [poolBuffer]);
    ...
    describe('Buffer Deletion', () => {
        ...
        it('should add buffer to pool when space available', () => {
            ...
            expect((webglManager as any).contextState.bufferPool.get(mockWebGLContext.ARRAY_BUFFER)).toContain(buffer);
        });
    });
    ...
    describe('Buffer Updates', () => {
        it('should update buffer data', () => {
            ...
            webglManager.updateBuffer(buffer as WebGLBuffer, newData, mockWebGLContext.ARRAY_BUFFER);
            expect(mockWebGLContext.bindBuffer).toHaveBeenCalledWith(mockWebGLContext.ARRAY_BUFFER, buffer);
            expect(mockWebGLContext.bufferData).toHaveBeenCalledWith(
                mockWebGLContext.ARRAY_BUFFER,
                newData,
    ...
    describe('Buffer Pool Management', () => {
        it('should clean up buffer pool when threshold exceeded', () => {
            ...
            (webglManager as any).contextState.bufferPool.set(mockWebGLContext.ARRAY_BUFFER, buffers);
    ...
    describe('Buffer Usage Tracking', () => {
        it('should track buffer usage', () => {
            ...
            expect((webglManager as any).contextState.activeBuffers.has(buffer)).toBe(true);
            expect((webglManager as any).contextState.bufferUsage.get(buffer)).toBe(mockWebGLContext.ARRAY_BUFFER);
        });
        ...
        it('should remove buffer from tracking when deleted', () => {
            ...
            webglManager.createBuffer(data, mockWebGLContext.ARRAY_BUFFER);
            webglManager.deleteBuffer(buffer as WebGLBuffer);
            expect((webglManager as any).contextState.activeBuffers.has(buffer)).toBe(false);
            expect((webglManager as any).contextState.bufferUsage.has(buffer)).toBe(false);
        });
    });
}); 