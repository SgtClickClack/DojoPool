...
} from './performance-visualizer';
import {
  ResourceVisualizer,
  type ResourceVisualizerOptions,
} from './resource-visualizer';
import { WebGLProfiler, type WebGLTimingInfo } from './webgl-profiler';
...
export class PerformanceMonitor extends BaseManager<PerformanceMonitor> {
  ...
  protected constructor(options: MonitorOptions = {}) {
    ...
    if (options.resourceVisualizer) {
      this.resourceVisualizer = ResourceVisualizer.getInstance(
        options.resourceVisualizer
      );
    }
  ...
  public static override getInstance(
    ...
    return (
      BaseManager.getInstance.call(PerformanceMonitor) ||
    ...
    this.monitoringInterval = window.setInterval(() => {
      ...
      if (this.resourceVisualizer) {
        ...
        if (this.webglProfiler) {
          this.resourceVisualizer.updateWebGLResources(
            this.webglProfiler.getResourceStats()
          );
        }
        ...
        const workerMetrics = Array.from(
          this.workerProfiler.getAllMetrics().values()
        );
        const activeWorkers = workerMetrics.filter(
          (m) => m.queueLength > 0
        ).length;
        const idleWorkers = workerMetrics.length - activeWorkers;
        const queuedTasks = workerMetrics.reduce(
          (sum, m) => sum + m.queueLength,
          0
        );
  ...
  private generateReport(): PerformanceReport {
    ...
    const snapshot: PerformanceSnapshot & { drawCalls: number } = {
      ...
      workerUtilization:
        totalWorkerMetrics.totalUtilization / workerMetrics.length,
    ...
    return {
      ...
      workerMetrics: {
        ...totalWorkerMetrics,
        averageProcessingTime:
          totalWorkerMetrics.averageProcessingTime / workerMetrics.length,
