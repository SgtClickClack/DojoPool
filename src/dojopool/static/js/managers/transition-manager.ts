...
export class TransitionManager extends BaseManager<TransitionManager> {
    ...
    constructor(contextManager: WebGLContextManager, shaderManager: ShaderManager) {
        super();
    ...
    public startTransition(targetQuality: number): void {
        ...
        if (targetQuality < TransitionManager.MIN_QUALITY ||
    ...
    private readonly updateTransition = (): void => {
        ...
        const easedProgress = this.calculateEasedProgress(this.currentTransition.progress);
    ...
    private interpolateQuality(start: number, end: number, progress: number): number {
