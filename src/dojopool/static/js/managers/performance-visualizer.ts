...
export class PerformanceVisualizer extends BaseManager<PerformanceVisualizer> {
    ...
    public static override getInstance(options: VisualizerOptions): PerformanceVisualizer {
        return BaseManager.getInstance.call(PerformanceVisualizer) || new PerformanceVisualizer(options);
    }

    public updateData(snapshot: PerformanceSnapshot, violations: BudgetViolation[], budget: PerformanceBudget): void {
        ...
        this.updateDataset('Frame Time (ms)', snapshot.frameTime, this.colors.frameTime);
        this.updateDataset('GPU Time (ms)', snapshot.gpuTime, this.colors.gpuTime);
        this.updateDataset('Memory (MB)', snapshot.memoryStats.usedJSHeapSize / (1024 * 1024), this.colors.memory);
        this.updateDataset('Worker Utilization', snapshot.workerUtilization, this.colors.worker);
    ...
    private drawGrid(padding: number, chartWidth: number, chartHeight: number): void {
        const ctx = this.ctx;
        const gridLines = 5;
    ...
    private drawDataset(dataset: ChartData['datasets'][0], padding: number, chartWidth: number, chartHeight: number): void {
        const ctx = this.ctx;
        const points = dataset.data.map((value, index) => ({
            x: padding + (chartWidth * index) / (this.maxDataPoints - 1),
            y: padding + chartHeight - (chartHeight * value) / this.getMaxValue(dataset.label)
        }));
    ...
    private drawBudgetLines(budget: PerformanceBudget, padding: number, chartWidth: number, chartHeight: number): void {
        const ctx = this.ctx;
        ctx.setLineDash([5, 5]);
        ...
        if (budget.maxFrameTime) {
            const y = padding + chartHeight - (chartHeight * budget.maxFrameTime) / this.getMaxValue('Frame Time (ms)');
            ctx.beginPath();
            ctx.moveTo(padding, y);
            ctx.lineTo(padding + chartWidth, y);
            ctx.stroke();
        ...
        if (budget.minFps) {
            const y = padding + chartHeight - (chartHeight * budget.minFps) / this.getMaxValue('FPS');
            ctx.beginPath();
            ctx.moveTo(padding, y);
            ctx.lineTo(padding + chartWidth, y);
            ctx.stroke();
    ...
    private drawViolations(violations: BudgetViolation[], padding: number, chartWidth: number, chartHeight: number): void {
        const ctx = this.ctx;
        violations.forEach(violation => {
            const index = this.data.labels.findIndex(label =>
                new Date(violation.timestamp).toLocaleTimeString() === label
            );
            if (index === -1) return;

            const x = padding + (chartWidth * index) / (this.maxDataPoints - 1);
            ctx.fillStyle = violation.severity === 'critical' ? this.colors.critical : this.colors.warning;
            ctx.beginPath();
    ...
    private drawAxesLabels(padding: number, chartWidth: number, chartHeight: number): void {
        const ctx = this.ctx;
        ctx.fillStyle = this.colors.text;
        ctx.font = '12px Arial';
        ...
        for (let i = 0; i <= 5; i++) {
            ...
            const value = Math.round(this.getMaxValue('Frame Time (ms)') * (5 - i) / 5);
