...
export class WebGLProfiler extends BaseManager<WebGLProfiler> {
    ...
    public static override getInstance(gl: WebGLRenderingContext | WebGL2RenderingContext): WebGLProfiler {
    ...
    public startFrame(): void {
        ...
        if (this.activeQuery) {
            ...
            if (available) {
                this.lastGPUTime = this.gl.getQueryParameter(this.activeQuery, this.gl.QUERY_RESULT) / 1000000; // Convert to ms
    ...
    public trackResourceCreation(type: 'texture' | 'buffer' | 'shader' | 'program'): void {
        this.resourceStats[`${type}s` as keyof WebGLResourceStats]++;
    }

    public trackResourceDeletion(type: 'texture' | 'buffer' | 'shader' | 'program'): void {
        this.resourceStats[`${type}s` as keyof WebGLResourceStats]--;
    }
