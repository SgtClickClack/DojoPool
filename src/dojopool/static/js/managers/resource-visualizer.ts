...
import { WebGLResourceStats } from './webgl-profiler';
...
export class ResourceVisualizer extends BaseManager<ResourceVisualizer> {
    ...
    public static override getInstance(options: ResourceVisualizerOptions): ResourceVisualizer {
        return BaseManager.getInstance.call(ResourceVisualizer) || new ResourceVisualizer(options);
    }
    ...
    public updateWorkerStatus(active: number, idle: number, queued: number, errors: number): void {
        this.data.workers = [
    ...
    private drawPieChart(data: PieChartData[], centerX: number, centerY: number, radius: number, title: string): void {
        const ctx = this.ctx;
