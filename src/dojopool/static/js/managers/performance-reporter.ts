...
import { type PerformanceReport } from './performance-monitor';
...
export class PerformanceReporter extends BaseManager<PerformanceReporter> {
  ...
  public static override getInstance(
    ...
    return (
      BaseManager.getInstance.call(PerformanceReporter) ||
      new PerformanceReporter(options)
    );
  }
  ...
  private generateSummary(): any {
    ...
    const workerUtil = this.reports.map(
      (r) => r.workerMetrics.totalUtilization
    );
  ...
  private generateRecommendations(): any {
    ...
    if (summary.memory.growth > 1024 * 1024) {
      // More than 1MB/s growth
      recommendations.push({
        type: 'memory',
        severity: 'high',
