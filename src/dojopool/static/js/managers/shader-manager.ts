...
export class ShaderManager extends BaseManager<ShaderManager> {
    ...
    public registerShader(name: string, definitions: ShaderDefinition[]): void {
        ...
        const sortedDefs = [...definitions].sort((a, b) => a.complexity - b.complexity);
        const variants: ShaderVariant[] = [];

        // Create variants for different quality levels
        for (let quality = 1; quality <= this.contextManager.getQualityLevel(); quality++) {
            ...
            try {
                ...
                console.error(`Failed to compile shader ${name} for quality ${quality}:`, error);
            }
        }
    ...
    private selectDefinitionForQuality(definitions: ShaderDefinition[], quality: number): ShaderDefinition {
    ...
    private createProgram(vertexSource: string, fragmentSource: string): ShaderProgram | null {
        const vertexShader = this.compileShader(vertexSource, this.context.VERTEX_SHADER);
        const fragmentShader = this.compileShader(fragmentSource, this.context.FRAGMENT_SHADER);
        ...
        if (!this.context.getProgramParameter(program, this.context.LINK_STATUS)) {
            console.error('Failed to link program:', this.context.getProgramInfoLog(program));
    ...
    private compileShader(source: string, type: number): WebGLShader | null {
        ...
        if (!this.context.getShaderParameter(shader, this.context.COMPILE_STATUS)) {
            console.error('Failed to compile shader:', this.context.getShaderInfoLog(shader));
    ...
    private getAttributes(program: WebGLProgram): Map<string, number> {
        ...
        const numAttributes = this.context.getProgramParameter(program, this.context.ACTIVE_ATTRIBUTES);

        for (let i = 0; i < numAttributes; i++) {
            const info = this.context.getActiveAttrib(program, i);
            if (info) {
                attributes.set(info.name, this.context.getAttribLocation(program, info.name));
            }
        }
    ...
    private getUniforms(program: WebGLProgram): Map<string, WebGLUniformLocation> {
        const uniforms = new Map<string, WebGLUniformLocation>();
        const numUniforms = this.context.getProgramParameter(program, this.context.ACTIVE_UNIFORMS);
    ...
    public override cleanup(): void {
        ...
        this.contextManager.removeContextListener(this.handleContextEvent.bind(this));
        this.onCleanup();
    }
} 