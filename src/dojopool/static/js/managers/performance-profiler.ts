...
export class PerformanceProfiler extends BaseManager<PerformanceProfiler> {
  ...
  private checkWarningThresholds(metrics: PerformanceMetrics): void {
    ...
    if (metrics.frameProcessingTime > warningThresholds.frameTime) {
      warnings.push(
        `High frame processing time: ${metrics.frameProcessingTime.toFixed(2)}ms`,
      );
    }
  ...
  public getAverageMetrics(): Partial<PerformanceMetrics> {
    ...
    const sum = this.metrics.reduce((acc, metrics) => ({
      frameProcessingTime:
        acc.frameProcessingTime + metrics.frameProcessingTime,
      ...
      memoryUsage: {
        ...
        externalMemory:
          acc.memoryUsage.externalMemory + metrics.memoryUsage.externalMemory,
      },
      shotDetectionTime:
        (acc.shotDetectionTime || 0) + (metrics.shotDetectionTime || 0),
      shotConfidence: (acc.shotConfidence || 0) + (metrics.shotConfidence || 0),
    }));

    const count = this.metrics.length;
    const shotMetricsCount = this.metrics.filter(
      (m) => m.shotDetectionTime !== undefined,
    ).length;
  ...
  public endMark(
    markName: keyof PerformanceMarks,
    measureName: string,
  ): number {
  ...
  private detectAnomalies(
    ...
    // Check FPS anomalies
    const fpsDeviation =
      Math.abs(snapshot.fps - trend.averageFps) / trend.fpsStdDev;
    ...
    const frameTimeDeviation =
      Math.abs(snapshot.frameTime - trend.averageFrameTime) /
      trend.frameTimeStdDev;
