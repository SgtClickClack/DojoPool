...
interface ContextState {
  ...
  atlasRegionMap: Map<
    string,
    { atlas: TextureAtlas; region: TextureAtlasRegion }
  >;
  webgl2Features: WebGL2Features | null;
  textureWorker: Worker | null;
  pendingWorkerTasks: Map<
    string,
    { resolve: (data: ArrayBufferView) => void; reject: (error: Error) => void }
  >;
}
...
export class WebGLContextManager extends BaseManager<WebGLContextManager> {
  ...
  private configureContext(
    ...
    gl.hint(
      ...
      quality.textureQuality > 0.7 ? gl.NICEST : gl.FASTEST
    );
  ...
  private detectTextureFormats(
    gl: WebGLRenderingContext | WebGL2RenderingContext
  ): Map<string, TextureFormat> {
  ...
  private createTextureWorker(): Worker | null {
    try {
      const blob = new Blob([WebGLContextManager.WORKER_SCRIPT], {
      ...
      console.warn(
        'Failed to create texture worker:',
  ...
  private async processTextureInWorker(
  ...
  private checkPerformance(): void {
    ...
    const currentQuality =
      WebGLContextManager.QUALITY_LEVELS[
  ...
  private async attemptContextRecovery(): Promise<void> {
    ...
    if (timeSinceLastLoss < WebGLContextManager.CONTEXT_RECOVERY_DELAY) {
      await new Promise((resolve) =>
        setTimeout(
          resolve,
          WebGLContextManager.CONTEXT_RECOVERY_DELAY - timeSinceLastLoss
        )
      );
    }
  ...
  private checkMemoryUsage(): void {
    ...
    const now = Date.now();
    if (
      now - this.contextState.lastMemoryCheck <
      WebGLContextManager.MEMORY_CHECK_INTERVAL
    )
      return;
    ...
    if (
      this.contextState.texturePool.length >
      WebGLContextManager.MAX_TEXTURE_POOL_SIZE *
        WebGLContextManager.TEXTURE_POOL_CLEANUP_THRESHOLD
    ) {
    ...
    if (
      this.deviceProfile.getCapabilities().isLowEndDevice ||
      this.deviceProfile.isMobileDevice()
    ) {
  ...
  private cleanupTexturePool(force: boolean = false): void {
    ...
    const maxPoolSize = force
      ? Math.floor(WebGLContextManager.MAX_TEXTURE_POOL_SIZE / 2)
      : WebGLContextManager.MAX_TEXTURE_POOL_SIZE;
  ...
  public createTexture(
    ...
    const { gl, texturePool, activeTextures, textureFormats } =
      this.contextState;
    ...
    if (isMobile && data) {
      ...
      if (textureFormats.has('etc2')) {
        // Android
        format = textureFormats.get('etc2')!;
      } else if (textureFormats.has('pvrtc')) {
        // iOS
  ...
  private async processStreamingQueue(): Promise<void> {
    ...
    const { streamingQueue, streamingTextures, currentStreamingLoad } =
      this.contextState;
  ...
  private async loadStreamingTexture(
  ...
  private async createTextureFromWorkerData(
    ...
    if (webgl2Features) {
      ...
      gl.texStorage2D(
        gl.TEXTURE_2D,
        generateMipmaps ? Math.log2(Math.max(width, height)) + 1 : 1,
        gl.RGBA8,
        width,
        height
      );
  ...
  public createPreloadGroup(
    id: string,
    textures: TextureStreamOptions[],
    priority: number = 0
  ): void {
    if (!this.contextState) return;
    ...
    const group: TexturePreloadGroup = {
      ...
      onProgress: (progress: number) => {
        console.debug(
          `Preload group ${id}: ${Math.round(progress * 100)}% complete`
        );
      },
    };
  ...
  private async preloadTexture(options: TextureStreamOptions): Promise<void> {
    ...
    try {
      ...
      if (texture && this.contextState.streamingTextures.has(options.url)) {
        const streamingTexture = this.contextState.streamingTextures.get(
          options.url
        )!;
  ...
  private async createProgressiveData(
    image: HTMLImageElement,
    options: TextureStreamOptions
  ): Promise<ProgressiveTextureData> {
  ...
  private async processProgressiveQueue(): Promise<void> {
    ...
    for (const url of progressiveLoadQueue) {
      ...
      const nextQualityIndex = Math.min(
        Math.floor(
          texture.currentQuality *
            WebGLContextManager.PROGRESSIVE_QUALITY_LEVELS.length
        ),
  ...
  private async findFreeRegion(
    ...
    for (const region of atlas.regions.values()) {
      for (
        ...
        x += WebGLContextManager.MIN_ATLAS_REGION
      ) {
        used.add(x);
      }
    }
    ...
    const possibleX = Array.from({
      length: atlas.width / WebGLContextManager.MIN_ATLAS_REGION,
    })
  ...
  private async defragmentAtlases(): Promise<void> {
    ...
    for (const atlas of textureAtlases) {
      ...
      if (
        atlas.usedSpace / (atlas.width * atlas.height) <
        WebGLContextManager.ATLAS_CLEANUP_THRESHOLD
      ) {
  ...
  private cleanupAtlases(): void {
    ...
    if (
      totalMemory >
      WebGLContextManager.STREAMING_MEMORY_LIMIT *
        WebGLContextManager.MEMORY_PRESSURE_THRESHOLD
    ) {
