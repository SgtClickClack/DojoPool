...
class AnalyticsView {
  ...
  private initializeCharts(): void {
    // Heatmap
    const heatmapCtx = document.getElementById(
      'heatmap-chart'
    ) as HTMLCanvasElement;
    this.charts.set(
      'heatmap',
      new Chart(heatmapCtx, this.createHeatmapConfig())
    );

    // Time Distribution
    const timeDistCtx = document.getElementById(
      'time-distribution-chart'
    ) as HTMLCanvasElement;
    this.charts.set(
      'timeDistribution',
      new Chart(timeDistCtx, this.createTimeDistributionConfig())
    );

    // Intervals
    const intervalsCtx = document.getElementById(
      'intervals-chart'
    ) as HTMLCanvasElement;
    this.charts.set(
      'intervals',
      new Chart(intervalsCtx, this.createIntervalsConfig())
    );

    // Accuracy
    const accuracyCtx = document.getElementById(
      'accuracy-chart'
    ) as HTMLCanvasElement;
    this.charts.set(
      'accuracy',
      new Chart(accuracyCtx, this.createAccuracyConfig())
    );

    // Processing Time
    const processingCtx = document.getElementById(
      'processing-time-chart'
    ) as HTMLCanvasElement;
    this.charts.set(
      'processingTime',
      new Chart(processingCtx, this.createProcessingTimeConfig())
    );

    // Model Performance Chart
    const modelPerfCtx = document.getElementById(
      'model-performance-chart'
    ) as HTMLCanvasElement;
    this.charts.set(
      'modelPerformance',
      new Chart(modelPerfCtx, this.createModelPerformanceConfig())
    );
  }
  ...
  private updatePredictionMarkers(
    predictions: Array<{
  ...
  private updateCharts(): void {
    ...
    if (timeDistribution) {
      timeDistribution.data.datasets[0].data = Object.values(
    ...
    if (intervals) {
      intervals.data.datasets[0].data = Object.values(
        analysis.patterns.intervalDistribution
      );
      intervals.update();
    }
