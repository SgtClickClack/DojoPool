document.addEventListener('DOMContentLoaded', function() {
    ...
    const achievementDetailsModal = new bootstrap.Modal(document.getElementById('achievementDetailsModal'));
    ...
    function createAchievementCard(achievement) {
        const progress = achievement.target_value ? (achievement.progress / achievement.target_value * 100) : 0;
        const card = document.createElement('div');
        card.className = 'achievement-card mb-3 p-3 bg-light rounded cursor-pointer';
    ...
    function renderLeaderboard(leaderboard) {
        ...
        leaderboard.forEach((entry, index) => {
            ...
            item.className = 'leaderboard-item d-flex align-items-center p-3 border-bottom';
    ...
    function showAchievementDetails(achievement) {
        ...
        modal.querySelector('.achievement-icon').src = achievement.icon_url || '/static/img/default-achievement.png';
        ...
        modal.querySelector('.progress-value').textContent = `${achievement.progress} / ${achievement.target_value}`;
        modal.querySelector('.progress-bar').style.width = `${progress}%`;
        modal.querySelector('.achievement-points').textContent = `${achievement.points} points`;
        modal.querySelector('.achievement-completed-date').textContent = achievement.completed_at ? 
            new Date(achievement.completed_at).toLocaleDateString() : 'Not completed';
