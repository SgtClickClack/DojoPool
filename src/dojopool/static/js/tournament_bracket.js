class TournamentBracket {
    ...
    async fetchBracketData() {
        const response = await fetch(`/api/v1/tournaments/${this.tournamentId}/brackets`);
        if (!response.ok) {
            throw new Error('Failed to fetch bracket data');
        }
        const data = await response.json();
        this.format = data.format;
        this.rounds = Object.entries(data.brackets).sort((a, b) => parseInt(a[0]) - parseInt(b[0]));
    }
    ...
    createMatchElement(match) {
        ...
        if (match.status === 'pending' && match.player1 && match.player2) {
            matchContainer.addEventListener('click', () => this.handleMatchClick(match));
        }
    ...
    addConnectors() {
        const rounds = this.container.querySelectorAll('.tournament-bracket__round');
