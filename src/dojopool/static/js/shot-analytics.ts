...
class ShotAnalytics extends EventEmitter {
    ...
    private calculateAverageConfidence(): number {
        ...
        return this.shots.reduce((sum, shot) => sum + shot.confidence, 0) / this.shots.length;
    }

    private calculateAverageProcessingTime(): number {
        if (this.shots.length === 0) return 0;
        return this.shots.reduce((sum, shot) => sum + shot.processingTime, 0) / this.shots.length;
    }
