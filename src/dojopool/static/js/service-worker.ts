...
async function getPendingActions(): Promise<GameAction[]> {
  ...
  const store = db.transaction('pendingActions', 'readonly').objectStore('pendingActions');
...
async function removePendingAction(actionId: string): Promise<void> {
  const db = await openDB();
  const store = db.transaction('pendingActions', 'readwrite').objectStore('pendingActions');
  return new Promise((resolve, reject) => {
