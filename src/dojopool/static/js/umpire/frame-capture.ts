...
export async function captureFrame(): Promise<ImageData | null> {
    const videoElement = document.getElementById('game-video') as HTMLVideoElement;
    ...
    return captureContext.getImageData(0, 0, captureCanvas.width, captureCanvas.height);
}

export async function validateFrame(imageData: ImageData | null): Promise<ImageData | null> {
    if (!imageData) {
