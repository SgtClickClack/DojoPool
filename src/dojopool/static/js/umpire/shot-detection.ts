...
import { DOMElements, showSuccess, updateGameStats } from './ui';

export function handleShotDetection(
    domElements: DOMElements,
    data: { position: { x: number; y: number }, confidence?: number; timestamp: number }
): void {
...
export function updateMonitoringStatus(state: UmpireState, data: { stats: any }): void {
    state.stats = {
        ...state.stats,
        ...data.stats,
    };
