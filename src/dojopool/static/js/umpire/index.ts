...
async function initializeSocket(): Promise<void> {
    try {
        ...
        socket.on('connect_error', (error) => handlers.handleConnectionError(umpireState, error));
        socket.on('disconnect', (reason) => handlers.handleDisconnect(umpireState, socket, reason));
        socket.on('error', (error) => handlers.handleSocketError(umpireState, error));
        socket.on('reconnect_attempt', handlers.handleReconnectAttempt);
        socket.on('reconnect_failed', () => handlers.handleReconnectFailed(umpireState));
    } catch (error) {
