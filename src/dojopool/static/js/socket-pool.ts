...
class SocketPool {
  ...
  async getSocket(): Promise<Socket> {
    ...
    const availableSocket = this.pool
      .filter(
        (socket) =>
          socket.isAvailable && socket.connected && !socket.isProcessing,
      )
      .sort(
        (a, b) =>
          a.messageQueue.length +
          a.reconnectAttempts -
          (b.messageQueue.length + b.reconnectAttempts),
      )[0];
  ...
  private async waitForAvailableSocket(): Promise<Socket> {
    ...
    while (attempt < this.backoffTimes.length) {
      await new Promise((resolve) =>
        setTimeout(resolve, this.backoffTimes[attempt]),
      );

      const availableSocket = this.pool.find(
        (s) => s.isAvailable && s.connected && !s.isProcessing,
      );
      if (availableSocket) {
        availableSocket.isAvailable = false;
        availableSocket.lastUsed = Date.now();
  ...
  private createSocket(): PooledSocket {
    const socket = io('ws://localhost:3102', this.options) as PooledSocket;
