...
class AlertSystem {
  ...
  loadThresholds() {
    const saved: any = localStorage.getItem('alertThresholds');
    if (saved) {
      try {
    ...
    switch (type) {
      case 'loadTime':
        if (this.thresholds.loadTime.enabled) {
          const duration: any = data.duration;
      ...
      case 'memory':
        if (this.thresholds.memory.enabled) {
          const usagePercent: any = (data.usage / data.limit) * 100;
  ...
  acknowledgeAlert(alertId) {
    const alert: any = this.alerts.find((a) => a.id === alertId);
    if (alert) {
  ...
  notify(alert) {
    if (this.notificationPermission) {
      const notification: any = new Notification(`${alert.category} Alert`, {
        body: alert.message,
...
const alertSystem: any = new AlertSystem();
