...
// Type imports
// TODO: Import types from ./alertSystem.js
...
class AlertConfig {
    ...
    createConfigPanel() {
        ...
        // Create tabs
        const tabs: any = document.createElement('div');
    ...
    createThresholdConfig() {
        const thresholds: any = alertSystem.thresholds;
    ...
    updateHistoryView() {
        const history: any = alertSystem.getAlertHistory();
        const filter: any = this.container.querySelector('.history-filter').value;
        const listContainer: any = this.container.querySelector('.history-list');

        const filteredHistory: any = filter === 'all' ?
            history :
    ...
    updateThresholdInputs() {
        const thresholds: any = alertSystem.thresholds;
        for (const [category, config] of Object.entries(thresholds)) {
            const toggle: any = this.container.querySelector(
                `.threshold-toggle[data-category="${category}"]`
            );
            const warningInput: any = this.container.querySelector(
                `.threshold-input[data-category="${category}"][data-level="warning"]`
            );
            const errorInput: any = this.container.querySelector(
...
const alertConfig: any = new AlertConfig();
