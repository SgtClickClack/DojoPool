...
import { ShotData } from './shot-analytics';
...
class ShotPredictor extends EventEmitter {
    ...
    private async initializeModel(): Promise<void> {
        try {
            ...
            this.model = await tf.loadLayersModel('indexeddb://shot-prediction-model');
    ...
    private preprocessShots(shots: ShotData[]): {
        ...
        for (let i = this.sequenceLength; i < shots.length; i++) {
            ...
            for (let j = i - this.sequenceLength; j < i; j++) {
                const timeDelta = j > 0 ? shots[j].timestamp - shots[j - 1].timestamp : 0;
                sequence.push([
    ...
    public async train(shots: ShotData[]): Promise<void> {
        if (this.isTraining || !this.model || shots.length < this.sequenceLength + 1) {
            return;
        }
    ...
    public async predict(recentShots: ShotData[]): Promise<PredictionResult | null> {
        ...
        const sequence = recentShots.slice(-this.sequenceLength).map(shot => {
            const timeDelta = shot.timestamp - (recentShots[recentShots.length - 2]?.timestamp || 0);
            return [
                shot.position.x / 100,
                shot.position.y / 100,
                shot.confidence,
    ...
    public async evaluateModel(testShots: ShotData[]): Promise<{
        ...
        const evaluation = await this.model.evaluate(sequences, targets) as tf.Scalar[];
        ...
        for (let i = 0; i < testSequences.length; i++) {
            const prediction = await this.predict(testShots.slice(i, i + this.sequenceLength));
            if (prediction) {
