...
class MemoryPool {
    ...
    initialize(frameSize: number): void {
        ...
        const requiredFloat64Size = Math.ceil(requiredUint32Size / PROCESSING_CHUNK_SIZE);
    ...
    release(buffers: { uint32View: Uint32Array; brightnessSums: Float64Array }): void {
...
async function processFrame(imageData: ImageData): Promise<Partial<ProcessedResult>> {
...
function processChunksSequential(uint32View: Uint32Array, brightnessSums: Float64Array, chunks: number, width: number): void {
    let maxBrightness = 0;
    let maxBrightnessX = 0;
    let maxBrightnessY = 0;
...
async function processChunksParallel(uint32View: Uint32Array, brightnessSums: Float64Array, chunks: number, width: number): Promise<void> {
    const chunkPromises = [];
    const chunkSize = Math.ceil(chunks / 4); // Process in 4 parallel chunks

    for (let i = 0; i < 4; i++) {
        const start = i * chunkSize * PROCESSING_CHUNK_SIZE;
        const end = Math.min(start + chunkSize * PROCESSING_CHUNK_SIZE, uint32View.length);
        chunkPromises.push(processChunkRange(uint32View, brightnessSums, start, end, width));
    }

    await Promise.all(chunkPromises);
}

function processChunk(uint32View: Uint32Array, brightnessSums: Float64Array, chunkIndex: number, start: number, end: number, width: number): void {
...
async function processChunkRange(uint32View: Uint32Array, brightnessSums: Float64Array, start: number, end: number, width: number): Promise<void> {
    ...
    for (let i = 0; i < chunks; i++) {
        ...
        processChunk(uint32View, brightnessSums, Math.floor(chunkStart / PROCESSING_CHUNK_SIZE), chunkStart, chunkEnd, width);
    }
}

function calculateResults(uint32View: Uint32Array, brightnessSums: Float64Array, width: number): Partial<ProcessedResult> {
    const avgBrightness = brightnessSums.reduce((a, b) => a + b, 0) / uint32View.length;
    ...
    return {
        ...
        confidence: shotDetected ? (maxBrightness - BRIGHTNESS_THRESHOLD) / MAX_CONFIDENCE : 0
    };
} 