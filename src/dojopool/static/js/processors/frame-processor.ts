...
export class FrameProcessor {
  ...
  private createBufferPair(): {
    ...
    if (!frameBuffer || !texture) {
      throw this.createError(
        "BUFFER",
        "Failed to create frame buffer or texture",
        true,
      );
    }
  ...
  public processFrame(videoElement: HTMLVideoElement): ImageData | null {
    ...
    this.gl.bindTexture(
      this.gl.TEXTURE_2D,
      this.resources.textures[this.currentTextureIndex],
    );
    this.gl.texImage2D(
      this.gl.TEXTURE_2D,
      0,
    ...
    this.gl.bindFramebuffer(
      this.gl.FRAMEBUFFER,
      this.resources.frameBuffers[nextIndex],
    );
