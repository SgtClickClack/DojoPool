...
describe('FrameProcessor', () => {
    ...
    describe('Initialization', () => {
        ...
        it('should initialize shaders', () => {
            ...
            expect(shaderManager.registerShader).toHaveBeenCalledWith('shotDetection', expect.any(Object));
        });

        it('should set up event listeners', () => {
            const contextManager = WebGLContextManager.getInstance();
            expect(contextManager.addContextListener).toHaveBeenCalled();
            expect(window.addEventListener).toHaveBeenCalledWith('qualitychange', expect.any(Function));
        });
    });
    ...
    describe('Error Handling', () => {
        it('should handle WebGL context loss', () => {
            const contextManager = WebGLContextManager.getInstance();
            const contextListener = (contextManager.addContextListener as jest.Mock).mock.calls[0][0];
        ...
        it('should handle quality change events', () => {
            ...
            const qualityChangeHandler = (window.addEventListener as jest.Mock).mock.calls.find(
                call => call[0] === 'qualitychange'
            )?.[1];
    ...
    describe('Resource Management', () => {
        it('should clean up resources', () => {
            ...
            expect(window.removeEventListener).toHaveBeenCalledWith('qualitychange', expect.any(Function));
        });
