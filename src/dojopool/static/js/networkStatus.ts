...
class NetworkStatus {
  ...
  handleConnectionChange() {
    const oldQuality: any = this.status.quality;
    this.updateStatus();
  ...
  pauseNonCriticalRequests() {
    ...
    window.dispatchEvent(
      new CustomEvent("pauseNonCritical", {
  ...
  updateStatus() {
    const oldStatus: any = { ...this.status };
  ...
  detectConnectionQuality() {
    ...
    if ("performance" in window && "memory" in window.performance) {
      ...
      if (memory.jsHeapSizeLimit < 2147483648) {
        // Less than 2GB
  ...
  notifyStatusChange(reason) {
    window.dispatchEvent(
      new CustomEvent("networkQualityChange", {
        detail: {
          ...this.status,
          reason,
          timestamp: Date.now(),
...
const networkStatus: any = new NetworkStatus();
