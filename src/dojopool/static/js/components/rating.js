class Rating {
    ...
    async submitRating() {
        try {
            const response = await fetch(`/api/v1/ratings/${this.options.targetType}/${this.options.targetId}`, {
                method: 'POST',
                headers: {
    ...
    updateReviewsList(ratings) {
        ...
        if (ratings.length === 0) {
            this.reviewsContainer.innerHTML = '<p class="text-muted">No reviews yet</p>';
            return;
        }
        
        this.reviewsContainer.innerHTML = ratings.map(rating => `
        ...
        this.reviewsContainer.querySelectorAll('.delete-review').forEach(btn => {
            btn.addEventListener('click', () => this.deleteRating(btn.dataset.ratingId));
        });
    }
