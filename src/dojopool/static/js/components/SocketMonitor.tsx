...
import { ConnectionState, type ConnectionStatus } from '../socket_types';
    ...
    <div className="p-4 bg-white rounded-lg shadow-md">
      ...
      <div className="mb-4">
        ...
        <div className="flex items-center">
          <div
            className={`w-3 h-3 rounded-full mr-2 ${getStatusColor(status.state)}`}
          />
          <span>{status.state}</span>
        </div>
        {status.lastError && (
          <div className="text-red-500 mt-1">Error: {status.lastError}</div>
        )}
