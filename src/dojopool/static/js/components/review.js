class ReviewComponent {
    ...
    createElements() {
        ...
        this.responseInput = this.container.querySelector('.response-form textarea');
        this.submitResponseButton = this.container.querySelector('.submit-response');
    }
    ...
    bindEvents() {
        ...
        if (this.options.showVoting) {
            this.helpfulButton?.addEventListener('click', () => this.voteReview(true));
            this.unhelpfulButton?.addEventListener('click', () => this.voteReview(false));
        }
        
        this.reportButton?.addEventListener('click', () => this.reportReview());
        
        if (this.options.showResponses) {
            this.submitResponseButton?.addEventListener('click', () => this.submitResponse());
        }
    }
    ...
    async voteReview(isHelpful) {
        try {
            const response = await fetch(`/api/v1/reviews/${this.options.ratingId}/vote`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    async reportReview() {
        ...
        try {
            const response = await fetch(`/api/v1/reviews/${this.options.ratingId}/report`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    async submitResponse() {
        try {
            ...
            const response = await fetch(`/api/v1/reviews/${this.options.ratingId}/responses`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    displayReview(data) {
        ...
        this.reviewDate.textContent = new Date(data.created_at).toLocaleDateString();
    ...
    displayResponses(responses) {
        ...
        this.responsesList.innerHTML = responses.map(response => `
