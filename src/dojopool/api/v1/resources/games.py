...
class GameResource(BaseResource):
    ...
    def get(self, game_id):
        ...
        if not (current_user.has_role('admin') or
                current_user.id in [game.player1.user_id, game.player2.user_id]):
...
class GameListResource(BaseResource):
    ...
    def get(self):
        ...
        if player_id:
            query = query.filter(
                (Game.player1.has(user_id=player_id)) |
                (Game.player2.has(user_id=player_id))
            )
...
class GameStatsResource(BaseResource):
    ...
    def get(self, game_id: Optional[int] = None):
        ...
        games = Game.query.filter(
            (Game.player1.has(user_id=user_id)) |
            (Game.player2.has(user_id=user_id))
        ).all()
