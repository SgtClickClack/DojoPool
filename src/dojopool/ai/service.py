...
from typing import Dict, List, Any, Optional, TypedDict, Tuple, cast, Union
...
from ..models import Match, Game, Event, Shot, User
...
class MatchAnalyzer:
    ...
    def _handle_cache(self, match_id: int, analysis: Optional[MatchAnalysis] = None) -> Optional[MatchAnalysis]:
        """
    ...
    def _evaluate_event_impact(self, event_type: str, status: str, time_factor: float = 0.5) -> EventImpact:
        """
    ...
    def _analyze_performance(self, match: Match, shots: List[Shot], events: List[Event], player_id: Optional[int] = None) -> AnalysisMetrics:
        """
        ...
        if pressure_events:
            ...
            early_success = sum(1 for e in pressure_events[:len(pressure_events)//2] 
                              if e.status == 'success')
            late_success = sum(1 for e in pressure_events[len(pressure_events)//2:]
                             if e.status == 'success')
            adaptation = ((late_success / (len(pressure_events)//2)) - 
                         (early_success / (len(pressure_events)//2))) * 100
        else:
            success_rate = 0.0
            adaptation = 0.0
    ...
    async def _load_match_data(self, match_id: int) -> Tuple[Match, User, User, List[Event], List[Shot], Dict[str, Any], Dict[str, Any]]:
        """
    ...
    async def analyze_match(self, match_id: int) -> MatchAnalysis:
        ...
        try:
            ...
            # Load all match data
            match, player1, player2, events, shots, player1_levels, player2_levels = await self._load_match_data(match_id)
            
            # Perform various analyses
            key_moments = self._analyze_key_moments(match, events)
            performance_stats = self._analyze_performance(match, shots, events)  # Overall match stats
            tactical_analysis = cast(TacticalAnalysis, self._analyze_tactics(match, shots, events, player1_levels, player2_levels))
            improvement_areas = await self._identify_improvement_areas(match, shots, events, player1_levels, player2_levels)
            
            # Generate match summary and score progression
            match_summary, score_progression = await self._generate_match_summary(match, player1, player2, events)
            
            # Generate player insights using the same performance analysis method
            player1_insights = cast(AnalysisMetrics, self._analyze_performance(match, shots, events, match.player1_id))
            player2_insights = cast(AnalysisMetrics, self._analyze_performance(match, shots, events, match.player2_id))
    ...
    def _analyze_tactics(self, match: Match, shots: List[Shot], events: List[Event], player1_levels: Dict[str, Any], player2_levels: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if not shots or not events:
            return {
                ...
                'strategic_decisions': {'offensive_plays': 0.0, 'defensive_plays': 0.0, 'risk_taking': 0.0},
                'adaptations': {'style_changes': 0.0, 'response_to_pressure': 50.0, 'pattern_recognition': 50.0},
                'player_matchup': {
                    'style_clash': {'compatibility': 50.0, 'advantage': 50.0, 'dynamic_shifts': 0.0},
                    'strength_exploitation': {'targeting_weaknesses': 50.0, 'leveraging_strengths': 50.0, 'adaptation_rate': 0.0}
                }
            }
        ...
        style_change = abs(late_patterns['style']['aggressive'] - early_patterns['style']['aggressive']) / 100.0
        
        # Calculate pressure adaptation
        pressure_adaptation = 0.0
        if pressure_events:
            early_success_rate = sum(1 for e in pressure_events[:len(pressure_events)//2] if e.status == 'success') / (len(pressure_events)//2 or 1)
            late_success_rate = sum(1 for e in pressure_events[len(pressure_events)//2:] if e.status == 'success') / ((len(pressure_events) - len(pressure_events)//2) or 1)
        ...
        adaptations = {
            ...
            'pattern_recognition': round(late_patterns['style']['balanced'] - early_patterns['style']['balanced'] + 50, 2)
        }
        ...
        style_compatibility = min(
            ...
            abs(get_level(player1_levels, 'consistency') - get_level(player2_levels, 'consistency'))
        )
        
        player_matchup = {
            'style_clash': {
                'compatibility': round((1 - style_compatibility) * 100, 2),
                'advantage': round((get_level(player1_levels, 'skill') - get_level(player2_levels, 'skill') + 1) * 50, 2),
                'dynamic_shifts': round(style_change * 100, 2)
            },
            'strength_exploitation': {
                'targeting_weaknesses': round((1 - min(get_level(player1_levels, 'defense'), get_level(player2_levels, 'defense'))) * 100, 2),
                'leveraging_strengths': round(max(get_level(player1_levels, 'offense'), get_level(player2_levels, 'offense')) * 100, 2),
    ...
    async def _identify_improvement_areas(self, match: Match, shots: List[Shot], events: List[Event], player1_levels: Dict[str, Any], player2_levels: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        try:
            ...
            match_data = {
                ...
                'player_levels': {
                    'player1': player1_levels,
                    'player2': player2_levels
                }
            }
    ...
    def _process_match_events(self, events: List[Event], match: Match) -> Tuple[List[Dict[str, float]], List[str]]:
        """
        ...
        # Add final score
        final_score = match.score if isinstance(match.score, dict) else {'player1': 0.0, 'player2': 0.0}
        score_progression.append({
            'time': match_duration,
    ...
    async def _generate_match_summary(self, match: Match, player1: User, player2: User, events: List[Event]) -> Tuple[str, List[Dict[str, float]]]:
        """
        ...
        return summary or f"Match between {player1.username} and {player2.username}", score_progression