...
from dojopool.core.models import MediaFile, User
...
def get_file(media_id: str):
    ...
    return jsonify({
        ...
        'thumbnails': [{
            ...
            'url': storage_client.get_url(f"thumbnails/{size[0]}x{size[1]}/{media_file.id}.jpg")
        } for size in THUMBNAIL_SIZES] if media_file.content_type.startswith('image/') else None
    }) 