...
export const usePerformanceMonitor = () => {
    ...
    const applyOptimization = useCallback(async (recommendationId: string) => {
        try {
            ...
            setError(err instanceof Error ? err.message : 'An error occurred while applying optimization');
            setLoading(false);
        }
    }, []);
    ...
    const acknowledgeRecommendation = useCallback(async (recommendationId: string) => {
        try {
            ...
            setError(err instanceof Error ? err.message : 'An error occurred while acknowledging recommendation');
        }
    }, []);
