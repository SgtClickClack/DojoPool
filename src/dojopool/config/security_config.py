...
class SecurityConfig(BaseModel):
    ...
    TOKEN_KEYS_DIR: str = Field(
        "instance/keys", description="Directory to store token encryption keys"
    )
    TOKEN_KEY_ROTATION_INTERVAL: timedelta = Field(
        timedelta(days=1), description="How often to rotate token encryption keys"
    )

    # Token Expiration Times
    ACCESS_TOKEN_EXPIRY: timedelta = Field(
        timedelta(minutes=15), description="Access token expiration time"
    )
    REFRESH_TOKEN_EXPIRY: timedelta = Field(
        timedelta(days=7), description="Refresh token expiration time"
    )

    # Project Paths
    PROJECT_ROOT: str = Field("src/dojopool", description="Root path of the project")
    ...
    SESSION_COOKIE_SAMESITE: str = Field(
        "Lax", description="SameSite session cookie policy"
    )
    PERMANENT_SESSION_LIFETIME: timedelta = Field(
        timedelta(days=1), description="Session lifetime"
    )
    ...
    VULNERABILITY_SCAN_PATHS: list[str] = Field(
        ["src", "tests"], description="Paths to scan for vulnerabilities"
    )
    ...
    SEVERITY_THRESHOLDS: Dict[str, Dict[str, int]] = Field(
        {
            ...
            "code": {"critical": 0, "high": 0, "moderate": 2, "low": 3},
            "configuration": {"critical": 0, "high": 0, "moderate": 1, "low": 2},
        },
    ...
    MAX_SCANS_PER_HOUR: int = Field(
        4, description="Maximum number of vulnerability scans per hour"
    )
    SCAN_COOLDOWN_MINUTES: int = Field(
        15, description="Minutes to wait between vulnerability scans"
    )
