...
function createInfoWindowContent(place, nameLower) {
  const venueType = nameLower.includes('pool') || nameLower.includes('snooker') || nameLower.includes('billiard')
...
function checkVenueUnlock(venue, playerPosition) {
  ...
  if (distance <= VenueProgression.UNLOCK_RADIUS) {
    ...
    const nearbyVenues = getNearbyVenues(venue.geometry.location, VenueProgression.INFLUENCE_RADIUS);
...
function checkUnlockRequirements(venue, nearbyVenues) {
  ...
  const capturedNearby = nearbyVenues.filter(v => v.status === VenueStatus.OWNED).length;
...
function updateCaptureProgress(venue, timeSpent) {
  const progress = Math.min((timeSpent / VenueProgression.MIN_VISIT_TIME) * 100, 100);
...
function generateEventIndicator(event) {
  const timeLeft = Math.max(0, (event.startTime + event.duration - Date.now()) / 1000);
...
function generateTerritoryBoundary(territory) {
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    const lat = territory.center.lat() + (territory.radius / 111300) * Math.cos(angle);
    const lng = territory.center.lng() + (territory.radius / (111300 * Math.cos(territory.center.lat() * Math.PI / 180))) * Math.sin(angle);
    points.push({ lat, lng });
  }
...
function showTerritoryInfo(territory) {
  const venue = markers.find(m => m.venue.place_id === territory.venueId)?.venue;
  if (!venue) return;
