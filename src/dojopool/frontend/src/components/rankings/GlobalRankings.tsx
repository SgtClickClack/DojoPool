...
import {
  ...
  Paper, Typography, Box, Avatar, Chip, IconButton, useTheme,
  Pagination, CircularProgress, Alert
} from '@mui/material';
import {
  EmojiEvents, Star, TrendingUp, TrendingDown, Remove,
  ArrowUpward, ArrowDownward, Timeline, Info, Sync
...
const GlobalRankings: React.FC = () => {
  ...
  const { data: rankings, isLoading, error } = useQuery(
    ...
    async () => {
      ...
      const response = await axios.get(`/api/rankings/global?start_rank=${start}&end_rank=${end}`);
          ...
          <TableBody>
            {rankings?.map((player: RankingData) => (
              ...
              >
                ...
                <TableCell align="center">
                  <Typography
                    color={player.rank_movement > 0 ? 'success.main' : player.rank_movement < 0 ? 'error.main' : 'inherit'}
                  >
                    {player.rank_movement > 0 ? '+' : ''}{player.rank_movement}
                  </Typography>
                </TableCell>
                <TableCell align="center">
                  {getStreakIndicator(player.rank_streak, player.rank_streak_type)}
                </TableCell>
                <TableCell align="center">
                  <IconButton
                    size="small"
