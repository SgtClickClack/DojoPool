...
const DojoDiscovery: React.FC = () => {
  ...
  const fetchRegionalLeaderboard = async (lat: number, lng: number) => {
    try {
      const response = await fetch(`/api/geo/leaderboard/regional?lat=${lat}&lng=${lng}`);
    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
                ...
                <StyledPaper>
                  ...
                  <Box sx={{ mb: 2 }}>
                    <Chip
                      ...
                      sx={{ mr: 1 }}
                    />
                    <Chip label={`${selectedDojo.distance} km away`} variant="outlined" />
                  </Box>
                  <Button
                    variant="contained"
                    fullWidth
      ...
      >
          ...
          {challenge && (
            <>
              ...
              <Typography variant="body2" paragraph>
                ...
                <br />• Complete within {challenge.requirements.time_limit_days} days
              </Typography>
              <Typography variant="subtitle2" gutterBottom>
                Rewards:
              </Typography>
              <Typography variant="body2">
                ...
                <br />• Title: {challenge.rewards.title}
              </Typography>
            </>
          )}
        </DialogContent>
        <DialogActions>
          ...
          <Button variant="contained" onClick={() => setShowChallengeDialog(false)}>
            Accept Challenge
          </Button>
        </DialogActions>
      </Dialog>
