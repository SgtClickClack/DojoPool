...
export class DataCompressor {
    ...
    private archives: Map<string, { data: Uint8Array; metadata: ArchiveMetadata }> = new Map();
    ...
    async archiveData(key: string, data: { data: any; metadata: ArchiveMetadata }): Promise<void> {
        const compressedData = await this.compressData(data.data);
        this.archives.set(key, { data: compressedData, metadata: data.metadata });
    }

    async retrieveArchive(key: string): Promise<{ data: any; metadata: ArchiveMetadata } | null> {
    ...
    getArchiveStats(): { totalSize: number; oldestDate: Date; compressionRatio: number } {
        let totalSize = 0;
    ...
    shouldArchive(dataSize: number): boolean {
        const totalSize = Array.from(this.archives.values())
            .reduce((sum, archive) => sum + archive.data.length, 0);
