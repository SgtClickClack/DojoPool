...
export class GameML {
  ...
  private async initializeModels(): Promise<void> {
    try {
      ...
      this.shotModel = await tf.loadLayersModel('indexeddb://dojopool-shot-model');
      this.skillModel = await tf.loadLayersModel('indexeddb://dojopool-skill-model');
  ...
  public async assessPlayerSkill(playerId: string): Promise<{
    ...
    const avgMetrics = playerMetrics.reduce((acc, curr) => ({
      accuracyRate: acc.accuracyRate + curr.accuracyRate,
      averageShotDifficulty: acc.averageShotDifficulty + curr.averageShotDifficulty,
  ...
  private async trainShotModel(): Promise<tf.History> {
    ...
    const xs = tf.tensor2d(this.shotData.map(shot => [
      shot.angle / 360,
      shot.force / 100,
      (shot.spin + 1) / 2,
      shot.distance / 100,
    ...
    const ys = tf.tensor2d(this.shotData.map(shot => [
      shot.success ? 1 : 0,
      shot.difficulty,
      shot.force / 100,
      (shot.spin + 1) / 2
  ...
  private async trainSkillModel(): Promise<tf.History> {
    ...
    const xs = tf.tensor2d(allSkillData.map(data => [
      data.accuracyRate,
    ...
    const ys = tf.tensor2d(allSkillData.map(data => [
  ...
  public getModelSummaries(): {
    ...
    if (this.shotModel) {
      this.shotModel.summary(undefined, undefined, line => shotSummary.push(line));
    }
    if (this.skillModel) {
      this.skillModel.summary(undefined, undefined, line => skillSummary.push(line));
    }
