...
export class PatternRecognition {
  ...
  private analyzePlayStyle(patterns: PatternData[]): PlayStyle {
    ...
    const aggression = patterns.reduce((sum, p) => 
      sum + (p.difficulty * (p.shotType.includes('bank') ? 1.2 : 1)), 0) / patterns.length;

    // Calculate consistency based on success rate
    const successRate = patterns.filter(p => p.outcome === 'success').length / patterns.length;
    const consistency = successRate * 0.7 + this.calculatePositionalConsistency(patterns) * 0.3;

    // Calculate complexity based on shot types
    const complexity = patterns.reduce((sum, p) => 
      sum + (this.getShotComplexity(p.shotType)), 0) / patterns.length;
  ...
  private analyzeStrengthsWeaknesses(patterns: PatternData[]): {
    ...
    const shotTypeStats = new Map<string, { attempts: number; success: number }>();
    
    patterns.forEach(pattern => {
      const stats = shotTypeStats.get(pattern.shotType) || { attempts: 0, success: 0 };
      stats.attempts++;
    ...
    shotTypeStats.forEach((stats, shotType) => {
      ...
      if (stats.attempts >= 5) { // Only consider shots with enough attempts
        if (successRate >= 0.7) {
          strengths.push(`${shotType} shots (${(successRate * 100).toFixed(1)}% success)`);
        } else if (successRate <= 0.4) {
          weaknesses.push(`${shotType} shots (${(successRate * 100).toFixed(1)}% success)`);
        }
      }
    });
  ...
  private async generateRecommendations(
    ...
    if (playStyle.aggression > 0.8) {
      recommendations.push('Consider incorporating more safety plays in your strategy');
    } else if (playStyle.aggression < 0.3) {
      recommendations.push('Look for opportunities to take calculated aggressive shots');
    }
  ...
  private calculatePositionalConsistency(patterns: PatternData[]): number {
    ...
    for (let i = 0; i < patterns.length - 1; i++) {
      ...
      if (currentShot.outcome === 'success') {
        ...
        const positionQuality = this.evaluatePosition(currentShot.position, nextShot.position);
  ...
  private evaluatePosition(currentPos: { x: number; y: number }, nextPos: { x: number; y: number }): number {
    ...
    const distanceScore = Math.min(1, Math.max(0, 1 - Math.abs(distance - 20) / 40));

    // Consider table position (center of table is generally better)
    const centerScore = 1 - (Math.abs(nextPos.x - 50) / 50 + Math.abs(nextPos.y - 50) / 50) / 2;
