export class GameMetricsMonitor {
    ...
    recordGameCompletion(playerId: string, score: number, time: number) {
        ...
        this.metrics.averageScore = (this.metrics.averageScore * (this.metrics.completions - 1) + score) / this.metrics.completions;
        this.metrics.averageTime = (this.metrics.averageTime * (this.metrics.completions - 1) + time) / this.metrics.completions;
    }
    ...
    async getHistoricalMetrics(startDate: Date, endDate: Date): Promise<GameMetrics[]> {
