...
export class OptimizationEngine {
  ...
  public async generateSuggestions(
    ...
    const difficultyGap = Math.abs(metrics.averageDifficulty - skillAssessment.recommendedDifficulty);
    if (difficultyGap > 0.2) {
      suggestions.push({
        type: 'strategy',
        priority: 7,
        title: 'Adjust Shot Selection',
        description: metrics.averageDifficulty > skillAssessment.recommendedDifficulty
