...
const VenueDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setDeals([
      { id: "1", title: "Happy Hour", description: "50% off tables 5-7pm", active: true },
      { id: "2", title: "Ladies Night", description: "Free entry for women on Thursdays", active: false },
    ]);
    setAnalytics({
      occupancyRate: 0.67,
      revenueToday: 320,
    ...
    <Box sx={{ p: 2 }}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        ...
        <Box display="flex" gap={1}>
          <Button variant="contained" startIcon={<Add />} onClick={handleCreateDeal}>
            Add Deal
          </Button>
            ...
            {analytics && (
              <>
                <Box display="flex" alignItems="center" gap={1} mb={1}>
                  ...
                  <Typography variant="body2">Occupancy Rate: {(analytics.occupancyRate * 100).toFixed(0)}%</Typography>
                </Box>
                <Box display="flex" alignItems="center" gap={1} mb={1}>
                  <BarChart fontSize="small" />
                  <Typography variant="body2">Revenue Today: ${analytics.revenueToday}</Typography>
                </Box>
                <Box display="flex" alignItems="center" gap={1} mb={1}>
                  <TableBar fontSize="small" />
                  <Typography variant="body2">Games Played: {analytics.gamesPlayed}</Typography>
                </Box>
                <Box display="flex" alignItems="center" gap={1}>
                  <Schedule fontSize="small" />
                  <Typography variant="body2">Avg. Session: {analytics.avgSessionTime} min</Typography>
                </Box>
              </>
            )}
          </Paper>
      ...
      {/* Create Deal Dialog */}
      <Dialog open={showCreateDeal} onClose={() => setShowCreateDeal(false)} maxWidth="sm" fullWidth>
