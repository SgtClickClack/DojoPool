...
const CreateExperiment: React.FC<CreateExperimentProps> = ({
  onExperimentCreated,
}) => {
  ...
  const validateForm = () => {
    if (!id || !name || !description || !startDate || variants.length < 2) {
      setError(
        "Please fill in all required fields and add at least one variant besides control",
      );
      return false;
    }
