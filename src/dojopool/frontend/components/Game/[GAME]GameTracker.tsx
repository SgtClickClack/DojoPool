...
export const GameTracker: React.FC<GameTrackerProps> = ({ gameId, player1Id, player2Id }) => {
    const { gameState, updateBallStatus, endTurn, endGame } = useGameState(gameId);
                        ...
                        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                            {Array.from({ length: 15 }, (_, i) => i + 1).map((ball) => (
                                <Button
                                    ...
                                    disabled={gameState?.ballStatuses[ball] === BallStatus.POCKETED}
                                >
                                    {ball}
                                </Button>
                            ))}
