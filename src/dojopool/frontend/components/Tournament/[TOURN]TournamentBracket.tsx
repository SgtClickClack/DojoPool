...
export const TournamentBracket: React.FC<TournamentBracketProps> = ({
  ...
  const loserMatchesByRound = tournament.loserRounds?.reduce(
    (acc, round) => {
      acc[round.roundNumber] = round.matches;
      return acc;
    },
      ...
      return (
        <Box>
          ...
          <Box sx={{ display: "flex", gap: 6, overflowX: "auto", p: 2 }}>
            {/* Winner Bracket */}
            <Box>
              <Typography variant="subtitle1" align="center">Winners Bracket</Typography>
            ...
            {/* Loser Bracket */}
            <Box>
              <Typography variant="subtitle1" align="center">Losers Bracket</Typography>
              {Object.entries(loserMatchesByRound).map(([roundNumber, matches]) => (
                <Box key={roundNumber} sx={{ mb: 2 }}>
                  <Typography variant="subtitle2">Round {roundNumber}</Typography>
                  {matches.map((match) => (
                    <MatchBox key={match.id} match={match} players={playerLookup} />
        ...
        <Box>
          ...
          {swissRounds.map((round, idx) => (
            <Box key={round.roundNumber || idx} sx={{ mb: 2 }}>
              <Typography variant="subtitle2">Round {round.roundNumber}</Typography>
