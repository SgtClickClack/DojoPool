...
export const performanceHelpers = {
  ...
  monitorMemoryUsage(operation: () => void) {
    cy.monitorMemory(
      operation,
      performanceThresholds.memory.maxSessionIncrease
    );
  },
  ...
  monitorResourceUsage() {
    cy.window().then((win) => {
      ...
      expect(cpuUsage).to.be.lessThan(
        performanceThresholds.resources.maxCPUUsage
      );

      // @ts-ignore
      const memoryUsage =
        win.performance?.memory?.usedJSHeapSize / (1024 * 1024);
      expect(memoryUsage).to.be.lessThan(
        performanceThresholds.resources.maxRAMUsage
      );
    });
  },
  ...
  monitorAPIPerformance(endpoint: string) {
    ...
    cy.wait('@apiRequest').then(() => {
      ...
      expect(responseTime).to.be.lessThan(
        performanceThresholds.api.maxResponseTime
      );
    });
  },
    ...
    cy.wrap(null).then(() => {
      if (messageCount > 0) {
        ...
        expect(avgLatency).to.be.lessThan(
          performanceThresholds.realtime.maxLatency
        );
      }
    });
  },
  ...
  monitorAssetLoading() {
    ...
    cy.window().then((win) => {
      ...
      expect(maxLoadTime).to.be.lessThan(
        performanceThresholds.assets.maxImageLoadTime
      );
    });
  },
  ...
  monitorVenuePerformance(venueId: string) {
    ...
    cy.window().then((win) => {
      const playerCount = win.document.querySelectorAll(
        '[data-testid="player-avatar"]'
      ).length;
      expect(playerCount).to.be.lessThan(
        performanceThresholds.venue.maxConcurrentPlayers
      );
    });
  },
};
