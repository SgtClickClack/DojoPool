  ...
  describe('Critical Path Performance', () => {
        ...
        cy.findByRole('button', { name: /make shot/i })
          ...
          .then(() => {
            cy.window().then((win) => {
              ...
              win.performance.measure('game-initialization', 'game-init-start', 'game-init-end');
              
              const measure = win.performance.getEntriesByName('game-initialization')[0];
    ...
    it('should maintain shot response time under load', () => {
      cy.createGame(users.validUser.id, users.existingUser.id).then((gameId) => {
        ...
        const shots = Array(10).fill(null).map((_, i) => i + 1);
        ...
        shots.forEach(ball => {
          ...
          cy.window().then((win) => {
            ...
            win.performance.measure(`shot-${ball}`, `shot-${ball}-start`, `shot-${ball}-end`);
            
            const measure = win.performance.getEntriesByName(`shot-${ball}`)[0];
    ...
    it('should implement effective caching', () => {
      ...
      cy.visit('/venues').then(() => {
        ...
        cy.window().then((win) => {
          ...
          win.performance.measure('cached-load', 'cache-visit-start', 'cache-visit-end');
