  ...
  describe('Error Recovery', () => {
    ...
    describe('State Recovery', () => {
      it('should restore game state after crash', () => {
        cy.createGame(users.validUser.id, users.existingUser.id).then((gameId) => {
          ...
          // Simulate crash
          cy.window().then((win) => {
            win.localStorage.setItem('lastGameState', JSON.stringify({
              gameId,
              shots: [1, 2],
    ...
    describe('Form Recovery', () => {
      it('should preserve form data during errors', () => {
        ...
        cy.findByLabelText(/player 2/i)
          .should('have.value', 'opponent@example.com');
        cy.findByLabelText(/venue/i)
          .should('have.value', venues.venues[0].id);
      });
  ...
  describe('Accessibility', () => {
    ...
    describe('Screen Reader Support', () => {
      it('should provide appropriate ARIA labels', () => {
        ...
        cy.findByTestId('shot-controls').within(() => {
          cy.findByRole('button', { name: /1/i })
            .should('have.attr', 'aria-label', 'Shot ball 1');
        });

        // Check turn indicator
        cy.findByTestId('turn-indicator')
          .should('have.attr', 'aria-label', /Current turn/i);
      });
      ...
      it('should announce dynamic updates', () => {
        ...
        // Verify announcement
        cy.findByRole('alert')
          .should('have.text', /Shot successful/i);
        ...
        cy.findByRole('alert')
          .should('have.text', /Turn changed/i);
      });
    });
