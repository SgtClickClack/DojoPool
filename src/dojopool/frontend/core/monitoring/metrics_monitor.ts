...
class GameMetricsMonitor {
  ...
  public recordGameCompletion(
    ...
    metrics.completionRate =
      metrics.totalGamesCompleted / Math.max(metrics.activeGames, 1);
    metrics.averageCompletionTime =
      (metrics.averageCompletionTime * (metrics.totalGamesCompleted - 1) +
        time) /
      metrics.totalGamesCompleted;
  ...
  public recordError(
    ...
    this.addAlert(AlertSeverity.ERROR, `Error in game ${gameId}: ${message}`, {
