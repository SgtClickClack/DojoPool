...
class ABTestingService {
        ...
        try {
            ...
            if (stored) {
                ...
                assignments.forEach((assignment: TestAssignment) => {
                    this.assignments.set(this.getAssignmentKey(assignment.testId, assignment.userId), assignment);
                });
            }
        } catch (error) {
    ...
    private saveAssignments() {
        try {
            localStorage.setItem(this.storageKey, JSON.stringify(Array.from(this.assignments.values())));
        } catch (error) {
    ...
    trackMetric(testId: string, userId: string, metricName: string, value: number) {
        const assignment = this.assignments.get(this.getAssignmentKey(testId, userId));
