...
class ValidationService {
    ...
    private registerDefaultRules(): void {
        ...
        this.registerRule('min', (value, min) => {
            if (!value) return true;
            return Number(value) >= min;
        }, 'This field must be at least {0}');

        // Maximum value validation
        this.registerRule('max', (value, max) => {
            if (!value) return true;
        ...
        this.registerRule('futureDate', (value) => {
            if (!value) return true;
            const date = new Date(value);
        ...
        this.registerRule('pastDate', (value) => {
            if (!value) return true;
            const date = new Date(value);
            return date < new Date();
    ...
    public async validate(schema: ValidationSchema, data: Record<string, any>): Promise<ValidationResult> {
        ...
        for (const [field, rules] of Object.entries(schema)) {
            for (const rule of rules) {
                if (!this.rules.has(rule.type)) {
                    throw new Error(`Unknown validation rule type '${rule.type}' in schema for field '${field}'`);
                }
            }
        }
        return schema;
