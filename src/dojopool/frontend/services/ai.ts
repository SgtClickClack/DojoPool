...
export async function generateTrainingStory(request: StoryGenerationRequest): Promise<StoryResponse> {
    ...
    const response = await api.post('/ai/generate-story', { prompt: sanitizedPrompt });
    return response.data;
}

export async function getPersonalizedRecommendations(request: RecommendationRequest): Promise<TechniqueRecommendation[]> {
...
export async function generateTechniqueDescription(techniqueId: string, skillLevel: string): Promise<{
    description: string;
    keyPoints: string[];
