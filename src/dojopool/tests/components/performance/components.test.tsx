...
describe('SystemStatus', () => {
  ...
  it('should render system metrics correctly', () => {
    ...
    expect(
      screen.getByText(`Process CPU: ${mockStatus.process_metrics.cpu_percent}%`)
    ).toBeInTheDocument();
    expect(
      screen.getByText(`Process Memory: ${mockStatus.process_metrics.memory_percent}%`)
    ).toBeInTheDocument();
    expect(
      screen.getByText(`Open Files: ${mockStatus.process_metrics.open_files}`)
    ).toBeInTheDocument();
    expect(
      screen.getByText(`Active Connections: ${mockStatus.process_metrics.connections}`)
    ).toBeInTheDocument();
  });
});
...
describe('MetricsChart', () => {
  ...
  it('should render chart with data', () => {
    ...
    render(<MetricsChart data={mockData} timeRange="1h" onTimeRangeChange={onTimeRangeChange} />);
