...
from ...models.marketplace import MarketplaceItem, UserInventory, Wallet
...
def test_purchase_flow(test_client, auth_headers, test_user, test_wallet, test_items):
    ...
    # Purchase items
    purchase_data = {"items": [{"id": str(test_items[0]._id), "quantity": 1}]}
    ...
    inventory = response.json
    purchased_item = next(
        (item for item in inventory if item["id"] == str(test_items[0]._id)), None
    )
...
def test_purchase_validation(test_client, auth_headers, test_user, test_wallet, test_items):
    ...
    purchase_data = {"items": [{"id": str(expensive_item._id), "quantity": 10}]}
    response = test_client.post(
        "/api/marketplace/purchase", json=purchase_data, headers=auth_headers
    )
    ...
    purchase_data = {"items": [{"id": str(test_items[0]._id), "quantity": 100}]}
    response = test_client.post(
        "/api/marketplace/purchase", json=purchase_data, headers=auth_headers
    )
    assert response.status_code == 400
