...
class Shot(BaseModel):
    ...
    def get_player_shots(
        cls, player_id: int, time_range: Optional[Dict[str, datetime]] = None
    ) -> List["Shot"]:
        ...
        if time_range:
            query = query.filter(
                cls.timestamp >= time_range.get("start"), cls.timestamp <= time_range.get("end")
            )
    ...
    def get_player_stats(
        ...
        power_ranges = {"soft": (0, 30), "medium": (31, 70), "power": (71, 100)}
        ...
        return {
            ...
            "shot_distribution": {
                name: count / total_shots for name, count in shot_distribution.items()
            },
        }
