...
from .models import TrainingProgram, Exercise, UserProgress
...
class TrainingService:
    ...
    def generate_exercise(self, program_id: int, user_id: int) -> Exercise:
        ...
        ).order_by(desc(UserProgress.completion_date)).first()
    ...
    def _calculate_program_match(self, program: TrainingProgram, improvement_areas: List[Dict]) -> float:
        """Calculate how well a program matches user's improvement areas."""
        match_score = 0
        for area in improvement_areas:
            # Check if program exercises target the improvement area
            relevant_exercises = [e for e in program.exercises if area['aspect'] in e.description.lower()]
            if relevant_exercises:
                match_score += 1 * (1 if area['priority'] == 'high' else 0.7 if area['priority'] == 'medium' else 0.4)
        return match_score / len(improvement_areas) if improvement_areas else 0
        
    def _get_recommendation_reasons(self, program: TrainingProgram, improvement_areas: List[Dict]) -> List[str]:
        """Generate reasons for program recommendation."""
        reasons = []
        for area in improvement_areas:
            relevant_exercises = [e for e in program.exercises if area['aspect'] in e.description.lower()]
            if relevant_exercises:
                reasons.append(f"Addresses {area['aspect']} improvement area with {len(relevant_exercises)} targeted exercises")
        return reasons
        
    def _generate_exercise_params(self, program_difficulty: str, recent_metrics: Optional[Dict]) -> Dict:
