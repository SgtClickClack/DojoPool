...
class ImageCompressionService:
    ...
    def _get_optimal_chunk_size(self) -> int:
        ...
        logger.info(f"Calculated optimal chunk size: {optimal_size} based on {available_memory / (1024*1024):.2f}MB available memory")
        return optimal_size
    ...
    def batch_compress_directory(
        ...
        try:
            ...
            for root, _, files in os.walk(input_dir):
                for file in files:
                    if any(file.lower().endswith(ext) for format_info in self._supported_formats.values() for ext in format_info['extensions']):
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                for i in range(0, len(image_files), chunk_size):
                    ...
                    logger.info(f"Progress: {progress:.1f}% ({i + len(chunk)}/{len(image_files)} images)")
