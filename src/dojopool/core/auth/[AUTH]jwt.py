...
class JWTManager:
    ...
    def create_tokens(self, user_id: int, claims: Dict[str, Any] = None) -> Tuple[str, str]:
        ...
        # Base claims
        base_claims = {"sub": user_id, "iat": now}
    ...
    def _encode(self, claims: Dict[str, Any]) -> str:
        ...
        Returns:
            JWT string
        """
        return jwt.encode(claims, current_app.config["SECRET_KEY"], algorithm=self.algorithm)
    ...
    def _decode(self, token: str) -> Dict[str, Any]:
        ...
        Raises:
            jwt.InvalidTokenError: If token is invalid
        """
        return jwt.decode(token, current_app.config["SECRET_KEY"], algorithms=[self.algorithm])
