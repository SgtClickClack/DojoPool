...
def init_app(app):
    ...
    def handle_dojo_pool_error(error: DojoPoolError) -> Tuple[Response, int]:
        """Handle DojoPool custom errors."""
        log_error(error, level=logging.WARNING if error.status_code < 500 else logging.ERROR)
    ...
    def handle_http_error(error: HTTPException) -> Tuple[Response, int]:
        ...
        log_error(
            error,
            level=logging.WARNING if error.code < 500 else logging.ERROR,
...
def register_blueprint_error_handlers(blueprint):
    ...
    def handle_blueprint_error(error: DojoPoolError) -> Tuple[Response, int]:
