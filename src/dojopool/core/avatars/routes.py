...
def generate_avatar():
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "data": {"avatar": f"data:image/png;base64,{avatar_base64}"}}
        )
...
def update_avatar():
    ...
    try:
        ...
        if "avatar" not in data:
            return jsonify({"status": "error", "message": "No avatar data provided"}), 400

        # Decode base64 avatar
        try:
            avatar_data = base64.b64decode(data["avatar"].split(",")[1])
        except:
            return jsonify({"status": "error", "message": "Invalid avatar data format"}), 400
...
def animate_avatar():
    ...
    try:
        ...
        if animation_type not in animation_manager.available_animations:
            return jsonify({"status": "error", "message": "Invalid animation type"}), 400
        ...
        animation = animation_manager.create_animation(
            avatar_image, animation_type, data.get("options", {})
        )
