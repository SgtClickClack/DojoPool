...
from .models import Venue, PoolTable, VenueRating
...
class VenueService:
    ...
    def update_venue(self, venue_id: int, venue_data: Dict) -> Optional[Venue]:
        ...
        try:
            ...
            if any(venue_data.get(field) != getattr(venue, field)
                   for field in ['address', 'city', 'state', 'country']):
                address = f"{venue_data.get('address', venue.address)}, " \
        ...
        try:
            ...
            if filters:
                if filters.get('has_parking'):
                    query = query.filter(Venue.has_parking == True)
                if filters.get('is_accessible'):
                    query = query.filter(Venue.is_accessible == True)
                if filters.get('has_food'):
                    query = query.filter(Venue.has_food_service == True)
                if filters.get('has_bar'):
                    query = query.filter(Venue.has_bar == True)
