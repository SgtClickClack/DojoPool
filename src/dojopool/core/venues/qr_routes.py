...
def generate_table_qr(venue_id: int, table_id: int):
    ...
    try:
        if not qr_manager:
            return jsonify({"error": "QR code manager not initialized"}), 500
...
def generate_venue_qr_codes(venue_id: int):
    ...
    try:
        ...
        if qr_codes:
            return jsonify({"venue_id": venue_id, "qr_codes": qr_codes}), 200
        return jsonify({"error": "Failed to generate QR codes"}), 400
...
def table_qr_action():
    ...
    try:
        ...
        if not qr_data or not action:
            return jsonify({"error": "QR data and action are required"}), 400
...
def get_table_status_from_qr():
    ...
    try:
        ...
        return jsonify({"error": "Failed to get table status"}), 400

    except Exception as e:
        logger.error(f"Failed to get table status: {str(e)}")
        return jsonify({"error": "Failed to get table status"}), 500
