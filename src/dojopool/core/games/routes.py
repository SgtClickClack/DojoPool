...
def list_games():
    ...
    games = query.all()
    return jsonify({"status": "success", "data": {"games": [game.to_dict() for game in games]}})
...
def update_game(game_id):
    ...
    if game.player_id != current_user.id and not current_user.is_admin:
        return jsonify({"status": "error", "message": "Not authorized to update this game"}), 403
...
def delete_game(game_id):
    ...
    if game.player_id != current_user.id and not current_user.is_admin:
        return jsonify({"status": "error", "message": "Not authorized to delete this game"}), 403
...
def end_game(game_id):
    ...
    if game.player_id != current_user.id and not current_user.is_admin:
        return jsonify({"status": "error", "message": "Not authorized to end this game"}), 403
