...
class MatchDeclinedEvent(MatchEvent):
    ...
    def __init__(self, user_id: int, match_id: int, reason: Optional[str] = None):
        super().__init__(
            ...
            user_id=user_id,
            data={"match_id": match_id, "reason": reason},
        )
...
class MatchStartedEvent(MatchEvent):
    ...
    def __init__(self, match_id: int, user1_id: int, user2_id: int, venue_id: int):
        super().__init__(
            ...
            user_id=user1_id,
            data={"match_id": match_id, "opponent_id": user2_id, "venue_id": venue_id},
        )
...
class EventManager:
    ...
    def clear_history(
        ...
        self.event_history = [
            event
            for event in self.event_history
            if (before_time and event.timestamp >= before_time)
            or (event_type and event.event_type != event_type)
        ]
