...
from ..config.matchmaking import (
    MATCHMAKING_WEIGHTS, SKILL_LEVELS, PLAY_STYLES,
    TIME_SETTINGS, LOCATION_SETTINGS, SOCIAL_SETTINGS,
    QUEUE_SETTINGS, PREFERENCES
)
...
class Matchmaker:
    ...
    def add_to_queue(self, user: User, preferences: Dict) -> bool:
        ...
        entry = QueueEntry(
            ...
            priority=QUEUE_SETTINGS['priority_levels'].get(
                ...
                QUEUE_SETTINGS['priority_levels']['standard']
            )
        )
    ...
    def find_match(self, user: User, preferences: Dict) -> Optional[User]:
        ...
        sorted_queue = sorted(
            ...
            key=lambda x: (
                x.priority,
                (datetime.now() - x.join_time).total_seconds()
            ),
            reverse=True
        )
    ...
    def _calculate_time_compatibility(self, prefs1: Dict, prefs2: Dict) -> float:
        ...
        overlapping_slots = calculate_time_overlap(
            prefs1['available_times'],
            prefs2['available_times']
        )
    ...
    def _is_blocked(self, user1_id: int, user2_id: int) -> bool:
        ...
        """
        return (user1_id, user2_id) in self.blocked_pairs or (user2_id, user1_id) in self.blocked_pairs
