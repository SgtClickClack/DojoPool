...
from .constants import ErrorCodes, RoomState, RoomTypes
...
class RoomInitializer:
    ...
    async def initialize_room(
        ...
        try:
            async with self._init_lock:
                ...
                room = await room_manager.create_room(room_type, max_members, metadata)
                ...
                if not room:
                    ...
                    return format_error_response(
                        ...
                        {"room_type": room_type, "metadata": metadata},
                    )
                ...
                # Configure room
                if config:
                    error = await self._configure_room(room.room_id, config)
                    if error:
                        await room_manager.delete_room(room.room_id)
                        self._init_stats["failed_initializations"] += 1
                        return error
    ...
    async def _configure_room(self, room_id: str, config: Dict[str, Any]):
        ...
        try:
            ...
            error = self._validate_room_config(room_state["room_type"], config)
            if error:
                return error
            ...
            await room_persistence.save_room_config(room_id, config)
            ...
            for key, value in config.items():
                ...
                if key not in valid_options:
                    return format_error_response(
                        ...
                        {"option": key, "valid_options": list(valid_options.keys())},
                    )
    ...
    async def reinitialize_room(
        self, room_id: str, config: Optional[Dict[str, Any]] = None
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            await room_state_manager.transition_state(
                ...
                {"reinitialization": True, "timestamp": datetime.utcnow().isoformat()},
            )
