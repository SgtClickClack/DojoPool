...
class RoomBroadcaster:
        ...
        try:
            if not self._socket:
                return format_error_response(
                    ErrorCodes.NO_SOCKET, "WebSocket not initialized", None
                )
                ...
                try:
                    ...
                    if exclude_sid:
                        await self._socket.emit(
                            event, broadcast_data, room=room_id, skip_sid=exclude_sid
                        )
                    else:
                        await self._socket.emit(event, broadcast_data, room=room_id)
    ...
    async def broadcast_to_user(
        ...
        try:
            ...
            async with self._broadcast_lock:
                ...
                try:
                    # Emit to user
                    await self._socket.emit(event, broadcast_data, room=user_id)
    ...
    async def broadcast_system_message(
        ...
        return await self.broadcast_to_room(
            room_id, "system_message", {"message": message, "level": level}
        )

    async def broadcast_error(
        self, room_id: str, error: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
