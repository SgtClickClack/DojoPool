...
class RoomRecovery:
    ...
    async def _perform_recovery(
        ...
        try:
            ...
            await room_notifications.broadcast_room_event(
                room_id,
                "recovery_started",
                {"timestamp": datetime.utcnow().isoformat(), "error_data": error_data},
            )

            # Attempt state restoration
            await self._restore_room_state(room_id, persisted_data, room_stats)
            ...
            await room_state_manager.transition_state(
                room_id,
                RoomState.ACTIVE,
                {"recovery": True, "timestamp": datetime.utcnow().isoformat()},
            )
    ...
    async def _restore_room_state(
        ...
        try:
            ...
            await room_manager.create_room(
                room_id, persisted_data["room_type"], persisted_data["metadata"]
            )
            ...
            if room_stats:
                room_monitor.update_room_stats(room_id, room_stats)
    ...
    async def _verify_room_state(self, room_id: str) -> bool:
        ...
        try:
            ...
            if (
                room_state["room_type"] != persisted_data["room_type"]
                or room_state["metadata"] != persisted_data["metadata"]
            ):
                return False
