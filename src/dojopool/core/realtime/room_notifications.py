...
from .constants import ErrorCodes, MetricTypes, NotificationPriority
...
class RoomNotifications:
    ...
    async def _record_metric(
        self, metric_type: MetricTypes, value: float, error: bool = False
    ) -> None:
        ...
        try:
            async with self._notification_lock:
                ...
                if priority == NotificationPriority.NORMAL:
                    ...
                    if (
                        len(self._notification_batch) >= self._batch_size
                        or (datetime.utcnow() - self._last_batch_time).total_seconds()
                        >= self._batch_timeout
                    ):
    ...
    async def _adjust_batch_size(self) -> None:
        ...
        try:
            ...
            recent_latencies = [
                value
                for value, timestamp in latency_metrics
            ...
            for room_id, notifications in room_notifications.items():
                ...
                if subscribers:
                    await room_broadcaster.broadcast_to_room(
                        room_id, "batch_notifications", {"notifications": notifications}
                    )
    ...
    def get_filtered_notifications(
        self, user_id: str, filters: Dict[str, Any], limit: Optional[int] = None
    ) -> List[Dict[str, Any]]:
    ...
    def _matches_filters(
        self, notification: Dict[str, Any], user_id: str, filters: Dict[str, Any]
    ) -> bool:
