    ...
    async def _cleanup_loop(
        ...
        while True:
            try:
                ...
                await self.cleanup_rooms(idle_timeout, error_timeout, max_age)
            except asyncio.CancelledError:
                break
                ...
                for room_id, room_state in room_state_manager._room_states.items():
                    try:
                        ...
                        if age > max_age:
                            ...
                            await self._close_room(room_id, "Room exceeded maximum age")
                            rooms_cleaned += 1
                            continue
                        ...
                        if room_state == RoomState.ERROR:
                            ...
                            if error_age > error_timeout:
                                await self._close_room(
                                    room_id, "Room in error state exceeded timeout"
                                )
                                rooms_cleaned += 1
                                continue
                        ...
                        if room_stats:
                            ...
                            if last_activity:
                                idle_time = (
                                    current_time - datetime.fromisoformat(last_activity)
                                ).total_seconds()
    ...
    async def _close_room(self, room_id: str, reason: str) -> None:
        ...
        try:
            ...
            await room_state_manager.transition_state(
                room_id, RoomState.CLOSING, {"reason": reason}
            )
