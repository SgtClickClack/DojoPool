...
def log_error(
    error: Exception, message: Optional[str] = None, level: int = logging.ERROR, **kwargs: Any
) -> None:
...
def audit_log(
    action: str, resource: str, success: bool, user_id: Optional[int] = None, **kwargs: Any
) -> None:
    """Log an audit event.
    ...
    # Add request context if available
    if has_request_context():
        context.update({"method": request.method, "url": request.url, "ip": request.remote_addr})

    logger.info(f"{action} on {resource} {'succeeded' if success else 'failed'}", extra=context)
