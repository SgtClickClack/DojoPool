...
from .models import Tournament, TournamentParticipant, TournamentBracket, TournamentMatch, TournamentMatchPlayer, TournamentPrize
...
class TournamentService:
    ...
    def _generate_first_round_matches(self, bracket: TournamentBracket, participants: List[TournamentParticipant]) -> None:
    ...
    def update_match_result(self, match_id: int, winner_id: int, score: str) -> TournamentMatch:
        ...
        winner = TournamentParticipant.query.filter_by(
            tournament_id=match.bracket.tournament_id,
            user_id=winner_id
        ).first()
        ...
        loser = next(
            (p for p in match.players if p.user_id != winner_id),
            None
        )
        if loser:
    ...
    def _progress_tournament(self, tournament_id: int) -> None:
        ...
        tournament = Tournament.query.get(tournament_id)
        active_matches = TournamentMatch.query.join(TournamentBracket).filter(
    ...
    def _distribute_prizes(self, tournament: Tournament, winner: TournamentParticipant) -> None:
        ...
        prizes = TournamentPrize.query.filter_by(
            tournament_id=tournament.id
        ).order_by(TournamentPrize.position).all()
        ...
        runners_up = TournamentParticipant.query.filter_by(
            tournament_id=tournament.id,
            status='eliminated'
        ).order_by(TournamentParticipant.updated_at.desc()).limit(2).all()
    ...
    def get_tournament_status(self, tournament_id: int) -> Dict:
        ...
        for bracket in brackets:
            ...
            for match in matches:
                players = TournamentParticipant.query.join(
                    TournamentMatchPlayer
                ).filter(
                    TournamentMatchPlayer.match_id == match.id
                ).all()
