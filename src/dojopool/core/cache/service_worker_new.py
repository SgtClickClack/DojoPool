...
from typing import Optional, Dict, Any, List, Tuple, Coroutine, Union
...
import asyncio
import aiohttp
from .cache_manager_new import CacheManager, CacheConfig
...
class ServiceWorker:
    ...
    def __init__(
        ...
        if start_background_sync:
            ...
            if loop.is_running():
                ...
                loop.create_task(self._run_background_sync())
    ...
    async def _process_sync_task(self, task: SyncTask) -> bool:
        ...
        try:
            ...
            if task.type == 'upload':
                success = await self._process_upload_task(task)
            elif task.type == 'process':
                success = await self._process_processing_task(task)
            elif task.type == 'delete':
                success = await self._process_delete_task(task)
            else:
            ...
            if success:
                ...
                if task_path.exists():
                    try:
                        ...
                        logger.error(f"Error removing task file: {str(e)}")
                
                if task in self._sync_queue:
                    self._sync_queue.remove(task)
            else:
                # Update retry count
                task.retries += 1
