...
class PhysicsEngine:
    ...
    def resolve_collision(self, ball1, ball2):
        ...
        impulse = (-(1 + self.BALL_ELASTICITY) * 
    ...
    def is_simulation_active(self):
        return any(ball.is_active and 
                  (ball.velocity.magnitude() > 0.01 or abs(ball.spin) > 0.01) 
                  for ball in self.balls)
...
def calculate_shot(
    ...
    initial_velocity = Vector2D(
        power * 500 * math.cos(angle),  # Max speed of 500 cm/s
        power * 500 * math.sin(angle)
    )
    ...
    # Calculate impulse
    j = -(1 + 0.95) * (  # 0.95 is coefficient of restitution
