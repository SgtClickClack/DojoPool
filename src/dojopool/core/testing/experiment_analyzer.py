...
from .metrics import MetricType, MetricsManager
...
class ExperimentAnalyzer:
    ...
    def analyze_experiment(
        ...
        try:
            ...
            # Check sample size
            if len(control_values) < self._min_sample_size or len(variant_values) < self._min_sample_size:
            ...
            percent_change = (difference / control_mean) * 100 if control_mean != 0 else float('inf')
            ...
            pooled_std = math.sqrt(((n1-1)*control_std**2 + (n2-1)*variant_std**2) / (n1+n2-2))
            effect_size = abs(difference) / pooled_std if pooled_std != 0 else 0
            
            # Calculate statistical power
            power = float(1 - stats.t.cdf(
                stats.t.ppf(1-alpha/2, n1+n2-2) - effect_size*math.sqrt(n1*n2/(n1+n2)),
                n1+n2-2
            ))
    ...
    def determine_winner(
        ...
        try:
            ...
            if (result.is_significant and 
                result.effect_size >= min_effect_size and 
                result.power >= required_power):
