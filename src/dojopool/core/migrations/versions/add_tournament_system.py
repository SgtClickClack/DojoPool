...
def upgrade():
    # Create enum types
    op.execute("CREATE TYPE tournament_status AS ENUM ('upcoming', 'registration_open', 'in_progress', 'completed', 'cancelled')")
    op.execute("CREATE TYPE participant_status AS ENUM ('registered', 'checked_in', 'eliminated', 'active', 'winner')")
    op.execute("CREATE TYPE payment_status AS ENUM ('pending', 'paid', 'refunded')")
    op.execute("CREATE TYPE match_status AS ENUM ('scheduled', 'in_progress', 'completed', 'cancelled')")
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('upcoming', 'registration_open', 'in_progress', 'completed', 'cancelled', name='tournament_status'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('registered', 'checked_in', 'eliminated', 'active', 'winner', name='participant_status'), nullable=False),
        sa.Column('payment_status', sa.Enum('pending', 'paid', 'refunded', name='payment_status'), nullable=False),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['bracket_id'], ['tournament_brackets.id'], ),
        sa.ForeignKeyConstraint(['next_match_id'], ['tournament_matches.id'], ),
    ...
    op.create_table(
        ...
        sa.Column('winner_id', sa.Integer(), nullable=True),
        sa.Column('distribution_status', sa.Enum('pending', 'distributed', 'claimed', name='prize_status'), nullable=False),
