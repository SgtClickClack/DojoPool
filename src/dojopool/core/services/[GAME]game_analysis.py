...
class GameAnalyzer:
    ...
    def analyze_game(
        ...
        results = {
            ...
            "heatmaps": {"shots": shot_heatmap, "positions": position_heatmap},
            "strategy": {
    ...
    def _analyze_patterns(
        ...
        strategy_patterns = self._extract_strategy_patterns(shots, positions, outcomes)
            ...
            patterns.append(
                {
                    ...
                    "control_area": next(
                        (
                            a
                            for a in control_areas
                            if np.allclose(a["center"], key_position["position"])
                        ),
                        None,
                    ),
                }
    ...
    def _generate_shot_heatmap(self, shots: List[Dict]):
        ...
        positions = np.array(
            [[shot["start_position"][0], shot["start_position"][1]] for shot in shots]
        )
        ...
        if len(positions) > 1:
            ...
            return {"x": x.tolist(), "y": y.tolist(), "density": Z.tolist()}
        else:
            return {"x": [], "y": [], "density": []}
    ...
    def _generate_strategy(
        ...
        recommendations = self._generate_recommendations(
            strengths, weaknesses, patterns, metrics, style
        )
    ...
    def _suggest_position_adaptations(
        self, position: Dict, metrics: GameMetrics, style: PlayerStyle
    ) -> List[Dict]:
