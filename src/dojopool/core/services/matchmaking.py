...
class MatchmakingService:
    ...
    def find_matches(self, user_id, game_type=None, venue_id=None, max_matches=5):
        ...
        if game_type:
            query = query.filter(
                User.preferred_game_types.contains([game_type])
            )
            
        if venue_id:
            query = query.filter(
                User.preferred_venues.contains([venue_id])
            )
    ...
    def _calculate_availability_compatibility(self, user, opponent):
        ...
        Returns:
            float: Compatibility score (0-1)
        """
        # Check if both players are currently active
    ...
    def _calculate_social_compatibility(self, user, opponent):
        ...
        games_together = Game.query.filter(
            Game.players.any(user_id=user.id),
            Game.players.any(user_id=opponent.id),
    ...
    def _get_compatibility_details(self, user, opponent):
        ...
        return {
            ...
            'social': {
                ...
                'mutual_friends': len(
                    set(user.friends) & set(opponent.friends)
                )
            }
        }
