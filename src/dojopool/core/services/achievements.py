...
class AchievementManager:
    ...
    def check_achievements(self, user_id):
        ...
        for tier, data in self.achievements["GAMES_PLAYED"].items():
            if games_played >= data["count"]:
                achievement = self._award_achievement(
                    user, "GAMES_PLAYED", tier, data["points"], data["icon"]
                )
                if achievement:
        ...
        for tier, data in self.achievements["GAMES_WON"].items():
            if games_won >= data["count"]:
                achievement = self._award_achievement(
                    user, "GAMES_WON", tier, data["points"], data["icon"]
                )
                if achievement:
                    new_achievements.append(achievement)
        ...
        for tier, data in self.achievements["WIN_STREAK"].items():
            if current_streak >= data["count"]:
                achievement = self._award_achievement(
                    user, "WIN_STREAK", tier, data["points"], data["icon"]
                )
                if achievement:
                    new_achievements.append(achievement)
        ...
        for tier, data in self.achievements["VENUES_VISITED"].items():
            if venues_visited >= data["count"]:
                achievement = self._award_achievement(
                    user, "VENUES_VISITED", tier, data["points"], data["icon"]
                )
                if achievement:
                    new_achievements.append(achievement)
    ...
    def _award_achievement(self, user, achievement_type, tier, points, icon):
        ...
        existing = (
            ...
            .filter_by(user_id=user.id, type=achievement_type, tier=tier)
            .first()
        )
        ...
        emit_event("achievement_earned", {"user_id": user.id, "achievement": achievement.to_dict()})
    ...
    def _check_point_milestones(self, user):
        ...
        for points, reward in self.rewards["POINT_MILESTONES"].items():
            if current_points >= points:
                ...
                existing = (
                    db.session.query(Achievement)
                    .filter_by(user_id=user.id, type="POINT_MILESTONE", points=points)
                    .first()
                )
    ...
    def _award_special_reward(self, user, reward_type):
        ...
        existing = (
            db.session.query(Achievement)
            .filter_by(user_id=user.id, type=f"SPECIAL_{reward_type}")
            .first()
        )
    ...
    def _get_current_streak(self, user_id):
        ...
        for game in recent_games:
            if game.winner_id == user_id:
                if (
                    last_game_time
                    and (game.end_time - last_game_time).total_seconds()
                    > self.settings["STREAK_TIMEOUT"]
                ):
                    break
                streak += 1
    ...
    def _count_unique_venues(self, user_id):
        ...
        """
        return (
            db.session.query(Game.venue_id)
            .distinct()
            .filter(Game.players.any(user_id=user_id))
            .count()
        )
