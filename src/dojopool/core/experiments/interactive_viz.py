...
class InteractiveVisualizer:
    ...
    def plot_power_curve(
        ...
        fig.add_hline(y=0.8, line_dash="dash", line_color="gray",
                     annotation=dict(text="0.8 Power Threshold"))
    ...
    def plot_distributions(
        ...
        fig.add_vline(x=control_mean, line_dash="dash", line_color="blue",
                     annotation=dict(text=f"Control Mean: {control_mean:.2f}"))
        fig.add_vline(x=variant_mean, line_dash="dash", line_color="red",
                     annotation=dict(text=f"Variant Mean: {variant_mean:.2f}"))
    ...
    def create_statistical_summary(
        ...
        fig = go.Figure(data=[go.Table(
            ...
            cells=dict(
                values=[
                    stats['Metric'],
                    [f"{x:,.2f}" if isinstance(x, float) else f"{x:,}" for x in stats['Control']],
                    [f"{x:,.2f}" if isinstance(x, float) else f"{x:,}" for x in stats['Variant']]
                ],
                fill_color='lavender',
                align='right'
            )
        ...
        fig.update_layout(
            title=f"Statistical Summary for {control_events[0].metric_name}",
            width=600
        )
    ...
    def plot_time_series(
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='markers',
            name='Control Raw',
            marker=dict(color='blue', size=4, opacity=0.1),
            showlegend=False
        ))
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='markers',
            name='Variant Raw',
            marker=dict(color='red', size=4, opacity=0.1),
            showlegend=False
        ))
        ...
        fig.add_trace(go.Scatter(
            ...
            y=(control_mean + control_std).tolist() + (control_mean - control_std).tolist()[::-1],
            fill='toself',
        ...
        fig.add_trace(go.Scatter(
            ...
            y=(variant_mean + variant_std).tolist() + (variant_mean - variant_std).tolist()[::-1],
            fill='toself',
    ...
    def create_time_dashboard(
        ...
        def add_daily_pattern(events: List[MetricEvent], name: str, row: int, col: int):
            ...
            fig.add_trace(go.Scatter(
                ...
                mode='lines',
                name=f'{name} Mean',
                line=dict(color='blue' if name == 'Control' else 'red')
            ), row=row, col=col)
            
            fig.add_trace(go.Scatter(
                x=list(range(24)) + list(range(24))[::-1],
                y=(hourly_mean + hourly_std).tolist() + (hourly_mean - hourly_std).tolist()[::-1],
                fill='toself',
        ...
        if save_path:
            with open(save_path, 'w') as f:
                ...
                for i, key in enumerate(segment_keys):
                    f.write(f'<button class="tablinks" onclick="openTab(event, \'tab{i}\')">{key}</button>')
                f.write('</div>')
    ...
    def _optimize_data_for_visualization(
        ...
        if len(df) > max_points:
            ...
            sampled = non_outliers.sample(
                n=min(sample_size, len(non_outliers)),
                random_state=42
            )
            ...
            for key2 in segment_keys[i+1:]:
                ...
                for val1 in key1_vals:
                    for val2 in key2_vals:
                        ...
                        c_values = np.array(control_df[c_mask]['value'].to_numpy(), dtype=np.float64)
                        v_values = np.array(variant_df[v_mask]['value'].to_numpy(), dtype=np.float64)
