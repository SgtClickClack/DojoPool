...
class ResultsDashboard:
    ...
    def _optimize_data(
        self, events: List[MetricEvent], max_points: int = 10000
    ) -> List[MetricEvent]:
    ...
    def create_summary(
        ...
        if time_window:
            if not isinstance(time_window, str) or len(time_window) < 2:
                raise ValueError(
                    "time_window must be a string in format '<number><unit>' (e.g., '7D')"
                )
            try:
        ...
        fig.add_trace(self._create_stats_table(result), row=3, col=1)
    ...
    def create_segment_view(
        ...
        if invalid_segments:
            raise ValueError(
                f"Invalid segment keys: {invalid_segments}. Available segments: {available_segments}"
            )
    ...
    def _filter_time_window(
        ...
        if unit == "D":
            ...
            raise ValueError(
                f"Unsupported time window unit: {unit}. Use 'D' for days or 'H' for hours"
            )
    ...
    def _export_dashboard(
        self, fig: go.Figure, base_path: str, formats: List[str]
    ) -> Dict[str, str]:
