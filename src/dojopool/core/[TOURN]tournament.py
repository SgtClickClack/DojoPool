...
from ..models import db, Tournament, TournamentParticipant, TournamentMatch
from ..models.tournament import TournamentStatus, TournamentFormat
...
class TournamentService:
    ...
    def _generate_swiss_brackets(tournament: Tournament):
        ...
        num_participants = len(participants)
        num_rounds = math.floor(math.log2(num_participants))  # Recommended rounds for Swiss
    ...
    @staticmethod
    def _update_single_elimination_brackets(tournament: Tournament, completed_match: TournamentMatch) -> None:
    ...
    @staticmethod
    def _update_double_elimination_brackets(tournament: Tournament, completed_match: TournamentMatch) -> None:
        ...
        if completed_match.round_number > 0:
            ...
            loser_id = completed_match.player1_id if completed_match.winner_id == completed_match.player2_id else completed_match.player2_id
            
            # Find appropriate losers bracket match
            losers_round = -completed_match.round_number
            losers_match = TournamentMatch.query.filter_by(
                tournament_id=tournament.id,
                round_number=losers_round
            ).filter(
                (TournamentMatch.player1_id == None) | 
                (TournamentMatch.player2_id == None)
            ).first()
    ...
    def get_standings(tournament_id: int) -> List[Dict]:
        ...
        for participant in participants:
            standing = {
                ...
                'win_percentage': (participant.matches_won / participant.matches_played * 100 
                                 if participant.matches_played > 0 else 0)
            }
