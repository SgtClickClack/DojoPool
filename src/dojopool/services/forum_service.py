...
class ForumService:
    ...
    def create_topic(
        ...
        topic = ForumTopic(title=title, slug=slug, category_id=category_id, user_id=user_id)
        db.session.add(topic)
        db.session.flush()  # Get topic ID

        # Create initial post
        post = ForumPost(topic_id=topic.id, user_id=user_id, content=content)
        db.session.add(post)
        ...
        # Invalidate caches
        cache.delete_many(
            [f"{CACHE_PREFIX}category:{category_id}", f"{CACHE_PREFIX}topics:{category_id}"]
        )
    ...
    def create_post(self, user_id: int, topic_id: int, content: str) -> Dict[str, Any]:
        ...
        # Invalidate caches
        cache.delete_many([f"{CACHE_PREFIX}topic:{topic_id}", f"{CACHE_PREFIX}posts:{topic_id}"])
    ...
    def get_posts(self, topic_id: int, page: int = 1, per_page: int = 20) -> Dict[str, Any]:
    ...
    def add_reaction(self, user_id: int, post_id: int, reaction_type: str) -> Dict[str, Any]:
        ...
        reaction = ForumPostReaction(post_id=post_id, user_id=user_id, reaction_type=reaction_type)
    ...
    def subscribe(
        self, user_id: int, topic_id: Optional[int] = None, category_id: Optional[int] = None
    ) -> Dict[str, Any]:
    ...
    def _create_notification(self, user_id: int, topic: ForumTopic, post: ForumPost) -> None:
