...
class OfflineSyncService:
    ...
    def sync_offline_data(
        ...
        processed_matches = self._process_matches(
            user_id,
            offline_data.get('matches', [])
        )

        # Process shots
        processed_shots = self._process_shots(
            user_id,
            offline_data.get('shots', [])
        )

        # Process venue check-ins
        processed_checkins = self._process_checkins(
            user_id,
            offline_data.get('checkins', [])
        )
