...
from dojopool.models.tournament import TournamentMatch, TournamentParticipant, MatchStatus
...
class MatchStatsService:
    ...
    def _update_participant_stats(match: TournamentMatch) -> None:
        ...
        for participant in [match.player1, match.player2]:
            ...
            breaks = [b['points'] for b in match.stats.get('breaks', []) 
                     if b['player_id'] == participant.id]
            if breaks:
                participant.stats['total_breaks'] += len(breaks)
                participant.stats['highest_break'] = max(
                    participant.stats['highest_break'],
                    max(breaks)
                )
                participant.stats['average_break'] = (
                    sum(breaks) / len(breaks)
                )
            
            # Update foul count
            participant.stats['total_fouls'] += len([
                f for f in match.stats.get('fouls', [])
