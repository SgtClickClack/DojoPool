...
class MLService:
    ...
    def analyze_player_performance(self, player_id: int) -> Dict:
        ...
        matches = self.db.query(GameAnalytics).filter(
            GameAnalytics.player_id == player_id
        ).all()
        
        if not matches:
            return {
                'status': 'error',
                'message': 'No match data found for player'
            }
    ...
    def predict_player_performance(
        ...
        # Get player's match history
        player_matches = self.db.query(GameAnalytics).filter(
            GameAnalytics.player_id == player_id
        ).all()
        ...
        if opponent_id:
            opponent_matches = self.db.query(GameAnalytics).filter(
                GameAnalytics.player_id == opponent_id
            ).all()
            if opponent_matches:
        ...
        return {
            ...
            'opponent_prediction': opponent_prediction.tolist() if opponent_prediction is not None else None,
    ...
    def analyze_match_patterns(self, match_id: int) -> Dict:
        ...
        match = self.db.query(GameAnalytics).filter(
            GameAnalytics.match_id == match_id
        ).first()
    ...
    def generate_training_recommendations(self, player_id: int) -> Dict:
        ...
        recent_matches = self.db.query(GameAnalytics).filter(
            GameAnalytics.player_id == player_id
        ).order_by(
            GameAnalytics.timestamp.desc()
        ).limit(10).all()
        
        if not recent_matches:
            return {
                'status': 'error',
                'message': 'No recent matches found for player'
            }
