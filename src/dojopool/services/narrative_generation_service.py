...
class NarrativeGenerationService:
    def __init__(self):
        self.shot_analysis = ShotAnalysisService()
    ...
    def _generate_match_highlights(
        ...
        analyzed_shots = [
            (shot, self.shot_analysis.analyze_shot(shot.to_dict()))
            for shot in shots
        ]
        sorted_shots = sorted(
            analyzed_shots,
    ...
    def _generate_shot_description(
        ...
        templates = self.narrative_templates['shot_description'].get(
            shot_type,
            self.narrative_templates['shot_description']['position']
        )
    ...
    def _calculate_match_pace(self, shots: List[Shot]) -> str:
        ...
        time_diffs = [
            (t2 - t1).total_seconds()
            for t1, t2 in zip(shot_times[:-1], shot_times[1:])
        ]
