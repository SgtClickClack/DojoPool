    ...
    def _ensure_firebase_initialized(self):
        ...
        if self.app is None:
            try:
                ...
                try:
                    cred_path = current_app.config.get('FIREBASE_CREDENTIALS_PATH', 'firebase-credentials.json')
                    if os.path.exists(cred_path):
                        cred = credentials.Certificate(cred_path)
                        self.app = firebase_admin.initialize_app(cred)
                    else:
                        current_app.logger.warning('Firebase credentials not found. Push notifications will be disabled.')
                except RuntimeError:
    ...
    def _handle_failed_tokens(
        ...
        for idx, response in enumerate(responses):
            if not response.success:
                ...
                device = next(
                    (d for d in devices if d.push_token == token),
                    None
                )
                if device:
                    if (
