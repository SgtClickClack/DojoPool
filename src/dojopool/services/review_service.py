...
class ReviewService:
    ...
    def create_review(rating_id: int, content: str) -> Review:
        ...
        review = Review(rating_id=rating_id, content=content)
    ...
    def add_response(review_id: int, user_id: int, content: str) -> ReviewResponse:
        ...
        response = ReviewResponse(review_id=review_id, user_id=user_id, content=content)
    ...
    def report_review(
        ...
        report = ReviewReport(review_id=review_id, user_id=user_id, reason=reason, details=details)
    ...
    def vote_review(review_id: int, user_id: int, is_helpful: bool) -> ReviewVote:
        ...
        existing_vote = ReviewVote.query.filter_by(review_id=review_id, user_id=user_id).first()
    ...
    def get_review_analytics(target_type: str, target_id: int) -> dict:
        ...
        return {
            ...
            "response_rate": (
                reviews.filter(ReviewResponse.review_id is not None).count() / total_reviews
                if total_reviews > 0
                else 0
            ),
        }
