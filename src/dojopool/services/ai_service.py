...
from ..models.analytics import UserMetrics, GameMetrics
from ..extensions import db, cache
from ..utils.ml_utils import prepare_features, normalize_data
...
class AIService:
    ...
    def generate_story(
        ...
        if result is None:
            ...
            recent_matches = Match.query.filter_by(user_id=user_id)\
            ...
            ).order_by(UserMetrics.date.desc()).limit(10).all()
    ...
    def get_personalized_recommendations(
        self,
        user_id: int,
        recommendation_type: str = 'training'
    ) -> List[Dict[str, Any]]:
        """Get personalized recommendations based on user data and behavior."""
        cache_key = f"{CACHE_PREFIX}recommendations:{user_id}:{recommendation_type}"
        result = cache.get(cache_key)
        ...
        if result is None:
            ...
            metrics = UserMetrics.query.filter_by(user_id=user_id)\
                .order_by(UserMetrics.date.desc())\
                .limit(30).all()
            
            # Prepare features for recommendation model
            features = prepare_features(recent_matches, metrics)
    ...
    def analyze_match(
        ...
        cache_key = f"{CACHE_PREFIX}analysis:{match_id}:{analysis_type}"
        result = cache.get(cache_key)
        ...
        if result is None:
            ...
            # Get match metrics
            metrics = GameMetrics.query.filter_by(game_id=match.game_id)\
                .order_by(GameMetrics.created_at).all()
            
            # Prepare data for analysis
            match_data = self._prepare_match_data(match, metrics)
            
            # Perform analysis based on type
            if analysis_type == 'full':
                analysis = self._perform_full_analysis(match_data)
            elif analysis_type == 'performance':
                analysis = self._analyze_performance(match_data)
            elif analysis_type == 'strategy':
                analysis = self._analyze_strategy(match_data)
            else:
                analysis = self._perform_basic_analysis(match_data)
    ...
    def _generate_recommendations(
        ...
        predictions = self.recommendation_model.predict(features)
    ...
    def _analyze_performance(
        ...
        anomalies = self.anomaly_detector.fit_predict(
    ...
    def _analyze_patterns(self, timeline: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for sequence in shot_sequences:
            pattern = {
                ...
                'position_pattern': self._analyze_position_pattern([shot['position'] for shot in sequence]),
                'timing_pattern': self._analyze_timing_pattern([shot['duration'] for shot in sequence])
            }
    ...
    def _analyze_risk_taking(self, timeline: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            'risk_success_rate': sum(1 for shot in difficult_shots if shot['result'] == 'made') / (len(difficult_shots) + 1e-6),
            'safe_success_rate': sum(1 for shot in safe_shots if shot['result'] == 'made') / (len(safe_shots) + 1e-6)
        }
    ...
    def _analyze_time_management(self, timeline: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            'time_consistency': float(1.0 - np.std(shot_durations) / (np.mean(shot_durations) + 1e-6)),
    ...
    def _identify_preferred_areas(self, positions: List[Dict[str, float]]) -> List[Dict[str, Any]]:
        ...
        for pos in positions:
            for area in areas:
                if (area['x_range'][0] <= pos['x'] < area['x_range'][1] and
                    area['y_range'][0] <= pos['y'] < area['y_range'][1]):
                    area['count'] += 1
    ...
    def _identify_movement_patterns(self, positions: List[Dict[str, float]]) -> List[Dict[str, Any]]:
    ...
    def _analyze_time_pressure(self, shots: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        pressure_success = (
            ...
            len(pressure_shots)
        ) if pressure_shots else 0.0
        
        normal_success = (
            sum(1 for shot in normal_shots if shot['result'] == 'made') /
            len(normal_shots)
        ) if normal_shots else 0.0
    ...
    def _get_pattern_key(self, sequence: List[Dict[str, Any]]) -> str:
        ...
        return '_'.join(
            f"{shot['type']}_{shot['difficulty']:.1f}"
