...
class VenueManagementService:
    ...
    def get_nearby_venues(self, latitude: float, longitude: float, radius: float = 10.0) -> List[Venue]:
        ...
        return Venue.query.filter(
            func.ST_DWithin(
                func.ST_SetSRID(func.ST_MakePoint(Venue.longitude, Venue.latitude), 4326)::Geography,
    ...
    def get_venue_stats(self, venue_id: int) -> Optional[Dict]:
        ...
        unique_players = db.session.query(func.count(func.distinct(Game.player_id))).filter_by(venue_id=venue_id).scalar()
    ...
    def get_venue_occupancy(self, venue_id: int) -> Optional[Dict]:
        ...
        checked_in_players = VenueCheckin.query.filter(
            ...
            VenueCheckin.check_in_time >= three_hours_ago
        ).count()
