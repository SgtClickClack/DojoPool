...
class ImageCompressionService:
    ...
    def _compress_single_variant(
        self,
        img: Image.Image,
        variant_name: str,
        variant_config: Dict[str, Any],
        output_format: Literal["JPEG", "WebP"],
    ) -> bytes:
            ...
            with Image.open(input_path) as img:
                ...
                for variant_name, variant_config in self.config[
                    ...
                    if self.config["keep_original"]:
                        ...
                        compressed = self._compress_single_variant(
                            img, variant_name, variant_config, "JPEG"
                        )
                    ...
                    if self.config["convert_to_webp"]:
                        ...
                        compressed = self._compress_single_variant(
                            img, variant_name, variant_config, "WebP"
                        )
                        with open(output_path, "wb") as f:
                            f.write(compressed)
    ...
    def batch_compress_directory(self, input_dir: str, output_dir: str):
        ...
        # Split into chunks
        chunks = [
            image_files[i : i + self.config["chunk_size"]]
