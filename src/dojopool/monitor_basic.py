...
def format_optimization_details(details: List[str]) -> str:
    """Format optimization details for better readability."""
    return "\n    - " + "\n    - ".join(details)


def compute_hash(data: str) -> str:
    # Replace MD5 with SHA-256 for improved security
...
class BasicMonitor:
    def __init__(self, auto_optimize: bool = False):
        ...
        self._metric_batch_size = 5  # Number of readings to batch before analysis
        self._current_batch = {"cpu": [], "memory": [], "disk": []}
        self._last_full_scan = 0
        self._full_scan_interval = 300  # Full system scan every 5 minutes
        self._process_scan_interval = 60  # Process scan every minute
    ...
    def _collect_metrics(self) -> Dict:
        """Collect comprehensive system metrics with optimized scanning."""
        current_time = time.time()

        # Determine if we need a full system scan
        need_full_scan = (
            current_time - self._last_full_scan
        ...
        metrics = {
            ...
            "disk_percent": (
                ...
                if need_full_scan
                else self.metrics_history["disk"][-1]
            ),
            "processes": self._get_process_details_optimized(need_full_scan),
        }

        # Only collect network metrics during full scan
        if need_full_scan:
            ...
            metrics["network"] = self.previous_network

        return metrics

    def _get_process_details_optimized(self, full_scan: bool = False) -> List[Dict]:
        """Get process details with caching and optimized scanning."""
        current_time = time.time()
        processes = []

        try:
            # Use cached process info for non-full scans
            if not full_scan and self._process_cache:
                ...
                # Full scan: update process cache
                ...
                for proc in psutil.process_iter(
                    ...
                    try:
                        ...
                        if pinfo["cpu_percent"] > 0 or pinfo["memory_percent"] > 0:
                            self._process_cache[pinfo["pid"]] = pinfo
                            processes.append(pinfo)
                    except (psutil.NoSuchProcess, psutil.AccessDenied):
                        continue
        except Exception as e:
            self.logger.error(f"Error collecting process details: {str(e)}")
            return []

        return sorted(processes, key=lambda x: x["cpu_percent"], reverse=True)

    def _update_history(self, metrics: Dict):
        """Update metrics history with batching."""
        self._current_batch["cpu"].append(metrics["cpu_percent"])
        self._current_batch["memory"].append(metrics["memory_percent"])
        self._current_batch["disk"].append(metrics["disk_percent"])

        # Process batch when enough samples are collected
        if len(self._current_batch["cpu"]) >= self._metric_batch_size:
            # Calculate averages
            self.metrics_history["cpu"].append(
                sum(self._current_batch["cpu"]) / self._metric_batch_size
            )
            self.metrics_history["memory"].append(
                sum(self._current_batch["memory"]) / self._metric_batch_size
            )
            self.metrics_history["disk"].append(
                sum(self._current_batch["disk"]) / self._metric_batch_size
            )
    ...
    def _calculate_trend(self, values: deque) -> str:
        ...
        avg_change = sum(b - a for a, b in zip(recent[:-1], recent[1:])) / (
            len(recent) - 1
        )
    ...
    def _check_thresholds(self, metrics: Dict):
        """Check metrics against thresholds with optimized warnings."""
        warnings = []

        # Only check CPU and memory frequently
        if metrics["cpu_percent"] > self.thresholds["cpu_percent"]:
            top_process = (
                metrics["processes"][0] if metrics["processes"] else {"name": "Unknown"}
            )
            warnings.append(
                f"⚠️ High CPU usage: {metrics['cpu_percent']}% - "
                f"Top process: {top_process['name']}"
            )

        if metrics["memory_percent"] > self.thresholds["memory_percent"]:
            memory_processes = sorted(
                metrics["processes"], key=lambda x: x["memory_percent"], reverse=True
            )
            top_memory_process = (
                memory_processes[0] if memory_processes else {"name": "Unknown"}
            )
            warnings.append(
                f"⚠️ High memory usage: {metrics['memory_percent']}% - "
                f"Top memory process: {top_memory_process['name']}"
            )

        # Check disk and Cursor processes less frequently
        if time.time() - self._last_full_scan < 10:  # Only during full scans
            if metrics["disk_percent"] > self.thresholds["disk_percent"]:
                warnings.append(f"⚠️ High disk usage: {metrics['disk_percent']}%")

            cursor_processes = [
                p for p in metrics["processes"] if "cursor" in p["name"].lower()
            ]
            if len(cursor_processes) > self.thresholds["process_count"]:
                warnings.append(
                    f"⚠️ High number of Cursor processes: {len(cursor_processes)} - "
    ...
    def _generate_optimization_recommendations(self, metrics: Dict) -> List[Dict]:
        ...
        if self.previous_network:
            time_diff = (
                metrics["timestamp"] - self.previous_network["timestamp"]
            ).total_seconds()
            bytes_sent_per_sec = (
                metrics["network"]["bytes_sent"] - self.previous_network["bytes_sent"]
            ) / time_diff
            bytes_recv_per_sec = (
                metrics["network"]["bytes_recv"] - self.previous_network["bytes_recv"]
            ) / time_diff

            if (
                bytes_sent_per_sec > self.thresholds["network_bytes_sent"]
                or bytes_recv_per_sec > self.thresholds["network_bytes_recv"]
            ):
                recommendations.append(
                    {
                        "type": "network",
                        ...
                        "details": self._get_network_optimization_details(
        ...
        if recommendations:
            self.optimization_history.append(
                {"timestamp": metrics["timestamp"], "recommendations": recommendations}
            )
    ...
    def _apply_automatic_optimizations(self, analysis: Dict):
        ...
        for rec in analysis.get("optimization_recommendations", []):
            ...
            if (
                self.optimization_attempts.get(opt_key, 0)
            ...
            if (
                last_attempt
                and (current_time - last_attempt).total_seconds()
            ...
            if success:
                self.logger.info(
                    f"Successfully applied automatic optimization: {rec['action']}"
                )
                self.last_optimization[opt_key] = current_time
            else:
                self.optimization_attempts[opt_key] = (
                    self.optimization_attempts.get(opt_key, 0) + 1
                )

    def _optimize_cursor_automatically(self, recommendation: Dict) -> bool:
        """Apply automatic optimizations for Cursor with caching."""
        try:
            config_path = os.path.expanduser("~/AppData/Roaming/Cursor/config.json")
            if not os.path.exists(config_path):
                return False

            # Cache config reading
            if not hasattr(self, "_cursor_config_cache"):
                with open(config_path, "r") as f:
                    self._cursor_config_cache = json.load(f)
                    self._cursor_config_last_read = time.time()
            elif (
                time.time() - self._cursor_config_last_read > 300
            ...
            if not os.path.exists(backup_path):
                with open(backup_path, "w") as f:
                    json.dump(self._cursor_config_cache, f)
            ...
            for key, value in optimizations.items():
                if self._cursor_config_cache.get(key) != value:
                    self._cursor_config_cache[key] = value
                    config_changed = True
            ...
            for proc in target_processes:
                if proc.info["memory_percent"] > 10:  # High memory usage
                    ...
                    if (
                        ...
                        or "cursor" in process_name.lower()
                    ):
                        # Clear browser cache
                        cache_dir = os.path.expanduser(
                            "~/AppData/Local/Cursor/User Data/Default/Cache"
                        )
                        if os.path.exists(cache_dir):
                            try:
                                subprocess.run(
                                    ["rd", "/s", "/q", cache_dir], shell=True
                                )
    ...
    def _get_cursor_optimization_details(
        self, cursor_processes: List[Dict]
    ) -> List[str]:
    ...
    def _get_network_optimization_details(
        self, bytes_sent_per_sec: float, bytes_recv_per_sec: float
    ) -> List[str]:
    ...
    def get_optimization_summary(self) -> Dict:
        ...
        recent_recommendations = self.optimization_history[
            -10:
        ...
        for entry in recent_recommendations:
            for rec in entry["recommendations"]:
                # Count by type
                summary["by_type"][rec["type"]] = (
                    summary["by_type"].get(rec["type"], 0) + 1
                )
                # Count by severity
                if "severity" in rec:
                    summary["by_severity"][rec["severity"]] = (
                        summary["by_severity"].get(rec["severity"], 0) + 1
                    )
                # Count by process
                if "process" in rec:
                    summary["most_common_processes"][rec["process"]] = (
                        summary["most_common_processes"].get(rec["process"], 0) + 1
                    )
                ...
                severity_icon = (
                    ...
                    else "🟡" if rec.get("severity") == "medium" else "🟢"
                )
                formatted_rec = (
                    f"{severity_icon} [{rec['type'].upper()}] {rec['action']}\n"
                    f"   Details:{format_optimization_details(rec['details'])}"
                )
                summary["formatted_summary"].append(formatted_rec)
