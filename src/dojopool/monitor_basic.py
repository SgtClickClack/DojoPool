...
class BasicMonitor:
    ...
    def _collect_metrics(self) -> Dict:
        ...
        need_full_scan = (current_time - self._last_full_scan) >= self._full_scan_interval
        
        metrics = {
            'timestamp': datetime.now(),
            'cpu_percent': psutil.cpu_percent(interval=0.1),  # Reduced interval
            'memory_percent': psutil.virtual_memory().percent,
            'disk_percent': psutil.disk_usage('/').percent if need_full_scan else self.metrics_history['disk'][-1],
    ...
    def _update_history(self, metrics: Dict):
        ...
        if len(self._current_batch['cpu']) >= self._metric_batch_size:
            # Calculate averages
            self.metrics_history['cpu'].append(sum(self._current_batch['cpu']) / self._metric_batch_size)
            self.metrics_history['memory'].append(sum(self._current_batch['memory']) / self._metric_batch_size)
            self.metrics_history['disk'].append(sum(self._current_batch['disk']) / self._metric_batch_size)
    ...
    def _check_thresholds(self, metrics: Dict):
        ...
        if metrics['cpu_percent'] > self.thresholds['cpu_percent']:
            top_process = metrics['processes'][0] if metrics['processes'] else {'name': 'Unknown'}
            warnings.append(
        ...
        if metrics['memory_percent'] > self.thresholds['memory_percent']:
            memory_processes = sorted(metrics['processes'], key=lambda x: x['memory_percent'], reverse=True)
            top_memory_process = memory_processes[0] if memory_processes else {'name': 'Unknown'}
            warnings.append(
    ...
    def _optimize_cursor_automatically(self, recommendation: Dict) -> bool:
        ...
        try:
            ...
            elif time.time() - self._cursor_config_last_read > 300:  # Refresh cache every 5 minutes
                with open(config_path, 'r') as f:
                    self._cursor_config_cache = json.load(f)
                    self._cursor_config_last_read = time.time()
