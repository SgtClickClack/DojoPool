...
def validate_date_range(start_date: Union[str, datetime], end_date: Union[str, datetime]) -> Tuple[bool, Optional[str]]:
    """
...
def validate_hours_format(hours: dict) -> Tuple[bool, Optional[str]]:
    ...
    for day in days:
        ...
        if not re.match(time_pattern, day_hours['open']) or not re.match(time_pattern, day_hours['close']):
...
def validate_features(features: list) -> Tuple[bool, Optional[str]]:
    ...
    """
    valid_features = {
        'parking', 'food_service', 'bar', 'tournaments', 'lessons',
        'equipment_rental', 'wifi', 'air_conditioning', 'pro_shop',
        'billiards_supplies', 'snooker_tables', 'pool_tables',
        'dart_boards', 'arcade_games', 'smoking_area', 'outdoor_seating',
        'private_rooms', 'event_space', 'live_music', 'tv_screens'
    }
