...
import { VectorClock, type VectorTimestamp } from '../consistency/VectorClock';
import { LWWRegister } from '../consistency/CRDT';
import {
  RaftConsensus,
  type ConsensusConfig,
} from '../consensus/RaftConsensus';
import {
  type GameEvent,
  type GameAction,
  type Player,
...
import { PhysicsEngine, type PhysicsObject } from '../../utils/physics';
import {
  AIRefereeService,
  type RefereeInput,
  type RefereeResult,
  FoulType,
} from '../../services/ai/AIRefereeService';
...
export class GameState extends EventEmitter {
    ...
    this.consensus.on('eventApplied', (event: GameEvent) => {
      ...
      if (consensusEvent.nodeId !== this.nodeId) {
        ...
        this.applyActionLocally(
          consensusEvent.action,
          consensusEvent.action.timestamp
        );
      } else {
        console.log(
          `Ignoring consensus event from self: ${consensusEvent.action.type}`
        );
      }
    });
  }
  ...
  private applyActionLocally(
    ...
    console.log(
      `Applying action locally: ${action.type} at ${actionTimestamp}`
    );
    ...
    switch (action.type) {
        ...
        if (tableRegister && gameTable) {
          ...
          if (!currentTableData.players.includes(action.playerId)) {
            ...
            if (player) {
              ...
              console.error(
                `JOIN_TABLE: Failed to create player object for ${action.playerId}`
              );
            }
          } else {
            console.warn(
      ...
      case 'LEAVE_TABLE':
        if (tableRegister && gameTable && action.playerId) {
          const currentTableData = tableRegister.getValue();
          if (currentTableData.players.includes(action.playerId)) {
            const updatedPlayersArray = currentTableData.players.filter(
            ...
            console.warn(
              `Attempted LEAVE_TABLE for player ${action.playerId} not on table ${action.tableId}`
            );
          }
        } else {
          console.warn(
            `Attempted LEAVE_TABLE for invalid table or player ID: ${action.tableId}, ${action.playerId}`
          );
        }
        break;

      case 'START_GAME':
        if (gameTable && gameTable.currentTurn === action.playerId) {
          // Only current player can start?
          this.startGame(action.tableId);
        } else {
          console.warn(
            `Player ${action.playerId} attempted START_GAME on table ${action.tableId} but conditions not met.`
          );
        }
        break;
      ...
      case 'PLACE_BALL':
        if (
          gameTable &&
          action.playerId === gameTable.currentTurn &&
          gameTable.ballInHand
        ) {
          const newPosition = action.data.position as Vector2D;
          if (!newPosition) {
            console.error(
              `PLACE_BALL action missing position data for table ${action.tableId}`
            );
            return;
          }
          ...
          if (
            newPosition.x < 0 ||
            newPosition.x > gameTable.tableWidth ||
            newPosition.y < 0 ||
            newPosition.y > gameTable.tableHeight
          ) {
            console.warn(
              `PLACE_BALL: Invalid position (${newPosition.x}, ${newPosition.y}) - out of bounds.`
            );
            return;
          }
          ...
          if (!cueBall) {
            console.error(
              `PLACE_BALL: Cue ball not found on table ${action.tableId}.`
            );
            return;
          }
          ...
          console.log(
            `Applied PLACE_BALL: Player ${action.playerId} placed cue ball at (${newPosition.x}, ${newPosition.y}) on table ${action.tableId}. Ball-in-hand consumed.`
          );
          ...
          console.warn(
            `PLACE_BALL: Invalid conditions for player ${action.playerId} on table ${action.tableId}. Has ball-in-hand: ${gameTable?.ballInHand}`
          );
        }
        break;
      ...
      case 'END_GAME':
        console.log(
          `applyActionLocally: Handling END_GAME for table ${action.tableId}`
        );
      ...
      case 'CHANGE_TURN':
        console.log(
          `applyActionLocally: Handling CHANGE_TURN for table ${action.tableId}`
        );
        ...
        if (gameTable && !gameTable.gameEnded) {
          ...
          if (nextPlayerId) {
            ...
            if (gameTable.currentTurn === nextPlayerId) {
              // Ensure turn actually changed
              gameTable.ballInHand = hasBallInHand;
            }
          } else {
            console.warn(
              `CHANGE_TURN action missing nextPlayerId for table ${action.tableId}`
            );
          }
        }
        break;
      ...
      case 'ENTRY_COMMITTED':
        console.log(
          `applyActionLocally: Ignoring system/unhandled action type: ${action.type}`
        );
        break;

      default:
        console.warn(
          `Unhandled action type in applyActionLocally: ${action.type}`
        );
  ...
  public requestMakeShot(
    ...
    if (table.ballInHand) {
      throw new Error(
        'Cannot make shot: Player has ball-in-hand. Place the cue ball first.'
      );
    }
    this.proposeAction({
      type: 'MAKE_SHOT',
  ...
  public requestPlaceBall(
    ...
    if (
      position.x < 0 ||
      position.x > table.tableWidth ||
      position.y < 0 ||
      position.y > table.tableHeight
    ) {
  ...
  private startGame(tableId: string): void {
    ...
    gameTable.balls = this.createInitialBallState(
  ...
  private makeShot(
    ...
    if (!gameTable || !gameTable.balls) {
      console.error(
        `makeShot Error: Invalid table or balls state for ${tableId}`
      );
      return;
    }
    ...
    if (gameTable.ballInHand) {
      console.error(
        `makeShot Error: Player ${playerId} has ball-in-hand. PLACE_BALL action required first.`
      );
    ...
    if (gameTable.physicsActive) {
      console.warn(
        `makeShot Warning: Physics already active for table ${tableId}. Ignoring shot.`
      );
      return;
    }
    ...
    const isBreak =
      gameTable.pocketedBalls.length === 0 &&
      (!gameTable.pocketedBallsBeforeShot ||
        gameTable.pocketedBallsBeforeShot.length === 0);
    if (isBreak) {
  ...
  private endGame(tableId: string, winnerId: string | null): void {
    ...
    console.log(
      `Ending game on table ${tableId}. Winner: ${winnerId || 'None'}`
    );
  ...
  private changeTurn(tableId: string, nextPlayerId: string): void {
    ...
    if (!gameTable || !gameTable.players[nextPlayerId]) {
      console.error(
        `changeTurn: Invalid next player ID ${nextPlayerId} for table ${tableId}`
      );
      return;
    }
    if (gameTable.currentTurn !== nextPlayerId) {
  ...
  private async analyzeShotOutcome(tableId: string): Promise<void> {
    ...
    if (!gameTable) {
      console.error(
        `analyzeShotOutcome: Missing gameTable for table ${tableId}`
      );
      return;
    }
    ...
    if (!refereeInput) {
      ...
      if (currentPlayerId !== opponentPlayerId) {
        ...
        console.warn(
          `Cannot determine opponent for table ${tableId}, turn remains.`
        );
      }
    ...
    try {
      ...
      console.error(
        `Error calling AI Referee service for table ${tableId}:`,
        error
      );
    ...
    if (refereeResult.foul) {
      ...
      console.log(
        `Ball-in-hand awarded due to foul: ${refereeResult.isBallInHand}`
      );

      // Handle special case: Break foul scratch gives different placement rules
      if (
        refereeResult.foul === FoulType.BREAK_FOUL &&
        refereeResult.reason?.includes('Scratch')
      ) {
    ...
    if (
      ...
      const pocketedThisShot = gameTable.balls
        .filter(
          (b) =>
            b.pocketed &&
            !refereeInput.tableStateBeforeShot.balls?.find(
              (pb) => pb.number === b.number
            )?.pocketed
        )
        .map((b) => b.number);
      ...
      if (firstLegalPocket) {
        ...
        if (pocketedType === 'solid') {
          ...
          console.log(
            `Assigned Solids to ${currentPlayerId}, Stripes to ${opponentPlayerId}`
          );
          ...
          console.log(
            `Assigned Stripes to ${currentPlayerId}, Solids to ${opponentPlayerId}`
          );
        }
      }
    }
    ...
    const eightBallPocketed =
      gameTable.balls.find((b) => b.number === 8)?.pocketed === true;
    const wasEightBallPocketedBefore =
      refereeInput.tableStateBeforeShot.balls?.find((b) => b.number === 8)
        ?.pocketed === true;
    ...
    if (eightBallPocketed && !wasEightBallPocketedBefore) {
      ...
      const legalTargets =
        playerType === 'solids'
          ...
          : playerType === 'stripes'
            ? [9, 10, 11, 12, 13, 14, 15]
            : [];
      const playerGroupCleared = legalTargets.every(
        (num) => gameTable.balls.find((b) => b.number === num)?.pocketed
      );
      ...
      if (refereeResult.foul) {
        console.log(
          `Game Over: ${currentPlayerId} pocketed 8-ball on a foul shot. ${opponentPlayerId} wins.`
        );
        winnerId = opponentPlayerId;
        loserId = currentPlayerId;
      } else if (playerType !== 'open' && !playerGroupCleared) {
        console.log(
          `Game Over: ${currentPlayerId} pocketed 8-ball before their group was cleared. ${opponentPlayerId} wins.`
        );
        winnerId = opponentPlayerId;
        loserId = currentPlayerId;
      } else if (playerType !== 'open' && playerGroupCleared) {
        console.log(
          `Game Over: ${currentPlayerId} legally pocketed the 8-ball. ${currentPlayerId} wins.`
        );
        ...
        if (!refereeResult.foul) {
          console.log(
            `Game Over: ${currentPlayerId} pocketed the 8-ball on the break shot legally. ${currentPlayerId} wins.`
          );
          winnerId = currentPlayerId;
          loserId = opponentPlayerId;
        } else {
          ...
          console.log(
            `Game Over: ${currentPlayerId} pocketed the 8-ball on break but committed a foul. ${opponentPlayerId} wins.`
          );
          winnerId = opponentPlayerId;
          loserId = currentPlayerId;
        }
      ...
      if (winnerId) {
        ...
        this.emit('game:ended', {
          tableId: tableId,
          winner: gameTable.players[winnerId],
        });
        gameTable.lastUpdatedLocally = Date.now();
        this.emit('state:updated', { tableId, state: gameTable });
    ...
    if (gameTable.currentTurn !== refereeResult.nextPlayerId) {
      ...
      if (refereeResult.isBallInHand) {
        console.log(
          `Ball-in-hand set for incoming player ${refereeResult.nextPlayerId}`
        );
    ...
    gameTable.lastUpdatedLocally = Date.now();
    console.log(
      `Shot analysis complete for table ${tableId}. Final state:`,
      gameTable
    );
    this.emit('state:updated', { tableId, state: gameTable });
  }
  ...
  public tick(tableId: string, deltaTime: number): void {
    ...
    gameTable.balls = gameTable.balls.map((originalBall, index) => {
      ...
      if (!updatedPhysics) {
        console.error(
          `Mismatch in physics update for ball index ${index}, number ${originalBall.number}`
        );
        return originalBall;
      }
      return {
    ...
    for (let i = 0; i < gameTable.balls.length; i++) {
      ...
      if (ball.number === 0 && !cueBallContactMade) {
        for (let j = 0; j < gameTable.balls.length; j++) {
          if (
            i === j ||
            gameTable.balls[j].number === 0 ||
            gameTable.balls[j].pocketed
          )
            continue;
      ...
      if (
        pos.x - radius < 0 ||
        pos.x + radius > gameTable.tableWidth ||
        pos.y - radius < 0 ||
        pos.y + radius > gameTable.tableHeight
      ) {
        hitRail = true;
    ...
    if (!anyBallMoving) {
      ...
      if (shotData.isBreakShot && shotData.ballsHittingRailOnBreak) {
        shotData.numberOfBallsHittingRailOnBreak =
          shotData.ballsHittingRailOnBreak.size;
        console.log(
          `Break shot rail count: ${shotData.numberOfBallsHittingRailOnBreak}`
        );
      ...
      this.analyzeShotOutcome(tableId).catch((error) => {
        console.error(
          `Error during async analyzeShotOutcome for table ${tableId}:`,
          error
        );
  ...
  private startGameLoop(tableId: string, intervalMs = 16): void {
    ...
    const loop = () => {
      ...
      try {
        ...
        console.error(
          `Error during game loop tick for table ${tableId}:`,
          error
        );
  ...
  async analyzeShot(shotData: ShotData): Promise<ShotResult> {
    ...
    const path = [
      cueBall,
      targetBall || {
        x: cueBall.x + Math.cos(angle) * 100,
        y: cueBall.y + Math.sin(angle) * 100,
      },
    ];
    const collisions = [];
