    ...
    def validate(cls) -> Dict[str, str]:
        ...
        for provider, config in cls.OAUTH_PROVIDERS.items():
            if config['client_id'] or config['client_secret']:
                if not config['client_id']:
                    errors[f'{provider.upper()}_CLIENT_ID'] = f'{provider} OAuth enabled but client_id missing'
                if not config['client_secret']:
                    errors[f'{provider.upper()}_CLIENT_SECRET'] = f'{provider} OAuth enabled but client_secret missing'
        ...
        if cls.ENV != 'development' and not cls.MAIL_SUPPRESS_SEND:
            if not cls.MAIL_USERNAME or not cls.MAIL_PASSWORD:
                errors['MAIL_CREDENTIALS'] = 'Mail username and password required for sending emails'
