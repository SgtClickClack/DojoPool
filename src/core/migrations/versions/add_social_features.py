...
def upgrade():
    ...
    op.execute("CREATE TYPE challenge_status AS ENUM ('joined', 'in_progress', 'completed', 'failed')")
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('joined', 'in_progress', 'completed', 'failed', name='challenge_status'), nullable=False),
