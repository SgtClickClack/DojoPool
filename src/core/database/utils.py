...
def backup_database(backup_path: Optional[str] = None) -> str:
    ...
    if backup_path is None:
        ...
        backup_path = os.path.join(
            backup_dir,
            f"backup_{current_app.config['ENV']}_{db.engine.url.database}.sql"
        )
    
    # SQLite backup
    if 'sqlite' in current_app.config['SQLALCHEMY_DATABASE_URI']:
        import sqlite3
        source = sqlite3.connect(current_app.config['SQLALCHEMY_DATABASE_URI'].replace('sqlite:///', ''))
...
def restore_database(backup_path: str) -> None:
    ...
    if 'sqlite' in current_app.config['SQLALCHEMY_DATABASE_URI']:
        ...
        dest = sqlite3.connect(current_app.config['SQLALCHEMY_DATABASE_URI'].replace('sqlite:///', ''))
        source.backup(dest)
        source.close()
        dest.close()
    # PostgreSQL restore
