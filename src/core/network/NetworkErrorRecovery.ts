...
export class NetworkErrorRecovery extends EventEmitter {
  ...
  private readonly failureWindow: Map<
    string,
    Array<{ timestamp: number; success: boolean }>
  >;
  private readonly retryBudget: Map<
    string,
  ...
  private updateFailureWindow(nodeId: string, success: boolean): void {
    ...
    while (
      window.length > 0 &&
      now - window[0].timestamp > this.config.circuitBreaker.slidingWindowSize
    ) {
      window.shift();
    }
  ...
  private incrementFailureCount(nodeId: string): void {
    ...
    console.log(
      'DEBUG: incrementFailureCount',
      nodeId,
      'new count:',
      count,
      'full map:',
      Array.from(this.failureCount.entries())
    );
  }
