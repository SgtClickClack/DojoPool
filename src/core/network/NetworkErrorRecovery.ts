...
import { NetworkError, NetworkMessage, NetworkMessageType, NetworkStats } from './types';
...
export class NetworkErrorRecovery extends EventEmitter {
  ...
  constructor(transport: NetworkTransport, config: Partial<ErrorRecoveryConfig> = {}) {
    super();
  ...
  private isResponseMessage(type: NetworkMessageType): boolean {
    return type === NetworkMessageType.APPEND_ENTRIES_RESPONSE ||
  ...
  private checkCircuitBreaker(nodeId: string): void {
    ...
    if (state === CircuitState.CLOSED && failures >= this.config.circuitBreaker.failureThreshold) {
  ...
  private async retryMessage(messageId: string, pending: PendingMessage): Promise<void> {
    const delay = this.calculateRetryDelay(pending.retries);
    await new Promise(resolve => setTimeout(resolve, delay));

    try {
      await this.transport.send(pending.message.target, pending.message.type, pending.message.payload);
  ...
  private handleMessageFailure(messageId: string, pending: PendingMessage): void {
  ...
  public async send<T>(target: string, type: NetworkMessageType, payload: T): Promise<void> {
