...
import { type NetworkMetrics } from './NetworkMetricsCollector';
...
export const NetworkMetricsDashboard: React.FC<
  NetworkMetricsDashboardProps
> = ({ nodeId, onAlert }) => {
  const [metricsHistory, setMetricsHistory] = useState<MetricsHistory[]>([]);
  const [currentMetrics, setCurrentMetrics] = useState<NetworkMetrics | null>(
    null
  );
    ...
    <div className="network-metrics-dashboard">
      ...
      <div className="metrics-summary">
        ...
        <div className="metric-card">
          <h3>Throughput</h3>
          <div>
            Messages/s: {currentMetrics.throughput.messagesPerSecond.toFixed(2)}
          </div>
          <div>
            Bytes/s:{' '}
            {(currentMetrics.throughput.bytesPerSecond / 1024).toFixed(2)} KB/s
          </div>
        </div>
        ...
        <div className="metric-card">
          ...
          <div>
            Leader Changes: {currentMetrics.consensusHealth.leaderChanges}
          </div>
          <div>
            Last Heartbeat:{' '}
            {new Date(
              currentMetrics.consensusHealth.lastLeaderHeartbeat
            ).toLocaleTimeString()}
          </div>
        </div>
      </div>
