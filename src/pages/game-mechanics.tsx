...
const GameMechanicsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    gameMechanicsService.on('challengeCreated', (challenge) => {
      addNotification(`Challenge created: ${challenge.type} against ${challenge.defenderId}`);
    });
    ...
    gameMechanicsService.on('matchStarted', (match) => {
      addNotification(`Match started: ${match.player1Id} vs ${match.player2Id}`);
    });
  ...
  return (
    <Layout>
        ...
        <Container maxWidth="xl" sx={{ py: 4 }}>
                ...
                <Grid item xs={12} sm={6} md={3}>
                  <Box sx={{ textAlign: 'center' }}>
                    <Typography variant="h4" color="primary">
                      {gameState?.matchState?.isInMatch ? 'In Match' : 'Available'}
                    </Typography>
                ...
                <Grid item xs={12} sm={6} md={3}>
                  <Box sx={{ textAlign: 'center' }}>
                    <Typography variant="h4" color="success">
                      {gameState?.territoryControl?.controlledDojos?.length || 0}
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
          ...
          <Card>
            <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
              <Tabs value={tabValue} onChange={handleTabChange} aria-label="game mechanics tabs">
                <Tab 
                  icon={<EmojiEvents />} 
                  label="Challenges" 
                  iconPosition="start"
                      ...
                      {mockDojos.map((dojo) => (
                        ...
                        >
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <Chip 
                              label={dojo.status} 
                              color={dojo.status === 'available' ? 'success' : dojo.status === 'controlled' ? 'warning' : 'error'}
                              size="small"
                            />
                          </Box>
                          <Typography variant="body2" color="text.secondary">
                            {dojo.distance} km away
                            {dojo.controller && ` • Controlled by ${dojo.controller}`}
                          </Typography>
                        </Box>
                      ))}
                    </CardContent>
                    ...
                    <CardContent>
                      ...
                      {gameState?.activeChallenges?.length > 0 ? (
                        gameState.activeChallenges.map((challenge: any) => (
                          <Box key={challenge.id} sx={{ mb: 2, p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
                    ...
                    <CardContent>
                      ...
                      <Box sx={{ p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
                        <Typography variant="body1">
                          {gameState?.currentLocation?.dojoName || 'Unknown Location'}
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                          Lat: {gameState?.currentLocation?.latitude?.toFixed(4)}, 
                          Lng: {gameState?.currentLocation?.longitude?.toFixed(4)}
                        </Typography>
                      </Box>
                    </CardContent>
                  </Card>
                      ...
                      {gameState?.isTraveling ? (
                        <Box sx={{ p: 2, bgcolor: 'warning.light', borderRadius: 1 }}>
                          ...
                          <Typography variant="body2" color="warning.contrastText">
                            ETA: {gameState.destination?.estimatedArrival?.toLocaleTimeString()}
                          </Typography>
                        </Box>
                      ) : (
      ...
      >
        ...
        <DialogContent>
          <TerritoryControlPanel
            dojoId={selectedDojo?.id || 'dojo-1'}
            dojoName={selectedDojo?.name || 'Unknown Dojo'}
            currentController={selectedDojo?.controller ? { id: 'controller-1', name: selectedDojo.controller } : undefined}
            playerClanId="clan-1"
      ...
      >
        ...
        <DialogContent>
          <PlayerMovementPanel
            currentLocation={gameState?.currentLocation || { latitude: 0, longitude: 0 }}
      ...
      >
        ...
        <DialogContent>
          <RealTimeMatchTracker
            matchId="match-1"
            onMatchStart={(match) => {
              addNotification(`Match started: ${match.player1.name} vs ${match.player2.name}`);
            }}
            onMatchEnd={(match) => {
