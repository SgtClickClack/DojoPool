...
async function handleCreateShare(req: NextApiRequest, res: NextApiResponse, user: any) {
  const {
    content_type,
    content_id,
    title,
  ...
  const validTypes = ['game', 'tournament', 'achievement', 'profile', 'shot', 'venue'];
...
async function handleGetShares(req: NextApiRequest, res: NextApiResponse, user: any) {
  const {
    page = '1',
    per_page = '20',
    content_types,
  ...
  const types = content_types ? (content_types as string).split(',') : undefined;
...
async function handleDeleteShare(req: NextApiRequest, res: NextApiResponse, user: any) {
