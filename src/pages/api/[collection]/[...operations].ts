...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (collection === 'venues' && operations[0] === 'by-location') {
      const result = await getVenuesByLocation(req.query.location as string, Number(req.query.limit) || 10);
      return res.status(result.success ? 200 : 400).json(result);
    }

    if (collection === 'tournaments' && operations[0] === 'upcoming') {
      const result = await getUpcomingTournaments(Number(req.query.limit) || 10);
      return res.status(result.success ? 200 : 400).json(result);
    }
