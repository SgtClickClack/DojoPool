...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Fetch comments
    const commentsSnapshot = await db.collection('games').doc(id).collection('comments').get();
    const comments = commentsSnapshot.docs.map((doc: QueryDocumentSnapshot<DocumentData>) => ({
      id: doc.id,
      ...doc.data(),
    })) as Comment[];
    ...
    const player1Shots = shots.filter(shot => shot.playerId === gameData.player1Id);
    const player2Shots = shots.filter(shot => shot.playerId === gameData.player2Id);
    ...
    const calculateAdvancedStats = (playerShots: Shot[]) => {
      ...
      const shotAccuracyByPosition = playerShots.reduce((acc, shot) => {
      ...
      const offensivePlayPercentage = (successfulShots.length / totalShots) * 100;
    ...
    const achievements: Achievement[] = [
      {
        ...
        icon: 'FiAward',
        unlocked: player1AdvancedStats.breakAndRun === 1 || player2AdvancedStats.breakAndRun === 1,
      ...
      },
      {
        ...
        icon: 'FiTrendingUp',
        unlocked: player1AdvancedStats.highestBreak >= 5 || player2AdvancedStats.highestBreak >= 5,
        timestamp: gameData.endTime,
      },
      {
        id: 'perfect-game',
        ...
        icon: 'FiAward',
        unlocked: player1Score > player2Score && player1Shots.filter(s => s.type === 'foul').length === 0 ||
                 player2Score > player1Score && player2Shots.filter(s => s.type === 'foul').length === 0,
        timestamp: gameData.endTime,
      },
    ];
    ...
    const response = {
      ...
      statistics: {
        ...
        averageShotTime: shots.length > 1
          ? (shots[shots.length - 1].timestamp - shots[0].timestamp) / (shots.length - 1)
          : 0,
        shotsPerMinute: shots.length > 0
          ? (shots.length / ((gameData.endTime || Date.now()) - gameData.startTime)) * 60000
          : 0,
      },
    };
