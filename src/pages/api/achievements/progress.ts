...
export default async function handler(
  ...
  try {
    ...
    if (!achievement_id || typeof progress !== 'number') {
      return res.status(400).json({ error: 'Achievement ID and progress are required' });
    }

    // Update achievement progress
    const [success, error, updated_achievement] = await AchievementService.update_achievement_progress(
      user.id,
      achievement_id,
      progress,
      metadata
    ...
    if (!success) {
      return res.status(400).json({ error: error || 'Failed to update achievement progress' });
    }
