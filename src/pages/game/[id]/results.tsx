...
const GameResultsPage: React.FC = () => {
  ...
  const { gameResult, gameSpecificRewards, loading, error, fetchGameResults } = useGameResults();
  ...
  if (!gameId) {
    return (
      <ProtectedRoute>
        <AuthenticatedLayout>
          <Paper sx={{ padding: 2, margin: 2 }}><Typography>No Game ID provided.</Typography></Paper>
        </AuthenticatedLayout>
      </ProtectedRoute>
    );
  }
    ...
    <ProtectedRoute>
      <AuthenticatedLayout>
        <Paper sx={{ padding: 3, margin: 2 }}>
          ...
          {error && <Typography color="error">Error loading results: {typeof error === 'string' ? error : 'An error occurred'}</Typography>}
          ...
          <Typography variant="h6" gutterBottom>Overall Rewards</Typography>
          ...
          <Box mt={3} display="flex" justifyContent="space-between">
            ...
            <Button variant="contained" onClick={() => router.push('/game/new')}>
              Play Again
            </Button>
          </Box>
        </Paper>
