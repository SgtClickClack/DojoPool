...
export class RetryMechanism {
  ...
  static getInstance(config?: Partial<RetryConfig>): RetryMechanism {
    if (!RetryMechanism.instance) {
      RetryMechanism.instance = new RetryMechanism(config);
    }
  ...
  wrap<T extends (...args: any[]) => Promise<any>>(
    ...
    return ((...args: Parameters<T>): Promise<Awaited<ReturnType<T>>> => {
      return this.execute(() => fn(...args), onRetry);
    }) as T;
  }
  ...
  async executeWithRetry<T>(
    ...
    return this.execute(operation, onRetry);
  }
}
