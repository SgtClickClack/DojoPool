...
describe('Game State Management', () => {
  ...
  it('handles game state persistence', async () => {
    ...
    expect(screen.getByTestId('player-1-score')).toHaveTextContent(currentScore || '');
  });
  ...
  it('manages game rules and scoring', async () => {
    ...
    // Test legal shot
    await act(async () => {
      fireEvent.change(screen.getByTestId('power-slider'), { target: { value: '50' } });
      fireEvent.change(screen.getByTestId('angle-slider'), { target: { value: '45' } });
      fireEvent.click(screen.getByTestId('shot-button'));
      await new Promise(resolve => setTimeout(resolve, 1000));
    });
