...
describe('GamePatterns Component', () => {
  ...
  it('displays shot distribution data', () => {
    ...
    expect(screen.getByText('Analysis of shot type frequencies and spatial distribution')).toBeInTheDocument();
  });
  ...
  it('displays player positioning data', () => {
    ...
    expect(screen.getByText('Analysis of player movement and positioning strategies')).toBeInTheDocument();
  });
  ...
  it('renders success patterns correctly', () => {
    ...
    Object.entries(mockPatterns.success_patterns).forEach(([key, value]) => {
      expect(screen.getByText(key.replace(/_/g, ' ').toUpperCase())).toBeInTheDocument();
  ...
  it('handles long sequence descriptions', () => {
    const patternsWithLongDescriptions = {
      ...
      common_sequences: [
        {
          description: 'A very long and detailed description of a complex shot sequence that involves multiple balls and precise positioning on the table with specific angles and power levels',
          frequency: 10,
          success_rate: 0.75
        }
      ]
    ...
    renderWithProviders(<GamePatterns patterns={patternsWithLongDescriptions} />);
    
    expect(screen.getByText(patternsWithLongDescriptions.common_sequences[0].description)).toBeInTheDocument();
  });
  ...
  it('maintains consistent layout with varying content lengths', () => {
    const mixedLengthPatterns = {
      ...
      common_sequences: [
        ...
        {
          description: 'A much longer sequence description that should still fit in the layout without breaking it',
          frequency: 2,
          success_rate: 0.5
        }
      ],
      success_patterns: {
        'short': 'Brief',
        'very_long_pattern_name': 'A detailed explanation of the pattern that spans multiple lines potentially'
      }
    };
    ...
    expect(screen.getByText('A much longer sequence description that should still fit in the layout without breaking it')).toBeInTheDocument();
