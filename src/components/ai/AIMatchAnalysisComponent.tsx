...
export const AIMatchAnalysisComponent: React.FC<AIMatchAnalysisComponentProps> = ({
  matchId,
  player1Id,
  player2Id,
  onAnalysisUpdate
  ...
  const requestCoaching = async (situation: string) => {
    try {
      const coachingData = await aiService.provideRealTimeCoaching(matchId, player1Id, situation);
      setCoaching(coachingData);
    } catch (error) {
    ...
    <div className="bg-white rounded-lg shadow-lg p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <div className="bg-blue-50 p-4 rounded-lg">
          <h3 className="text-lg font-semibold text-blue-800 mb-2">Current Score</h3>
        ...
        <div className="bg-orange-50 p-4 rounded-lg">
          <h3 className="text-lg font-semibold text-orange-800 mb-2">Pressure Level</h3>
          <div className="flex items-center">
            ...
            <span className="text-lg font-bold text-orange-600">{analysis.pressureLevel}%</span>
          </div>
        </div>

        {/* Match Prediction */}
        <div className="bg-green-50 p-4 rounded-lg">
          <h3 className="text-lg font-semibold text-green-800 mb-2">AI Prediction</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Player 1 Analysis</h3>
          <div className="space-y-3">
            <div className="flex justify-between">
              <span>Accuracy:</span>
              <span className={getPerformanceColor(analysis.player1Analysis.accuracy)}>
                {analysis.player1Analysis.accuracy}%
              </span>
            </div>
            <div className="flex justify-between">
              <span>Consistency:</span>
              <span className={getPerformanceColor(analysis.player1Analysis.consistency)}>
                {analysis.player1Analysis.consistency}%
              </span>
            </div>
            <div className="flex justify-between">
              <span>Strategy:</span>
              <span className={getPerformanceColor(analysis.player1Analysis.strategy)}>
                {analysis.player1Analysis.strategy}%
              </span>
            </div>
            <div className="flex justify-between">
              <span>Pressure Handling:</span>
              <span className={getPerformanceColor(analysis.player1Analysis.pressureHandling)}>
        ...
        {/* Player 2 */}
        <div className="bg-gray-50 p-4 rounded-lg">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Player 2 Analysis</h3>
          <div className="space-y-3">
            <div className="flex justify-between">
              <span>Accuracy:</span>
              <span className={getPerformanceColor(analysis.player2Analysis.accuracy)}>
                {analysis.player2Analysis.accuracy}%
              </span>
            </div>
            <div className="flex justify-between">
              <span>Consistency:</span>
              <span className={getPerformanceColor(analysis.player2Analysis.consistency)}>
                {analysis.player2Analysis.consistency}%
              </span>
            </div>
            <div className="flex justify-between">
              <span>Strategy:</span>
              <span className={getPerformanceColor(analysis.player2Analysis.strategy)}>
                {analysis.player2Analysis.strategy}%
              </span>
            </div>
            <div className="flex justify-between">
              <span>Pressure Handling:</span>
              <span className={getPerformanceColor(analysis.player2Analysis.pressureHandling)}>
      ...
      {/* Recent Shots */}
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Recent Shots</h3>
        <div className="space-y-2 max-h-64 overflow-y-auto">
          {recentShots.map((shot, index) => (
            <div key={shot.shotId} className="bg-gray-50 p-3 rounded-lg">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <div className={`w-3 h-3 rounded-full ${getShotTypeColor(shot.shotType)}`}></div>
                  <span className="font-medium">{shot.shotType}</span>
                  <span className="text-sm text-gray-600">by {shot.playerId}</span>
                </div>
                <div className="flex items-center space-x-4">
                  <span className={`text-sm ${shot.success ? 'text-green-600' : 'text-red-600'}`}>
                    {shot.success ? 'Success' : 'Miss'}
                  </span>
                  <span className="text-sm text-gray-600">Accuracy: {shot.accuracy}%</span>
                  <span className="text-sm text-gray-600">Power: {shot.power}%</span>
                </div>
              </div>
      ...
      {/* Real-time Coaching */}
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">AI Coaching</h3>
        ...
        {coaching && (
          <div className="bg-blue-50 p-4 rounded-lg">
            <h4 className="font-semibold text-blue-800 mb-2">Current Situation: {coaching.currentSituation}</h4>
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-gray-50 p-3 rounded-lg text-center">
            <div className="text-2xl font-bold text-blue-600">{analysis.performanceMetrics.totalShots}</div>
            <div className="text-sm text-gray-600">Total Shots</div>
          </div>
          <div className="bg-gray-50 p-3 rounded-lg text-center">
            <div className="text-2xl font-bold text-green-600">{analysis.performanceMetrics.successfulShots}</div>
            <div className="text-sm text-gray-600">Successful</div>
          </div>
          <div className="bg-gray-50 p-3 rounded-lg text-center">
            <div className="text-2xl font-bold text-red-600">{analysis.performanceMetrics.fouls}</div>
            <div className="text-sm text-gray-600">Fouls</div>
          </div>
          <div className="bg-gray-50 p-3 rounded-lg text-center">
            <div className="text-2xl font-bold text-purple-600">{analysis.performanceMetrics.averageShotTime}s</div>
      ...
      {analysis.keyMoments.length > 0 && (
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Key Moments</h3>
          <div className="space-y-2">
            {analysis.keyMoments.slice(0, 5).map((moment) => (
              <div key={moment.id} className="bg-gray-50 p-3 rounded-lg">
                <div className="flex items-center justify-between mb-1">
                  <span className="font-medium text-gray-800">{moment.type}</span>
                  <span className={`text-sm px-2 py-1 rounded ${
                    moment.impact === 'positive' ? 'bg-green-100 text-green-800' :
                    moment.impact === 'negative' ? 'bg-red-100 text-red-800' :
      ...
      {analysis.tacticalRecommendations.length > 0 && (
        <div>
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Tactical Recommendations</h3>
          <div className="space-y-2">
            {analysis.tacticalRecommendations.slice(0, 3).map((rec, index) => (
              <div key={index} className="bg-green-50 p-3 rounded-lg">
                <div className="font-medium text-green-800 mb-1">{rec.situation}</div>
                <div className="text-green-700 mb-2">{rec.recommendedAction}</div>
