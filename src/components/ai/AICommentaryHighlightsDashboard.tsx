...
import { useAICommentaryHighlights, GeneratedHighlight, HighlightGenerationRequest } from '../../hooks/useAICommentaryHighlights';
...
interface CommentaryEvent {
  id: string;
  timestamp: Date;
  type: 'shot' | 'foul' | 'score' | 'highlight' | 'analysis' | 'blessing' | 'fluke';
  content: string;
  audioUrl?: string;
  excitementLevel: number;
  confidence: number;
...
export const AICommentaryHighlightsDashboard: React.FC<AICommentaryHighlightsDashboardProps> = ({ className = '' }) => {
  const {
    config,
    updateConfig,
    generateHighlights,
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'generate' | 'highlights' | 'config' | 'metrics'>('overview');
  ...
  const sampleMatches = [
    { id: 'match-1', name: 'Brisbane Championship Final', players: ['Player A', 'Player B'] },
    { id: 'match-2', name: 'Quarter Final Match', players: ['Player C', 'Player D'] },
    { id: 'match-3', name: 'Semi Final Match', players: ['Player E', 'Player F'] }
  ];
  ...
  const handleShareHighlight = async (highlightId: string) => {
    try {
      await shareHighlight(highlightId, ['twitter', 'facebook'], 'Check out this amazing pool highlight!');
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-lg shadow p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Total Highlights</h3>
          <p className="text-3xl font-bold text-blue-600">{metrics?.totalHighlights || 0}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Audio Generated</h3>
          <p className="text-3xl font-bold text-green-600">{metrics?.audioGenerated || 0}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Videos Generated</h3>
          <p className="text-3xl font-bold text-purple-600">{metrics?.videosGenerated || 0}</p>
        </div>
      </div>
  ...
  const renderGenerate = () => (
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Generate New Highlights</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Commentary Style</label>
  ...
  const renderHighlights = () => (
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">All Highlights</h3>
  ...
  const renderConfig = () => (
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">AI Commentary Configuration</h3>
            ...
            <div>
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e) => updateConfig({ realTimeCommentary: e.target.checked })}
                  className="mr-2"
                />
            ...
            <div>
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e) => updateConfig({ highlightGeneration: e.target.checked })}
                  className="mr-2"
                />
              ...
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e) => updateConfig({ 
                    audioSynthesis: { ...config.audioSynthesis, enabled: e.target.checked }
                  })}
                  className="mr-2"
                />
                Audio Synthesis
            ...
            <div>
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e) => updateConfig({ 
                    videoGeneration: { ...config.videoGeneration, enabled: e.target.checked }
                  })}
                  className="mr-2"
                />
                Video Generation
            ...
            <div>
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e) => updateConfig({ 
                    socialSharing: { ...config.socialSharing, enabled: e.target.checked }
                  })}
                  className="mr-2"
                />
                Social Sharing
        ...
        {metrics && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-blue-600">{metrics.totalHighlights}</p>
              <p className="text-sm text-gray-600">Total Highlights</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-green-600">{metrics.audioGenerated}</p>
              <p className="text-sm text-gray-600">Audio Generated</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-purple-600">{metrics.videosGenerated}</p>
              <p className="text-sm text-gray-600">Videos Generated</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-orange-600">{metrics.sharesGenerated}</p>
  ...
  return (
    <div className={`bg-gray-50 min-h-screen ${className}`}>
      <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">AI-Powered Match Commentary & Highlights</h1>
          <p className="mt-2 text-gray-600">
            Generate AI-powered commentary, highlights, and social media content for pool matches
          </p>
        </div>
