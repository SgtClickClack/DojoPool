...
const TournamentStreaming: React.FC = () => {
  ...
  const [commentaryType, setCommentaryType] = useState<'shot' | 'foul' | 'game_end' | 'highlight' | 'analysis'>('shot');
  ...
  const handleAddCommentary = () => {
    if (newCommentary.trim()) {
      streamingService.addCommentary({
        ...
        priority: commentaryExcitement > 7 ? 'high' : commentaryExcitement > 4 ? 'medium' : 'low',
      });
      setNewCommentary('');
    }
  };
    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="subtitle1" sx={{ color: '#00a8ff', mb: 2 }}>
          Advanced live streaming and broadcasting system with real-time commentary
        </Typography>
        ...
        >
          {isConnected ? 'Connected to streaming service' : 'Disconnected from streaming service'}
        </Alert>
      </Box>
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
        ...
        >
          ...
          <Tab label="Commentary" icon={<RecordVoiceOver />} iconPosition="start" />
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} lg={8}>
            <Card sx={{ bgcolor: '#1a1a1a', border: '1px solid #333', mb: 3 }}>
              <CardContent>
                <Box sx={{ position: 'relative', bgcolor: '#000', borderRadius: 2, p: 2, mb: 2 }}>
                  <Typography variant="h6" sx={{ color: '#ffffff', textAlign: 'center', py: 8 }}>
                ...
                <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                  <Button
                    ...
                    sx={{ bgcolor: isStreaming ? '#ff4444' : '#00ff9d', color: '#000' }}
                  >
              ...
              <CardContent>
                ...
                <List sx={{ maxHeight: 400, overflow: 'auto' }}>
                  {commentary.map((event) => (
                    <ListItem key={event.id} sx={{ borderBottom: '1px solid #333' }}>
                      <ListItemAvatar>
                      ...
                      <ListItemText
                        ...
                        secondary={
                          <Typography variant="body2" sx={{ color: '#888' }}>
                            {event.timestamp.toLocaleTimeString()} - Excitement: {event.excitement}/10
                          </Typography>
                        }
                      />
                    </ListItem>
            ...
            <Card sx={{ bgcolor: '#1a1a1a', border: '1px solid #333' }}>
              <CardContent>
                ...
                <Slider
                  ...
                  onChange={(e, value) => setCommentaryExcitement(value as number)}
                  min={1}
                  max={10}
                  sx={{ mb: 2 }}
                />
              ...
              <CardContent>
                ...
                <List sx={{ maxHeight: 400, overflow: 'auto', mb: 2 }}>
                  {chatMessages.map((message) => (
                    <ListItem key={message.id} sx={{ borderBottom: '1px solid #333' }}>
                      <ListItemAvatar>
                        <Avatar sx={{ bgcolor: message.type === 'moderator' ? '#00ff9d' : '#00a8ff' }}>
                        ...
                        primary={
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            ...
                            {message.type === 'moderator' && (
                              <Chip label="MOD" size="small" sx={{ bgcolor: '#00ff9d', color: '#000' }} />
                            )}
                            {message.donation && (
                              <Chip 
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  ...
                  <IconButton 
                    ...
                    sx={{ bgcolor: '#00ff9d', color: '#000', '&:hover': { bgcolor: '#00cc7a' } }}
                  >
                    <Send />
                  </IconButton>
                </Box>
              ...
              <CardContent>
                ...
                {stats && (
                  <Box>
                    ...
                    <Typography variant="body2" sx={{ color: '#888', mb: 1 }}>
                      Average Watch Time: {stats.averageWatchTime.toFixed(1)} minutes
                    </Typography>
                  </Box>
                )}
              </CardContent>
                    ...
                    <FormControlLabel
                      control={
                        <Switch
                          ...
                          onChange={(e) => handleConfigUpdate({ videoEnabled: e.target.checked })}
                    ...
                    <FormControlLabel
                      control={
                        <Switch
                          ...
                          onChange={(e) => handleConfigUpdate({ audioEnabled: e.target.checked })}
                          sx={{ '& .MuiSwitch-switchBase.Mui-checked': { color: '#00ff9d' } }}
                        />
                      }
                      label="Audio Enabled"
                    ...
                    <FormControlLabel
                      control={
                        <Switch
                          ...
                          onChange={(e) => handleConfigUpdate({ commentaryEnabled: e.target.checked })}
                          sx={{ '& .MuiSwitch-switchBase.Mui-checked': { color: '#00ff9d' } }}
                        />
                      }
                    ...
                    <FormControlLabel
                      control={
                        <Switch
                          ...
                          onChange={(e) => handleConfigUpdate({ chatEnabled: e.target.checked })}
                          sx={{ '& .MuiSwitch-switchBase.Mui-checked': { color: '#00ff9d' } }}
                        />
                      }
                      label="Chat Enabled"
                    ...
                    <FormControlLabel
                      control={
                        <Switch
                          ...
                          onChange={(e) => handleConfigUpdate({ recordingEnabled: e.target.checked })}
                          sx={{ '& .MuiSwitch-switchBase.Mui-checked': { color: '#00ff9d' } }}
                        />
                      }
