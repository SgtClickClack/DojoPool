...
describe('ShotAnalysis', () => {
  ...
  it('displays all tips and warnings', async () => {
    ...
    await waitFor(() => {
      // Check for tips
      expect(screen.getByText('Keep your cue level for better accuracy')).toBeInTheDocument();
      expect(screen.getByText('Focus on the target ball, not the cue ball')).toBeInTheDocument();
      expect(screen.getByText('Maintain a steady stance')).toBeInTheDocument();

      // Check for warnings
      expect(screen.getByText('High spin may affect accuracy')).toBeInTheDocument();
      expect(screen.getByText('Power level might be too high')).toBeInTheDocument();
