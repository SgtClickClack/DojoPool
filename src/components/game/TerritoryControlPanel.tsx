...
const TerritoryControlPanel: React.FC<TerritoryControlPanelProps> = ({
  ...
  const [contestType, setContestType] = useState<'standard' | 'high-stakes' | 'clan-war'>('standard');
  ...
  return (
    <Card sx={{ maxWidth: 600, width: '100%' }}>
              ...
              <CardContent>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                  <Typography variant="h6">
                    {isUnclaimed ? 'Unclaimed Territory' : 'Controlled Territory'}
                  </Typography>
                  <Chip
                ...
                {currentController ? (
                  <Box>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      <strong>Last Contested:</strong> {territoryStats.lastContested.toLocaleDateString()}
                    </Typography>
                  </Box>
                ) : (
                  <Typography variant="body2" color="text.secondary">
                    This territory is available for claiming. Be the first to establish control!
                  </Typography>
                )}
              </CardContent>
            </Card>
                  ...
                  <ListItemText
                    primary={
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        {defender.name}
                        {defender.isMaster && <Star color="secondary" fontSize="small" />}
                      </Box>
                    }
        ...
        <DialogContent>
          ...
          <Alert severity="info" sx={{ mt: 2 }}>
            <Typography variant="body2">
              Claiming a territory will make you responsible for defending it against challengers.
              You'll earn daily revenue and gain influence in the area.
            </Typography>
          </Alert>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={showContestDialog} onClose={() => setShowContestDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
                ...
                <Grid item xs={12} key={type}>
                  <Card 
                    ...
                    sx={{ 
                      ...
                      borderColor: contestType === type ? 'primary.main' : 'divider'
                    }}
                  ...
                  >
                    <CardContent>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <Chip label={`${requirements.cost} Coins`} color="primary" size="small" />
                      </Box>
                      <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                        Min Level: {requirements.minLevel} | Min Wins: {requirements.minWins}
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
          ...
          <Alert severity="warning" sx={{ mt: 2 }}>
            <Typography variant="body2">
              Contesting a territory will initiate a challenge against the current controller.
              Make sure you're ready for the battle!
            </Typography>
          </Alert>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setShowContestDialog(false)}>
            Cancel
          </Button>
          <Button
            variant="contained"
