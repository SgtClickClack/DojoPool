...
export const ShotAnalysis: React.FC<ShotAnalysisProps> = ({
  ...
  const simulateShotAnalysis = async (): Promise<ShotAnalysisResult> => {
    ...
    const successProbability = Math.max(0, 1 - (powerDiff + angleDiff + spinDiff) / 3);

    return {
      successProbability,
      difficulty: successProbability > 0.7 ? 'easy' : successProbability > 0.4 ? 'medium' : 'hard',
      recommendedPower: 70,
      ...
      <VStack spacing={4} align="stretch">
        ...
        <Box>
          <HStack justify="space-between" mb={2}>
            ...
            <Badge colorScheme={analysis.successProbability > 0.7 ? 'green' : analysis.successProbability > 0.4 ? 'yellow' : 'red'}>
              {Math.round(analysis.successProbability * 100)}%
            </Badge>
          </HStack>
          <Progress
            value={analysis.successProbability * 100}
            colorScheme={analysis.successProbability > 0.7 ? 'green' : analysis.successProbability > 0.4 ? 'yellow' : 'red'}
            size="sm"
          />
        </Box>
        ...
        {/* Shot Difficulty */}
        <Box>
          <Text fontWeight="bold" mb={2}>Shot Difficulty</Text>
          <Badge
            colorScheme={
              analysis.difficulty === 'easy' ? 'green' :
              analysis.difficulty === 'medium' ? 'yellow' : 'red'
            }
          >
        ...
        {/* Warnings */}
        <Box>
          <Text fontWeight="bold" mb={2}>Warnings</Text>
          <VStack spacing={2} align="stretch">
