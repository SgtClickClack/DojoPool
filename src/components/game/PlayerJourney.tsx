...
const PlayerJourney: React.FC = () => {
  ...
  const [selectedObjective, setSelectedObjective] = useState<StoryObjective | null>(null);

  const getObjectiveIcon = (type: string) => {
    switch (type) {
      case 'main': return <FlagOutlined style={{ color: '#ff4d4f' }} />;
      case 'side': return <StarOutlined style={{ color: '#faad14' }} />;
      case 'social': return <TeamOutlined style={{ color: '#52c41a' }} />;
      case 'tournament': return <TrophyOutlined style={{ color: '#1890ff' }} />;
  ...
  const getEventIcon = (type: string) => {
    switch (type) {
      case 'social': return <TeamOutlined style={{ color: '#52c41a' }} />;
      case 'tournament': return <TrophyOutlined style={{ color: '#faad14' }} />;
      case 'achievement': return <StarOutlined style={{ color: '#1890ff' }} />;
      case 'venue_discovery': return <CompassOutlined style={{ color: '#722ed1' }} />;
      case 'venue_mastery': return <FireOutlined style={{ color: '#ff4d4f' }} />;
      case 'venue_special': return <FlagOutlined style={{ color: '#13c2c2' }} />;
      default: return <BookOutlined />;
    }
  };
  ...
  const experienceProgress = (playerStats.experience / playerStats.experienceToNext) * 100;

  return (
    <div className="player-journey" style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto' }}>
      {/* Player Stats Header */}
      <Card 
        title={
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <Badge count={playerStats.level} showZero style={{ backgroundColor: '#52c41a' }} />
                <span>Level {playerStats.level}</span>
                {playerStats.clanRank && (
                  <Badge count={playerStats.clanRank} style={{ backgroundColor: '#1890ff' }} />
                )}
              </div>
            </div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
          <div>
            ...
            <Progress 
              ...
              format={() => `${playerStats.experience}/${playerStats.experienceToNext} XP`}
              strokeColor="#52c41a"
            />
          </div>
          <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {currentObjectives.map((objective) => (
              <Card
                ...
                style={{ 
                  cursor: 'pointer',
                  border: selectedObjective?.id === objective.id ? '2px solid #1890ff' : '1px solid #d9d9d9'
                }}
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                  ...
                  <Badge 
                    ...
                    style={{ 
                      backgroundColor: objective.type === 'main' ? '#ff4d4f' : 
                                   objective.type === 'side' ? '#faad14' : '#52c41a'
                    }} 
                  />
                </div>
                <p style={{ margin: '8px 0', color: '#666' }}>{objective.description}</p>
                {objective.location && (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '8px' }}>
                  <Progress 
                    ...
                    size="small"
                    format={() => `${objective.progress}/${objective.maxProgress}`}
                  />
                  <span style={{ fontSize: '12px', color: '#52c41a' }}>Reward: {objective.reward}</span>
                </div>
                {objective.isActive && (
                  <Button 
                    type="primary" 
              ...
              <Alert
                ...
                description={
                  <div>
                    ...
                    <span style={{ fontSize: '10px', color: '#999' }}>{event.timestamp.toLocaleString()}</span>
                  </div>
                }
                type="info"
                showIcon={false}
      ...
      {selectedObjective && (
        <Card 
          ...
          extra={
            ...
            >
              {selectedObjective.isActive ? 'Begin Challenge' : 'Objective Locked'}
            </Button>
          }
        >
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
            <div>
              ...
              {selectedObjective.location && (
                <p><strong>Location:</strong> {selectedObjective.location}</p>
              )}
              <p><strong>Reward:</strong> {selectedObjective.reward}</p>
            </div>
            <div>
              <h4>Progress</h4>
              <Progress 
                percent={(selectedObjective.progress / selectedObjective.maxProgress) * 100}
                format={() => `${selectedObjective.progress}/${selectedObjective.maxProgress}`}
                strokeColor="#52c41a"
              />
              <Divider />
              <h4>Tips</h4>
