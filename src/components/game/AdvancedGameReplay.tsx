...
const AdvancedGameReplay: React.FC = () => {
  ...
  const handleCreateSession = async () => {
    try {
      await service.createReplaySession({
        ...
        duration: (newSession.endTime.getTime() - newSession.startTime.getTime()) / 1000,
        events: [],
        highlights: [],
        analysis: {
          totalEvents: 0,
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {tabValue === 1 && (
        <Box>
          <Typography variant="h6" sx={{ mb: 2 }}>Replay Player</Typography>
          
          {selectedSession ? (
            <Box>
              <Card sx={{ mb: 2 }}>
                <CardContent>
                  <Typography variant="h6" sx={{ mb: 2 }}>{selectedSession.title}</Typography>
              ...
              <Typography variant="h6" sx={{ mb: 2 }}>Session Events</Typography>
              <Grid container spacing={2}>
                          ...
                          <AccordionDetails>
                            <Box>
                              <Typography variant="body2" sx={{ mb: 1 }}>
                                <strong>Technique:</strong> {event.aiAnalysis.technique}
                              </Typography>
                              <Typography variant="body2" sx={{ mb: 1 }}>
                                <strong>Strategy:</strong> {event.aiAnalysis.strategy}
                              </Typography>
                              <Typography variant="body2" sx={{ mb: 1 }}>
                                <strong>Success Probability:</strong> {(event.aiAnalysis.prediction.successProbability * 100).toFixed(1)}%
                              </Typography>
                              <Typography variant="body2" sx={{ mb: 1 }}>
                                <strong>Improvements:</strong>
                              </Typography>
                              <List dense>
                                {event.aiAnalysis.improvement.map((improvement, index) => (
                                  <ListItem key={index}>
      ...
      {tabValue === 2 && (
        <Box>
          <Typography variant="h6" sx={{ mb: 2 }}>All Replay Events</Typography>
      ...
      {tabValue === 3 && metrics && (
        <Box>
          <Typography variant="h6" sx={{ mb: 2 }}>Replay Analytics</Typography>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12}>
              <Card>
                <CardContent>
                  <Typography variant="h6" sx={{ mb: 2 }}>Popular Sessions</Typography>
                  <Grid container spacing={2}>
                      ...
                      <FormControlLabel
                        control={
                          <Switch
                            ...
                            onChange={(e) => setConfig({ ...config, highlightDetection: e.target.checked })}
                          />
                        }
                      ...
                      <FormControlLabel
                        control={
                          <Switch
                            ...
                            onChange={(e) => setConfig({ ...config, publicByDefault: e.target.checked })}
                          />
                        }
          ...
          <Grid container spacing={2} sx={{ pt: 2 }}>
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={(e) => setNewSession({ ...newSession, matchId: e.target.value })}
              />
            </Grid>
            <Grid item xs={12} md={6}>
              <TextField
          ...
          <Grid container spacing={2} sx={{ pt: 2 }}>
            ...
            <Grid item xs={12}>
              <TextField
                ...
                onChange={(e) => setNewComment({ ...newComment, eventId: e.target.value })}
              />
            </Grid>
          </Grid>
        </DialogContent>
      ...
      <Dialog open={!!selectedSession} onClose={() => setSelectedSession(null)} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          {selectedSession && (
            <Box sx={{ pt: 2 }}>
              <Typography variant="h6" sx={{ mb: 2 }}>{selectedSession.title}</Typography>
              <Typography variant="body1" sx={{ mb: 2 }}>{selectedSession.description}</Typography>
              ...
              <Grid container spacing={2} sx={{ mb: 2 }}>
                ...
                <Grid item xs={6}>
                  <Typography variant="body2" color="text.secondary">
                    Average Quality: {selectedSession.analysis.averageShotQuality.toFixed(1)}%
                  </Typography>
                </Grid>
                <Grid item xs={6}>
                  <Typography variant="body2" color="text.secondary">
              ...
              <Accordion>
                ...
                <AccordionDetails>
                  <Box>
                    ...
                    <Typography variant="h6" sx={{ mb: 1 }}>Improvement Areas:</Typography>
                    <List dense>
                      {selectedSession.analysis.improvementAreas.map((area, index) => (
                        <ListItem key={index}>
      ...
      <Dialog open={!!selectedEvent} onClose={() => setSelectedEvent(null)} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          {selectedEvent && (
            <Box sx={{ pt: 2 }}>
              <Typography variant="h6" sx={{ mb: 2 }}>{selectedEvent.description}</Typography>
              ...
              <Accordion>
                ...
                <AccordionDetails>
                  <Box>
                    <Typography variant="body1" sx={{ mb: 2 }}>
                      <strong>Technique:</strong> {selectedEvent.aiAnalysis.technique}
                    </Typography>
                    <Typography variant="body1" sx={{ mb: 2 }}>
                      <strong>Strategy:</strong> {selectedEvent.aiAnalysis.strategy}
                    </Typography>
                    <Typography variant="body1" sx={{ mb: 2 }}>
                      <strong>Success Probability:</strong> {(selectedEvent.aiAnalysis.prediction.successProbability * 100).toFixed(1)}%
                    </Typography>
                    <Typography variant="body1" sx={{ mb: 2 }}>
                      <strong>Expected Outcome:</strong> {selectedEvent.aiAnalysis.prediction.expectedOutcome}
                    </Typography>
                    
                    <Typography variant="h6" sx={{ mb: 1 }}>Improvements:</Typography>
                    <List dense>
                      {selectedEvent.aiAnalysis.improvement.map((improvement, index) => (
                        <ListItem key={index}>
                          <ListItemText primary={improvement} />
                        </ListItem>
                      ))}
