...
const mockEvents = [
  ...
  { id: 2, time: '14:33:12', message: 'Player 1 shot detected', severity: 'info' },
  { id: 3, time: '14:34:27', message: 'Foul detected on black ball', severity: 'warning' },
  { id: 4, time: '14:35:41', message: 'Illegal ball movement detected', severity: 'error' },
  { id: 5, time: '14:36:18', message: 'Player 2 shot detected', severity: 'info' },
  { id: 6, time: '14:37:55', message: 'Ball in pocket detected: red', severity: 'info' },
...
const AIReferee: React.FC = () => {
  const [status, setStatus] = useState<'Idle' | 'Monitoring' | 'Paused'>('Idle');
  const [events, setEvents] = useState(mockEvents);
  const [ballCount, setBallCount] = useState<number>(0);
  const [detectionConfidence, setDetectionConfidence] = useState<number>(0);
  const [lastAnalysisTime, setLastAnalysisTime] = useState<string>('');
  const [circles, setCircles] = useState<Array<{x: number, y: number, radius: number}>>([]);
  ...
  useEffect(() => {
    ...
    const socket = io(process.env.NEXT_PUBLIC_WS_URL || '/socket.io', socketOptions);
  ...
  return (
    <Box sx={{ width: '100%', minHeight: '80vh', mt: 2 }}>
      ...
      <Paper sx={cyberpunkPaper}>
        <Typography variant="h4" sx={{ fontWeight: 700, color: '#00fff7', mb: 3, letterSpacing: 2, textAlign: 'center' }}>
          AI Referee System
        </Typography>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={8}>
            <Paper sx={videoFeedStyle}>
              {isLoading ? (
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                  <VideocamIcon sx={{ fontSize: 80, color: '#00fff7', opacity: 0.7, mb: 2 }} />
                  <Typography variant="h6" sx={{ color: '#00fff7', textAlign: 'center' }}>
                    Video Feed
                  </Typography>
                  <Typography variant="body2" sx={{ color: '#aaa', textAlign: 'center', mt: 1 }}>
                    {status === 'Idle' 
          ...
          <Grid item xs={12} md={4}>
            <Paper sx={statusPanelStyle}>
              <Typography variant="h6" sx={{ color: '#ff00ea', mb: 2, borderBottom: '1px solid #ff00ea', pb: 1 }}>
                Status Panel
              </Typography>
              
              <Box sx={{ mb: 2 }}>
                <Typography variant="body2" sx={{ color: '#aaa', mb: 0.5 }}>Current State:</Typography>
                <Chip 
                  label={status} 
                  sx={{ 
                    bgcolor: status === 'Monitoring' ? '#00ff9d' : status === 'Paused' ? '#ffaa00' : '#aaa',
                    color: '#000',
                    fontWeight: 'bold',
                    fontSize: '1rem',
                    py: 2,
              ...
              <Box sx={{ mb: 2 }}>
                <Typography variant="body2" sx={{ color: '#aaa', mb: 0.5 }}>Frame Analysis:</Typography>
              ...
              <Box sx={{ mb: 2 }}>
                <Typography variant="body2" sx={{ color: '#aaa', mb: 0.5 }}>Balls Detected:</Typography>
                <Typography variant="h6" sx={{ color: '#00fff7' }}>{ballCount}</Typography>
              </Box>

              <Box sx={{ mb: 2 }}>
                <Typography variant="body2" sx={{ color: '#aaa', mb: 0.5 }}>Detection Confidence:</Typography>
                <Typography variant="h6" sx={{ color: '#00fff7' }}>{detectionConfidence}%</Typography>
              </Box>
              
              <Box sx={{ mb: 2 }}>
                <Typography variant="body2" sx={{ color: '#aaa', mb: 0.5 }}>Last Analysis:</Typography>
                <Typography variant="body1" sx={{ color: '#00fff7' }}>
                  {lastAnalysisTime ? new Date(lastAnalysisTime).toLocaleTimeString('en-US', { hour12: false }) : 'N/A'}
                </Typography>
              </Box>
              
              <Box sx={{ mb: 2 }}>
                <Typography variant="body2" sx={{ color: '#aaa', mb: 0.5 }}>Active Rules:</Typography>
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                  <Chip label="8-Ball" size="small" sx={{ bgcolor: '#00fff7', color: '#222' }} />
                  <Chip label="Fouls" size="small" sx={{ bgcolor: '#00fff7', color: '#222' }} />
                  <Chip label="Scratches" size="small" sx={{ bgcolor: '#00fff7', color: '#222' }} />
                </Box>
              </Box>
              
              <Box>
                <Typography variant="body2" sx={{ color: '#aaa', mb: 0.5 }}>System Health:</Typography>
                <Chip label="Optimal" sx={{ bgcolor: '#00ff9d', color: '#222' }} />
              </Box>
            </Paper>
          </Grid>
              ...
              <List dense>
                {events.map((event, index) => (
                  <React.Fragment key={event.id}>
                    ...
                    {index < events.length - 1 && <Divider sx={{ borderColor: 'rgba(0, 255, 247, 0.2)' }} />}
                  </React.Fragment>
                ))}
              </List>
            </Paper>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'center', mt: 2 }}>
              ...
              <Button
                ...
                sx={{
                  ...
                  borderColor: '#00fff7',
                  color: '#00fff7',
                  '&:hover': { borderColor: '#00cccc', color: '#00cccc', bgcolor: 'rgba(0, 255, 247, 0.1)' }
                }}
              >
                Settings
              </Button>
