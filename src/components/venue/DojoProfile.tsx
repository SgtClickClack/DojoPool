...
const DojoProfileComponent: React.FC = () => {
  ...
  const [selectedStyle, setSelectedStyle] = useState<VenueTheme['backgroundStyle']>('cyberpunk');
  ...
  const handleGenerateTheme = async () => {
    ...
    try {
      const newTheme = await profileService.generateTheme(themePrompt, selectedStyle);
      if (profile) {
  ...
  const handleCreateReward = async (reward: Omit<DojoCoinReward, 'id' | 'currentRedemptions'>) => {
    try {
      const newReward = await profileService.createReward(reward);
      if (profile) {
        const updatedProfile = { ...profile, rewards: [...profile.rewards, newReward] };
        setProfile(updatedProfile);
        await profileService.updateProfile(updatedProfile);
      }
  ...
  const handleCreateSpecial = async (special: Omit<VenueSpecial, 'id'>) => {
    try {
      ...
      if (profile) {
        const updatedProfile = { ...profile, specials: [...profile.specials, newSpecial] };
        setProfile(updatedProfile);
        await profileService.updateProfile(updatedProfile);
      }
  ...
  const handleCreateStory = async (story: Omit<VenueStory, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
      const newStory = await profileService.createStory(story);
      if (profile) {
        const updatedProfile = { ...profile, stories: [...profile.stories, newStory] };
        setProfile(updatedProfile);
        await profileService.updateProfile(updatedProfile);
      }
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
      <Box sx={{ mb: 4 }}>
        ...
        >
          {isConnected ? 'Connected to Dojo Profile service' : 'Disconnected from Dojo Profile service'}
        </Alert>
      </Box>
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
        ...
        >
          <Tab label="Theme & Branding" icon={<Palette />} iconPosition="start" />
            ...
            <Card sx={{ bgcolor: '#1a1a1a', border: '1px solid #333' }}>
              <CardContent>
                ...
                <FormControl fullWidth sx={{ mb: 2 }}>
                  ...
                  <Select
                    value={selectedStyle}
                    onChange={(e) => setSelectedStyle(e.target.value as VenueTheme['backgroundStyle'])}
                  >
              ...
              <CardContent>
                ...
                <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
                  ...
                  <Chip 
                    ...
                    sx={{ bgcolor: profile.theme.secondaryColor, color: '#000' }}
                  />
                  <Chip 
              ...
              <Card sx={{ bgcolor: '#1a1a1a', border: '1px solid #333' }}>
                <CardContent>
                  ...
                  {reward.maxRedemptions && (
                    <Typography variant="body2" sx={{ color: '#888' }}>
                      {reward.currentRedemptions}/{reward.maxRedemptions} redemptions
                    </Typography>
                  )}
                </CardContent>
              </Card>
