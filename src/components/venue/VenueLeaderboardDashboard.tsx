...
import { useVenueLeaderboard, LeaderboardEntry, DojoMaster, PlayerPerformance } from '../../hooks/useVenueLeaderboard';
import { 
  Card, 
  CardContent, 
  CardHeader, 
...
export const VenueLeaderboardDashboard: React.FC<VenueLeaderboardDashboardProps> = ({
  selectedVenueId,
  onVenueSelect
}) => {
  const {
  ...
  const [activeTab, setActiveTab] = useState<'leaderboard' | 'dojo-masters'>('leaderboard');
  ...
  const calculateReignDuration = (designationDate: Date) => {
    const days = Math.floor((Date.now() - new Date(designationDate).getTime()) / (1000 * 60 * 60 * 24));
    return days;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Venue Leaderboard</h1>
          <p className="text-gray-600 mt-1">Track venue performance and Dojo Masters</p>
        </div>
        <Button onClick={refreshLeaderboard} variant="outline">
          <RefreshCw className="w-4 h-4 mr-2" />
          Refresh
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <CardTitle className="text-xl">{entry.venueName}</CardTitle>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  <div className="text-center">
                    <div className="text-lg font-semibold">{entry.performance.totalMatches}</div>
                    <div className="text-sm text-gray-500">Matches</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-semibold">{entry.performance.totalTournaments}</div>
                    <div className="text-sm text-gray-500">Tournaments</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-semibold">{entry.performance.totalPlayers}</div>
                    <div className="text-sm text-gray-500">Players</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-semibold">{formatCurrency(entry.performance.revenue)}</div>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-purple-700">
                          Reign: {calculateReignDuration(entry.dojoMaster.designationDate)} days
                        </div>
                      </div>
                    </div>
                  </div>
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              <div className="text-sm text-gray-500">
                                {player.matchesWon}/{player.matchesPlayed} wins ({player.winRate.toFixed(1)}%)
                              </div>
                            </div>
                          </div>
                          <div className="text-right">
      ...
      {activeTab === 'dojo-masters' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {dojoMasters.map((master) => (
            <Card key={`${master.venueId}-${master.masterId}`} className="hover:shadow-lg transition-shadow">
              <CardHeader>
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <CardTitle className="text-lg">{master.masterName}</CardTitle>
                    <div className="text-sm text-gray-500">{master.venueName}</div>
                  </div>
                </div>
              </CardHeader>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="text-center">
                    <div className="text-lg font-semibold">{calculateReignDuration(master.designationDate)}</div>
                    <div className="text-sm text-gray-500">Days Reign</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-semibold">{master.successfulDefenses}</div>
          ...
          {dojoMasters.length === 0 && (
            <div className="col-span-full text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-600 mb-2">No Dojo Masters Yet</h3>
              <p className="text-gray-500">Dojo Masters will appear here once they are designated.</p>
            </div>
          )}
        </div>
      )}
