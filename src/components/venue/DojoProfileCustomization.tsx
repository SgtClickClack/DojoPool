...
import { DojoProfileCustomizationService, CustomizationConfig, TextToImageResponse } from '../../services/venue/DojoProfileCustomizationService';
...
const DojoProfileCustomization: React.FC = () => {
  ...
  const [customization, setCustomization] = useState<CustomizationConfig | null>(null);
  ...
  const [selectedImage, setSelectedImage] = useState<TextToImageResponse | null>(null);
  ...
  useEffect(() => {
    ...
    service.on('imageGenerated', (image: TextToImageResponse) => {
      setGeneratedImages(prev => [...prev.filter(img => img.id !== image.id), image]);
    });

    service.on('imageGenerationFailed', (data: { id: string; error: string }) => {
    ...
    try {
      switch (type) {
        case 'theme':
          ...
          break;
        case 'attributes':
          const attributes = await service.generateAttributesFromText(description);
        ...
        <Card sx={{ bgcolor: '#1a1a1a', border: '1px solid #333' }}>
          <CardContent>
            ...
            <Button
              ...
              onClick={() => handleGenerateFromText('theme', 'modern futuristic pool venue')}
              disabled={generating}
          ...
          <CardContent>
            ...
            {generatedImages.length > 0 ? (
                    ...
                    <Box sx={{ position: 'relative' }}>
                      ...
                      <Chip
                        ...
                        sx={{
                          ...
                          top: 8,
                          right: 8,
                          bgcolor: image.status === 'completed' ? '#00ff9d' : '#ff6b6b',
                          color: '#000'
                        }}
                      />
                    </Box>
              ...
              <Typography variant="body2" sx={{ color: '#888' }}>
                No images generated yet. Use the AI generation tools above to create custom images.
              </Typography>
            )}
          </CardContent>
        </Card>
        ...
        <Card sx={{ bgcolor: '#1a1a1a', border: '1px solid #333' }}>
          <CardContent>
            ...
            <Button
              variant="contained"
              startIcon={<AutoAwesome />}
              onClick={() => handleGenerateFromText('attributes', 'modern tournament pool venue')}
              disabled={generating}
              sx={{ bgcolor: '#00ff9d', color: '#000' }}
            >
          ...
          <CardContent>
            ...
            <FormControl fullWidth sx={{ mb: 3 }}>
              ...
              <Select
                ...
                onChange={(e) => service.updateAtmosphere(venueId, { lighting: e.target.value })}
                sx={{ color: '#fff' }}
              >
            ...
            <FormControl fullWidth sx={{ mb: 3 }}>
              ...
              <Select
                ...
                onChange={(e) => service.updateAtmosphere(venueId, { music: e.target.value })}
                sx={{ color: '#fff' }}
              >
            ...
            <FormControl fullWidth sx={{ mb: 3 }}>
              ...
              <Select
                ...
                onChange={(e) => service.updateAtmosphere(venueId, { decor: e.target.value })}
                sx={{ color: '#fff' }}
              >
            ...
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={(e) => service.updateAtmosphere(venueId, { vibe: e.target.value })}
                sx={{ color: '#fff' }}
              >
        ...
        <Card sx={{ bgcolor: '#1a1a1a', border: '1px solid #333' }}>
          <CardContent>
            ...
            {customization?.atmosphere && (
              <Box>
                <Typography variant="h5" sx={{ color: '#00ff9d', mb: 2, fontStyle: 'italic' }}>
                  {customization.atmosphere.vibe.charAt(0).toUpperCase() + customization.atmosphere.vibe.slice(1)} Atmosphere
                </Typography>
                
                <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 2 }}>
                  <Chip label={customization.atmosphere.lighting} sx={{ bgcolor: '#333', color: '#00a8ff' }} />
                  <Chip label={customization.atmosphere.music} sx={{ bgcolor: '#333', color: '#00a8ff' }} />
                  <Chip label={customization.atmosphere.decor} sx={{ bgcolor: '#333', color: '#00a8ff' }} />
                </Box>
                
                <Typography variant="body1" sx={{ color: '#fff' }}>
                  Experience the perfect blend of {customization.atmosphere.lighting} lighting, 
                  {customization.atmosphere.music} music, and {customization.atmosphere.decor} decor 
                  for an {customization.atmosphere.vibe} gaming environment.
                </Typography>
              </Box>
            )}
          </CardContent>
        ...
        <Card sx={{ bgcolor: '#1a1a1a', border: '1px solid #333' }}>
          <CardContent>
            ...
            <TextField
              ...
              onChange={(e) => service.updateBranding(venueId, { tagline: e.target.value })}
              sx={{ mb: 3 }}
            ...
            <TextField
              ...
              onChange={(e) => service.updateBranding(venueId, { mission: e.target.value })}
              sx={{ mb: 3 }}
              InputProps={{ sx: { color: '#fff' } }}
              InputLabelProps={{ sx: { color: '#888' } }}
            />
            ...
            <TextField
              ...
              onChange={(e) => service.updateBranding(venueId, { values: e.target.value.split(',').map(v => v.trim()) })}
              sx={{ mb: 3 }}
              InputProps={{ sx: { color: '#fff' } }}
              InputLabelProps={{ sx: { color: '#888' } }}
            />
              ...
              <Box>
                ...
                <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                  {customization.branding.values?.map((value) => (
                    <Chip key={value} label={value} sx={{ bgcolor: '#333', color: '#00a8ff' }} />
                  ))}
                </Box>
              </Box>
            )}
    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="subtitle1" sx={{ color: '#00a8ff', mb: 2 }}>
          Customize your venue's digital presence with AI-powered text-to-image generation
        </Typography>
        ...
        >
          {isConnected ? 'Connected to customization service' : 'Disconnected from customization service'}
        </Alert>
      </Box>
      ...
      >
        ...
        <DialogActions>
          <Button onClick={() => setImageDialogOpen(false)} sx={{ color: '#00ff9d' }}>
            Close
          </Button>
          <Button
            startIcon={<Download />}
