...
import { EquipmentManagementService, Equipment, EquipmentInventory, MaintenanceTask, EquipmentAlert, PerformanceMetrics } from '../../services/venue/EquipmentManagementService';
...
const EquipmentManagement: React.FC = () => {
  ...
  const [scheduleMaintenanceDialog, setScheduleMaintenanceDialog] = useState(false);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setMaintenanceTasks(equipmentService.getMaintenanceSchedule(selectedVenue));
      setAlerts(equipmentService.getCriticalAlerts(selectedVenue));
      setPerformanceMetrics(equipmentService.getPerformanceMetrics(selectedVenue));
    ...
    <Box sx={{ flexGrow: 1, p: 3 }}>
      ...
      {!isConnected && (
        <Alert severity="warning" sx={{ mb: 3 }}>
          Equipment Management Service is disconnected. Some features may be limited.
        </Alert>
      )}
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Card sx={{ background: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)', color: 'white' }}>
              <CardContent>
                ...
                <Typography variant="body2">
                  {((inventory.operationalEquipment / inventory.totalEquipment) * 100).toFixed(1)}% of total
                </Typography>
              </CardContent>
            </Card>
          </Grid>
      ...
      <Card sx={{ background: 'rgba(255, 255, 255, 0.05)', backdropFilter: 'blur(10px)' }}>
                    ...
                    <TableCell>
                      <Box sx={{ display: 'flex', gap: 1 }}>
                        <Tooltip title="View Details">
                          <IconButton size="small" onClick={() => setSelectedEquipment(eq)}>
                            <Visibility />
                          </IconButton>
                        </Tooltip>
                        <Tooltip title="Edit">
        ...
        <TabPanel value={tabValue} index={1}>
          <Typography variant="h6" gutterBottom>Equipment Health Overview</Typography>
                ...
                <Card sx={{ 
                  ...
                  <CardContent>
                    ...
                    <Box sx={{ mb: 2 }}>
                      ...
                      <Typography variant="body2" gutterBottom>
                        Uptime: {Math.floor(eq.health.uptime / 3600)}h {Math.floor((eq.health.uptime % 3600) / 60)}m
                      </Typography>
                      <Typography variant="body2" gutterBottom>
                    ...
                    {eq.health.alerts.length > 0 && (
                      <Box>
                        <Typography variant="subtitle2" gutterBottom>
                          Active Alerts ({eq.health.alerts.filter(a => !a.resolved).length})
                        </Typography>
                        ...
                        {maintenanceTasks.map((task) => (
                          <TableRow key={task.id}>
                            ...
                            <TableCell>
                              <Chip
                                label={task.status}
                                color={task.status === 'overdue' ? 'error' : task.status === 'completed' ? 'success' : 'warning'}
                                size="small"
                              />
                            </TableCell>
          ...
          <Grid container spacing={3}>
            {performanceMetrics.map((metrics, index) => (
              <Grid item xs={12} sm={6} md={4} key={index}>
                <Card sx={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.1) 0%, rgba(255,255,255,0.05) 100%)' }}>
                  <CardContent>
                    <Typography variant="h6" gutterBottom>
                    ...
                    <Box sx={{ mb: 2 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                        ...
                        <Typography variant="body2">{metrics.efficiency}%</Typography>
                      </Box>
                      <LinearProgress
                        variant="determinate"
                        value={metrics.efficiency}
                        color={metrics.efficiency >= 80 ? 'success' : metrics.efficiency >= 60 ? 'warning' : 'error'}
                      />
                    </Box>
                    ...
                    <Box sx={{ mb: 2 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                        ...
                        <Typography variant="body2">{metrics.reliability}%</Typography>
                      </Box>
                      <LinearProgress
                        variant="determinate"
                        value={metrics.reliability}
                        color={metrics.reliability >= 80 ? 'success' : metrics.reliability >= 60 ? 'warning' : 'error'}
                      />
                    </Box>
        ...
        <TabPanel value={tabValue} index={4}>
          ...
          {alerts.length === 0 ? (
            <Alert severity="success">
              No critical alerts at this time. All equipment is operating normally.
            </Alert>
          ) : (
            <List>
              {alerts.map((alert) => (
                <ListItem key={alert.id} sx={{ mb: 2 }}>
                  <Card sx={{ width: '100%', background: 'linear-gradient(135deg, #ff9a9e 0%, #fecfef 100%)' }}>
                    <CardContent>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <Box sx={{ display: 'flex', gap: 1 }}>
                          <Button
                            size="small"
                            variant="outlined"
                            onClick={() => equipmentService.acknowledgeAlert(alert.id, alert.id)}
                          >
                            Acknowledge
                          </Button>
                          <Button
                            size="small"
                            variant="contained"
                            onClick={() => equipmentService.resolveAlert(alert.id, alert.id)}
                          >
                            Resolve
                          </Button>
                        </Box>
      ...
      <Dialog open={addEquipmentDialog} onClose={() => setAddEquipmentDialog(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 1 }}>
            ...
            <Grid item xs={12}>
              <TextField fullWidth label="Installation Date" type="date" InputLabelProps={{ shrink: true }} />
            </Grid>
          </Grid>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={scheduleMaintenanceDialog} onClose={() => setScheduleMaintenanceDialog(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 1 }}>
            ...
            <Grid item xs={12} sm={6}>
              <TextField fullWidth label="Estimated Duration (minutes)" type="number" />
            </Grid>
            <Grid item xs={12}>
              <TextField fullWidth label="Instructions" multiline rows={4} />
            </Grid>
            <Grid item xs={12}>
              <TextField fullWidth label="Scheduled Date" type="date" InputLabelProps={{ shrink: true }} />
            </Grid>
          </Grid>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setScheduleMaintenanceDialog(false)}>Cancel</Button>
