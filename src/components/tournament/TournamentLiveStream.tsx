...
export const TournamentLiveStream: React.FC<TournamentLiveStreamProps> = ({
  ...
  useEffect(() => {
    const unsubscribe = tournamentStreamingService.subscribeToStream(streamKey, (data) => {
  ...
  const startStream = () => {
    ...
    const success = tournamentStreamingService.startStream(tournamentId, matchId, config);
    if (success) {
      setIsLive(true);
    }
  };
  ...
  const stopStream = () => {
    const success = tournamentStreamingService.stopStream(tournamentId, matchId);
    if (success) {
      setIsLive(false);
    }
  };
            ...
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                <Chip
                  ...
                  sx={{
                    background: isLive ? neonColors.primary : theme.palette.grey[600],
                ...
                <IconButton onClick={() => setIsFullscreen(!isFullscreen)} sx={{ color: neonColors.info }}>
                  <Fullscreen />
                </IconButton>
              </Box>
              ...
              <StreamVideo>
                {isLive ? (
                  ...
                  <Box sx={{ textAlign: 'center', color: neonColors.secondary }}>
                    <Typography variant="h4" sx={{ mb: 2 }}>
                      📺 Stream Offline
                    </Typography>
                    <Typography variant="body1">
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', mt: 2, gap: 1 }}>
                {!isLive ? (
                  ...
                  <NeonButton
                    ...
                    onClick={stopStream}
                    variant="contained"
                    sx={{ background: `linear-gradient(45deg, ${neonColors.error} 30%, ${neonColors.warning} 90%)` }}
                  >
                    Stop Stream
                  </NeonButton>
                )}

                <IconButton onClick={() => setIsMuted(!isMuted)} sx={{ color: neonColors.info }}>
                  {isMuted ? <VolumeOff /> : <VolumeUp />}
                </IconButton>

                <IconButton onClick={() => setIsLiked(!isLiked)} sx={{ color: isLiked ? neonColors.error : neonColors.secondary }}>
        ...
        <Box sx={{ flex: { lg: 1 } }}>
          <CyberpunkCard>
            <CardContent>
              ...
              <Typography variant="h6" sx={{ color: neonColors.warning, mb: 2 }}>
                Live Commentary
              </Typography>
