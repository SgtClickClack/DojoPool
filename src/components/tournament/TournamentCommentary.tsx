...
const TournamentCommentary: React.FC = () => {
            ...
            <ListItemText
              ...
              secondary={
                <Typography variant="caption" sx={{ color: '#888', mt: 1 }}>
                  {event.timestamp.toLocaleTimeString()} • {event.metadata.style} • {event.metadata.duration}s
                </Typography>
              }
            />
          </ListItem>
  ...
  const renderAnalytics = () => (
    <Box>
      <Typography variant="h6" sx={{ ...neonTextStyle, color: '#ff00ea', mb: 2 }}>
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md={6}>
          <Card sx={cyberpunkPaper}>
            <Typography variant="subtitle1" sx={{ color: '#00fff7', fontWeight: 600, mb: 2 }}>
            ...
            <Box sx={{ mb: 2 }}>
              ...
              <LinearProgress
                variant="determinate"
                value={commentaryEvents.filter(e => e.excitementLevel >= 90).length / Math.max(commentaryEvents.length, 1) * 100}
                sx={{
                  height: 8,
                  borderRadius: 4,
            ...
            <Box sx={{ mb: 2 }}>
              ...
              <LinearProgress
                variant="determinate"
                value={commentaryEvents.filter(e => e.excitementLevel >= 60 && e.excitementLevel < 90).length / Math.max(commentaryEvents.length, 1) * 100}
                sx={{
                  height: 8,
                  borderRadius: 4,
        ...
        <Grid item xs={12} md={6}>
          <Card sx={cyberpunkPaper}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              {['shot_analysis', 'match_update', 'player_performance', 'tournament_insight', 'excitement_moment'].map((type) => {
                const count = commentaryEvents.filter(e => e.type === type).length;
                const percentage = commentaryEvents.length > 0 ? (count / commentaryEvents.length) * 100 : 0;
                return (
          ...
          <Card sx={cyberpunkPaper}>
            ...
            <FormControl fullWidth sx={{ mb: 2 }}>
              ...
              <Select
                ...
                onChange={(e) => handleStyleChange(e.target.value as CommentaryConfig['style'])}
                sx={{
                  color: '#fff',
