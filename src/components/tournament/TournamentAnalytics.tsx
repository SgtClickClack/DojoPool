...
const TournamentAnalytics: React.FC<TournamentAnalyticsProps> = ({
  ...
  const loadAnalytics = async () => {
    try {
      ...
      const dashboard = await analyticsService.generateAnalyticsDashboard(tournament);
      ...
      const trends = await Promise.all(
        dashboard.playerPerformance.slice(0, 5).map(player =>
          analyticsService.calculateHistoricalTrends(player.playerId, [tournament])
        )
      );
  ...
  const setupSubscriptions = () => {
    const unsubscribeStats = analyticsService.subscribe('tournamentStatistics', (data) => {
      setStatistics(data);
    });

    const unsubscribePerformance = analyticsService.subscribe('playerPerformance', (data) => {
      setPlayerPerformance(data);
    });

    const unsubscribeMatchAnalytics = analyticsService.subscribe('matchAnalytics', (data) => {
        ...
        <CyberpunkCard sx={{ mb: 3 }}>
          <CardContent>
            ...
            <Box sx={{ mt: 3 }}>
              ...
              <ProgressBar variant="determinate" value={statistics.completionRate} />
            </Box>
          </CardContent>
        </CyberpunkCard>
      )}
        ...
        <CardContent>
          ...
          <TableContainer component={Paper} sx={{ background: 'transparent' }}>
            <Table>
              <TableHead>
                <TableRow>
                  <TableCell sx={{ color: '#00d4ff', fontWeight: 'bold' }}>Rank</TableCell>
                  <TableCell sx={{ color: '#00d4ff', fontWeight: 'bold' }}>Player</TableCell>
                  <TableCell sx={{ color: '#00d4ff', fontWeight: 'bold' }}>Matches</TableCell>
                  <TableCell sx={{ color: '#00d4ff', fontWeight: 'bold' }}>Win Rate</TableCell>
                  <TableCell sx={{ color: '#00d4ff', fontWeight: 'bold' }}>Avg Points</TableCell>
                  <TableCell sx={{ color: '#00d4ff', fontWeight: 'bold' }}>Highest Break</TableCell>
                  <TableCell sx={{ color: '#00d4ff', fontWeight: 'bold' }}>Status</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                  ...
                  <TableRow key={player.playerId} hover>
                    ...
                    <TableCell sx={{ color: '#ffffff' }}>
                      <Box display="flex" alignItems="center">
                        ...
                        {player.winRate > 60 ? (
                          <TrendingUp sx={{ color: '#00ff00', ml: 1, fontSize: 16 }} />
                        ) : (
                          <TrendingDown sx={{ color: '#ff0000', ml: 1, fontSize: 16 }} />
                        )}
                      </Box>
                    </TableCell>
                    <TableCell sx={{ color: '#ffffff' }}>
                    ...
                    <TableCell>
                      <Chip
                        label={player.eliminationRound ? `Eliminated R${player.eliminationRound}` : 'Active'}
                        size="small"
                        sx={{
                          background: player.eliminationRound ? '#ff0000' : '#00ff00',
                          color: '#ffffff',
                        }}
                      />
                    </TableCell>
          ...
          <CyberpunkCard>
            <CardContent>
              ...
              {matchAnalytics.slice(0, 5).map((match) => (
                <Box key={match.matchId} sx={{ mb: 2, p: 2, border: '1px solid #333', borderRadius: '8px' }}>
                  <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
                  ...
                  <Box display="flex" justifyContent="space-between" sx={{ color: '#cccccc' }}>
              ...
              {performanceTrends.slice(0, 3).map((trend) => (
                <Box key={trend.playerId} sx={{ mb: 2 }}>
                  ...
                  <Box display="flex" alignItems="center" mb={1}>
                    <Typography variant="caption" sx={{ color: '#cccccc', mr: 1 }}>
                      Win Rate Trend:
                    </Typography>
                    <Box display="flex" alignItems="center">
