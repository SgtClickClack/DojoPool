...
export const AdvancedVenueManagementDashboard: React.FC = () => {
  ...
  const handleVenueSelect = async (venueId: string) => {
    ...
    const [analytics, tables, engagements, revenue, optimizations] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold">Advanced Venue Management & Analytics</h1>
      ...
      {metrics && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{metrics.averageUtilization.toFixed(1)}%</div>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                Last updated: {new Date(metrics.lastActivity).toLocaleDateString()}
              </p>
            </CardContent>
          </Card>
        </div>
                ...
                <CardContent>
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span>Revenue:</span>
                      <span className="font-semibold">${venue.revenue.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Utilization:</span>
                      <span className="font-semibold">{venue.tableUtilization.toFixed(1)}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Rating:</span>
                      <span className="font-semibold">{venue.averageRating.toFixed(1)}/5</span>
                    </div>
                    <Badge variant={venue.playerEngagement > 80 ? 'default' : 'secondary'}>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="text-2xl font-bold">
                            {performances.find(v => v.venueId === selectedVenue)?.totalMatches}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm text-muted-foreground">Total Players</p>
                          <p className="text-2xl font-bold">
                            {performances.find(v => v.venueId === selectedVenue)?.totalPlayers}
                          </p>
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="flex gap-1 mt-1">
                          {performances.find(v => v.venueId === selectedVenue)?.peakHours.map((hour, i) => (
                            <Badge key={i} variant="outline">{hour}</Badge>
                          ))}
                        </div>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Popular Game Types</p>
                        <div className="flex gap-1 mt-1">
                          {performances.find(v => v.venueId === selectedVenue)?.popularGameTypes.map((type, i) => (
                    ...
                    <div className="space-y-4">
                      <div>
                        <div className="flex justify-between text-sm">
                          ...
                          <span>{performances.find(v => v.venueId === selectedVenue)?.tableUtilization.toFixed(1)}%</span>
                        </div>
                        <Progress 
                          value={performances.find(v => v.venueId === selectedVenue)?.tableUtilization} 
                          className="mt-1" 
                        />
                      </div>
                      <div>
                        <div className="flex justify-between text-sm">
                          ...
                          <span>{performances.find(v => v.venueId === selectedVenue)?.playerEngagement.toFixed(1)}%</span>
                        </div>
                        <Progress 
                          value={performances.find(v => v.venueId === selectedVenue)?.playerEngagement} 
                          className="mt-1" 
                        />
                      </div>
                      <div>
                        <div className="flex justify-between text-sm">
                          ...
                          <span>{performances.find(v => v.venueId === selectedVenue)?.averageRating.toFixed(1)}/5</span>
                        </div>
                        <Progress 
                          value={(performances.find(v => v.venueId === selectedVenue)?.averageRating || 0) * 20} 
                          className="mt-1" 
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="text-xl font-bold">${analyticsData.metrics.totalRevenue.toLocaleString()}</p>
                        </div>
                        <div>
                          <p className="text-sm text-muted-foreground">Total Matches</p>
                          <p className="text-xl font-bold">{analyticsData.metrics.totalMatches}</p>
                        </div>
                        <div>
                          <p className="text-sm text-muted-foreground">Unique Players</p>
                          <p className="text-xl font-bold">{analyticsData.metrics.uniquePlayers}</p>
                        </div>
                        <div>
                          <p className="text-sm text-muted-foreground">Utilization Rate</p>
                          <p className="text-xl font-bold">{analyticsData.metrics.tableUtilizationRate.toFixed(1)}%</p>
                        </div>
                      </div>
                    </div>
                  </CardContent>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-xl font-bold text-green-600">+{analyticsData.trends.revenue.toFixed(1)}%</p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Player Growth</p>
                        <p className="text-xl font-bold text-blue-600">+{analyticsData.trends.playerGrowth.toFixed(1)}%</p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Engagement Trend</p>
                        <p className="text-xl font-bold text-purple-600">+{analyticsData.trends.engagement.toFixed(1)}%</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <ul className="space-y-1">
                          {analyticsData.insights.map((insight: string, i: number) => (
            ...
            {tableData.length > 0 ? (
                    ...
                    <CardContent>
                      <div className="space-y-2">
                        <div className="flex justify-between">
                          ...
                          <span className="font-semibold">{table.totalMatches}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Utilization:</span>
                          <span className="font-semibold">{table.utilizationRate.toFixed(1)}%</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Revenue:</span>
                          <span className="font-semibold">${table.revenue.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Rating:</span>
                          <span className="font-semibold">{table.averageRating.toFixed(1)}/5</span>
                        </div>
                        <div>
                          <p className="text-xs text-muted-foreground">
                            Last maintenance: {new Date(table.lastMaintenance).toLocaleDateString()}
                          </p>
                        </div>
                      </div>
                    </CardContent>
              ...
              <Card>
                <CardContent className="pt-6">
                  <p className="text-center text-muted-foreground">No table data available</p>
                </CardContent>
              </Card>
            )}
          </TabsContent>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {engagementData.slice(0, 9).map((engagement) => (
                    <Card key={engagement.playerId}>
                      <CardHeader>
                        <CardTitle className="text-sm">Player {engagement.playerId}</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <div className="space-y-2">
                          <div className="flex justify-between">
                            <span>Visits:</span>
                            <span className="font-semibold">{engagement.totalVisits}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Matches:</span>
                            <span className="font-semibold">{engagement.totalMatches}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Spent:</span>
                            <span className="font-semibold">${engagement.totalSpent}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Engagement:</span>
                            <span className="font-semibold">{engagement.engagementScore.toFixed(1)}%</span>
                          </div>
                          <Progress value={engagement.engagementScore} className="mt-2" />
                        </div>
                      </CardContent>
                    </Card>
                  ))}
                ...
                <Card>
                  ...
                  <CardContent>
                    <ResponsiveContainer width="100%" height={300}>
                      <PieChart>
                        <Pie
                          data={Object.entries(revenueData.revenueBySource).map(([key, value]) => ({
                          ...
                          dataKey="value"
                        >
                          {Object.entries(revenueData.revenueBySource).map((entry, index) => (
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <p className="text-xl font-bold text-green-600">+{revenueData.growthRate.toFixed(1)}%</p>
                      </div>
                      <div>
                        <p className="text-sm text-muted-foreground">Top Revenue Drivers</p>
                        <div className="flex gap-1 mt-1">
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="flex justify-between items-center">
                      ...
                      <Badge variant={optimizationData.priority === 'critical' ? 'destructive' : 'default'}>
                        ...
                        <Card key={rec.id} className="p-4">
                          <div className="flex justify-between items-start">
                            ...
                            <div className="text-right">
                              <p className="text-sm font-semibold">${rec.cost.toLocaleString()}</p>
                              <p className="text-xs text-muted-foreground">{rec.implementationTime} days</p>
                            </div>
                          </div>
                          <div className="grid grid-cols-3 gap-2 mt-3">
                            <div className="text-center">
                              <p className="text-xs text-muted-foreground">Revenue</p>
                              <p className="text-sm font-semibold text-green-600">+{rec.impact.revenue}%</p>
                            </div>
                            <div className="text-center">
                              <p className="text-xs text-muted-foreground">Efficiency</p>
                              <p className="text-sm font-semibold text-blue-600">+{rec.impact.efficiency}%</p>
                            </div>
                            <div className="text-center">
                              <p className="text-xs text-muted-foreground">Satisfaction</p>
                              <p className="text-sm font-semibold text-purple-600">+{rec.impact.satisfaction}%</p>
                            </div>
                          </div>
                        </Card>
                      ))}
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-2xl font-bold text-blue-600">
                          +{optimizationData.expectedImpact.efficiencyGain.toFixed(1)}%
                        </p>
                      </div>
                      <div className="text-center">
                        <p className="text-sm text-muted-foreground">Customer Satisfaction</p>
                        <p className="text-2xl font-bold text-purple-600">
                          +{optimizationData.expectedImpact.customerSatisfaction.toFixed(1)}%
                        </p>
                      </div>
                    </div>
                  </CardContent>
                ...
                <div className="space-y-2">
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => updateConfig({ realTimeTracking: e.target.checked })}
                    />
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => updateConfig({ performanceAnalytics: e.target.checked })}
                    />
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => updateConfig({ revenueTracking: e.target.checked })}
                    />
                ...
                <div className="space-y-2">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateConfig({ retentionPeriod: parseInt(e.target.value) * 24 * 60 * 60 * 1000 })}
                      className="w-full p-2 border rounded"
                    />
                  </div>
                </div>
