...
import { useAdvancedPlayerAnalytics, PlayerPerformance, PlayerInsights, PlayerComparison } from '../../hooks/useAdvancedPlayerAnalytics';

interface AdvancedPlayerAnalyticsDashboardProps {
  className?: string;
}

export const AdvancedPlayerAnalyticsDashboard: React.FC<AdvancedPlayerAnalyticsDashboardProps> = ({ className = '' }) => {
  const {
    config,
    updateConfig,
    metrics,
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'players' | 'performance' | 'skills' | 'analysis' | 'insights' | 'comparison'>('overview');
  ...
  const handleComparePlayers = async () => {
    if (selectedPlayerId && comparisonPlayerId) {
      const comparison = await comparePlayers(selectedPlayerId, comparisonPlayerId);
      if (comparison) {
  ...
  const handleAddMetric = async (playerId: string, metricType: string, value: number) => {
  ...
  const handleUpdateSkill = async (playerId: string, skillArea: string, improvement: number, reason: string) => {
  ...
  return (
    <div className={`bg-white rounded-lg shadow-lg p-6 ${className}`}>
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Advanced Player Analytics</h1>
        <p className="text-gray-600">Comprehensive player performance tracking and analysis system</p>
      </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-blue-50 p-4 rounded-lg">
              <h3 className="text-lg font-semibold text-blue-900">Total Players</h3>
              <p className="text-3xl font-bold text-blue-600">{metrics?.totalPlayers || 0}</p>
            </div>
            <div className="bg-green-50 p-4 rounded-lg">
              <h3 className="text-lg font-semibold text-green-900">Active Players</h3>
              <p className="text-3xl font-bold text-green-600">{metrics?.activePlayers || 0}</p>
            </div>
            <div className="bg-purple-50 p-4 rounded-lg">
              <h3 className="text-lg font-semibold text-purple-900">Avg Skill Level</h3>
              <p className="text-3xl font-bold text-purple-600">{metrics?.averageSkillLevel?.toFixed(1) || 0}</p>
            </div>
            <div className="bg-orange-50 p-4 rounded-lg">
              <h3 className="text-lg font-semibold text-orange-900">Avg Win Rate</h3>
              <p className="text-3xl font-bold text-orange-600">{metrics?.averageWinRate?.toFixed(1) || 0}%</p>
            </div>
          </div>
              ...
              <div className="space-y-2">
                {topPerformers.slice(0, 5).map((player, index) => (
                  <div key={player.playerId} className="flex justify-between items-center p-2 bg-gray-50 rounded">
                    ...
                    <span className="text-green-600 font-semibold">{player.winRate.toFixed(1)}%</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2">
                {mostImproved.slice(0, 5).map((player, index) => (
                  <div key={player.playerId} className="flex justify-between items-center p-2 bg-gray-50 rounded">
                    ...
                    <span className="text-blue-600 font-semibold">{player.reputation} pts</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <table className="min-w-full bg-white border rounded-lg">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Reputation</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {performances.map(player => (
                  <tr key={player.playerId} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{player.totalMatches}</td>
                    <td className="px-4 py-3 whitespace-nowrap">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                        player.skillLevel === 'master' ? 'bg-purple-100 text-purple-800' :
                        player.skillLevel === 'expert' ? 'bg-red-100 text-red-800' :
                        player.skillLevel === 'advanced' ? 'bg-blue-100 text-blue-800' :
                        player.skillLevel === 'intermediate' ? 'bg-green-100 text-green-800' :
                        'bg-gray-100 text-gray-800'
                      }`}>
                        {player.skillLevel}
                      </span>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{player.reputation}</td>
      ...
      {activeTab === 'performance' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {performances.map(player => (
              <div key={player.playerId} className="bg-white border rounded-lg p-4">
                <h3 className="font-semibold mb-3">{player.playerName}</h3>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span>Shot Accuracy:</span>
                    <span className="font-medium">{player.shotAccuracy.toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Break Success:</span>
                    <span className="font-medium">{player.breakSuccessRate.toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Safety Shot:</span>
                    <span className="font-medium">{player.safetyShotRate.toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Avg Score:</span>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{player.currentWinStreak}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      {activeTab === 'insights' && (
        <div className="space-y-6">
          ...
          <div className="flex space-x-4 mb-4">
            ...
            <button
              onClick={() => selectedPlayerId && handleGenerateInsights(selectedPlayerId)}
          ...
          {playerInsights && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white border rounded-lg p-4">
                <h3 className="font-semibold mb-3 text-red-700">Areas for Improvement</h3>
                <ul className="space-y-1">
              ...
              <div className="bg-white border rounded-lg p-4">
                <h3 className="font-semibold mb-3 text-blue-700">Recommended Drills</h3>
                <ul className="space-y-1">
                  {playerInsights.recommendedDrills.map((drill, index) => (
                    <li key={index} className="text-sm text-gray-700">• {drill}</li>
                  ))}
                </ul>
              </div>

              <div className="bg-white border rounded-lg p-4">
                <h3 className="font-semibold mb-3 text-purple-700">Performance Predictions</h3>
                <div className="space-y-2">
          ...
          {playerComparison && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white border rounded-lg p-4 lg:col-span-2">
                ...
                <ul className="space-y-1">
                  {playerComparison.recommendations.map((recommendation, index) => (
