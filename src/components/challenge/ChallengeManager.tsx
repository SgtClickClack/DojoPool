...
const ChallengeManager: React.FC<ChallengeManagerProps> = ({ isOpen, onClose }) => {
  ...
  const handleRespond = async (challengeId: string, response: 'accept' | 'decline') => {
    setLoading(true);
    try {
      if (response === 'accept') {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[2000]">
      <div className="bg-gray-900 border-2 border-cyan-400 rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-cyan-400">Challenge Manager</h2>
          <button 
            onClick={onClose}
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {/* Incoming Challenges */}
            <div>
              <h3 className="text-lg font-bold text-white mb-3">Incoming Challenges</h3>
              {incomingChallenges.length > 0 ? (
                      ...
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <p className="text-white font-semibold">
                            {ChallengeService.getChallengeTypeName(challenge.type)} Challenge
                          </p>
                ...
                <p className="text-gray-500 text-center py-4">No incoming challenges</p>
              )}
            </div>

            {/* Outgoing Challenges */}
            <div>
              <h3 className="text-lg font-bold text-white mb-3">Outgoing Challenges</h3>
