...
const DojoProfilePanel: React.FC<DojoProfilePanelProps> = ({ dojo, onClose, isLocked = false }) => {
  ...
  const handleChallenge = async (type: 'pilgrimage' | 'gauntlet') => {
    ...
    try {
      ...
      setMessage(`Challenge sent! ${type === 'pilgrimage' ? 'Pilgrimage' : 'Gauntlet'} challenge created.`);
    } catch (error) {
    ...
    <div className="absolute bottom-0 left-0 right-0 bg-gray-900 bg-opacity-95 border-t-4 border-cyan-400 p-6 z-[1000] animate-slide-up">
      ...
      {isLocked ? (
        <div className="text-center py-8">
          ...
          <p className="text-gray-300 mt-2">You must increase your influence in the current zone to challenge Dojos here.</p>
        </div>
      ) : (
