...
import { ShotAnalysisService, ShotData } from '../../ai/shot-analysis/ShotAnalysisService';
...
export const ShotTracker: React.FC<ShotTrackerProps> = ({ onShotComplete }) => {
  ...
  const updateCanvas = (positions: { cueBall: { x: number; y: number }; targetBall: { x: number; y: number } }) => {
  ...
  return (
    <Paper elevation={3} sx={{ p: 2, maxWidth: 600, mx: 'auto' }}>
      ...
      <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center' }}>
        ...
        <button
          onClick={() => handleCompleteShot(true)}
          disabled={!isTracking}
        >
          Shot Success
        </button>
