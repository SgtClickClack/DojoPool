...
const TournamentSecurity: React.FC = () => {
  ...
  const getAlertTypeIcon = (type: string) => {
    switch (type) {
      case 'fraud': return <BugReport />;
      case 'suspicious_activity': return <Warning />;
      case 'security_breach': return <Security />;
      case 'unauthorized_access': return <Visibility />;
      case 'data_leak': return <Timeline />;
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'false_positive': return '#888888';
      default: return '#888888';
    }
  };
  ...
  const handleUpdateAlertStatus = async (alertId: string, status: SecurityAlert['status']) => {
    setLoading(true);
    try {
      ...
      <Box sx={{ 
        ...
        <Box sx={{ 
          ...
          <Box>
            ...
            <Typography variant="subtitle1" sx={{ color: '#000', opacity: 0.8 }}>
        ...
        {metrics && (
          <Box sx={{ 
            ...
            <Box sx={{ flex: 1 }}>
              <Typography variant="h6" sx={{ color: getThreatLevelColor(metrics.threatLevel) }}>
                Threat Level: {metrics.threatLevel.toUpperCase()}
              </Typography>
              <Typography variant="body2" sx={{ color: '#ccc' }}>
                Security Score: {metrics.securityScore}/100 | Open Alerts: {metrics.openAlerts} | Compliance: {metrics.complianceStatus}
              </Typography>
            </Box>
            <LinearProgress
              variant="determinate"
                  ...
                  <CardContent>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        <Typography variant="h6" sx={{ color: getSeverityColor(alert.severity) }}>
                          {alert.title}
                        </Typography>
                      </Box>
                      <Chip
              ...
              <Grid item xs={12} md={6} key={pattern.id}>
                <Card sx={{ 
                  ...
                  <CardContent>
                    ...
                    <Typography variant="body2" sx={{ color: '#ffaa00', mb: 1 }}>
                    ...
                    <List dense>
                      {pattern.detectionRules.map((rule, index) => (
                        <ListItem key={index} sx={{ py: 0 }}>
                          <ListItemText 
                            primary={rule} 
                            sx={{ '& .MuiListItemText-primary': { color: '#888', fontSize: '0.875rem' } }}
                          />
                        </ListItem>
                      ))}
                    </List>
                    <Typography variant="body2" sx={{ color: '#ccc', mb: 1, mt: 2 }}>
                      Mitigation Actions:
                    </Typography>
                    <List dense>
                      {pattern.mitigationActions.map((action, index) => (
                        <ListItem key={index} sx={{ py: 0 }}>
                          <ListItemText 
                            primary={action} 
                            sx={{ '& .MuiListItemText-primary': { color: '#00ff88', fontSize: '0.875rem' } }}
                          />
                        </ListItem>
                      ))}
                    </List>
          ...
          <Grid container spacing={2}>
            {securityEvents.map((event) => (
              <Grid item xs={12} md={6} key={event.id}>
                <Card sx={{ 
                  background: 'rgba(0, 0, 0, 0.6)',
                  border: event.flagged ? '1px solid #ff4444' : '1px solid #333',
                  '&:hover': { borderColor: '#ff4444' }
                }}>
                  <CardContent>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                      ...
                      <Chip
                        ...
                        sx={{
                          background: event.flagged ? 'rgba(255, 68, 68, 0.2)' : 'rgba(0, 255, 136, 0.2)',
                    ...
                    <Typography variant="body2" sx={{ color: event.success ? '#00ff88' : '#ff4444', mb: 1 }}>
                ...
                <Card sx={{ 
                  ...
                  <CardContent>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                      <Typography variant="body2" sx={{ color: '#ccc' }}>Total Alerts:</Typography>
                      <Typography variant="body2" sx={{ color: '#ff4444' }}>{metrics.totalAlerts}</Typography>
                    </Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                      <Typography variant="body2" sx={{ color: '#ccc' }}>Open Alerts:</Typography>
                      <Typography variant="body2" sx={{ color: '#ffaa00' }}>{metrics.openAlerts}</Typography>
                    </Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                      <Typography variant="body2" sx={{ color: '#ccc' }}>Resolved Alerts:</Typography>
                      <Typography variant="body2" sx={{ color: '#00ff88' }}>{metrics.resolvedAlerts}</Typography>
                    </Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                      <Typography variant="body2" sx={{ color: '#ccc' }}>False Positives:</Typography>
                      <Typography variant="body2" sx={{ color: '#888' }}>{metrics.falsePositives}</Typography>
                    </Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                      <Typography variant="body2" sx={{ color: '#ccc' }}>Avg Response Time:</Typography>
                      <Typography variant="body2" sx={{ color: '#00ccff' }}>{Math.round(metrics.averageResponseTime)}m</Typography>
                    </Box>
                  </CardContent>
                </Card>
              </Grid>
              <Grid item xs={12} md={6}>
                <Card sx={{ 
                  ...
                  <CardContent>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                      <Typography variant="body2" sx={{ color: '#ccc' }}>Threat Level:</Typography>
                      <Chip
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                      <Typography variant="body2" sx={{ color: '#ccc' }}>Security Score:</Typography>
                      <Typography variant="body2" sx={{ color: getThreatLevelColor(metrics.threatLevel) }}>
                        {metrics.securityScore}/100
                      </Typography>
                    </Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                      <Typography variant="body2" sx={{ color: '#ccc' }}>Compliance:</Typography>
                      <Chip
                        label={metrics.complianceStatus.toUpperCase()}
                        color={metrics.complianceStatus === 'compliant' ? 'success' : 
                               metrics.complianceStatus === 'warning' ? 'warning' : 'error'}
                        size="small"
                      />
                    </Box>
                    {metrics.lastIncident && (
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                        <Typography variant="body2" sx={{ color: '#ccc' }}>Last Incident:</Typography>
                        <Typography variant="body2" sx={{ color: '#888' }}>
                    ...
                    <FormControlLabel
                      control={
                        <Switch
                          ...
                          onChange={(e) => handleConfigChange({ fraudDetectionEnabled: e.target.checked })}
                          sx={{
                    ...
                    <FormControlLabel
                      control={
                        <Switch
                          ...
                          onChange={(e) => handleConfigChange({ automatedActions: e.target.checked })}
                          sx={{
                            '& .MuiSwitch-switchBase.Mui-checked': {
                              color: '#00ff88'
                            },
                  ...
                  <CardContent>
                    ...
                    <Box sx={{ mb: 2 }}>
                      ...
                      <Slider
                        ...
                        onChange={(e, value) => handleConfigChange({ 
                          alertThresholds: { ...config.alertThresholds, low: value as number }
                        })}
                        min={0}
                        max={1}
                        step={0.1}
                    ...
                    <Box sx={{ mb: 2 }}>
                      <Typography variant="body2" sx={{ color: '#ccc', mb: 1 }}>
                        Medium: {Math.round(config.alertThresholds.medium * 100)}%
                      </Typography>
                      <Slider
                        value={config.alertThresholds.medium}
                        onChange={(e, value) => handleConfigChange({ 
                          alertThresholds: { ...config.alertThresholds, medium: value as number }
                        })}
                        min={0}
                        max={1}
                        step={0.1}
                    ...
                    <Box sx={{ mb: 2 }}>
                      ...
                      <Slider
                        ...
                        onChange={(e, value) => handleConfigChange({ 
                          alertThresholds: { ...config.alertThresholds, high: value as number }
                        })}
                        min={0}
                        max={1}
                        step={0.1}
                    ...
                    <Box>
                      <Typography variant="body2" sx={{ color: '#ccc', mb: 1 }}>
                        Critical: {Math.round(config.alertThresholds.critical * 100)}%
                      </Typography>
                      <Slider
                        value={config.alertThresholds.critical}
                        onChange={(e, value) => handleConfigChange({ 
                          alertThresholds: { ...config.alertThresholds, critical: value as number }
                        })}
                        min={0}
                        max={1}
                        step={0.1}
