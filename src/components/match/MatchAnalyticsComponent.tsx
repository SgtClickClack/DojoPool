...
import { MatchResult, MatchAnalytics } from '../../services/RealTimeMatchTrackingService';
...
export const MatchAnalyticsComponent: React.FC<MatchAnalyticsComponentProps> = ({
  matchResult,
  onClose,
}) => {
  ...
  const generateInsights = () => {
    ...
    if (analytics.fouls === 0) {
      newInsights.push({
        type: 'positive',
        title: 'Clean Game',
        description: 'No fouls recorded - excellent discipline and rule adherence.',
        icon: '✨',
      });
  ...
  const getMetricColor = (metric: string, value: number): string => {
    switch (metric) {
      ...
      case 'fouls':
        return value === 0 ? 'text-green-400' : value > 3 ? 'text-red-400' : 'text-yellow-400';
      case 'highlights':
    ...
    <div className="bg-gray-900 text-white p-6 rounded-lg border border-cyan-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-gray-800 p-4 rounded-lg">
          ...
          <p className="text-3xl font-bold text-cyan-400">{matchResult.analytics.totalShots}</p>
        </div>
        <div className="bg-gray-800 p-4 rounded-lg">
        ...
        <div className="bg-gray-800 p-4 rounded-lg">
          ...
          <p className="text-3xl font-bold text-cyan-400">{matchResult.analytics.fouls}</p>
        </div>
        <div className="bg-gray-800 p-4 rounded-lg">
          <h4 className="text-lg font-semibold mb-2">Highlights</h4>
          <p className="text-3xl font-bold text-cyan-400">{matchResult.analytics.highlights.length}</p>
        </div>
      </div>

      {/* Player Comparison */}
      <div className="bg-gray-800 p-4 rounded-lg mb-6">
        <h4 className="text-lg font-semibold mb-4">Player Performance Comparison</h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {Object.entries(matchResult.analytics.playerStats).map(([playerId, stats]) => {
            const value = getMetricValue(playerId, selectedMetric);
            const displayValue = selectedMetric === 'accuracy' ? `${(value * 100).toFixed(1)}%` : value;
            
            return (
              <div key={playerId} className="bg-gray-700 p-4 rounded-lg">
                <div className="flex justify-between items-center mb-2">
                  <h5 className="font-semibold">{playerId}</h5>
                  <span className={`text-xl font-bold ${getMetricColor(selectedMetric, value)}`}>
                    {displayValue}
                  </span>
                </div>
        ...
        <div className="space-y-3">
          {matchResult.analytics.accuracy < 0.7 && (
            <div className="bg-blue-900/20 border border-blue-500 p-3 rounded-lg">
              <h5 className="font-semibold text-blue-400 mb-2">Improve Shot Accuracy</h5>
          ...
          {matchResult.analytics.fouls > 2 && (
            <div className="bg-yellow-900/20 border border-yellow-500 p-3 rounded-lg">
              <h5 className="font-semibold text-yellow-400 mb-2">Reduce Fouls</h5>
              <ul className="text-sm text-gray-300 space-y-1">
          ...
          {matchResult.analytics.totalShots < 20 && (
            <div className="bg-green-900/20 border border-green-500 p-3 rounded-lg">
              <h5 className="font-semibold text-green-400 mb-2">Efficient Play</h5>
              <ul className="text-sm text-gray-300 space-y-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <div className="space-y-1 text-sm">
              <p><span className="text-gray-400">Duration:</span> {Math.floor(matchResult.duration / 60000)}m {Math.floor((matchResult.duration % 60000) / 1000)}s</p>
              <p><span className="text-gray-400">Winner Score:</span> {matchResult.winnerScore}</p>
              <p><span className="text-gray-400">Loser Score:</span> {matchResult.loserScore}</p>
              <p><span className="text-gray-400">Score Difference:</span> {Math.abs(matchResult.winnerScore - matchResult.loserScore)}</p>
            </div>
          </div>
          <div>
            <h5 className="font-semibold mb-2">Performance Metrics</h5>
            <div className="space-y-1 text-sm">
              <p><span className="text-gray-400">Average Shot Time:</span> {matchResult.analytics.averageShotTime > 0 ? `${Math.round(matchResult.analytics.averageShotTime / 1000)}s` : 'N/A'}</p>
              <p><span className="text-gray-400">Shots per Minute:</span> {Math.round((matchResult.analytics.totalShots / (matchResult.duration / 60000)) * 10) / 10}</p>
              <p><span className="text-gray-400">Foul Rate:</span> {matchResult.analytics.fouls > 0 ? `${((matchResult.analytics.fouls / matchResult.analytics.totalShots) * 100).toFixed(1)}%` : '0%'}</p>
              <p><span className="text-gray-400">Highlight Rate:</span> {matchResult.analytics.highlights.length > 0 ? `${((matchResult.analytics.highlights.length / matchResult.analytics.totalShots) * 100).toFixed(1)}%` : '0%'}</p>
            </div>
          </div>
        </div>
      </div>
