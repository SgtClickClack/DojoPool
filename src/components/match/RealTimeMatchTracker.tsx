...
const RealTimeMatchTracker: React.FC<RealTimeMatchTrackerProps> = ({
  ...
  const handleMatchCompleted = ({ matchResult }: { matchResult: MatchResult }) => {
    setIsTracking(false);
  ...
  const handleAnalyticsUpdated = ({ matchId, analytics }: { matchId: string; analytics: MatchAnalytics }) => {
    if (matchData && matchData.id === matchId) {
      setMatchData(prev => prev ? { ...prev, matchAnalytics: analytics } : null);
    }
  };
  ...
  const activateMatch = () => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : 'Failed to activate match');
    }
  };
  ...
  const endMatch = async () => {
    ...
    try {
      const winnerId = matchData.score.player1 > matchData.score.player2 
        ? matchData.player1Id 
        : matchData.player2Id;
  ...
  const simulateMatchProgress = () => {
    ...
    const randomPlayer = Math.random() > 0.5 ? matchData.player1Id : matchData.player2Id;
  ...
  const getEventDescription = (type: string, playerId: string): string => {
    const playerName = playerId === matchData?.player1Id ? 'Player 1' : 'Player 2';
    ...
    <Card sx={{ maxWidth: 1000, width: '100%' }}>
      <CardContent>
        ...
        <TabPanel value={tabValue} index={0}>
          ...
          <List dense>
            {matchData.events.slice(-10).reverse().map((event) => (
                      ...
                      <Table size="small">
                        <TableBody>
                          <TableRow>
                            ...
                            <TableCell>{matchData.matchAnalytics.totalShots}</TableCell>
                          </TableRow>
                          <TableRow>
                            <TableCell>Successful Shots</TableCell>
                            <TableCell>{matchData.matchAnalytics.successfulShots}</TableCell>
                          </TableRow>
                          <TableRow>
                            <TableCell>Fouls</TableCell>
                            <TableCell>{matchData.matchAnalytics.fouls}</TableCell>
                          </TableRow>
                          <TableRow>
                            <TableCell>Breaks</TableCell>
                            <TableCell>{matchData.matchAnalytics.breaks}</TableCell>
                          </TableRow>
                          <TableRow>
                            ...
                            <TableCell>
                              <LinearProgress 
                                variant="determinate" 
                                value={matchData.matchAnalytics.excitementLevel * 100} 
                                sx={{ width: 100 }}
                              />
                            </TableCell>
                          </TableRow>
                ...
                <Card variant="outlined">
                  <CardContent>
                    ...
                    <Box sx={{ mb: 2 }}>
                      ...
                      <LinearProgress 
                        variant="determinate" 
                        value={matchData.matchAnalytics.playerPerformance.player1.accuracy * 100} 
                        sx={{ mb: 1 }}
                      />
                      <Typography variant="caption">
                        Accuracy: {Math.round(matchData.matchAnalytics.playerPerformance.player1.accuracy * 100)}%
                      </Typography>
                    </Box>
                    <Box>
                      ...
                      <LinearProgress 
                        variant="determinate" 
                        value={matchData.matchAnalytics.playerPerformance.player2.accuracy * 100} 
                        sx={{ mb: 1 }}
                      />
                      <Typography variant="caption">
                        Accuracy: {Math.round(matchData.matchAnalytics.playerPerformance.player2.accuracy * 100)}%
                      </Typography>
                    </Box>
                  </CardContent>
                </Card>
