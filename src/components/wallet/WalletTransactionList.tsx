...
export const WalletTransactionList: React.FC<WalletTransactionListProps> = ({ transactions }) => {
  const getTransactionIcon = (type: string | undefined, amount: number) => {
    ...
    if (upperType === 'REWARD') return <EmojiEvents sx={{ color: '#ff00ff', textShadow: '0 0 10px #ff00ff' }} />;
    if (upperType === 'PURCHASE') return <ShoppingCart sx={{ color: '#00a8ff', textShadow: '0 0 10px #00a8ff' }} />;
    return amount > 0 ? 
              ...
              primary={
                <Box display="flex" alignItems="center" gap={1} flexWrap="wrap">
                  ...
                  <Chip
                    ...
                    size="small"
                    sx={cyberChipStyle(getTransactionColor(transaction.type, transaction.amount))}
                  />
                </Box>
              }
              secondary={
                <React.Fragment>
                  ...
                  >
                    {transaction.date ? formatDistanceToNow(new Date(transaction.date), { addSuffix: true }) : 'Date unavailable'}
                  </Typography>
                  <Typography
                    component="span"
                    variant="h6"
                  ...
                  >
                    {transaction.amount > 0 ? '+' : ''}{formatCurrency(Math.abs(transaction.amount), transaction.currency || 'DP')}
                  </Typography>
                </React.Fragment>
              }
            />
