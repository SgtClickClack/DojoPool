...
import AvatarCreationService, { ClothingItem, AvatarAsset } from '../../services/avatar/AvatarCreationService';
...
const AvatarCreationFlow: React.FC<AvatarCreationFlowProps> = ({
  ...
  const loadScanningGuidelines = async () => {
    try {
      ...
      const guidelines = await AvatarCreationService.getScanningGuidelines(platform);
...
Would you like to continue on mobile or use the photogrammetry fallback?`;
  ...
  const simulatePhotogrammetryScanning = async () => {
    ...
    try {
      ...
      const mockImages = Array(12).fill(0).map(() => generateMockImageData());
      const result = await AvatarCreationService.submitPhotogrammetryScan(userId, mockImages);
  ...
  const pollBaseMeshStatus = async (scanId: string) => {
    ...
    const checkStatus = async () => {
      try {
        const meshStatus = await AvatarCreationService.getBaseMeshStatus(scanId);
            ...
            <CardContent>
              ...
              {scanningGuidelines && (
                <Box sx={{ mb: 3 }}>
                  ...
                  {Object.entries(scanningGuidelines.requirements).map(([key, value]) => (
                    <Chip
                      key={key}
                      label={`${key}: ${value}`}
                      size="small"
          ...
          <Card>
            <CardContent>
              ...
              <Typography variant="body1" sx={{ mb: 2 }}>
                Your 3D scan is being processed using Laplacian Mesh Deformation to create your base avatar.
              </Typography>
              {baseMeshId ? (
          ...
          <Card>
            <CardContent>
              ...
              <Typography variant="body1" sx={{ mb: 3 }}>
                Select clothing items for your avatar. You can choose one item per category.
              </Typography>
                  ...
                  <Grid item xs={12} sm={6} md={4} key={item.id}>
                    <Card
                      sx={{
                        ...
                        borderColor: selectedClothing.includes(item.id) ? 'primary.main' : 'divider',
        ...
        return (
          <Card>
            <CardContent>
              ...
              <Typography variant="body1" sx={{ mb: 3 }}>
                Your 3D avatar has been successfully created and optimized with Draco compression and KTX2 textures.
              </Typography>
