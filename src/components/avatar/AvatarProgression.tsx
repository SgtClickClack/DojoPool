...
import type { AvatarProgression, AvatarFeature, StoryAchievement, VenueMastery } from '../../services/avatar/AvatarProgressionService';
...
const AvatarProgression: React.FC<AvatarProgressionProps> = ({ userId }) => {
  ...
  const [selectedFeature, setSelectedFeature] = useState<AvatarFeature | null>(null);
  const [featureDialogOpen, setFeatureDialogOpen] = useState(false);
  const [storyEventDialogOpen, setStoryEventDialogOpen] = useState(false);
  const [storyEventMessage, setStoryEventMessage] = useState<string | null>(null);

  useEffect(() => {
    const userProgression = avatarProgressionService.getUserAvatarProgression(userId);
  ...
  const getFeatureIcon = (type: string) => {
    switch (type) {
      case 'outfit': return <StyleIcon />;
      case 'accessory': return <PersonIcon />;
      case 'effect': return <EffectIcon />;
      case 'badge': return <BadgeIcon />;
      case 'animation': return <ViewIcon />;
  ...
  const handleStartDemoStoryArc = () => {
    const demoArc = {
      ...
      steps: [
        { id: 'step1', description: 'Win your first match', isComplete: false, eventType: 'win_match' },
        { id: 'step2', description: 'Defeat a rival', isComplete: false, eventType: 'defeat_rival' },
        { id: 'step3', description: 'Master a venue', isComplete: false, eventType: 'venue_mastery' }
      ],
      currentStep: 0,
      isComplete: false,
      startedAt: new Date()
    ...
    setProgression({ ...avatarProgressionService.getUserAvatarProgression(userId) });
  ...
  return (
    <Box sx={{ p: 2 }}>
      ...
      {!progression.storyArc && (
        <Button variant="contained" color="secondary" sx={{ mb: 3 }} onClick={handleStartDemoStoryArc}>
          Start Demo Story Arc
        </Button>
      )}
      {progression.storyArc && !progression.storyArc.isComplete && (
        <Button variant="contained" color="primary" sx={{ mb: 3 }} onClick={handleAdvanceStoryArc}>
          Advance Story Arc
        </Button>
      )}
        ...
        <Card sx={{ mb: 3 }}>
          <CardContent>
            ...
            <Typography variant="body2" color="text.secondary">
              Step {progression.storyArc.currentStep + 1} / {progression.storyArc.steps.length}: {progression.storyArc.steps[progression.storyArc.currentStep]?.description}
            </Typography>
            {progression.storyArc.rivalUserId && (
              <Chip label={`Rival: ${progression.storyArc.rivalUserId}`} color="error" sx={{ mt: 1 }} />
            )}
            <LinearProgress 
              variant="determinate" 
              value={((progression.storyArc.currentStep + (progression.storyArc.isComplete ? 1 : 0)) / progression.storyArc.steps.length) * 100} 
      ...
      {/* Pool God Blessings */}
      {progression.poolGodBlessings && progression.poolGodBlessings.length > 0 && (
        <Card sx={{ mb: 3 }}>
          <CardContent>
            ...
            <List dense>
              {progression.poolGodBlessings.map((blessing) => (
                <ListItem key={blessing.id} sx={{ border: 1, borderColor: 'divider', borderRadius: 1, mb: 1 }}>
                  <ListItemIcon>
                      ...
                      secondary={
                        <Box>
                          ...
                          {feature.storyContext && (
                            <Chip label={feature.storyContext} size="small" sx={{ ml: 1 }} />
                          )}
                        </Box>
                      }
                    />
        ...
        <Grid item xs={12} md={6}>
          <Card>
            <CardContent>
              ...
              <List dense>
                {progression.storyAchievements.slice(-5).reverse().map((achievement) => (
                  <ListItem key={achievement.id} sx={{ border: 1, borderColor: 'divider', borderRadius: 1, mb: 1 }}>
                    <ListItemIcon>
                          ...
                          <Box sx={{ display: 'flex' }}>
                            {[1, 2, 3, 4, 5].map((level) => (
                              <StarIcon 
                                key={level}
                                sx={{ 
                                  color: level <= mastery.masteryLevel ? 'warning.main' : 'grey.300',
                                  fontSize: 16
                                }}
                              />
                            ))}
                    ...
                    <Grid item xs={12} sm={6} md={4} key={effect.id}>
                      <Card variant="outlined" sx={{ backgroundColor: 'secondary.light' }}>
                        <CardContent>
                          ...
                          <Typography variant="body2" color="text.secondary">
                            {effect.type} • Intensity: {Math.round(effect.intensity * 100)}%
                          </Typography>
                          {effect.duration > 0 && (
                            <Typography variant="body2" color="text.secondary">
      ...
      <Dialog open={featureDialogOpen} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
        {selectedFeature && (
          <>
      ...
      <Dialog open={storyEventDialogOpen} onClose={() => setStoryEventDialogOpen(false)}>
