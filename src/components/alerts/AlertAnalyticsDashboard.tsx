...
const useAlertAnalytics = (): AlertAnalyticsHook => {
    ...
    useEffect(() => {
        const unsubscribe = subscribe('analytics_update', (data: AlertAnalyticsData) => {
            setAnalytics(data);
        ...
        try {
            ...
            setAnalytics({
                ...
                alertsByStatus: {
                    ...
                    [AlertStatus.ACKNOWLEDGED]: data.alertsByStatus[AlertStatus.ACKNOWLEDGED] || 0,
                    [AlertStatus.DISMISSED]: data.alertsByStatus[AlertStatus.DISMISSED] || 0
                }
            });
            setLastUpdated(new Date());
        } catch (err) {
...
const AlertTable: React.FC<{ data: AlertAnalyticsData }> = ({ data }) => {
    ...
    const [orderBy, setOrderBy] = useState<keyof Pick<ChartData, 'name' | 'value'>>('value');

    const handleRequestSort = (property: keyof Pick<ChartData, 'name' | 'value'>) => {
    ...
    const sortedData = useMemo(() => {
        return [...Object.entries(data.alertsByType).map(([name, value]) => ({ name, value }))]
            .sort((a, b) => {
...
export const AlertAnalyticsDashboard: React.FC = () => {
    ...
    const { analytics, loading, error, refresh, lastUpdated, isConnected } = useAlertAnalytics();
    ...
    const filteredData = useMemo(() => {
        ...
        return {
            ...
            alertsByType: {
                [AlertType.ERROR]: filters.types.length === 0 || filters.types.includes(AlertType.ERROR) ? analytics.alertsByType[AlertType.ERROR] : 0,
                [AlertType.WARNING]: filters.types.length === 0 || filters.types.includes(AlertType.WARNING) ? analytics.alertsByType[AlertType.WARNING] : 0,
                [AlertType.SUCCESS]: filters.types.length === 0 || filters.types.includes(AlertType.SUCCESS) ? analytics.alertsByType[AlertType.SUCCESS] : 0,
                [AlertType.INFO]: filters.types.length === 0 || filters.types.includes(AlertType.INFO) ? analytics.alertsByType[AlertType.INFO] : 0
            },
            alertsByStatus: {
                [AlertStatus.OPEN]: filters.statuses.length === 0 || filters.statuses.includes(AlertStatus.OPEN) ? analytics.alertsByStatus[AlertStatus.OPEN] : 0,
                [AlertStatus.ACKNOWLEDGED]: filters.statuses.length === 0 || filters.statuses.includes(AlertStatus.ACKNOWLEDGED) ? analytics.alertsByStatus[AlertStatus.ACKNOWLEDGED] : 0,
                [AlertStatus.DISMISSED]: filters.statuses.length === 0 || filters.statuses.includes(AlertStatus.DISMISSED) ? analytics.alertsByStatus[AlertStatus.DISMISSED] : 0
            }
        };
    }, [analytics, filters]);
    ...
    if (loading) {
        return (
            <Container maxWidth="lg" role="region" aria-label="Alert Analytics Dashboard">
                <Box 
                    sx={{ 
                        display: 'flex', 
    ...
    return (
        <Container maxWidth="lg" role="region" aria-label="Alert Analytics Dashboard">
            <Box sx={{ my: 4 }}>
                ...
                >
                    <DialogTitle id="export-dialog-title">Export Analytics Data</DialogTitle>
                    <DialogContent>
