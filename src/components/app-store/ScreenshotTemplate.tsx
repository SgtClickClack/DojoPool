...
import { Wifi as WifiIcon, SignalCellularAlt as SignalIcon, BatteryFull as BatteryIcon } from '@mui/icons-material'; // Example icons
...
const DeviceFrame = styled(Box, {
  shouldForwardProp: (prop) => prop !== 'frameBorderRadius' && prop !== 'borderColor',
...
const StatusBar = styled(Box, {
  ...
  backgroundColor: theme.palette.mode === 'dark' ? theme.palette.grey[900] : theme.palette.grey[100], // Or a solid color
  display: 'flex',
...
const DefaultStatusBarContent: React.FC = () => {
  ...
  return (
    <>
      <Typography variant="caption" sx={{ fontWeight: 'medium', color: iconColor }}>
        {time}
      </Typography>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
        ...
        <Typography variant="caption" sx={{ fontWeight: 'medium', ml: 0.5, color: iconColor }}>100%</Typography>
      </Box>
    </>
  );
};
...
export const ScreenshotTemplate: React.FC<ScreenshotTemplateProps> = ({
  ...
  return (
    <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', p: {xs: 2, md: 4}, width: 'fit-content', margin: 'auto' }}> {/* Outer centering wrapper */}
      <ScreenshotContainer dimensions={dimensions} bgColor={backgroundColor}>
        {showDeviceFrame && <DeviceFrame frameBorderRadius={dimensions.borderRadius} borderColor={frameColor} />}
        {showStatusBar && (
          <StatusBar height={dimensions.statusBarHeight}>
            {statusBarContent !== undefined ? statusBarContent : <DefaultStatusBarContent />}
          </StatusBar>
        )}
        <AppContentContainer>
          {children}
