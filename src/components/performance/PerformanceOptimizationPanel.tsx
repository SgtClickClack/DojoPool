  ...
  const loadPerformanceData = () => {
    ...
    setRecommendations([
      ...
      },
      {
        ...
        description: 'Memory usage is at 65%. Consider optimizing memory usage.',
        impact: 6,
        effort: 4,
          ...
          <TabPanels>
            ...
            <TabPanel>
              <VStack spacing={6} align="stretch">
                    ...
                    <CardBody>
                      <HStack spacing={8} wrap="wrap">
                        <Stat>
                          ...
                          <StatNumber>{performanceMetrics.responseTime.toFixed(1)}ms</StatNumber>
                          <StatHelpText>
                            <Badge colorScheme={performanceMetrics.responseTime > 150 ? 'red' : 'green'}>
                              {performanceMetrics.responseTime > 150 ? 'High' : 'Good'}
                            </Badge>
                          </StatHelpText>
                        </Stat>
                        ...
                        <Stat>
                          ...
                          <StatNumber>{performanceMetrics.throughput.toFixed(0)} req/s</StatNumber>
                          <StatHelpText>
                        ...
                        <Stat>
                          ...
                          <StatNumber>{performanceMetrics.errorRate.toFixed(2)}%</StatNumber>
                          <StatHelpText>
                            <Badge colorScheme={performanceMetrics.errorRate > 5 ? 'red' : 'green'}>
                              {performanceMetrics.errorRate > 5 ? 'High' : 'Low'}
                            </Badge>
                          </StatHelpText>
                        </Stat>
                        ...
                        <Stat>
                          ...
                          <StatNumber>{performanceMetrics.cacheHitRate.toFixed(1)}%</StatNumber>
                          <StatHelpText>
                            <Badge colorScheme={performanceMetrics.cacheHitRate < 80 ? 'yellow' : 'green'}>
                              {performanceMetrics.cacheHitRate < 80 ? 'Low' : 'Good'}
                            </Badge>
                          </StatHelpText>
                        </Stat>
                      </HStack>
                      ...
                      <Box mt={4}>
                        ...
                        <Progress 
                          ...
                          colorScheme={performanceMetrics.memoryUsage > 0.8 ? 'red' : 'green'}
                          size="lg"
                        />
                        <Text fontSize="sm" mt={1}>
                          {(performanceMetrics.memoryUsage * 100).toFixed(1)}% used
                        </Text>
                      </Box>
                      ...
                      <Box mt={4}>
                        ...
                        <Progress 
                          ...
                          colorScheme={performanceMetrics.cpuUsage > 80 ? 'red' : 'green'}
                          size="lg"
                        />
                        <Text fontSize="sm" mt={1}>
                ...
                <Alert status={performanceMetrics ? getPerformanceStatus(performanceMetrics) : 'info'}>
                  <AlertIcon />
                  {performanceMetrics ? (
                    ...
                    <CardBody>
                      <VStack spacing={4} align="stretch">
                        <HStack justify="space-between">
                          <Text>Heap Used</Text>
                          <Text fontWeight="bold">{formatBytes(memoryMetrics.heapUsed)}</Text>
                        </HStack>

                        <HStack justify="space-between">
                          <Text>Heap Total</Text>
                          <Text fontWeight="bold">{formatBytes(memoryMetrics.heapTotal)}</Text>
                        </HStack>

                        <HStack justify="space-between">
                          <Text>External Memory</Text>
                          <Text fontWeight="bold">{formatBytes(memoryMetrics.external)}</Text>
                        </HStack>

                        <HStack justify="space-between">
                          <Text>Array Buffers</Text>
                          <Text fontWeight="bold">{formatBytes(memoryMetrics.arrayBuffers)}</Text>
                        </HStack>

                        <HStack justify="space-between">
                          <Text>RSS (Resident Set Size)</Text>
                          <Text fontWeight="bold">{formatBytes(memoryMetrics.rss)}</Text>
                        </HStack>
                        ...
                        <Box>
                          ...
                          <Progress 
                            value={(memoryMetrics.heapUsed / memoryMetrics.heapTotal) * 100}
                            colorScheme={(memoryMetrics.heapUsed / memoryMetrics.heapTotal) > 0.8 ? 'red' : 'green'}
                            size="lg"
                          />
                          <Text fontSize="sm" mt={1}>
                            {((memoryMetrics.heapUsed / memoryMetrics.heapTotal) * 100).toFixed(1)}% of heap used
                          </Text>
                        </Box>
                    ...
                    <CardBody>
                      <VStack spacing={4} align="stretch">
                        <HStack justify="space-between">
                          ...
                          <Text fontWeight="bold">{formatBytes(bundleMetrics.totalSize)}</Text>
                        </HStack>

                        <HStack justify="space-between">
                          <Text>Gzipped Size</Text>
                          <Text fontWeight="bold">{formatBytes(bundleMetrics.gzippedSize)}</Text>
                        </HStack>

                        <HStack justify="space-between">
                          <Text>Chunk Count</Text>
                          <Text fontWeight="bold">{bundleMetrics.chunkCount}</Text>
                        </HStack>

                        <HStack justify="space-between">
                          <Text>Optimization Score</Text>
                          <Badge 
                            colorScheme={bundleMetrics.optimizationScore > 80 ? 'green' : bundleMetrics.optimizationScore > 60 ? 'yellow' : 'red'}
                            fontSize="md"
                            p={2}
                          >
                        ...
                        <Box>
                          <Text mb={3} fontWeight="bold">Bundle Chunks</Text>
                            ...
                            <Tbody>
                              {bundleMetrics.chunks.map((chunk, index) => (
                                <Tr key={index}>
                                  ...
                                  <Td>
                                    <Badge colorScheme={chunk.gzippedSize / chunk.size < 0.3 ? 'green' : 'yellow'}>
                                      {((chunk.gzippedSize / chunk.size) * 100).toFixed(1)}%
                                    </Badge>
                                  </Td>
                                </Tr>
                              ))}
                        ...
                        {bundleMetrics.recommendations.length > 0 && (
                          <Box>
                            <Text mb={3} fontWeight="bold">Recommendations</Text>
                            <List spacing={2}>
                    ...
                    <VStack spacing={4} align="stretch">
                      ...
                      <HStack justify="space-between">
                        ...
                        <Text fontWeight="bold">
                          {performanceMetrics ? `${performanceMetrics.cacheHitRate.toFixed(1)}%` : 'N/A'}
                        </Text>
                      </HStack>
                    ...
                    <VStack spacing={4} align="stretch">
                      {recommendations.map((rec, index) => (
                        ...
                        >
                          ...
                          <Text mb={3} color="gray.600">{rec.description}</Text>
                          
                          <HStack spacing={4} mb={3}>
                            <Badge colorScheme="blue">Impact: {rec.impact}/10</Badge>
                            <Badge colorScheme="purple">Effort: {rec.effort}/10</Badge>
                          </HStack>
                  ...
                  <CardBody>
                    <VStack spacing={4} align="stretch">
                      ...
                      <HStack justify="space-between">
                        ...
                        <Text>{performanceMetrics?.timestamp.toLocaleTimeString() || 'N/A'}</Text>
                      </HStack>
