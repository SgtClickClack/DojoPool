...
import NFTMarketplaceService, { NFTMetadata, NFTListing, NFTBid, NFTRarity, BlockchainNetwork, NFTMarketplaceStats } from '../../services/blockchain/NFTMarketplaceService';
...
export const NFTMarketplaceComponent: React.FC<NFTMarketplaceComponentProps> = ({
  userAddress,
  onNFTMinted,
  onNFTPurchased
}) => {
  const [activeTab, setActiveTab] = useState<'browse' | 'my-nfts' | 'achievements' | 'collections'>('browse');
  ...
  const [bidCurrency, setBidCurrency] = useState<'ETH' | 'DOJO' | 'USDC'>('ETH');
  ...
  const loadData = async () => {
    try {
      ...
      const listedNFTs = activeListings.map(listing => 
        marketplaceService.getNFT(listing.tokenId, listing.contractAddress, listing.network)
  ...
  const handleListNFT = async (nft: NFTMetadata, price: string, currency: 'ETH' | 'DOJO' | 'USDC') => {
    try {
      if (!userAddress) {
    ...
    <div className="bg-white rounded-lg shadow-lg p-6">
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-blue-50 p-4 rounded-lg text-center">
            <div className="text-2xl font-bold text-blue-600">{stats.totalVolume} ETH</div>
          ...
          <div className="bg-orange-50 p-4 rounded-lg text-center">
            <div className="text-2xl font-bold text-orange-600">{stats.floorPrice} ETH</div>
      ...
      <div className="flex space-x-1 mb-6 bg-gray-100 p-1 rounded-lg">
        ...
        <button
          ...
          className={`flex-1 py-2 px-4 rounded-md font-medium transition-colors ${
            activeTab === 'my-nfts' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-800'
          }`}
        >
          My NFTs
        </button>
        <button
          ...
          className={`flex-1 py-2 px-4 rounded-md font-medium transition-colors ${
            activeTab === 'achievements' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-800'
          }`}
        >
          Achievements
        </button>
        <button
          ...
          className={`flex-1 py-2 px-4 rounded-md font-medium transition-colors ${
            activeTab === 'collections' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-800'
          }`}
        >
          Collections
        </button>
      ...
      <div className="space-y-6">
        {activeTab === 'browse' && (
          <div>
            <h3 className="text-lg font-semibold text-gray-800 mb-4">Available NFTs</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {nfts.map((nft) => {
                ...
                return (
                  <div key={nft.tokenId} className="bg-gray-50 rounded-lg p-4 hover:shadow-md transition-shadow">
                    <img 
                      src={nft.image} 
                      alt={nft.name}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between text-sm text-gray-500">
                        <span>{getNetworkIcon(nft.network)} {nft.network}</span>
        ...
        {activeTab === 'my-nfts' && (
          <div>
            <h3 className="text-lg font-semibold text-gray-800 mb-4">My NFTs</h3>
            {myNFTs.length === 0 ? (
        ...
        {activeTab === 'achievements' && (
          <div>
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm mt-2">Complete challenges to earn unique NFTs</p>
            </div>
          </div>
        )}
        ...
        {activeTab === 'collections' && (
          <div>
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm mt-2">Browse curated collections of related NFTs</p>
            </div>
          </div>
        )}
      </div>
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              ...
              <select
                ...
                onChange={(e) => setMintForm({...mintForm, rarity: e.target.value as NFTRarity})}
                className="w-full p-2 border rounded"
              >
              ...
              <select
                ...
                onChange={(e) => setMintForm({...mintForm, network: e.target.value as BlockchainNetwork})}
                className="w-full p-2 border rounded"
              >
      ...
      {showBidDialog && selectedNFT && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold mb-4">Place Bid on {selectedNFT.name}</h3>
            <div className="space-y-4">
              ...
              <select
                value={bidCurrency}
                onChange={(e) => setBidCurrency(e.target.value as 'ETH' | 'DOJO' | 'USDC')}
                className="w-full p-2 border rounded"
              >
