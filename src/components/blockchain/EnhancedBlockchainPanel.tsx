...
export const EnhancedBlockchainPanel: React.FC = () => {
  ...
  const handleCrossChainTransfer = async () => {
    if (!crossChainForm.fromUserId || !crossChainForm.toUserId || !crossChainForm.amount) {
      return;
    }
  ...
  const handleListNFT = async () => {
    if (!nftForm.tokenId || !nftForm.contractAddress || !nftForm.seller || !nftForm.price) {
      return;
    }
  ...
  const getNetworkIcon = (network: string) => {
    switch (network) {
      case 'ethereum': return <Network className="w-4 h-4" />;
      case 'polygon': return <Zap className="w-4 h-4" />;
      case 'solana': return <Activity className="w-4 h-4" />;
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="overview" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {analytics ? `${formatBalance(analytics.totalVolume)} DOJO` : 'Loading...'}
                </div>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Cross-Chain Transfers</CardTitle>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">NFT Listings</CardTitle>
            ...
            <CardContent>
              {loadingHealth ? (
                ...
                <div className="grid grid-cols-3 gap-4">
                  {Object.entries(healthStatus).map(([network, status]) => (
                    network !== 'overall' && (
            ...
            <CardContent>
              <div className="space-y-4">
                {Array.from(balances.values()).map((balance) => (
                  <div key={`${balance.userId}-${balance.network}`} className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex items-center gap-3">
                      {getNetworkIcon(balance.network)}
                      <div>
                        <p className="font-medium">User {balance.userId}</p>
                        <p className="text-sm text-muted-foreground capitalize">{balance.network}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-bold">{formatBalance(balance.balance)} DOJO</p>
                      <p className="text-xs text-muted-foreground">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={transferForm.network} onValueChange={(value) => setTransferForm(prev => ({ ...prev, network: value }))}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <Button onClick={handleTransfer} className="mt-4" disabled={loadingBalances}>
        ...
        <TabsContent value="cross-chain" className="space-y-4">
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {Array.from(crossChainTransactions.values()).map((transaction) => (
                  <div key={transaction.id} className="p-4 border rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        {getStatusIcon(transaction.status)}
                        <span className="font-medium">Transfer #{transaction.id.slice(-8)}</span>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <p className="font-medium">{transaction.fromNetwork}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="font-medium">{formatBalance(transaction.amount)} DOJO</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Bridge Fee</p>
                        <p className="font-medium">{formatBalance(transaction.bridgeFee)} DOJO</p>
                      </div>
                    </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setCrossChainForm(prev => ({ ...prev, fromUserId: e.target.value }))}
                    placeholder="Enter sender user ID"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setCrossChainForm(prev => ({ ...prev, toUserId: e.target.value }))}
                    placeholder="Enter recipient user ID"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setCrossChainForm(prev => ({ ...prev, amount: e.target.value }))}
                    placeholder="Enter amount"
                  />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="fromNetwork">From Network</Label>
                  <Select value={crossChainForm.fromNetwork} onValueChange={(value) => setCrossChainForm(prev => ({ ...prev, fromNetwork: value }))}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={crossChainForm.toNetwork} onValueChange={(value) => setCrossChainForm(prev => ({ ...prev, toNetwork: value }))}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <Button onClick={handleCrossChainTransfer} className="mt-4" disabled={loadingCrossChain}>
                    ...
                    <div key={item.id} className="border rounded-lg p-4">
                      <div className="aspect-square bg-muted rounded-lg mb-3 flex items-center justify-center">
                        <img 
                          ...
                          onError={(e) => {
                            e.currentTarget.src = '/images/nft-placeholder.webp';
                          }}
                        />
                      </div>
                      <h3 className="font-semibold mb-1">{item.metadata.name}</h3>
                      <p className="text-sm text-muted-foreground mb-2">{item.metadata.description}</p>
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-1">
                          {getNetworkIcon(item.network)}
                          <span className="text-xs capitalize">{item.network}</span>
                        </div>
                        <Badge variant="outline">{item.currency}</Badge>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="font-bold">{formatBalance(item.price)} DOJO</span>
                        <Button size="sm" onClick={() => handleBuyNFT(item.id, item.price)}>
                          Buy
                        </Button>
                      </div>
                    </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setNftForm(prev => ({ ...prev, contractAddress: e.target.value }))}
                    placeholder="Enter contract address"
                  />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="nftNetwork">Network</Label>
                  <Select value={nftForm.network} onValueChange={(value) => setNftForm(prev => ({ ...prev, network: value }))}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setNftForm(prev => ({ ...prev, seller: e.target.value }))}
                ...
                <div className="space-y-2">
                  ...
                  <Select value={nftForm.currency} onValueChange={(value: 'DOJO' | 'ETH' | 'SOL') => setNftForm(prev => ({ ...prev, currency: value }))}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
        ...
        <TabsContent value="analytics" className="space-y-4">
          <Card>
            ...
            <CardContent>
              {loadingAnalytics ? (
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <h3 className="font-semibold mb-3">Network Distribution</h3>
                      <div className="space-y-2">
                        {Object.entries(analytics.networkDistribution).map(([network, count]) => (
                          <div key={network} className="flex items-center justify-between">
                            <span className="capitalize">{network}</span>
                            <div className="flex items-center gap-2">
                              <Progress value={(count / analytics.totalTransactions) * 100} className="w-20" />
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{formatBalance(analytics.totalVolume)} DOJO</span>
                        </div>
                        <div className="flex justify-between">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{analytics.crossChainTransfers}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>NFT Listings</span>
                          <span className="font-medium">{analytics.nftListings}</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <Button onClick={refreshAnalytics} className="mt-4" disabled={loadingAnalytics}>
