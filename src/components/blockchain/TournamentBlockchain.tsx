...
const TournamentBlockchain: React.FC = () => {
  ...
  const [selectedNetwork, setSelectedNetwork] = useState<'ethereum' | 'solana' | 'polygon'>('ethereum');
  ...
  const handleCreateTransaction = async () => {
    ...
    try {
      await blockchainService.createTransaction({
        ...
        from: wallets[0]?.address || '0x0000000000000000000000000000000000000000',
    ...
    <Box sx={{ 
                ...
                <Card sx={{ 
                  ...
                  <CardContent>
                    ...
                    <Typography variant="body2" sx={{ color: '#ccc', mb: 1 }}>
                      Amount: <span style={{ color: getCurrencyColor(tx.currency) }}>
                  ...
                  <CardContent>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                      ...
                      <Chip
                        ...
                        sx={{ 
                          background: wallet.isConnected ? 'rgba(0, 255, 136, 0.2)' : 'rgba(255, 68, 68, 0.2)',
                          border: `1px solid ${wallet.isConnected ? '#00ff88' : '#ff4444'}`
                        }}
                      />
                    </Box>
                    <Typography variant="body2" sx={{ color: '#ccc', mb: 1 }}>
                      Address: {wallet.address.substring(0, 10)}...{wallet.address.substring(wallet.address.length - 8)}
                    </Typography>
                    <Typography variant="body2" sx={{ color: '#ff6b35', mb: 1 }}>
                      DOJO: {wallet.balance.DOJO.toFixed(2)}
                    </Typography>
                    <Typography variant="body2" sx={{ color: '#627eea', mb: 1 }}>
                      ETH: {wallet.balance.ETH.toFixed(4)}
                    </Typography>
                    <Typography variant="body2" sx={{ color: '#9945ff', mb: 1 }}>
              ...
              <Grid item xs={12} md={6} key={contract.address}>
                <Card sx={{ 
                  ...
                  <CardContent>
                    ...
                    <Typography variant="body2" sx={{ color: '#ccc', mb: 1 }}>
                      Address: {contract.address.substring(0, 10)}...{contract.address.substring(contract.address.length - 8)}
                    </Typography>
                    <Typography variant="body2" sx={{ color: '#888' }}>
                ...
                <Card sx={{ 
                  ...
                  <CardContent>
                    ...
                    <Chip
                      label={nft.rarity}
                      sx={{
                        background: `rgba(${nft.rarity === 'legendary' ? '255, 215, 0' : 
                          nft.rarity === 'epic' ? '138, 43, 226' : 
                          nft.rarity === 'rare' ? '0, 191, 255' : '128, 128, 128'}, 0.2)`,
                        border: `1px solid ${nft.rarity === 'legendary' ? '#ffd700' : 
                          nft.rarity === 'epic' ? '#8a2be2' : 
                          nft.rarity === 'rare' ? '#00bfff' : '#808080'}`,
              ...
              <Grid item xs={12} md={6} key={pool.id}>
                <Card sx={{ 
                  ...
                  <CardContent>
                    ...
                    <Typography variant="body2" sx={{ color: '#00ccff', mb: 1 }}>
      ...
      <Dialog open={transactionDialog} onClose={() => setTransactionDialog(false)} maxWidth="sm" fullWidth>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) => setTransactionForm({ ...transactionForm, currency: e.target.value as any })}
                  sx={{ color: '#fff', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#333' } }}
                >
            ...
            <Grid item xs={12} md={4}>
              <TextField
                ...
                onChange={(e) => setTransactionForm({ ...transactionForm, tournamentId: e.target.value })}
                sx={{ '& .MuiOutlinedInput-root': { color: '#fff', '& fieldset': { borderColor: '#333' } } }}
              />
            </Grid>
            <Grid item xs={12} md={4}>
              <TextField
                ...
                onChange={(e) => setTransactionForm({ ...transactionForm, matchId: e.target.value })}
                sx={{ '& .MuiOutlinedInput-root': { color: '#fff', '& fieldset': { borderColor: '#333' } } }}
              />
            </Grid>
            <Grid item xs={12} md={4}>
              <TextField
                ...
                onChange={(e) => setTransactionForm({ ...transactionForm, playerId: e.target.value })}
                sx={{ '& .MuiOutlinedInput-root': { color: '#fff', '& fieldset': { borderColor: '#333' } } }}
              />
            </Grid>
          </Grid>
        ...
        <DialogActions sx={{ background: '#1a1a2e' }}>
          <Button onClick={() => setTransactionDialog(false)} sx={{ color: '#888' }}>
            Cancel
          </Button>
          <Button
      ...
      {/* NFT Dialog */}
      <Dialog open={nftDialog} onClose={() => setNftDialog(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ background: '#1a1a2e', color: '#00ff88' }}>
          Mint New NFT
        </DialogTitle>
        <DialogContent sx={{ background: '#1a1a2e', pt: 2 }}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) => setNftForm({ ...nftForm, rarity: e.target.value as any })}
                  sx={{ color: '#fff', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#333' } }}
                >
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={(e) => setNftForm({ ...nftForm, tournamentId: e.target.value })}
                sx={{ '& .MuiOutlinedInput-root': { color: '#fff', '& fieldset': { borderColor: '#333' } } }}
              />
            </Grid>
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={(e) => setNftForm({ ...nftForm, playerId: e.target.value })}
                sx={{ '& .MuiOutlinedInput-root': { color: '#fff', '& fieldset': { borderColor: '#333' } } }}
              />
            </Grid>
          </Grid>
      ...
      {/* Pool Dialog */}
      <Dialog open={poolDialog} onClose={() => setPoolDialog(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ background: '#1a1a2e', color: '#00ff88' }}>
          Create DeFi Pool
        </DialogTitle>
        <DialogContent sx={{ background: '#1a1a2e', pt: 2 }}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              <TextField
                ...
                onChange={(e) => setPoolForm({ ...poolForm, liquidity: e.target.value })}
                sx={{ '& .MuiOutlinedInput-root': { color: '#fff', '& fieldset': { borderColor: '#333' } } }}
              />
            </Grid>
            <Grid item xs={12}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) => setPoolForm({ ...poolForm, tokens: e.target.value as string[] })}
                  sx={{ color: '#fff', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#333' } }}
                >
                  <MenuItem value="DOJO">DOJO</MenuItem>
                  <MenuItem value="ETH">ETH</MenuItem>
