...
import { TerritoryIncome } from '../../services/economy/PassiveIncomeService';

interface PassiveIncomeDashboardProps {
  className?: string;
}

export const PassiveIncomeDashboard: React.FC<PassiveIncomeDashboardProps> = ({ className = '' }) => {
  const {
    territoryIncomes,
    payoutHistory,
    config,
  ...
  const [selectedTerritory, setSelectedTerritory] = useState<TerritoryIncome | null>(null);
  ...
  const getActivityScore = (activity: any) => {
    ...
    return ((playerScore + matchScore + tournamentScore + revenueScore) / 4) * 100;
  };
  ...
  return (
    <Box className={className}>
      ...
      {stats && (
        <Grid container spacing={3} mb={3}>
          <Grid2 xs={12} sm={6} md={3}>
            <Card sx={{ backgroundColor: 'rgba(0, 255, 157, 0.1)', border: '1px solid #00ff9d' }}>
              <CardContent>
          ...
          <Grid2 xs={12} sm={6} md={3}>
            <Card sx={{ backgroundColor: 'rgba(0, 168, 255, 0.1)', border: '1px solid #00a8ff' }}>
              <CardContent>
                <Box display="flex" alignItems="center">
                ...
                <Typography variant="h4" sx={{ color: '#00a8ff', fontWeight: 'bold' }}>
          ...
          <Grid2 xs={12} sm={6} md={3}>
            <Card sx={{ backgroundColor: 'rgba(254, 202, 87, 0.1)', border: '1px solid #feca57' }}>
              <CardContent>
                <Box display="flex" alignItems="center">
          ...
          <Grid2 xs={12} sm={6} md={3}>
            <Card sx={{ backgroundColor: 'rgba(255, 71, 87, 0.1)', border: '1px solid #ff4757' }}>
              <CardContent>
                <Box display="flex" alignItems="center">
                ...
                <Typography variant="h4" sx={{ color: '#ff4757', fontWeight: 'bold' }}>
                  {stats.clanControlledTerritories}
                </Typography>
                <Typography variant="body2" color="gray">
                  {((stats.clanControlledTerritories / stats.totalTerritories) * 100).toFixed(1)}%
                </Typography>
              </CardContent>
            </Card>
          </Grid2>
      ...
      <Card sx={{ backgroundColor: 'rgba(30, 30, 40, 0.95)', border: '1px solid #333' }}>
        <CardContent>
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell sx={{ color: '#00ff9d' }}>Total Income/Hour</TableCell>
                  <TableCell sx={{ color: '#00ff9d' }}>Total Earned</TableCell>
                  <TableCell sx={{ color: '#00ff9d' }}>Activity Score</TableCell>
              ...
              <TableBody>
                {territoryIncomes.map((territory) => (
                  <TableRow key={territory.territoryId} sx={{ '&:hover': { backgroundColor: 'rgba(0, 255, 157, 0.05)' } }}>
                    <TableCell sx={{ color: 'white' }}>{territory.territoryId}</TableCell>
                    <TableCell sx={{ color: 'white' }}>{territory.ownerId}</TableCell>
                    <TableCell>
                      {territory.clanId ? (
                        <Chip label={territory.clanId} size="small" sx={{ backgroundColor: '#00a8ff' }} />
                      ) : (
                        <Chip label="None" size="small" sx={{ backgroundColor: '#666' }} />
                      )}
                    </TableCell>
                    <TableCell sx={{ color: 'white' }}>{formatCurrency(territory.baseIncome)}</TableCell>
                    <TableCell>
                      <Chip
                    ...
                    <TableCell sx={{ color: 'white' }}>{formatCurrency(territory.totalEarned)}</TableCell>
                    <TableCell>
                      <Chip
                        label={`${getActivityScore(territory.venueActivity).toFixed(0)}%`}
                        color={getActivityScore(territory.venueActivity) >= 50 ? 'success' : 'warning'}
                        size="small"
                      />
                    </TableCell>
                    <TableCell sx={{ color: 'white' }}>{formatTime(territory.nextPayout)}</TableCell>
                    <TableCell>
                      <Tooltip title="View Payouts">
                        <IconButton size="small" onClick={() => handleViewPayouts(territory)}>
      ...
      <Dialog open={showConfigDialog} onClose={() => setShowConfigDialog(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ color: '#00ff9d' }}>Passive Income Configuration</DialogTitle>
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 1 }}>
            <Grid2 xs={6}>
              <TextField
                ...
                onChange={(e) => setConfigForm(prev => ({ ...prev, baseRate: Number(e.target.value) }))}
                sx={{ '& .MuiInputLabel-root': { color: '#ccc' }, '& .MuiInputBase-input': { color: 'white' } }}
              />
            </Grid2>
            <Grid2 xs={6}>
              <TextField
                ...
                onChange={(e) => setConfigForm(prev => ({ ...prev, territoryBonus: Number(e.target.value) / 100 }))}
                sx={{ '& .MuiInputLabel-root': { color: '#ccc' }, '& .MuiInputBase-input': { color: 'white' } }}
              />
            </Grid2>
            <Grid2 xs={6}>
              <TextField
                ...
                onChange={(e) => setConfigForm(prev => ({ ...prev, venueActivityBonus: Number(e.target.value) / 100 }))}
                sx={{ '& .MuiInputLabel-root': { color: '#ccc' }, '& .MuiInputBase-input': { color: 'white' } }}
              />
            </Grid2>
            <Grid2 xs={6}>
              <TextField
                ...
                onChange={(e) => setConfigForm(prev => ({ ...prev, clanBonus: Number(e.target.value) / 100 }))}
                sx={{ '& .MuiInputLabel-root': { color: '#ccc' }, '& .MuiInputBase-input': { color: 'white' } }}
              />
            </Grid2>
            <Grid2 xs={6}>
              <TextField
                ...
                onChange={(e) => setConfigForm(prev => ({ ...prev, maxIncomePerHour: Number(e.target.value) }))}
                sx={{ '& .MuiInputLabel-root': { color: '#ccc' }, '& .MuiInputBase-input': { color: 'white' } }}
              />
            </Grid2>
            <Grid2 xs={6}>
              <TextField
                ...
                onChange={(e) => setConfigForm(prev => ({ ...prev, payoutInterval: Number(e.target.value) }))}
                sx={{ '& .MuiInputLabel-root': { color: '#ccc' }, '& .MuiInputBase-input': { color: 'white' } }}
              />
            </Grid2>
          </Grid>
        ...
        <DialogActions>
          <Button onClick={() => setShowConfigDialog(false)} sx={{ color: '#ccc' }}>
            Cancel
          </Button>
          <Button onClick={handleConfigUpdate} sx={{ backgroundColor: '#00ff9d', color: '#000' }}>
      ...
      <Dialog open={showPayoutDialog} onClose={() => setShowPayoutDialog(false)} maxWidth="md" fullWidth>
          ...
          {selectedTerritory && (
            <TableContainer>
              <Table>
                ...
                <TableBody>
                  {getPayoutHistory(selectedTerritory.territoryId).map((payout) => (
                    <TableRow key={payout.id}>
                      <TableCell sx={{ color: 'white' }}>{formatTime(payout.timestamp)}</TableCell>
                      <TableCell sx={{ color: 'white', fontWeight: 'bold' }}>
                        {formatCurrency(payout.amount)}
                      </TableCell>
                      <TableCell sx={{ color: 'white' }}>{formatCurrency(payout.breakdown.base)}</TableCell>
                      <TableCell sx={{ color: 'white' }}>{formatCurrency(payout.breakdown.territory)}</TableCell>
                      <TableCell sx={{ color: 'white' }}>{formatCurrency(payout.breakdown.activity)}</TableCell>
                      <TableCell sx={{ color: 'white' }}>{formatCurrency(payout.breakdown.clan)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
        ...
        <DialogActions>
          <Button onClick={() => setShowPayoutDialog(false)} sx={{ color: '#ccc' }}>
            Close
          </Button>
        </DialogActions>
      </Dialog>
