...
import {
  ...
  ChartOptions, // For typing options
  ChartData,  // For typing data
  Point,      // For onClick event
  ActiveElement // For onClick event
} from 'chart.js';
...
export interface Shot { // Export if it's defined here and used by parent
  id: string;
  type: string;
  success: boolean;
...
const TimelineChart: React.FC<TimelineChartProps> = ({
      ...
      datasets: [
        {
          ...
          pointRadius: (context) => (context.dataIndex === currentShotIndex ? 6 : 3),
          pointHoverRadius: 8,
  ...
  const chartOptions: ChartOptions<'line'> = useMemo(() => {
    return {
      ...
      scales: {
        ...
        y: { // Primary Y-axis (default for both datasets unless specified)
          beginAtZero: true,
  ...
  }, [legendColor, titleColor, chartTitle, tickColor, gridColor, maxYValue, onSelectShot]);


  if (!shots || shots.length === 0) {
    return (
      <Box height="400px" display="flex" alignItems="center" justifyContent="center" borderWidth="1px" borderRadius="md" borderColor="gray.200">
        <Text color="gray.500">No shot data available to display timeline.</Text>
      </Box>
    );
  }

  return (
    <Box height={{ base: "300px", md: "400px" }} width="100%"> {/* Responsive height */}
