...
const DojoProfilePanel: React.FC<DojoProfilePanelProps> = ({ dojo, onClose }) => {
  if (!dojo) return null;

  // Fallbacks for missing fields
  const leaderboard: LeaderboardPlayer[] = (dojo as any).leaderboard || mockLeaderboard;
  const allegianceMeter: ClanInfluence[] = (dojo as any).allegianceMeter || mockAllegianceMeter;

  const getAllegianceColor = () => {
    switch (dojo.allegiance) {
      case 'player': return '#22d3ee';
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <ul className="space-y-1">
            {leaderboard && leaderboard.length > 0 ? (
              leaderboard.slice(0, 10).map((player: LeaderboardPlayer, idx: number) => (
                <li key={player.id || idx} className="flex justify-between text-gray-200">
                  <span>{idx + 1}. {player.name}</span>
                  <span className="text-cyan-300 font-mono">{player.score}</span>
                </li>
              ))
            ) : (
        ...
        <div>
              ...
              allegianceMeter.map((clan: ClanInfluence, idx: number) => (
                <div key={clan.name || idx} className="flex items-center gap-2">
                  ...
                  <div className="flex-1 bg-gray-700 rounded h-3 overflow-hidden">
                    <div className="bg-cyan-500 h-3" style={{ width: `${clan.percent}%` }}></div>
                  </div>
                  <span className="text-cyan-300 font-mono w-10 text-right">{clan.percent}%</span>
                </div>
              ))
            ) : (
          ...
          <div className="space-y-3">
            <button className="w-full bg-red-600 hover:bg-red-700 p-3 rounded-lg font-bold">Challenge for Title (Gauntlet)</button>
            <button className="w-full bg-blue-600 hover:bg-blue-700 p-3 rounded-lg font-bold">Challenge for Badge (Pilgrimage)</button>
          </div>
        </div>
      </div>
    </div>
