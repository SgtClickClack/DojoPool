...
import { Clan, ClanMember, ClanWar, ClanInvite } from '../../services/clan/ClanSystemService';
...
export const ClanPanel: React.FC<ClanPanelProps> = ({ className = '' }) => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'members' | 'wars' | 'discover'>('overview');
      ...
      {!userClan ? (
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-4">
                <img src={userClan.avatar} alt={userClan.name} className="w-16 h-16 rounded-full" />
                <div>
                  <h2 className="text-2xl font-bold">{userClan.name}</h2>
                  <p className="text-gray-600">[{userClan.tag}] • Level {userClan.level}</p>
                </div>
              </div>
              {canManageClan() && (
                <button
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center p-3 bg-gray-50 rounded">
                <p className="text-2xl font-bold text-blue-600">{userClan.memberCount}</p>
                <p className="text-sm text-gray-600">Members</p>
              </div>
              <div className="text-center p-3 bg-gray-50 rounded">
                <p className="text-2xl font-bold text-green-600">{userClan.territoryCount}</p>
                <p className="text-sm text-gray-600">Territories</p>
              </div>
              <div className="text-center p-3 bg-gray-50 rounded">
                <p className="text-2xl font-bold text-purple-600">{userClan.totalWins}</p>
                <p className="text-sm text-gray-600">Wins</p>
              </div>
              <div className="text-center p-3 bg-gray-50 rounded">
                <p className="text-2xl font-bold text-orange-600">{Math.round(userClan.winRate * 100)}%</p>
          ...
          {clanInvites.length > 0 && (
            <div className="bg-white rounded-lg shadow p-6">
              <h3 className="text-lg font-semibold mb-4">Pending Invites ({clanInvites.length})</h3>
                ...
                {clanInvites.map((invite) => (
                  <div key={invite.id} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                    <div>
                      ...
                      {invite.message && <p className="text-sm text-gray-500 mt-1">{invite.message}</p>}
                    </div>
  ...
  const renderMembersTab = () => (
    <div className="bg-white rounded-lg shadow p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold">Members ({clanMembers.length})</h3>
        {canInviteMembers() && (
          <button
            onClick={() => setShowInvitePlayer(true)}
            className="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600"
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    {member.role} • {member.rank} • {member.territoryCount} territories
                  </p>
                </div>
              </div>
              ...
              <div key={war.id} className="p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`px-2 py-1 rounded text-sm ${
                    war.status === 'active' ? 'bg-green-100 text-green-800' :
                    war.status === 'preparing' ? 'bg-yellow-100 text-yellow-800' :
                ...
                <div className="flex items-center justify-between">
                  <div className="text-center">
                    ...
                    <p className="text-2xl font-bold text-blue-600">{war.clan1Score}</p>
                  </div>
                  <div className="text-center text-gray-500">VS</div>
                  <div className="text-center">
                    <p className="font-medium">{war.clan2Name}</p>
                    <p className="text-2xl font-bold text-red-600">{war.clan2Score}</p>
                  </div>
                </div>
          ...
          <div className="mt-4 space-y-2">
            {searchResults.map((clan) => (
              <div key={clan.id} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                <div className="flex items-center space-x-3">
                  <img src={clan.avatar} alt={clan.name} className="w-10 h-10 rounded-full" />
                  <div>
                    <p className="font-medium">{clan.name} [{clan.tag}]</p>
                    <p className="text-sm text-gray-600">{clan.memberCount} members • Level {clan.level}</p>
                  </div>
                </div>
                <button
                  onClick={() => invitePlayer(clan.leaderId, `Join ${clan.name}!`)}
          ...
          {topClans.map((clan, index) => (
            <div key={clan.id} className="flex items-center justify-between p-3 bg-gray-50 rounded">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="font-medium">{clan.name} [{clan.tag}]</p>
                  <p className="text-sm text-gray-600">{clan.memberCount} members • {Math.round(clan.winRate * 100)}% win rate</p>
                </div>
              </div>
              <div className="text-right">
                <p className="font-bold text-green-600">{clan.totalWins} wins</p>
                <p className="text-sm text-gray-500">{clan.territoryCount} territories</p>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              <input
                ...
                onChange={(e) => setWarForm({...warForm, targetClanId: e.target.value})}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg"
              />
              <input
                type="text"
                ...
                value={warForm.name}
                onChange={(e) => setWarForm({...warForm, name: e.target.value})}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg"
              />
              <textarea
                placeholder="Description"
