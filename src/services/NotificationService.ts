...
const stateService = {
  ...
  getState: (selector: (state: any) => any) => ({ ui: { notifications: [] } }),
};
...
const websocketService = {
  subscribe: (event: string, callback: (data: any) => void) => {
    console.log(`Mock websocket subscribe to: ${event}`);
    return () => {};
  },
...
class NotificationService {
  ...
  private cleanup(): void {
    ...
    if (expired.length > 0) {
      ...
      // Track cleanup
      analyticsService.trackEvent('notifications_cleaned_up', {
    ...
    if (this.state.notifications.length > this.MAX_NOTIFICATIONS) {
      ...
      analyticsService.trackEvent('notifications_limit_enforced', {
        removedCount: excess,
        timestamp: now,
      });
    }
  ...
  public async add(
    ...
    analyticsService.trackEvent('notification_added', {
      notificationId: id,
  ...
  public async markAsRead(id: string): Promise<void> {
    ...
    if (notification && !notification.read) {
      ...
      analyticsService.trackEvent('notification_read', {
        notificationId: id,
        timestamp: new Date().toISOString(),
      });
    }
  ...
  public async remove(id: string): Promise<void> {
    ...
    if (index !== -1) {
      ...
      analyticsService.trackEvent('notification_removed', {
        notificationId: id,
        timestamp: new Date().toISOString(),
      });
    }
  ...
  public async clear(): Promise<void> {
    ...
    if (count > 0) {
      ...
      analyticsService.trackEvent('notifications_cleared', {
        count,
        timestamp: new Date().toISOString(),
      });
    }
