...
const stateService = {
  ...
  getState: (selector: (state: any) => any) => ({ ui: { currentRoute: null } }),
};
...
class RouterService {
  ...
  private initialize(): void {
    ...
    analyticsService.trackEvent('router_initialized', {
  ...
  private async matchRoute(
    ...
    if (matchedRoute && routeMatch) {
      await this.navigateToRoute(matchedRoute, routeMatch, query, state);
    }
  }
  ...
  private async navigateToRoute(
    ...
    try {
      ...
      analyticsService.trackEvent('route_change_error', {
        from: this.state.previousRoute?.path,
        to: route.path,
        error: error.message,
        timestamp: new Date().toISOString(),
  ...
  public registerRoute(route: Route): void {
    ...
    analyticsService.trackEvent('route_registered', {
      path: route.path,
      name: route.name,
      timestamp: new Date().toISOString(),
    });
  ...
  public async navigate(
    ...
    if (matchedRoute && routeMatch) {
      ...
      // Navigate to route
      await this.navigateToRoute(matchedRoute, routeMatch, query, newState);
    }
  }
