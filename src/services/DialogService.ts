...
class DialogService {
  ...
  public open(config: Omit<DialogConfig, 'id'>): string {
    ...
    // Track dialog opened
    analyticsService.trackEvent('dialog_opened', {
      userId: 'system',
      dialogId: id,
  ...
  public close(id: string): void {
    ...
    // Track dialog closed
    analyticsService.trackEvent('dialog_closed', {
      userId: 'system',
      dialogId: id,
      title: dialog.title,
      timestamp: new Date().toISOString(),
  ...
  public closeAll(): void {
    ...
    if (dialogCount > 0) {
      ...
      analyticsService.trackEvent('all_dialogs_closed', {
        userId: 'system',
        count: dialogCount,
        timestamp: new Date().toISOString(),
      });
  ...
  public updateDialog(id: string, updates: Partial<DialogConfig>): void {
    ...
    if (dialog) {
      ...
      analyticsService.trackEvent('dialog_updated', {
        userId: 'system',
        dialogId: id,
  ...
  public setDialogButton(
    ...
    if (dialog && dialog.buttons && dialog.buttons[index]) {
      ...
      analyticsService.trackEvent('dialog_button_updated', {
        userId: 'system',
        dialogId: id,
        buttonIndex: index,
        updates: Object.keys(updates),
