 ...
 * ErrorLoggingService
 *
 * Centralized error logging service for DojoPool. Logs errors to Winston, sends to error tracking endpoint, and logs to Firebase Analytics.
 * Use the exported `logError` function for all error logging in the codebase.
 */
import { ErrorInfo } from "react";
import { logErrorToAnalytics } from "../utils/analyticsUtils";
...
class ErrorLoggingService {
  ...
  public log(
    ...
    // Log to console in development using Winston
    ...
    if (process.env.NODE_ENV === "production") {
      ...
      // Log to file in production using Winston
      logger.error("Error logged in production:", errorLog);
    }
