...
class ValidationService {
  ...
  public registerRule(
    ...
    analyticsService.trackEvent('validation_rule_registered', {
      userId: 'system',
      details: {
        ruleType: type,
  ...
  public async validate(
    ...
    try {
      ...
      analyticsService.trackEvent('validation_started', {
        userId: 'system',
        details: {
          timestamp: new Date().toISOString(),
        },
      ...
      analyticsService.trackEvent('validation_completed', {
        userId: 'system',
        details: {
          isValid,
    ...
    for (const rule of rules) {
      ...
      try {
        ...
        if (!isValid) {
          const message =
            rule.message ||
            this.customMessages.get(rule.type) ||
            'Invalid value';
  ...
  public async validateAsync(
    ...
    try {
      ...
      analyticsService.trackEvent('async_validation_started', {
        userId: 'system',
        details: {
      ...
      analyticsService.trackEvent('async_validation_completed', {
        userId: 'system',
        details: {
          isValid,
          errorCount: Object.keys(errors).length,
      ...
      analyticsService.trackEvent('async_validation_error', {
        userId: 'system',
        details: {
          error: error instanceof Error ? error.message : String(error),
          timestamp: new Date().toISOString(),
