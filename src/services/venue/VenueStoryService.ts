...
export class VenueStoryService {
      ...
      specialEvents: [
        {
          ...
          description: 'A local regular challenges you to a game. Win to earn their respect and unlock the "Local Hero" achievement.',
          triggerCondition: 'Play 3 games at this venue',
          isTriggered: false,
          rewards: { experience: 100, dojoCoins: 50, items: ['local_hero_badge'] }
        }
      ]
    });
      ...
      specialEvents: [
        {
          ...
          description: 'A secret tournament is being held tonight. Only the best players are invited. Can you prove yourself worthy?',
          triggerCondition: 'Win 5 consecutive games',
          isTriggered: false,
          rewards: { experience: 300, dojoCoins: 150, items: ['underground_champion_badge'] }
        }
      ]
    });
      ...
      specialEvents: [
        {
          ...
          description: 'The hall\'s resident master has noticed your skills. They challenge you to a match that could change your destiny.',
          triggerCondition: 'Reach level 20',
          isTriggered: false,
          rewards: { experience: 500, dojoCoins: 250, items: ['masters_apprentice_badge'] }
        }
      ]
    });
      ...
      specialEvents: [
        {
          ...
          description: 'Beat the arcade\'s high score to unlock a special digital avatar and earn the "Pixel Perfect" achievement.',
          triggerCondition: 'Score 1000+ points in a single game',
          isTriggered: false,
          rewards: { experience: 200, dojoCoins: 100, items: ['pixel_perfect_avatar'] }
        }
      ]
    });
      ...
      specialEvents: [
        {
          ...
          description: 'Your training is complete. Face the academy\'s grandmaster in a final test of skill and determination.',
          triggerCondition: 'Complete all academy training modules',
          isTriggered: false,
          rewards: { experience: 1000, dojoCoins: 500, items: ['grandmaster_badge'] }
        }
      ]
    });
  }
  ...
  getVenueStoriesByType(venueType: 'bar' | 'club' | 'hall' | 'arcade' | 'academy'): VenueStory[] {
    return this.getAllVenueStories().filter(story => story.venueType === venueType);
  }

  triggerVenueDiscovery(venueType: 'bar' | 'club' | 'hall' | 'arcade' | 'academy', venueId: string, venueName: string): void {
    progressionService.addVenueDiscoveryEvent(venueType, venueId, venueName);
  }

  triggerVenueMastery(venueType: 'bar' | 'club' | 'hall' | 'arcade' | 'academy', venueId: string, venueName: string, difficulty: 'beginner' | 'intermediate' | 'advanced' | 'expert'): void {
    progressionService.addVenueMasteryEvent(venueType, venueId, venueName, difficulty);
  }

  triggerSpecialEvent(venueType: 'bar' | 'club' | 'hall' | 'arcade' | 'academy', venueId: string, venueName: string, eventTitle: string, eventDescription: string): void {
    progressionService.addVenueSpecialEvent(venueType, venueId, venueName, eventTitle, eventDescription);
  }
  ...
  checkSpecialEventTriggers(venueId: string, playerStats: any): void {
    ...
    story.specialEvents.forEach(event => {
      if (!event.isTriggered) {
        ...
        if (this.evaluateTriggerCondition(event.triggerCondition, playerStats)) {
  ...
  private evaluateTriggerCondition(condition: string, playerStats: any): boolean {
  ...
  advanceStoryChapter(venueId: string): void {
    ...
    if (story && !story.isCompleted) {
      ...
      if (story.currentChapter > story.totalChapters) {
        ...
        this.triggerVenueMastery(story.venueType, story.venueId, story.venueName, 'expert');
      }
    }
  }
}
