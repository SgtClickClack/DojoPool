...
export class DojoProfileCustomizationService extends BrowserEventEmitter {
  ...
  public static getInstance(): DojoProfileCustomizationService {
    if (!DojoProfileCustomizationService.instance) {
      DojoProfileCustomizationService.instance = new DojoProfileCustomizationService();
    }
  ...
  private initializeWebSocket(): void {
    try {
      ...
      this.socket.on('text-to-image:failed', (data: { id: string; error: string }) => {
  ...
  public async generateImageFromText(request: TextToImageRequest): Promise<TextToImageResponse> {
  ...
  public async generateLogoFromText(venueId: string, description: string): Promise<string> {
  ...
  public async generateBackgroundFromText(venueId: string, description: string): Promise<string> {
    const request: TextToImageRequest = {
  ...
  public async generateGalleryImagesFromText(venueId: string, description: string): Promise<string[]> {
    const prompts = [
  ...
  public async generateThemeFromText(description: string): Promise<VenueTheme> {
    ...
    const backgroundUrl = await this.generateBackgroundFromText(themeId, description);
  ...
  public async generateAttributesFromText(description: string): Promise<VenueAttribute[]> {
  ...
  public updateAttribute(venueId: string, attributeId: string, updates: Partial<VenueAttribute>): void {
    const config = this.customizations.get(venueId);
    if (config) {
      const index = config.attributes.findIndex(attr => attr.id === attributeId);
      if (index !== -1) {
        config.attributes[index] = { ...config.attributes[index], ...updates };
        this.emit('attributeUpdated', { venueId, attribute: config.attributes[index] });
      }
    }
  }

  // Gallery Management
  public async generateGalleryFromText(description: string): Promise<VenueGallery[]> {
    const galleries: VenueGallery[] = [];
    const imageUrls = await this.generateGalleryImagesFromText('gallery', description);
    
    const titles = ['Venue Overview', 'Tournament Setup', 'Atmospheric Corner', 'Exterior View'];
    const categories: VenueGallery['category'][] = ['interior', 'tournaments', 'atmosphere', 'exterior'];
  ...
  public updateGalleryItem(venueId: string, itemId: string, updates: Partial<VenueGallery>): void {
    const config = this.customizations.get(venueId);
    if (config) {
      const index = config.gallery.findIndex(item => item.id === itemId);
      if (index !== -1) {
        config.gallery[index] = { ...config.gallery[index], ...updates };
        this.emit('galleryItemUpdated', { venueId, item: config.gallery[index] });
      }
    }
  }
  ...
  public async generateStoryFromText(description: string): Promise<VenueStory> {
    ...
    const featuredImage = await this.generateBackgroundFromText(storyId, description);
  ...
  public updateStory(venueId: string, storyId: string, updates: Partial<VenueStory>): void {
    const config = this.customizations.get(venueId);
    if (config) {
      const index = config.stories.findIndex(story => story.id === storyId);
      if (index !== -1) {
        config.stories[index] = { ...config.stories[index], ...updates, updatedAt: new Date() };
  ...
  public updateAtmosphere(venueId: string, atmosphere: Partial<CustomizationConfig['atmosphere']>): void {
    const config = this.customizations.get(venueId);
    if (config) {
      config.atmosphere = { ...config.atmosphere, ...atmosphere };
      this.emit('atmosphereUpdated', { venueId, atmosphere: config.atmosphere });
    }
  }

  // Branding Management
  public updateBranding(venueId: string, branding: Partial<CustomizationConfig['branding']>): void {
    const config = this.customizations.get(venueId);
    if (config) {
  ...
  private selectFontFromText(text: string): string {
    const fonts = ['Orbitron, monospace', 'Playfair Display, serif', 'Roboto, sans-serif', 'Poppins, sans-serif'];
  ...
  public getThemes(): VenueTheme[] {
    return Array.from(this.customizations.values()).map(config => config.theme);
  }
