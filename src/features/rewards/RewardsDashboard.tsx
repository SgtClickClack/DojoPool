...
import { Box, Typography, Paper, CircularProgress, Alert, List, ListItem, Button, Divider, ListItemText } from "@mui/material";
...
import { Transaction, FungibleToken } from "../../types/wallet";
...
const RewardsDashboard: React.FC<RewardsDashboardProps> = () => {
  ...
  const { rewards, loading: rewardsLoading, error: rewardsError, fetchRewards } = useRewardsService();
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setError("Failed to load wallet or rewards data: " + (err?.message || err));
    }).finally(() => {
      setLoading(false);
    });
  ...
  return (
    <Paper sx={{ p: 3, m: 2 }}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
        ...
        <Button variant="outlined" color="secondary" onClick={() => setShopOpen(true)}>
          Go to Rewards Shop
        </Button>
      </Box>
      <Dialog open={shopOpen} onClose={() => setShopOpen(false)} maxWidth="md" fullWidth>
        <RewardsShop />
        <Box display="flex" justifyContent="flex-end" p={2}>
          <Button onClick={() => setShopOpen(false)} color="primary">Close</Button>
        </Box>
      </Dialog>
      <Typography variant="h6">Wallet Balance</Typography>
      <Typography variant="body1" sx={{ mb: 2 }}>{walletData && walletData.balance !== null ? `${walletData.balance} Dojo Coins` : "-"}</Typography>
      <Divider sx={{ my: 2 }} />
        ...
        {rewards.map((reward: RewardItem) => (
          <ListItem key={reward.id} sx={{ display: "flex", flexDirection: "column", alignItems: "flex-start" }}>
            ...
            {reward.type === 'dojo_coins' && (
              <Typography variant="body2">Amount: {reward.points_cost} Dojo Coins</Typography>
            )}
            {reward.type === 'nft' && (
              <Box>
                {reward.imageUrl && <img src={reward.imageUrl} alt={reward.name} style={{ width: 50, height: 50, marginRight: 8 }} />}
        ...
        {otherTokens.length === 0 ? (
          ...
          otherTokens.map((token) => (
            <ListItem key={token.id}>
              <ListItemText primary={`${token.name} (${token.symbol}): ${token.balance}`} />
            </ListItem>
          ))
        )}
      </List>
      <EarnedNftsDisplay nfts={earnedNfts} onNftClick={handleNftClick} onActionClick={handleNftAction} />
      <Snackbar
        open={snackbarOpen}
