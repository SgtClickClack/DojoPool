...
export class PlayerRecommender {
  ...
  private async initializeModel() {
    try {
      this.model = await tf.loadLayersModel('/models/player-recommender/model.json');
    } catch (error) {
  ...
  private calculateSkillCompatibility(player1: PlayerStats, player2: PlayerStats): number {
  ...
  private calculateLocationCompatibility(player1: PlayerStats, player2: PlayerStats): number {
    const R = 6371; // Earth's radius in km
    const lat1 = player1.location.lat * Math.PI / 180;
    const lat2 = player2.location.lat * Math.PI / 180;
    const dLat = (player2.location.lat - player1.location.lat) * Math.PI / 180;
    const dLon = (player2.location.lng - player1.location.lng) * Math.PI / 180;
  ...
  private calculateScheduleCompatibility(player1: PlayerStats, player2: PlayerStats): number {
    const commonHours = player1.activeHours.filter(hour => 
      player2.activeHours.includes(hour)
    );
    return commonHours.length / Math.max(player1.activeHours.length, player2.activeHours.length);
  }

  private calculatePlayStyleCompatibility(player1: PlayerStats, player2: PlayerStats): number {
    const commonStyles = player1.playStyle.filter(style => 
      player2.playStyle.includes(style)
    );
    return commonStyles.length / Math.max(player1.playStyle.length, player2.playStyle.length);
  }

  private async predictMatchQuality(player1: PlayerStats, player2: PlayerStats): Promise<number> {
  ...
  public async getRecommendations(
    ...
    for (const [candidateId, candidateStats] of this.playerStats.entries()) {
      ...
      const skillDifference = Math.abs(player.skillLevel - candidateStats.skillLevel);
      if (skillDifference > criteria.skillRange) continue;

      const locationCompatibility = this.calculateLocationCompatibility(player, candidateStats);
      ...
      const recommendedGameTypes = this.getRecommendedGameTypes(player, candidateStats);
  ...
  private getRecommendedGameTypes(player1: PlayerStats, player2: PlayerStats): string[] {
    ...
    const commonMatches = this.matchHistory.filter(match => 
  ...
  public async updateRecommendationModel(matches: MatchHistory[]): Promise<void> {
