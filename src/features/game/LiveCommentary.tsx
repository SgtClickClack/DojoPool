...
import { Box, Typography, Paper, CircularProgress, Alert, List, ListItem } from "@mui/material";
...
const LiveCommentary: React.FC<LiveCommentaryProps> = ({ gameId }) => {
  ...
  return (
    <Paper sx={{ p: 2, mb: 2 }}>
      <Typography variant="h6" gutterBottom>Live Commentary</Typography>
      <Typography variant="body1" sx={{ mb: 1 }}>{latest.text}</Typography>
      {latest.audioUrl && (
        <audio ref={audioRef} controls src={latest.audioUrl} style={{ width: "100%" }} />
      )}
      <List dense>
