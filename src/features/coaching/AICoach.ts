...
export class AICoach {
  ...
  public async analyzeGame(playerId: string, shots: Shot[]): Promise<GameAnalysis> {
  ...
  private calculateAccuracy(shots: Shot[]): number {
    const successfulShots = shots.filter(shot => shot.result === 'success').length;
  ...
  private identifyCommonMistakes(shots: Shot[]): string[] {
    ...
    const englishIssues = missedShots.filter(shot => shot.english !== 'center').length;
    if (englishIssues / missedShots.length > 0.4) {
      mistakes.push('Unnecessary english application');
    }

    // Analyze angle selection
    const angleIssues = missedShots.filter(shot => Math.abs(shot.angle) > Math.PI / 3).length;
  ...
  private identifyStrengths(shots: Shot[]): string[] {
    ...
    const spinShots = successfulShots.filter(shot => shot.english !== 'center');
    if (spinShots.length / successfulShots.length > 0.4) {
      strengths.push('Spin control');
    }

    // Analyze angle shots
    const angleShots = successfulShots.filter(shot => Math.abs(shot.angle) > Math.PI / 4);
  ...
  private identifyWeaknesses(shots: Shot[]): string[] {
    ...
    if (patterns.closeRange > threshold) weaknesses.push('Close range accuracy');
  ...
  public async createCoachingSession(
    ...
    const recommendations = this.recommendDrills(focusAreas, gameAnalysis.weaknessAreas);
  ...
  private recommendDrills(focusAreas: string[], weaknesses: string[]): TrainingDrill[] {
    ...
    return recommendations
      .sort((a, b) => this.calculateDrillRelevance(b, focusAreas, weaknesses) -
  ...
  private calculateProgress(history: GameAnalysis[]): { [key: string]: number } {
  ...
  private getAreaScore(game: GameAnalysis, area: string): number {
    switch (area) {
      ...
      case 'power_control':
        return 1 - game.shots.filter(s => s.power > 0.8).length / game.shots.length;
      case 'spin_control':
        return game.shots.filter(s => 
          s.english !== 'center' && s.result === 'success'
        ).length / game.shots.length;
      case 'positioning':
        return 1 - game.commonMistakes.filter(m => 
          m.includes('position') || m.includes('angle')
        ).length / game.commonMistakes.length;
      default:
        return 0;
    }
  }
