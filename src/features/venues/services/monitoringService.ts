...
export class VenueMonitoringService {
  ...
  public recordAnalytics(venueId: string, data: VenueAnalyticsData): void {
    ...
    this.metrics.occupancyGauge.set({ venue_id: venueId }, data.averageOccupancy);

    // Record maintenance metrics
    Object.entries(data.maintenanceStats.maintenanceByReason).forEach(([reason, count]) => {
      this.metrics.maintenanceCounter.inc({ venue_id: venueId, reason }, count);
    });

    // Record table utilization metrics
    data.tableUtilization.forEach(table => {
      this.metrics.tableUtilizationHistogram.observe({ venue_id: venueId }, table.utilization);
    });
  }
  ...
  public async checkAlertConditions(venueId: string, data: VenueAnalyticsData): Promise<Alert[]> {
    const alerts: Alert[] = [];
    ...
    const lowUtilizationTables = data.tableUtilization.filter(table => table.utilization < 20);
