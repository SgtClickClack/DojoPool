...
export class AnalyticsExportService {
  static async exportToCSV(data: VenueAnalyticsData): Promise<string> {
    ...
    csvRows.push(`Total Maintenance Events,${data.maintenanceStats.totalMaintenance}`);
    csvRows.push('');
    ...
    Object.entries(data.maintenanceStats.maintenanceByReason).forEach(([reason, count]) => {
      csvRows.push(`${reason},${count}`);
    });
  ...
  static async exportToPDF(data: VenueAnalyticsData): Promise<Blob> {
    ...
    // Title
    doc.setFontSize(20);
    doc.text('Venue Analytics Report', pageWidth / 2, yPos, { align: 'center' });
    yPos += 20;
    ...
    const tableData = data.tableUtilization.map(table => [
      ...
      table.utilization > 80 ? 'High' : table.utilization > 50 ? 'Medium' : 'Low',
    ]);
  ...
  static async exportData(data: VenueAnalyticsData, format: 'csv' | 'json' | 'pdf'): Promise<Blob | string> {
    switch (format) {
      case 'csv':
  ...
  static downloadFile(content: Blob | string, format: 'csv' | 'json' | 'pdf', filename: string): void {
