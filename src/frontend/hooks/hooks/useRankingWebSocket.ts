...
interface RankingUpdate {
  ...
  data: any;
  timestamp: string;
  messages?: RankingUpdate[]; // Add optional messages property for batch updates
}
...
export const useRankingWebSocket = ({
  ...
  const featuresRef = useRef<ConnectionFeatures | null>(null);
  ...
  const connect = useCallback(() => {
    ...
    const wsUrl = userId
      ? `/api/ws/rankings/${userId}${
          reconnectTokenRef.current ? `?token=${reconnectTokenRef.current}` : ''
        }`
