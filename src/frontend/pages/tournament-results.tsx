...
const TournamentResults: React.FC<TournamentResultsProps> = ({ tournamentId }) => {
  ...
  const { tournament, loading: tournamentLoading, error: tournamentError } = useTournament(tournamentId) as { tournament: Tournament | null, loading: boolean, error: Error | null };
  const { rewards, claimReward, loading: rewardsLoading, error: rewardsError } = useRewards();

  // State for individual reward claim loading
  const [claimingRewardId, setClaimingRewardId] = useState<string | number | null>(null);

  const handleClaimReward = useCallback(async (rewardId: string | number) => {
  ...
  const getMatchResultDisplay = (match: Match) => {
    if (match.status === 'completed') {
      ...
      return <Typography variant="body2" color="info.main">In Progress</Typography>;
    } else if (match.status === 'pending') {
        return <Typography variant="body2" color="text.disabled">Pending</Typography>;
    }
    return <Typography variant="body2" color="text.disabled">{match.status.charAt(0).toUpperCase() + match.status.slice(1)}</Typography>;
  };

  // --- Loading and Error States ---
  if (tournamentLoading || rewardsLoading) { // Check loading state from both hooks
    return (
  ...
  const finalMatch = tournament.matches && tournament.matches.length > 0
    ? tournament.matches.reduce((prev, current) => ((prev.round || 0) > (current.round || 0) ? prev : current), tournament.matches[0])
    : null;
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      <Grid container spacing={3}>
        <Grid item xs={12}>
          <Typography variant="h3" component="h1" gutterBottom sx={{textAlign: {xs: 'center', md: 'left'}}}>
        ...
        <Grid item xs={12} md={7}> {/* Adjusted grid size */}
          <Card sx={{ p: { xs: 2, sm: 3 }, height: '100%' }}>
            <CardContent>
              ...
              <Typography variant="body1" gutterBottom>
                <strong>Status:</strong> <span style={{ color: tournament.status === 'completed' ? 'green' : 'orange' }}>
                    {tournament.status?.charAt(0).toUpperCase() + tournament.status?.slice(1) || 'N/A'}
                </span>
              </Typography>
              {finalMatch && (
                <Typography variant="body1" gutterBottom>
                  <strong>Final Match:</strong> {finalMatch.player1} vs {finalMatch.player2}
            ...
            <CardContent>
              ...
              {rewards && rewards.length > 0 ? (
                <List sx={{maxHeight: 300, overflowY: 'auto'}}>
                  {rewards.map((reward: Reward) => ( // Use Reward interface
                    <React.Fragment key={reward.id}>
                      <ListItem
                        secondaryAction={
                          !reward.claimed && ( // Only show claim button if not claimed
                            ...
                            >
                              {claimingRewardId === reward.id ? <CircularProgress size={20} color="inherit" /> : 'Claim'}
                            </Button>
                          )
                        }
                      ...
                      >
                        <ListItemAvatar>
                          ...
                          <Avatar sx={{bgcolor: reward.claimed ? 'action.disabledBackground' : 'primary.main' }}>{reward.type?.[0]?.toUpperCase() || 'R'}</Avatar>
                        </ListItemAvatar>
                        <ListItemText
                          primary={reward.name}
                          secondary={reward.claimed ? 'Already Claimed' : reward.description}
                        />
                      </ListItem>
                ...
                <Typography variant="body1" color="text.secondary">
                  No rewards available to claim for this tournament or rewards information is loading.
                </Typography>
              )}
            </CardContent>
          </Card>
          ...
          <Grid item xs={12}>
            <Card sx={{ p: { xs: 2, sm: 3 } }}>
              <CardContent>
                ...
                <Grid container spacing={2}> {/* Reduced spacing for denser match list */}
                  {tournament.matches.map((match: Match) => ( // Use Match interface
                    <Grid item xs={12} sm={6} md={4} key={match.id}>
                      ...
                      >
                        <CardContent sx={{ flexGrow: 1 }}>
                          <Typography variant="h6" component="h3" fontSize="1.1rem" gutterBottom>
                            {match.player1 || 'Player 1'} vs {match.player2 || 'Player 2'}
                          </Typography>
                          {match.status !== 'pending' && (match.score1 !== undefined && match.score2 !== undefined) && (
                            <Typography variant="body2" color="text.secondary" gutterBottom>
                              Score: {match.score1} - {match.score2}
                            </Typography>
                          )}
                          {getMatchResultDisplay(match)}
                           {match.round && <Typography variant="caption" display="block" color="text.disabled" sx={{mt:1}}>Round: {match.round}</Typography>}
                        </CardContent>
                      </Card>
                    </Grid>
                  ))}
