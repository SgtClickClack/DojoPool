...
const AvatarPage: React.FC<AvatarPageProps> = () => {
  ...
  const fetchAvatar = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: `Failed to fetch avatar: ${response.status}` }));
        throw new Error(errorData.message || `Failed to fetch avatar: ${response.status}`);
      }
      const data = await response.json();
      setCurrentAvatarUrl(data.avatarUrl); // Adjust based on your API response structure
    } catch (e: any) {
      const errorMessage = e.message || 'An unexpected error occurred while loading avatar.';
  ...
  const handleSaveAvatar = useCallback(async (imageData: string) => { // imageData is likely base64
    setSaving(true);
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: `Failed to save avatar: ${response.status}` }));
        throw new Error(errorData.message || `Failed to save avatar: ${response.status}`);
      }
      ...
      const errorMessage = e.message || 'An unexpected error occurred while saving avatar.';
  ...
  }, [enqueueSnackbar]); // API_BASE_URL if it were dynamic
  ...
  const renderContent = () => {
    ...
    return (
      <>
        ...
        {currentAvatarUrl && !saving && ( // Hide if saving to avoid flicker if URL changes mid-save
        ...
        {error && !initialLoading && ( // Don't show this error if initialLoading error is already shown
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 max-w-lg"> {/* Max width for better focus */}
        ...
        {initialLoading ? (
          <div className="text-center py-8">
           ...
           <div className="my-4 p-3 bg-red-100 border border-red-400 text-red-700 rounded text-center">
                ...
                <p className="mt-2">Could not load your current avatar. You can still try uploading a new one below.</p>
            </div>
