...
export async function generateTrainingStory(
  ...
  const response = await apiClient.post('/ai/generate-story', {
...
export async function generateTrainingPlan(
  ...
  const response = await apiClient.post('/ai/training-plan', {
    userId,
...
export async function generateFeedback(sessionId: string): Promise<string> {
  const response = await apiClient.post(`/ai/feedback/${sessionId}`);
...
export async function analyzeTrainingProgress(userId: string): Promise<{
  ...
  const response = await apiClient.get(`/ai/progress-analysis/${userId}`);
  return response.data;
}
...
export async function generateMotivationalMessage(context: {
  ...
  const response = await apiClient.post('/ai/motivate', context);
...
export async function getAdaptiveDifficulty(userId: string): Promise<{
  ...
  const response = await apiClient.get(`/ai/adaptive-difficulty/${userId}`);
  return response.data;
}
...
export async function generateTechniqueDescription(
  ...
  const response = await apiClient.get(
    `/ai/technique-description/${techniqueId}`,
    {
      params: { skillLevel },
    }
  );
