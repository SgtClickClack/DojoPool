...
const TournamentPayment: React.FC<TournamentPaymentProps> = ({ tournamentId }) => {
  const navigate = useNavigate();
  const { tournament, loading: tournamentLoading, error: tournamentError } = useTournament(tournamentId);
  const { wallet, loading: walletLoading, error: walletError, processPayment } = useWallet();
      ...
      <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
        <Box sx={{ display: 'flex', justifyContent: 'center', py: 4 }}>
          <Alert severity="error">
            {tournamentError?.message || walletError?.message || 'Failed to load data'}
          </Alert>
        </Box>
      </Container>
    );
            ...
            <CardContent>
              ...
              {wallet.balance < tournament.entryFee && (
                <Alert severity="warning" sx={{ mt: 2 }}>
                  Insufficient balance. Please add more Dojo Coins to your wallet.
                </Alert>
              )}
            </CardContent>
          </Card>
