...
import { Box, CircularProgress, Grid, Paper, Typography, Table, TableBody, TableCell, TableContainer, TableRow, Alert, Chip } from '@mui/material';
import { SignalCellularAlt, People, Speed, ErrorIcon, Refresh, Favorite, CompressOutlined, BatchPrediction } from '@mui/icons-material';
...
const WebSocketMonitor: React.FC = () => {
    ...
    const rateLimitRate = stats.messages_sent > 0
        ? ((stats.rate_limited_attempts / stats.messages_sent) * 100).toFixed(2)
        : '0.00';

    const reconnectionRate = stats.reconnection_attempts > 0
        ? ((stats.successful_reconnections / stats.reconnection_attempts) * 100).toFixed(2)
        : '100.00';

    const heartbeatFailureRate = stats.messages_sent > 0
        ? ((stats.failed_heartbeats / stats.messages_sent) * 100).toFixed(2)
        : '0.00';

    const compressionRatio = stats.compression_stats.total_bytes_raw > 0
        ? ((1 - stats.compression_stats.total_bytes_compressed / stats.compression_stats.total_bytes_raw) * 100).toFixed(2)
        : '0.00';

    const avgBatchSize = stats.batch_stats.batches_sent > 0
        ? (stats.batch_stats.messages_batched / stats.batch_stats.batches_sent).toFixed(1)
        : '0.0';
            ...
            <Grid container spacing={3} mb={3}>
                <Grid item xs={12} sm={6} md={3}>
                    <StatCard
                        ...
                        color={stats.current_connections > stats.rate_limits.max_total_connections * 0.8 ? '#f44336' : '#2196f3'}
                    />
                </Grid>
                <Grid item xs={12} sm={6} md={3}>
                    <StatCard
                            ...
                            <Table size="small">
                                <TableBody>
                                    <TableRow>
                                        ...
                                        <TableCell align="right">{stats.reconnection_attempts}</TableCell>
                                    </TableRow>
                                    <TableRow>
                            ...
                            <Table size="small">
                                <TableBody>
                                    ...
                                    <TableRow>
                                        ...
                                        <TableCell align="right">
                                            {formatBytes(stats.compression_stats.total_bytes_compressed)}
                                        </TableCell>
                                    </TableRow>
                                    <TableRow>
                                    ...
                                    <TableRow>
                                        ...
                                        <TableCell align="right">{stats.batch_stats.batches_sent}</TableCell>
                                    </TableRow>
                                    <TableRow>
                                        <TableCell>Messages in Batches</TableCell>
                                        <TableCell align="right">{stats.batch_stats.messages_batched}</TableCell>
                                    </TableRow>
                                    <TableRow>
