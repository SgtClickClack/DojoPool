...
const TournamentRewards: React.FC<TournamentRewardsProps> = ({ tournamentId }) => {
  const navigate = useNavigate();
  const { tournament, loading, error } = useTournament(tournamentId);
  const { rewards, claimReward, loading: rewardsLoading, error: rewardsError } = useTournamentRewards(tournamentId);
  ...
  return (
    <Box sx={{ p: 3 }}>
                ...
                >
                  <CardContent>
                    ...
                    >
                      ...
                      <Typography
                        variant="h6"
                        color={reward.rarity === 'legendary' ? 'error' : reward.rarity === 'epic' ? 'warning' : 'primary'}
                      >
                        {reward.rarity}
                      </Typography>
                    </Box>
                    ...
                    <Box sx={{ mt: 2 }}>
                      ...
                      <Typography variant="body2" color="text.secondary">
                        Available Until: {new Date(reward.expiryDate).toLocaleDateString()}
                      </Typography>
                    </Box>
                  </CardContent>
                  <Button
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)}>
        ...
        <DialogContent>
          <Typography>
            You have successfully claimed your reward! It will be added to your profile shortly.
          </Typography>
        </DialogContent>
        <DialogActions>
