...
const ActiveGamesList: React.FC = () => {
  ...
  const fetchGames = async () => {
    ...
    setError(null);
    try {
      const response = await axiosInstance.get<GameListItem[] | { results: GameListItem[] }>('/games');
  ...
  const getOpponentUsername = (game: GameListItem): string => {
    ...
    const playerArray = game.players || (game.player1 && game.player2 ? [game.player1, game.player2] : []);
    const opponent = playerArray.find(p => (p.id ?? p.user_id) !== profile.id);
  ...
  return (
    <Box sx={{ mt: 4 }}>
      ...
      {!isLoading && !error && (
        <List dense>
          {games.length === 0 ? (
            <ListItem><ListItemText primary="No active games found." /></ListItem>
          ) : (
            games.map((game) => (
              <ListItem
                key={game.id}
       ...
       <Button onClick={fetchGames} disabled={isLoading} sx={{ mt: 1 }}>Refresh List</Button>
    </Box>
  );
};
