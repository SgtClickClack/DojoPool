...
const WalletView: React.FC = () => {
    ...
    const fetchWalletData = async () => {
      ...
      try {
        ...
        if (!walletResponse.ok) {
          ...
          throw new Error(errorData?.error || `Failed to fetch wallet: ${walletResponse.statusText}`);
        }
        ...
        if (fetchedWalletId) {
          const txResponse = await fetch(`/api/v1/wallet/${fetchedWalletId}/transactions`); // Updated endpoint with ID
           if (!txResponse.ok) {
             const errorData = await txResponse.json().catch(() => ({})); // Try to parse error
             throw new Error(errorData?.error || `Failed to fetch transactions: ${txResponse.statusText}`);
           }
          const txData: Transaction[] = await txResponse.json();
          setTransactions(txData);
        } else {
          throw new Error('Could not determine wallet ID to fetch transactions.');
        }
  ...
  if (!walletInfo) {
    return <div className="text-center p-4">Could not load wallet information.</div>;
  }
    ...
    <div className="border p-4 rounded shadow">
      <div className="mb-6">
        ...
        <p className="text-3xl">
          {walletInfo.balance.toFixed(2)} <span className="text-lg">{walletInfo.currency}</span>
        </p>
      </div>
          ...
          <ul className="divide-y divide-gray-200">
            {transactions.map((tx) => (
              <li key={tx.id} className="py-3 flex justify-between items-center">
                <div>
                  <p className="font-medium capitalize">{tx.transaction_type}: {tx.description || 'N/A'}</p>
