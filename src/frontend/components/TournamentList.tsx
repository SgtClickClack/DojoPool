  ...
  useEffect(() => {
    const fetchTournaments = async () => {
      ...
      try {
        ...
        setTournaments([
          { id: 't1', name: 'Weekly 8-Ball Challenge', format: 'Single Elimination', status: 'Registration Open', startDate: '2024-08-05', venueName: 'Main St. Dojo' },
          { id: 't2', name: 'Summer Swiss Showdown', format: 'Swiss', status: 'Upcoming', startDate: '2024-08-12', venueName: 'Downtown Dojo' },
          { id: 't3', name: 'Invitational Pro League', format: 'Round Robin', status: 'In Progress', startDate: '2024-07-20', venueName: 'Main St. Dojo' },
        ]);
      } finally {
        setIsLoading(false);
      }
            ...
            <li key={t.id} className="py-3 hover:bg-gray-50">
              ...
              >
                <div className="flex justify-between items-center">
                  <span className="font-medium">{t.name} ({t.format})</span>
                  <span className={`text-sm px-2 py-0.5 rounded ${t.status === 'Registration Open' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                    {t.status}
                  </span>
                </div>
                <div className="text-sm text-gray-600">
                  Starts: {new Date(t.startDate).toLocaleDateString()} at {t.venueName}
                </div>
              </button>
            </li>
          ))}
