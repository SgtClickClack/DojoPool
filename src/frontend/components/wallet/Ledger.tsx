...
const Ledger: React.FC = () => {
  ...
  const [externalWalletBalance, setExternalWalletBalance] = useState<string>('0');
  ...
  useEffect(() => {
    if (isConnected && connection?.address) {
      getDojoCoinBalance()
        ...
        .catch(err => console.error('Failed to get external wallet balance:', err));
    }
  }, [isConnected, connection, getDojoCoinBalance]);

  const handleSendTransaction = () => {
    // Implement send transaction logic
    console.log('Sending transaction:', { amount: sendAmount, address: sendAddress });
  ...
  const getTransactionIcon = (type: string) => {
    switch (type) {
      case 'credit': return <Receipt color="success" />;
      case 'debit': return <Send color="error" />;
      default: return <Send color="error" />;
    }
  };
              ...
              {!isConnected ? (
                <Box>
                  <Typography variant="body1" sx={{ mb: 2 }}>
                    Connect your existing crypto wallet to view and manage Dojo Coin alongside your other tokens.
                  </Typography>
                  <Button
                    variant="contained"
                ...
                <Box>
                  ...
                  <Typography variant="h5" sx={{ mb: 2 }}>
                    External Wallet Balance: {parseFloat(externalWalletBalance).toFixed(2)} DOJO
                  </Typography>
            ...
            >
              ...
              {isConnected && (
                <Typography variant="h6" sx={{ mt: 2, opacity: 0.9 }}>
                  Combined: {(parseFloat(walletData?.balance || '0') + parseFloat(externalWalletBalance)).toFixed(2)} DOJO
                </Typography>
              )}
                    ...
                    <React.Fragment key={transaction.id}>
                      ...
                      >
                        ...
                        <Typography
                          variant="h6"
                          sx={{
                            color: transaction.type === 'credit' ? '#00ff9d' : '#ff6b6b',
                            fontFamily: 'Orbitron, monospace',
                            fontWeight: 700,
                          }}
                        >
