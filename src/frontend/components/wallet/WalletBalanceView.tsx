...
const WalletBalanceView: React.FC<WalletBalanceViewProps> = () => {
  ...
  return (
    <Paper elevation={3} sx={{ padding: 2 }}>
      ...
      {error && <Typography color="error">Error fetching balance: {typeof error === 'string' ? error : 'An error occurred'}</Typography>}
      {!loading && !error && balanceToDisplay !== undefined && balanceToDisplay !== null && (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Typography variant="h4" component="span" sx={{ fontWeight: 'bold' }}>
            {(balanceToDisplay).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </Typography>
          <Typography variant="subtitle1" component="span" sx={{ marginLeft: 1 }}>
      ...
      {!loading && !error && (balanceToDisplay === undefined || balanceToDisplay === null) && (
        <Typography>{user?.id ? 'No balance data available.' : 'Please log in to view your balance.'}</Typography>
      )}
    </Paper>
  );
};
