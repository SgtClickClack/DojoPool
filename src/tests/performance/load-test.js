...
export default function() {
  ...
  // Group 2: Game Start
  {
    ...
    const gameRes = http.post(`${baseUrl}/api/games/start`, JSON.stringify({
      venueId: 'test-venue',
      tableId: 'test-table',
    }), { headers });
  ...
  {
    const startTime = new Date();
    const analysisRes = http.post(`${baseUrl}/api/games/shot-analysis`, JSON.stringify({
      gameId: 'test-game',
      shotData: {
  ...
  {
    const startTime = new Date();
    const tournamentRes = http.post(`${baseUrl}/api/tournaments/join`, JSON.stringify({
