import { BenchmarkResult, BenchmarkConfig, NetworkBenchmark } from './NetworkTransport.bench';
...
export class NetworkRegressionTest {
  ...
  private async saveHistory(): Promise<void> {
    ...
    await fs.writeFile(this.config.historyFile, JSON.stringify(this.history, null, 2));
  }
  ...
  private checkRegression(current: BenchmarkResult, baseline: BenchmarkResult): string[] {
    const regressions: string[] = [];
    const thresholds = this.config.regressionThresholds;

    // Calculate percentage changes
    const changes = {
      opsPerSecond: ((current.opsPerSecond - baseline.opsPerSecond) / baseline.opsPerSecond) * 100,
      averageLatency: ((current.averageLatency - baseline.averageLatency) / baseline.averageLatency) * 100,
      p95Latency: ((current.p95Latency - baseline.p95Latency) / baseline.p95Latency) * 100,
      p99Latency: ((current.p99Latency - baseline.p99Latency) / baseline.p99Latency) * 100,
    ...
    if (changes.opsPerSecond < thresholds.opsPerSecond) {
      regressions.push(`Operations per second decreased by ${Math.abs(changes.opsPerSecond).toFixed(2)}%`);
    }
    if (changes.averageLatency > thresholds.averageLatency) {
      regressions.push(`Average latency increased by ${changes.averageLatency.toFixed(2)}%`);
    }
    if (changes.p95Latency > thresholds.p95Latency) {
      regressions.push(`P95 latency increased by ${changes.p95Latency.toFixed(2)}%`);
    }
    if (changes.p99Latency > thresholds.p99Latency) {
      regressions.push(`P99 latency increased by ${changes.p99Latency.toFixed(2)}%`);
    }
