...
export class NetworkBenchmark {
  ...
  async setup(): Promise<void> {
    ...
    const peerConfigs = Array.from(
      { length: this.config.concurrentConnections },
      (_, i) => ({
        nodeId: `node${i}`,
        host: "127.0.0.1",
        port: 3000 + i,
  ...
  private handleMessage(
    message: NetworkMessage<BenchmarkMessagePayload>,
  ): void {
    if (
    ...
    while (performance.now() < endTime) {
          ...
          if (targetIndex !== i) {
            try {
              const payload: BenchmarkMessagePayload = {
                data: Buffer.alloc(this.config.messageSize)
                  .fill("x")
                  .toString(),
              ...
              console.error(
                `Error sending from node${i} to node${targetIndex}:`,
                error,
              );
              this.handleError();
            }
      ...
      if (elapsed < messageInterval) {
        await new Promise((resolve) =>
          setTimeout(resolve, messageInterval - elapsed),
        );
      }
    }
