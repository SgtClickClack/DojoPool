...
describe('AdvancedMatchAnalysisService', () => {
  ...
  it('should provide real-time coaching', async () => {
    ...
    const coaching = await service.provideRealTimeCoaching(matchId, playerId, currentSituation);
  ...
  it('should generate personalized training program', async () => {
    ...
    const program = await service.generatePersonalizedTrainingProgram(playerId, performance);
  ...
  it('should predict performance', async () => {
    const playerId = 'player-1';
    const upcomingMatch = { opponentId: 'opponent-1', tournamentId: 'tournament-1' };
    
    const prediction = await service.predictPerformance(playerId, upcomingMatch);
