...
describe('RealTimeAICommentaryService', () => {
  ...
  it('should handle missing player info', async () => {
    await expect(
      service.handleMatchEvent({ matchId: 'match-10', eventType: 'shot' })
    ).resolves.not.toThrow();
  });

  it('should calculate metadata correctly', () => {
    const poolGod = service.getPoolGods()[0];
    const metadata = (service as any).calculateMetadata(
      { difficulty: 8, success: true, impact: 'high', rarity: 'rare' },
      poolGod
    );
