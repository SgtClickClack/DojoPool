import { DojoCoinEconomyService, DojoCoinTransaction, UserEconomyProfile } from '../../services/economy/DojoCoinEconomyService';
...
describe('DojoCoinEconomyService', () => {
  ...
  describe('earning mechanics', () => {
    it('should earn coins from match win', async () => {
      const transaction = await service.earnCoins('user-1', 100, 'match_win', 'Match victory');
    ...
    it('should apply category multipliers', async () => {
      const transaction = await service.earnCoins('user-1', 100, 'tournament_win', 'Tournament victory');
  ...
  describe('spending mechanics', () => {
    it('should spend coins', async () => {
      const transaction = await service.spendCoins('user-1', 50, 'avatar_upgrade', 'Avatar upgrade');
  ...
  describe('transfer system', () => {
    ...
    it('should transfer coins between users', async () => {
      const result = await service.transferCoins('user-1', 'user-2', 50, 'Gift');
  ...
  describe('daily and weekly limits', () => {
    ...
    it('should enforce weekly limits', async () => {
      ...
      for (let i = 0; i < 50; i++) {
        await service.earnCoins('user-1', 100, 'tournament_win', `Tournament ${i}`);
      }
      
      // Next transaction should fail
      await expect(
        service.earnCoins('user-1', 100, 'tournament_win', 'Exceed weekly limit')
      ).rejects.toThrow('Transaction limit exceeded');
    });
  });
  ...
  describe('leaderboards', () => {
    ...
    it('should get balance leaderboard', () => {
      ...
      expect(leaderboard[0].balance).toBeGreaterThanOrEqual(leaderboard[1].balance);
      expect(leaderboard[1].balance).toBeGreaterThanOrEqual(leaderboard[2].balance);
    });

    it('should get earnings leaderboard', () => {
      const leaderboard = service.getEarningsLeaderboard(3);
      expect(leaderboard).toHaveLength(3);
      expect(leaderboard[0].totalEarned).toBeGreaterThanOrEqual(leaderboard[1].totalEarned);
    });
  });
