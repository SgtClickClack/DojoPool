...
jest.mock('../../components/wallet/WalletStats', () => ({
  WalletStatsDisplay: ({ stats }: { stats: WalletStats | null }) => (
...
const mockTransactions: Transaction[] = [
  { id: '1', amount: 100, currency: 'DP', type: 'REWARD', description: 'Daily Login', date: new Date().toISOString() },
  { id: '2', amount: -50, currency: 'DP', type: 'TRANSFER', description: 'Sent to user 2', date: new Date().toISOString() },
];
...
describe('WalletDashboard', () => {
  ...
  it('shows empty state when there are no transactions or rewards', async () => {
    mockedWalletServiceInstance.getWallet.mockResolvedValue({ ...mockWallet, balance: 0 });
    mockedWalletServiceInstance.getWalletStats.mockResolvedValue({
    ...
    mockedWalletServiceInstance.getTransactionHistory.mockResolvedValue([]);

    renderComponent();
    await waitFor(() => expect(screen.getByText('0 DP')).toBeInTheDocument());
    expect(screen.getByText('0')).toBeInTheDocument(); // transactions
    expect(screen.getByTestId('wallet-stats')).toHaveTextContent('{"total_transactions":0,"total_volume":0,"total_incoming":0,"total_outgoing":0,"rewards":{}}');
    expect(screen.getByText('0 transactions')).toBeInTheDocument(); // transaction count
    // Transaction list empty state
    expect(screen.getByTestId('wallet-transaction-list')).toHaveTextContent('0 transactions');
  });

  it('handles undefined stats and rewards gracefully', async () => {
    mockedWalletServiceInstance.getWallet.mockResolvedValue({ ...mockWallet, balance: 0 });
    mockedWalletServiceInstance.getWalletStats.mockResolvedValue({
      total_transactions: undefined,
      total_volume: undefined,
      total_incoming: undefined,
      total_outgoing: undefined,
      rewards: undefined,
    } as any);
    mockedWalletServiceInstance.getTransactionHistory.mockResolvedValue([]);
    renderComponent();
    await waitFor(() => expect(screen.getByText('0 DP')).toBeInTheDocument());
    expect(screen.getByText('0')).toBeInTheDocument(); // transactions
    expect(screen.getByTestId('wallet-stats')).toHaveTextContent('{}');
  });

  it('disables transfer when balance is zero', async () => {
    mockedWalletServiceInstance.getWallet.mockResolvedValue({ ...mockWallet, balance: 0 });
    renderComponent();
    await waitFor(() => expect(screen.getByText('0 DP')).toBeInTheDocument());
    fireEvent.click(screen.getByRole('button', { name: /transfer coins/i }));
