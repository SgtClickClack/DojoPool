...
describe('API Security Tests', () => {
  ...
  beforeAll(() => {
    ...
    validToken = sign({ userId: 'test-user', role: 'user' }, jwtConfig.secret, { expiresIn: '1h' });
    expiredToken = sign({ userId: 'test-user', role: 'user' }, jwtConfig.secret, { expiresIn: '0s' });
  ...
  describe('Authorization Security', () => {
    test('should prevent unauthorized access to admin endpoints', async () => {
      const userToken = sign({ userId: 'test-user', role: 'user' }, jwtConfig.secret, { expiresIn: '1h' });
    ...
    test('should allow admin access to admin endpoints', async () => {
      const adminToken = sign({ userId: 'admin-user', role: 'admin' }, jwtConfig.secret, { expiresIn: '1h' });
      const response = await fetch(`${baseUrl}/api/admin/users`, {
        headers: {
  ...
  describe('Rate Limiting', () => {
    test('should enforce authentication rate limits', async () => {
      ...
      const requests = Array(attempts).fill(null).map(() =>
  ...
  describe('Input Validation', () => {
    ...
    test('should validate file upload types', async () => {
      ...
      const fakeFile = new Blob(['fake executable'], { type: 'application/x-msdownload' });
