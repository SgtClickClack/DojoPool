...
export class GameAnalysisService {
  ...
  private updatePlayerStats(event: GameEvent): void {
    ...
    if (event.success) {
      ...
      playerStats.longestStreak = Math.max(
    ...
    const shotTypeCount =
      playerStats.preferredShotTypes.get(event.shotType) || 0;
    ...
    playerStats.averageAccuracy =
      (playerStats.averageAccuracy * (playerStats.shotsTaken - 1) +
        event.accuracy) /
      playerStats.shotsTaken;
  }
  ...
  private updateMatchStats(event: GameEvent): void {
    ...
    this.matchStats.averageShotTime =
      (this.matchStats.averageShotTime * (this.matchStats.totalShots - 1) +
        event.duration) /
      this.matchStats.totalShots;
  }
  ...
  private updateGameStatus(event: GameEvent): void {
    ...
    this.gameState.currentPlayer =
      event.playerId === this.gameState.currentPlayer
    ...
    // Check for game end
    if (
      this.gameState.score.player1 >= 8 ||
      this.gameState.score.player2 >= 8
    ) {
