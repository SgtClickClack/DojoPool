...
from src.models import User, Game, Match, Tournament
...
class AdaptiveDifficulty:
    ...
    async def calculate_player_level(user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            games = await db.session.query(Game).filter(
            ...
            tournaments = await db.session.query(Tournament).join(
                Tournament.players
            ...
            for l, data in AdaptiveDifficulty.DIFFICULTY_LEVELS.items():
                if data['min_score'] > current_level_data['max_score']:
                    ...
                    progress = min(
                        100,
                        ((total_score - current_level_data['min_score']) / range_size) * 100
                    )
                    break
    ...
    async def generate_training_plan(user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_games = await db.session.query(Game).filter(
                (Game.player1_id == user_id) | (Game.player2_id == user_id)
            ...
            if patterns['losses']:
                loss_patterns = [g.game_data.get('loss_reason') for g in patterns['losses'] if g.game_data]
                if loss_patterns:
