...
export class TrainingService extends EventEmitter {
  ...
  private readonly EXERCISE_TYPES: TrainingExercise['type'][] = ['accuracy', 'power', 'spin', 'technique'];
  ...
  public getCurrentExercise(): TrainingExercise | null {
    ...
    return this.currentSession.exercises[this.currentSession.currentExerciseIndex];
  }
  ...
  private checkExerciseCompletion(): void {
    ...
    const successRate = exerciseShots.filter(shot => shot.success).length / exerciseShots.length;
    const averageAccuracy = exerciseShots.reduce((sum, shot) => sum + shot.accuracy, 0) / exerciseShots.length;
  ...
  private completeCurrentExercise(): void {
    ...
    if (this.currentSession.currentExerciseIndex >= this.currentSession.exercises.length) {
      this.endSession();
    } else {
  ...
  private updateProgress(): void {
    ...
    this.currentSession.progress.successRate = shots.filter(shot => shot.success).length / shots.length;
    this.currentSession.progress.averageAccuracy = shots.reduce((sum, shot) => sum + shot.accuracy, 0) / shots.length;
   ...
   */
  public generateExercises(weaknesses: string[], difficulty: number = 1): TrainingExercise[] {
   ...
   */
  private createExercise(type: TrainingExercise['type'], difficulty: number): TrainingExercise {
    const baseMetrics = {
