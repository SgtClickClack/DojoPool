...
import { doc, setDoc, query, where, getDocs, collection, deleteDoc, getDoc } from 'firebase/firestore';
...
export const sendVerificationEmail = async (user: User): Promise<{ success: boolean; error?: string }> => {
  try {
...
export const requestAccountDeletion = async (user: User): Promise<{ success: boolean; error?: string }> => {
  try {
    ...
    if (user.email) {
      await sendEmail({
        ...
        body: ACCOUNT_DELETION.EMAIL_TEMPLATES.DELETION_REQUEST.body(user.displayName || 'User')
      });
    }
...
export const cancelAccountDeletion = async (user: User): Promise<{ success: boolean; error?: string }> => {
  try {
    const deletionRef = doc(db, 'accountDeletions', user.uid);
    const deletionDoc = await getDoc(deletionRef);
    ...
    if (deletionData.status !== 'pending') {
      return { success: false, error: 'Deletion request is not in pending state' };
    }
    ...
    if (user.email) {
      await sendEmail({
        ...
        body: ACCOUNT_DELETION.EMAIL_TEMPLATES.DELETION_CANCELLED.body(user.displayName || 'User')
      });
    }
...
export const getDeletionRequestStatus = async (user: User): Promise<{ 
  success: boolean; 
...
export const deleteAccount = async (user: User): Promise<{ success: boolean; error?: string }> => {
  try {
    const deletionRef = doc(db, 'accountDeletions', user.uid);
    const deletionDoc = await getDoc(deletionRef);
    ...
    if (user.email) {
      await sendEmail({
        ...
        body: ACCOUNT_DELETION.EMAIL_TEMPLATES.DELETION_COMPLETED.body(user.displayName || 'User')
      });
    }
...
// Export user data
export const exportUserData = async (user: User): Promise<{ success: boolean; data?: any; error?: string }> => {
  try {
