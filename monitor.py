...
def group_processes_by_name(processes: List[Dict]) -> Dict[str, Dict]:
    ...
    groups = defaultdict(
        lambda: {"cpu_percent": 0, "memory_percent": 0, "instances": 0, "pids": []}
    )
...
def get_process_details(include_grouped=True) :
    ...
    if include_grouped:
        ...
        grouped = dict(
            sorted(grouped.items(), key=lambda x: x[1]["cpu_percent"], reverse=True)
        )
...
def analyze_process(pinfo: Dict) -> str:
    ...
    if pid in PROCESS_HISTORY:
        ...
        elif len(history["cpu_samples"]) > 6 and all(
            x > 30 for x in list(history["cpu_samples"])[-6:]
        ):
...
def display_metrics(metrics: Dict):
    ...
    print(
        f"  Used: {metrics['disk']['used_gb']:.1f}GB / {metrics['disk']['total_gb']:.1f}GB"
    )
...
def get_optimization_suggestions(metrics: Dict) -> List[str]:
    ...
    if metrics["cpu"]["percent"] > 70:
        ...
        high_cpu_apps = [
            name for name, stats in grouped_procs.items() if stats["cpu_percent"] > 20
        ]
        if high_cpu_apps:
            suggestions.append(
                f"🔄 Consider closing or investigating high CPU apps: {', '.join(high_cpu_apps)}"
            )
    ...
    if metrics["disk"]["percent"] > 85:
        suggestions.append(
            "💿 Disk space is running low. Consider running disk cleanup."
        )
