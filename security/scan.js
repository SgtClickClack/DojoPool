...
async function runSecurityScans() {
  ...
  try {
    ...
    try {
      ...
      report.details.vulnerabilities.push(...Object.values(npmAuditResult.vulnerabilities || {}));
    ...
    try {
      ...
      report.details.vulnerabilities.push(...(snykResult.vulnerabilities || []));
    } catch (error) {
    ...
    try {
      const eslintOutput = execSync('npx eslint --config .eslintrc.security.js "src/**/*.{js,ts,tsx}" --format json').toString();
      const eslintResult = JSON.parse(eslintOutput);
      report.details.linting = eslintResult;
      report.summary.linting.errors = eslintResult.reduce((acc, file) => acc + file.errorCount, 0);
      report.summary.linting.warnings = eslintResult.reduce((acc, file) => acc + file.warningCount, 0);
    } catch (error) {
    ...
    fs.writeFileSync(
      path.join(reportsDir, `security-report-${new Date().toISOString().replace(/[:.]/g, '-')}.json`),
