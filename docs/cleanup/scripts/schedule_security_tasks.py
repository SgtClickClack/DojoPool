...
class SecurityTaskScheduler:
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(log_dir / f"scheduler_{datetime.now(timezone.utc).strftime('%Y%m%d')}.log"),
    ...
    def run_security_check(self):
        ...
        try:
            script_path = self.root_dir / "docs" / "cleanup" / "scripts" / "automated_security_check.py"
    ...
    def check_documentation_age(self):
        ...
        try:
            ...
            if days_since_update > 30:
                logging.warning(f"Security documentation is {days_since_update} days old and needs review")
            else:
                logging.info(f"Security documentation is up to date (last updated {days_since_update} days ago)")
    ...
    def generate_status_report(self):
        ...
        try:
            ...
            if reports_dir.exists():
                latest_report = max(reports_dir.glob("*.json"), key=lambda x: x.stat().st_mtime, default=None)
                if latest_report:
            ...
            if cert_logs_dir.exists():
                latest_log = max(cert_logs_dir.glob("*.log"), key=lambda x: x.stat().st_mtime, default=None)
                if latest_log:
            ...
            report_file = status_dir / f"security_status_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
