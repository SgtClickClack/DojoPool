...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('cursor_optimizer.log'),
...
def cleanup_processes():
    ...
    try:
        # Get all processes
        cursor_processes = [p for p in psutil.process_iter(['pid', 'name', 'cpu_percent', 'memory_percent', 'create_time'])
                          if 'cursor' in p.info['name'].lower()]
        pwsh_processes = [p for p in psutil.process_iter(['pid', 'name', 'cpu_percent', 'memory_percent', 'create_time'])
        ...
        if len(cursor_processes) > 2:
            logging.info(f"Found {len(cursor_processes)} Cursor instances, keeping newest 2")
        ...
        if len(pwsh_processes) > 2:
            logging.info(f"Found {len(pwsh_processes)} PowerShell instances, keeping newest 2")
...
def clear_caches():
    ...
    try:
        ...
        cache_path = os.path.expandvars("%LOCALAPPDATA%\\Microsoft\\Windows\\PowerShell\\PSModuleAnalysisCache")
...
def optimize_cursor():
    ...
    try:
        ...
        cursor_processes = [p for p in psutil.process_iter(['pid', 'name', 'cpu_percent', 'memory_percent'])
                          if 'cursor' in p.info['name'].lower()]
        pwsh_processes = [p for p in psutil.process_iter(['pid', 'name', 'cpu_percent', 'memory_percent'])
                          if 'pwsh' in p.info['name'].lower()]
        ...
        total_cpu = sum(p.info['cpu_percent'] for p in cursor_processes + pwsh_processes)
        total_memory = sum(p.info['memory_percent'] for p in cursor_processes + pwsh_processes)
