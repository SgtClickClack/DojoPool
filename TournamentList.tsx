...
import { Box, Typography, List, ListItem, ListItemText, Paper, CircularProgress, Alert, Chip, Button } from '@mui/material';
...
const getStatusColor = (status: TournamentStatus): "success" | "warning" | "info" | "error" | "default" => {
    switch (status) {
...
const TournamentList: React.FC = () => {
  ...
  const [registrationStatus, setRegistrationStatus] = useState<{ [key: string]: { loading: boolean; error: string | null } }>({});
  ...
  const handleRegister = async (tournamentId: string) => {
    ...
    setRegistrationStatus(prev => ({ ...prev, [tournamentId]: { loading: true, error: null } }));
    try {
      await joinTournament(tournamentId);
      alert('Successfully registered!'); 
      setRegistrationStatus(prev => ({ ...prev, [tournamentId]: { loading: false, error: null } }));
    } catch (err) {
      console.error("Registration error:", err);
      const errorMessage = err instanceof Error ? err.message : 'Failed to register.';
      setRegistrationStatus(prev => ({ ...prev, [tournamentId]: { loading: false, error: errorMessage } }));
      alert(`Registration failed: ${errorMessage}`); 
    }
  };

  const renderActionButton = (tournament: Tournament) => {
    const regStatus = registrationStatus[tournament.id] || { loading: false, error: null };
    ...
    switch (tournament.status) {
      ...
      case TournamentStatus.FULL:
        return <Button variant="outlined" size="small" disabled sx={{ ml: 1 }}>Full</Button>;
      case TournamentStatus.ACTIVE:
      case TournamentStatus.COMPLETED:
      case TournamentStatus.CLOSED:
        return null;
      case TournamentStatus.CANCELLED:
         return <Button variant="outlined" color="error" size="small" disabled sx={{ ml: 1 }}>Cancelled</Button>;
      case TournamentStatus.UPCOMING:
        return (
          <Button 
            variant="contained" 
