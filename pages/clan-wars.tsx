        ...
        <Container maxWidth="lg" sx={{ py: 4, position: 'relative', zIndex: 1 }}>
                    ...
                    <CardContent sx={{ p: 3 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                        ...
                        <Chip
                          ...
                          sx={{
                            backgroundColor: war.status === 'active' ? '#00ff9d' : 
                                           war.status === 'preparing' ? '#feca57' : '#00a8ff',
                            color: '#000',
                            fontWeight: 'bold'
                          }}
                        />
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                        <Box sx={{ textAlign: 'center', flex: 1 }}>
                          ...
                          <Typography variant="h3" sx={{ color: '#00ff9d', fontWeight: 'bold' }}>
                            {war.clan1Score}
                          </Typography>
                        </Box>
          ...
          <Box sx={{ mb: 4 }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 3, justifyContent: 'center' }}>
              {mockClans.filter(clan => clan.id !== userClan?.id).map((clan) => (
                <Paper
                  key={clan.id}
                  sx={{
                    p: 3,
                  ...
                  onClick={() => {
                    setWarForm(prev => ({ ...prev, targetClanId: clan.id }));
                ...
                >
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                    <Box sx={{ textAlign: 'center' }}>
                      <Typography variant="body2" sx={{ color: '#ccc' }}>Members</Typography>
                      <Typography variant="h6" sx={{ color: '#00a8ff' }}>{clan.members}</Typography>
                    </Box>
                    <Box sx={{ textAlign: 'center' }}>
                      <Typography variant="body2" sx={{ color: '#ccc' }}>Territories</Typography>
                      <Typography variant="h6" sx={{ color: '#00a8ff' }}>{clan.territories}</Typography>
                    </Box>
                    <Box sx={{ textAlign: 'center' }}>
                      <Typography variant="body2" sx={{ color: '#ccc' }}>Rating</Typography>
                      <Typography variant="h6" sx={{ color: '#00a8ff' }}>{clan.rating}</Typography>
                    </Box>
                  </Box>
          ...
          {showDeclareWar && (
            ...
            >
              ...
              >
                <Typography variant="h5" sx={{ color: '#ff6b6b', mb: 3, fontFamily: 'Orbitron, monospace', textAlign: 'center' }}>
                  ⚔️ Declare War
                </Typography>
                
                <Box sx={{ mb: 3 }}>
                  <Typography variant="body2" sx={{ color: '#ccc', mb: 1 }}>War Name</Typography>
                  <input
                    type="text"
                    value={warForm.name}
                    onChange={(e) => setWarForm(prev => ({ ...prev, name: e.target.value }))}
                    style={{
                      width: '100%',
                      padding: '12px',
                ...
                <Box sx={{ mb: 3 }}>
                  <Typography variant="body2" sx={{ color: '#ccc', mb: 1 }}>Description</Typography>
                  <textarea
                    value={warForm.description}
                    onChange={(e) => setWarForm(prev => ({ ...prev, description: e.target.value }))}
                    style={{
                      width: '100%',
                      padding: '12px',
                      backgroundColor: 'rgba(255, 255, 255, 0.1)',
