...
import { Box, Container, Typography, Paper, Card, CardContent, Button, Chip, LinearProgress } from '@mui/material';
import { Mic, PlayArrow, Pause, VolumeUp, Analytics, TrendingUp, Psychology, SportsEsports } from '@mui/icons-material';
...
const AICommentaryPage: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'live' | 'analysis' | 'coaching' | 'highlights'>('live');
  const [isLiveCommentaryActive, setIsLiveCommentaryActive] = useState(false);
  const [commentaryStyle, setCommentaryStyle] = useState<'professional' | 'excited' | 'analytical' | 'casual'>('professional');
    ...
    shots: [
      {
        ...
        commentary: 'Incredible break shot! ShadowStriker shows why they\'re ranked 1850!'
      },
      {
        id: 'shot2',
        playerId: 'player2',
        ...
        commentary: 'Smart safety play by PoolMaster2024, forcing a difficult position.'
      },
      {
        id: 'shot3',
        playerId: 'player1',
  ...
  const [aiAnalysis, setAiAnalysis] = useState({
    ...
    keyMoments: [
      ...
      },
      {
        id: 'moment2',
        timestamp: new Date(Date.now() - 20000),
        description: 'PoolMaster2024 shows tactical brilliance with safety play',
        impact: 'medium',
        excitementLevel: 75
      },
      {
  ...
  const getStyleColor = (style: string) => {
    switch (style) {
      case 'professional': return '#00a8ff';
      case 'excited': return '#ff6b6b';
      case 'analytical': return '#00ff9d';
      case 'casual': return '#feca57';
        ...
        <Container maxWidth="lg" sx={{ py: 4, position: 'relative', zIndex: 1 }}>
            ...
            >
              <CardContent sx={{ p: 3 }}>
                <Typography variant="h5" sx={{ color: '#00ff9d', mb: 2, fontFamily: 'Orbitron, monospace' }}>
                  🎯 Live Match: {currentMatch.player1.name} vs {currentMatch.player2.name}
                </Typography>
                ...
                <Chip
                  ...
                  sx={{
                    backgroundColor: currentMatch.status === 'active' ? '#00ff9d' : '#00a8ff',
                    color: '#000',
                    fontWeight: 'bold'
                  }}
                />
          ...
          <Box sx={{ mb: 4 }}>
            <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2, flexWrap: 'wrap' }}>
              {[
                ...
                { key: 'highlights', label: 'Highlights', icon: <SportsEsports /> }
              ].map((tab) => (
                <Button
                  key={tab.key}
          ...
          <Box sx={{ mb: 4 }}>
            ...
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', justifyContent: 'center' }}>
              {['professional', 'excited', 'analytical', 'casual'].map((style) => (
                <Button
                  key={style}
                  variant={commentaryStyle === style ? 'contained' : 'outlined'}
            ...
            {activeTab === 'live' && (
              <Box>
                <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2, mb: 3 }}>
                  ...
                  >
                    {isLiveCommentaryActive ? 'Pause Commentary' : 'Start Live Commentary'}
                  </Button>
                  <Button
                    variant="outlined"
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
                      <Box sx={{ flex: 1, minWidth: '300px' }}>
                        ...
                        <Box sx={{ mb: 2 }}>
                          ...
                          />
                          <Typography variant="body2" sx={{ color: '#feca57', mt: 0.5 }}>
                    ...
                    <CardContent sx={{ p: 3 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                        ...
                        <Chip
                          ...
                          sx={{
                            backgroundColor: moment.excitementLevel > 90 ? '#ff6b6b' : '#feca57',
                            color: '#000',
                            fontWeight: 'bold'
                          }}
                        />
                      ...
                      <Typography variant="body2" sx={{ color: '#ccc' }}>
                        Impact: {moment.impact.toUpperCase()} • {moment.timestamp.toLocaleTimeString()}
                      </Typography>
                    </CardContent>
                  </Card>
                ))}
