...
import { DojoService, DojoCandidate, DojoNomination } from '../../src/services/DojoService';
...
const ChooseDojoScreen: React.FC = () => {
  ...
  const initializeOnboarding = async () => {
    try {
      ...
      setDojos(MOCK_DOJOS);
      setLocation({ latitude: -27.4568, longitude: 153.0364, city: 'Fortitude Valley' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCompleteOnboarding = async () => {
    ...
    try {
      setCompleting(true);
      const result = await OnboardingService.completeOnboarding(selectedDojo.id);
  ...
  const handleNominateDojo = async (nomination: DojoNomination) => {
    try {
      ...
      if (result.success) {
        alert('Dojo nomination sent! Thank you for helping build the DojoPool world!');
        ...
        <div className="bg-black bg-opacity-50 p-4">
          <div className="max-w-6xl mx-auto">
            ...
            <p className="text-blue-200">
              {location?.city || 'Fortitude Valley'} • Select where your journey begins
            </p>
          </div>
        </div>
