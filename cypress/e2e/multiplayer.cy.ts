describe('Multiplayer Game Flow', () => {
  ...
  it('creates and joins a multiplayer game', () => {
    ...
    cy.get('[data-testid=game-code]')
      ...
      .then((gameCode) => {
        ...
        cy.get('[data-testid=player-1-status]').should('have.text', 'Connected');
        cy.get('[data-testid=player-2-status]').should('have.text', 'Connected');
      });
  });
  ...
  it('handles game chat functionality', () => {
    ...
    cy.get('[data-testid=chat-input]')
      .type('Good game!{enter}');
    
    // Verify message appears
    cy.get('[data-testid=chat-messages]')
      .should('contain', 'Good game!');
  });
