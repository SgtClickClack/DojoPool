...
export class VenueLeaderboardService extends EventEmitter {
   ...
   */
  public updatePlayerPerformance(
    playerId: string,
    venueId: string,
    performance: any
  ): void {
    ...
    if (!player) {
      ...
      const newPlayer: PlayerPerformance = {
        ...
        winRate:
          performance.matchesPlayed > 0
            ? performance.matchesWon / performance.matchesPlayed
            : 0,
      ...
      player.tournamentsWon =
        performance.tournamentsWon || player.tournamentsWon;
      player.tournamentsPlayed =
        performance.tournamentsPlayed || player.tournamentsPlayed;
      player.winRate =
        player.matchesPlayed > 0 ? player.matchesWon / player.matchesPlayed : 0;
  ...
  private checkDojoMasterEligibility(playerId: string, venueId: string): void {
    ...
    const daysSinceActive =
      (Date.now() - player.lastActive.getTime()) / (1000 * 60 * 60 * 24);
