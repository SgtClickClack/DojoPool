...
export class EquipmentManagementService extends BrowserEventEmitter {
  ...
  private initializeSocket(): void {
    try {
      ...
      this.socket.on(
        'health_update',
        (data: { equipmentId: string; health: EquipmentHealth }) => {
      ...
      console.error(
        'Failed to initialize Equipment Management Service socket:',
        error
      );
      this.isConnected = false;
    }
  ...
  private handleReconnect(): void {
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      setTimeout(() => {
        console.log(
          `Attempting to reconnect Equipment Management Service (${this.reconnectAttempts}/${this.maxReconnectAttempts})`
        );
        this.initializeSocket();
  ...
  public completeMaintenance(
    ...
    try {
      ...
      // Update task status
      const task = equipment.maintenance.maintenanceSchedule.tasks.find(
        (t) => t.id === taskId
      );
      if (task) {
        task.status = 'completed';
      ...
      equipment.health.overallHealth = Math.min(
        100,
        equipment.health.overallHealth + 10
      );
      equipment.health.alerts = equipment.health.alerts.filter(
      ...
      if (this.socket) {
        this.socket.emit('complete_maintenance', {
          equipmentId,
          taskId,
          record,
        });
  ...
  public getExpiringWarranties(
    ...
    return warranties.filter(
      (warranty) => warranty.endDate >= now && warranty.endDate <= futureDate
    );
  }
