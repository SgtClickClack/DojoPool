...
class VenueHardwareService {
  ...
  private collectSensorData(): void {
    for (const device of this.devices.values()) {
      if (
        device.type === HardwareDeviceType.SENSOR &&
  ...
  private collectCameraData(): void {
    for (const device of this.devices.values()) {
      if (
        device.type === HardwareDeviceType.CAMERA &&
        device.status === DeviceStatus.ONLINE
      ) {
  ...
  private generateMockMotion(): MotionData {
    ...
    return {
      ...
      direction: motionDetected
        ? Object.values(MotionDirection)[Math.floor(Math.random() * 6)]
        : MotionDirection.UP,
  ...
  private updateDeviceHealth(): void {
    for (const device of this.devices.values()) {
      ...
      device.health.temperature = Math.max(
        20,
        Math.min(80, device.health.temperature)
      );
      device.health.cpuUsage = Math.max(
        0,
        Math.min(100, device.health.cpuUsage)
      );
      device.health.memoryUsage = Math.max(
        0,
        Math.min(100, device.health.memoryUsage)
      );
      device.health.networkLatency = Math.max(
        1,
        Math.min(50, device.health.networkLatency)
      );
  ...
  private checkDeviceAlerts(device: HardwareDevice): void {
    ...
    if (
      ...
      device.configuration.storage.usedSpace /
        device.configuration.storage.totalSpace >
        0.9
    ) {
      this.createAlert(
        device.id,
  ...
  getVenueStatus(venueId: string): VenueHardwareStatus {
    ...
    const onlineDevices = venueDevices.filter(
      (d) => d.status === DeviceStatus.ONLINE
    ).length;
    const offlineDevices = venueDevices.filter(
      (d) => d.status === DeviceStatus.OFFLINE
    ).length;
    const maintenanceDevices = venueDevices.filter(
      (d) => d.status === DeviceStatus.MAINTENANCE
    ).length;
    const errorDevices = venueDevices.filter(
    ...
    const performance: HardwarePerformance = {
      averageLatency:
        venueDevices.reduce((sum, d) => sum + d.health.networkLatency, 0) /
        totalDevices,
      averageThroughput: 100, // Mock value
      uptime:
        venueDevices.reduce((sum, d) => sum + d.health.uptime, 0) /
        totalDevices,
      errorRate: errorDevices / totalDevices,
      calibrationAccuracy:
        venueDevices.reduce(
          (sum, d) => sum + (d.configuration.calibration?.accuracy || 0),
          0
        ) / totalDevices,
    };
  ...
  async calibrateDevice(deviceId: string): Promise<boolean> {
    ...
    this.publish('device_status_changed', { deviceId, status: device.status });
    this.publish('calibration_completed', {
      deviceId,
      accuracy: device.configuration.calibration?.accuracy,
    });
  ...
  async updateDeviceConfiguration(
    ...
    this.publish('device_configuration_updated', {
      deviceId,
    ...
    if (subscribers) {
      subscribers.forEach((callback) => {
        try {
          ...
          console.error(
            'Error in hardware service subscriber callback:',
            error
          );
        }
      });
