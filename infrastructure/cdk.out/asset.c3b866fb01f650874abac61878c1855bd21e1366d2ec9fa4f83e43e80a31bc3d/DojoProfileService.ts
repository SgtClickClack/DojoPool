...
class DojoProfileService extends BrowserEventEmitter {
  ...
  public async createProfile(
    profile: Omit<DojoProfile, 'venueId'>
  ): Promise<DojoProfile> {
  ...
  // Theme Management
  public async generateTheme(
    prompt: string,
    style: VenueTheme['backgroundStyle']
  ): Promise<VenueTheme> {
  ...
  public getActiveRewards(venueId: string): DojoCoinReward[] {
    const now = new Date();
    return this.getRewards(venueId).filter(
      (reward) =>
        reward.isActive &&
  ...
  public async createAtmosphere(
    atmosphere: Omit<VenueAtmosphere, 'id'>
  ): Promise<VenueAtmosphere> {
    const newAtmosphere: VenueAtmosphere = {
      ...atmosphere,
      id: this.generateId(),
      generatedAttributes: await this.generateAtmosphereAttributes(
        atmosphere.vibe,
        atmosphere.lighting
      ),
    };
  ...
  private async generateAtmosphereAttributes(
    vibe: string,
    lighting: string
  ): Promise<string[]> {
  ...
  private loadMockData(): void {
    ...
    const mockProfile: DojoProfile = {
      ...
      theme: {
        ...
        generatedImages: [
          '/images/cyberpunk-bg1.jpg',
          '/images/cyberpunk-bg2.jpg',
        ],
        customElements: [],
      },
      atmosphere: {
        ...
        generatedAttributes: [
          ...
          'Neon lighting for focus',
          'Electronic beats for motivation',
        ],
      },
      rewards: [
        {
      ...
      stories: [
        {
          ...
          title: 'Our Mission',
          content:
            'To create the ultimate pool gaming experience with cutting-edge technology and community spirit.',
          category: 'mission',
          isPublic: true,
          createdAt: new Date(),
          updatedAt: new Date(),
