...
class VenueSpecialsService extends BrowserEventEmitter {
  ...
  public async createSpecial(
    ...
    if (conflicts.length > 0) {
      throw new Error(
        `Special conflicts with existing specials: ${conflicts.map((c) => c.name).join(', ')}`
      );
    }
  ...
  public async recordUsage(
    usage: Omit<SpecialUsage, 'id' | 'usageDate'>
  ): Promise<SpecialUsage> {
    ...
    if (special.perCustomerLimit) {
      const customerUsage = this.usage.filter(
        (u) =>
          u.specialId === usage.specialId && u.customerId === usage.customerId
      ).length;
  ...
  public getUsage(specialId?: string, customerId?: string): SpecialUsage[] {
    ...
    return filtered.sort(
      (a, b) => b.usageDate.getTime() - a.usageDate.getTime()
    );
  }
  ...
  public async generateAnalytics(specialId: string): Promise<SpecialAnalytics> {
    ...
    const totalDiscount = specialUsage.reduce(
    ...
    const conversionRate =
      totalUsage > 0 ? (customerCount / totalUsage) * 100 : 0;
    const revenueImpact = specialUsage.reduce(
      (sum, u) => sum + u.finalAmount,
      0
    );
  ...
  private hasTimeOverlap(
    special1: VenueSpecial,
    special2: VenueSpecial
  ): boolean {
    return (
      special1.startDate < special2.endDate &&
      special2.startDate < special1.endDate
    );
  }

  private hasItemOverlap(
    special1: VenueSpecial,
    special2: VenueSpecial
  ): boolean {
    return special1.applicableItems.some((item) =>
      special2.applicableItems.includes(item)
    );
  }
  ...
  private calculateCustomerSatisfaction(specialId: string): number {
    ...
    const avgDiscount =
      specialUsage.reduce((sum, u) => sum + u.discountApplied, 0) /
      specialUsage.length;
