...
export class VenueCustomizationService {
   ...
   */
  async generateVenueCustomization(
    options: VenueCustomizationOptions
  ): Promise<GeneratedVenueAttributes> {
    try {
  ...
  private parseBrandingResponse(response: string) {
    try {
      ...
      return {
        ...
        description:
          parsed.description ||
          'A premier gaming destination for pool enthusiasts.',
        missionStatement:
          parsed.missionStatement ||
          'To create an inclusive community where players can compete, connect, and grow.',
  ...
  private parseStoryResponse(response: string) {
    try {
      ...
      return {
        origin:
          parsed.origin ||
          'Founded by passionate pool enthusiasts who wanted to create a modern gaming experience.',
        community:
          parsed.community ||
          'A hub for local players, hosting regular tournaments and community events.',
        achievements:
          parsed.achievements ||
          'Recognized for excellence in tournament organization and player development.',
        future:
          parsed.future ||
          'Expanding to become the premier destination for competitive pool gaming.',
      };
    } catch (error) {
  ...
  async applyCustomization(
    venueId: string,
    customization: GeneratedVenueAttributes
  ): Promise<void> {
    try {
      ...
      console.log(
        `Venue customization applied successfully for venue ${venueId}`
      );
    } catch (error) {
