...
class DojoCoinRewardService extends BrowserEventEmitter {
  ...
  public async createReward(
    reward: Omit<
      DojoCoinReward,
      'id' | 'currentRedemptions' | 'redemptionHistory' | 'analytics'
    >
  ): Promise<DojoCoinReward> {
  ...
  public getActiveRewards(venueId: string): DojoCoinReward[] {
    ...
    return this.rewards.filter(
      (r) =>
        r.isActive && r.startDate <= now && (!r.endDate || r.endDate >= now)
    );
  }
  ...
  private updateRewardAnalytics(reward: DojoCoinReward): void {
    const uniquePlayers = new Set(
      reward.redemptionHistory.map((r) => r.playerId)
    ).size;
    const totalRedemptions = reward.redemptionHistory.length;
    const averageRate =
      totalRedemptions > 0 ? totalRedemptions / uniquePlayers : 0;
  ...
  private calculateEngagement(reward: DojoCoinReward): number {
    const recentRedemptions = reward.redemptionHistory.filter(
      (r) => r.redeemedAt > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
    );
  ...
  public async createLoyaltyTier(
    tier: Omit<LoyaltyTier, 'id'>
  ): Promise<LoyaltyTier> {
  ...
  public async updatePlayerLoyalty(loyalty: PlayerLoyalty): Promise<void> {
    const index = this.playerLoyalty.findIndex(
      (p) => p.playerId === loyalty.playerId
    );
    if (index !== -1) {
  ...
  private checkTierUpgrade(loyalty: PlayerLoyalty): LoyaltyTier | null {
    const nextTier = this.loyaltyTiers.find(
      (t) =>
  ...
  public async createCampaign(
    campaign: Omit<RewardCampaign, 'id' | 'analytics'>
  ...
  private processAutomatedRewards(): void {
    this.automatedRewards.forEach(async (automatedReward) => {
      ...
      if (
        shouldTrigger &&
        (!automatedReward.maxTriggers ||
          automatedReward.triggerCount < automatedReward.maxTriggers)
      ) {
  ...
  private async triggerAutomatedReward(
  ...
  public getRewardAnalytics(venueId: string): any {
    ...
    const totalRedemptions = venueRewards.reduce(
      (sum, r) => sum + r.currentRedemptions,
      0
    );
    const totalCoinsDistributed = venueRewards.reduce(
      (sum, r) => sum + r.currentRedemptions * r.coinAmount,
      0
    );
    ...
    return {
      ...
      averageRedemptionRate:
        totalRewards > 0 ? totalRedemptions / totalRewards : 0,
  ...
  private getLoyaltyStats(): any {
    ...
    const tierDistribution = this.loyaltyTiers.map((tier) => ({
      tier: tier.name,
      count: this.playerLoyalty.filter((p) => p.currentTier.id === tier.id)
        .length,
    }));
    ...
    return {
      ...
      averagePoints:
        totalPlayers > 0
          ? this.playerLoyalty.reduce((sum, p) => sum + p.totalPoints, 0) /
            totalPlayers
          : 0,
    };
  }
      ...
      },
      {
        ...
        benefits: [
          ...
          },
          {
            ...
            value: 1,
            description: 'Exclusive rewards access',
            isActive: true,
          },
        ],
        color: '#FFD700',
