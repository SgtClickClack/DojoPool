...
import {
  ...
  type PoolGodInteraction,
  type TournamentAIEvent,
} from '../services/tournament/EnhancedTournamentService';
...
export const useEnhancedTournament = (tournamentId: string) => {
  ...
  const completeTournament = useCallback(
    async (winner: TournamentParticipant, finalMatch: TournamentMatch) => {
      if (!state.tournament) return;
