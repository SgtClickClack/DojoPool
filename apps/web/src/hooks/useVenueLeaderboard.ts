...
export const useVenueLeaderboard = (): UseVenueLeaderboardReturn => {
  ...
  useEffect(() => {
    ...
    newSocket.on(
      'venue-leaderboard:dojo-master-designated',
      (dojoMaster: DojoMaster) => {
  ...
  const getVenuePerformance = useCallback(
    async (venueId: string): Promise<VenuePerformance | null> => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/venue-leaderboard/venue/${venueId}`
        );
        if (!response.ok) {
  ...
  const getPlayerPerformance = useCallback(
    async (
      playerId: string,
      venueId: string
    ): Promise<PlayerPerformance | null> => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/venue-leaderboard/player/${playerId}/${venueId}`
        );
        if (!response.ok) {
          if (response.status === 404) return null;
  ...
  const getDojoMaster = useCallback(
    async (venueId: string): Promise<DojoMaster | null> => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/venue-leaderboard/dojo-master/${venueId}`
        );
        if (!response.ok) {
          if (response.status === 404) return null;
  ...
  const getAllDojoMasters = useCallback(async (): Promise<DojoMaster[]> => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/venue-leaderboard/dojo-masters`
      );
      if (!response.ok) {
  ...
  const getTopPlayers = useCallback(
    async (
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/venue-leaderboard/top-players/${venueId}?limit=${limit}`
        );
        if (!response.ok) {
  ...
  const updateVenuePerformance = useCallback(
    async (venueId: string, activity: any): Promise<void> => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/venue-leaderboard/venue-performance`,
          {
            method: 'POST',
            headers: {
  ...
  const updatePlayerPerformance = useCallback(
    async (
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/venue-leaderboard/player-performance`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
        ...
        if (!result.success) {
          throw new Error(
            result.error || 'Failed to update player performance'
          );
        }
      } catch (err) {
  ...
  const designateDojoMaster = useCallback(
    async (
      venueId: string,
      playerId: string,
      playerName: string
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/venue-leaderboard/designate-master`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
