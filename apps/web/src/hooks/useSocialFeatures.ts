...
import {
  ...
  type SocialProfile,
} from '../services/social/SocialFeaturesService';
...
export const useSocialFeatures = () => {
  ...
  const acceptFriendRequest = useCallback(
    async (requestId: string) => {
      ...
      try {
        const success =
          await socialFeaturesService.acceptFriendRequest(requestId);
        if (success) {
          await loadFriends(); // Refresh friends list
        }
        return success;
  ...
  const sendMessage = useCallback(
    async (
      ...
      setError(null);
      try {
        const success = await socialFeaturesService.sendMessage(
          receiverId,
          content,
          type,
          metadata
  ...
  const commentOnActivity = useCallback(
    async (activityId: string, comment: string) => {
      setError(null);
      try {
        const success = await socialFeaturesService.commentOnActivity(
          activityId,
          comment
        );
        if (success) {
  ...
  const getProfile = useCallback(
    async (userId: string): Promise<SocialProfile | null> => {
