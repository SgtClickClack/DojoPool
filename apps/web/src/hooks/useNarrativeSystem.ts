...
import {
  ...
  type StoryArc,
  type WorldState,
} from '../services/narrative/NarrativeEventSystem';
...
export const useNarrativeSystem = (userId: string) => {
  ...
  // Start a story arc
  const startStoryArc = useCallback(
    async (arcId: string) => {
      try {
  ...
  // Advance a story arc
  const advanceStoryArc = useCallback(
    async (arcId: string) => {
      try {
  ...
  const getCharactersAtCurrentLocation = useCallback(() => {
    return state.characters.filter(
      (char) =>
        char.currentLocation === state.worldState.currentLocation &&
        char.isActive
    );
  ...
  const hasCharactersAtLocation = useCallback(
    (location: string) => {
      return state.characters.some(
        (char) => char.currentLocation === location && char.isActive
      );
    },
    [state.characters]
  );
