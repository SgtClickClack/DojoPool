...
import {
  ...
  type AdvancedCommentaryEvent,
  type AdvancedCommentaryConfig,
  type AdvancedCommentaryMetrics,
...
const generateAdvancedHighlights = async (
  request: AdvancedHighlightRequest
): Promise<AdvancedHighlight> => {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to generate advanced highlights: ${response.statusText}`
    );
  }
...
const generateAdvancedCommentary = async (
  request: AdvancedCommentaryRequest
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to generate advanced commentary: ${response.statusText}`
    );
  }
...
const updateConfig = async (
  config: Partial<AdvancedCommentaryConfig>
): Promise<AdvancedCommentaryConfig> => {
...
export const advancedCommentaryKeys = {
  ...
  match: (matchId: string) =>
    [...advancedCommentaryKeys.all, 'match', matchId] as const,
  commentary: (matchId: string) =>
    [...advancedCommentaryKeys.all, 'commentary', matchId] as const,
  highlight: (highlightId: string) =>
    [...advancedCommentaryKeys.all, 'highlight', highlightId] as const,
...
export const useAdvancedAIMatchCommentaryHighlights =
  (): UseAdvancedAIMatchCommentaryHighlightsReturn => {
    ...
    const generateHighlightsMutation = useMutation({
      ...
      onSuccess: (data) => {
        queryClient.invalidateQueries({
          queryKey: advancedCommentaryKeys.match(data.matchId),
        });
    ...
    const generateCommentaryMutation = useMutation({
      ...
      onSuccess: (data) => {
        queryClient.invalidateQueries({
          queryKey: advancedCommentaryKeys.commentary(data.matchId),
        });
        queryClient.invalidateQueries({
          queryKey: advancedCommentaryKeys.match(data.matchId),
        });
    ...
    const generateHighlights = useCallback(
      async (request: AdvancedHighlightRequest) => {
    ...
    const [metrics, setMetrics] = useState<AdvancedCommentaryMetrics | null>(
      null
    );
    ...
    const calculateShotScore = useCallback(
      async (shot: ShotReplayData): Promise<number> => {
        setLoading(true);
        setError(null);
    ...
    const generateCinematicReplay = useCallback(
      async (shot: ShotReplayData): Promise<CinematicShot> => {
        setLoading(true);
        setError(null);
    ...
    const analyzePlayerPatterns = useCallback(
      async (
        playerId: string,
        recentShots: ShotReplayData[]
      ): Promise<PlayerPattern> => {
    ...
    const generateCoachingAdvice = useCallback(
    ...
    // Match Management
    const addShotToMatch = useCallback(
      async (shot: ShotReplayData): Promise<void> => {
        setLoading(true);
        setError(null);
    ...
    const getCurrentMatchShots = useCallback(async (): Promise<
      ShotReplayData[]
    > => {
      setLoading(true);
      setError(null);
    ...
    const getMatchHighlights = useCallback(async (): Promise<
      CinematicShot[]
    > => {
      setLoading(true);
      setError(null);
    ...
    // Player Patterns
    const getPlayerPattern = useCallback(
      async (playerId: string): Promise<PlayerPattern | null> => {
        setLoading(true);
        setError(null);
    ...
    // Configuration & Metrics
    const getConfig =
      useCallback(async (): Promise<AdvancedCommentaryConfig> => {
        setLoading(true);
        setError(null);
    ...
    const getMetrics =
      useCallback(async (): Promise<AdvancedCommentaryMetrics> => {
        setLoading(true);
        setError(null);
    ...
    const getCommentaryEvents = useCallback(
      async (matchId: string): Promise<AdvancedCommentaryEvent[]> => {
        setLoading(true);
        setError(null);
...
export const useAdvancedCommentaryRealTime = (matchId: string) => {
  const { data: matchData, refetch: refetchMatch } =
    useMatchHighlights(matchId);
  const { data: commentaryData, refetch: refetchCommentary } =
    useMatchCommentary(matchId);
