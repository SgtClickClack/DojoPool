...
export const useGameFlow = (initialConfig?: GameFlowConfig) => {
      ...
      if (gameStatus === 'in-progress' && status.stage !== 'in-game') {
        ...
        setFlowData((prev) => ({
          ...
          results: {
            ...
            duration:
              gameState.gameState.endTime && gameState.gameState.startTime
                ? new Date(gameState.gameState.endTime).getTime() -
                  new Date(gameState.gameState.startTime).getTime()
                : 0,
  ...
  const setupGame = useCallback(
    async (gameConfig: GameFlowConfig) => {
      ...
      try {
        ...
        setStatus((prev) => ({
          ...
          isLoading: false,
          error:
            error instanceof Error ? error.message : 'Failed to setup game',
        }));
      }
    },
...
export const useTableSelection = (venueId: string) => {
  ...
  const reserveTable = useCallback(
    async (tableId: string, duration: number) => {
      setLoading(true);
      setError(null);

      try {
        const response = await fetch(
          `/api/venues/${venueId}/tables/${tableId}/reserve`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        setError(
          err instanceof Error ? err.message : 'Failed to reserve table'
        );
        throw err;
      } finally {
        setLoading(false);
