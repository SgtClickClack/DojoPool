...
export const useEnhancedVenueManagement = () => {
  ...
  const [optimization, setOptimization] = useState<RevenueOptimization | null>(
    null
  );
  ...
  const loadAnalytics = useCallback(async (venueId: string) => {
    try {
      ...
      const analyticsData =
        await enhancedVenueManagementService.getVenueAnalytics(venueId);
  ...
  const loadStatus = useCallback(async (venueId: string) => {
    try {
      ...
      const statusData =
        await enhancedVenueManagementService.getVenueStatus(venueId);
  ...
  const loadSchedules = useCallback(async (venueId: string) => {
    try {
      const schedulesData =
        await enhancedVenueManagementService.getTournamentSchedules(venueId);
  ...
  const loadOptimization = useCallback(async (venueId: string) => {
    try {
      ...
      const optimizationData =
        await enhancedVenueManagementService.getRevenueOptimization(venueId);
  ...
  const loadPerformance = useCallback(async (venueId: string) => {
    try {
      ...
      const performanceData =
        await enhancedVenueManagementService.getVenuePerformance(venueId);
  ...
  const updateVenue = useCallback(
    async (venueId: string, updates: Partial<Venue>) => {
      try {
        const updatedVenue = await enhancedVenueManagementService.updateVenue(
          venueId,
          updates
        );
        if (updatedVenue) {
          setVenues((prev) =>
            prev.map((v) => (v.id === venueId ? updatedVenue : v))
          );
  ...
  const updateTournamentSchedule = useCallback(
    async (scheduleId: string, updates: Partial<TournamentSchedule>) => {
      try {
        const updatedSchedule =
          await enhancedVenueManagementService.updateTournamentSchedule(
            scheduleId,
            updates
          );
        if (updatedSchedule) {
  ...
  const createTournament = useCallback(
    async (venueId: string, scheduleId: string) => {
      try {
        const result = await enhancedVenueManagementService.createTournament(
          venueId,
          scheduleId
        );
        if (result.success) {
  ...
  const resolveAlert = useCallback(
    async (venueId: string, alertId: string) => {
      try {
        const success = await enhancedVenueManagementService.resolveAlert(
          venueId,
          alertId
        );
        if (success) {
