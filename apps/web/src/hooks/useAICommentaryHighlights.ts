...
export const useAICommentaryHighlights =
  (): UseAICommentaryHighlightsReturn => {
    const [config, setConfig] = useState<AICommentaryConfig | null>(null);
    const [highlights, setHighlights] = useState<GeneratedHighlight[]>([]);
    const [metrics, setMetrics] = useState<CommentaryHighlightsMetrics | null>(
      null
    );
    ...
    const updateConfig = useCallback(
      async (newConfig: Partial<AICommentaryConfig>) => {
        try {
          setIsLoading(true);
          const response = await fetch(`${API_BASE}/config`, {
            method: 'PUT',
    ...
    const generateHighlights = useCallback(
      async (
        ...
        try {
          ...
          const errorMessage =
            err instanceof Error ? err.message : 'Error generating highlights';
    ...
    const matchHighlights = useCallback(
      (matchId: string): GeneratedHighlight[] => {
    ...
    const tournamentHighlights = useCallback(
      (tournamentId: string): GeneratedHighlight[] => {
    ...
    // Get user highlights
    const userHighlights = useCallback(
      (userId: string): GeneratedHighlight[] => {
      ...
      async (
        ...
        try {
          const response = await fetch(
            `${API_BASE}/highlights/${highlightId}/share`,
            {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
    ...
    // Download highlight
    const downloadHighlight = useCallback(
      async (highlightId: string): Promise<string> => {
        try {
          const response = await fetch(
            `${API_BASE}/highlights/${highlightId}/download`
          );
          const data = await response.json();
