...
import { type TrophyInfo } from '../../services/wallet/NFTWalletService';
...
export const NFTGallery: React.FC<NFTGalleryProps> = ({
  ...
  const filteredAndSortedTrophies = React.useMemo(() => {
    const filtered = userTrophies.filter((trophy) => {
      const matchesRarity =
        filterRarity === 'all' || trophy.rarity === filterRarity;
      const matchesType =
        filterType === 'all' || trophy.trophyType === filterType;
      const matchesSearch =
  ...
  const handleShare = (trophy: TrophyInfo) => {
    if (navigator.share) {
      ...
      navigator.clipboard.writeText(
        `${window.location.origin}/nft/${trophy.tokenId}`
      );
    }
  };
      ...
      <Box sx={{ position: 'relative' }}>
        ...
        <Chip
          ...
          sx={{
            ...
            left: 8,
            backgroundColor:
              rarityColors[trophy.rarity as keyof typeof rarityColors],
            color: 'white',
            fontWeight: 'bold',
          }}
        />
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <Chip
                  ...
                  sx={{
                    backgroundColor:
                      rarityColors[
                        selectedTrophy.rarity as keyof typeof rarityColors
                      ],
                    color: 'white',
                  }}
                />
            ...
            <DialogContent>
              <Grid container spacing={3}>
                <Grid item xs={12} md={6}>
                  <img
                    src={
                      selectedTrophy.imageURI || '/images/default-trophy.png'
                    }
                ...
                <Grid item xs={12} md={6}>
                  ...
                  <Stack spacing={1}>
                    ...
                    <Typography variant="body2">
                      ...
                      {new Date(selectedTrophy.mintedAt).toLocaleString()}
                    </Typography>
                    <Typography variant="body2">
                      <strong>Transferable:</strong>{' '}
                      {selectedTrophy.isTransferable ? 'Yes' : 'No'}
                    </Typography>
