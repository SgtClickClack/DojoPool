    ...
    if (isTraveling && currentMovement) {
      const interval = setInterval(() => {
        setTravelProgress((prev) => {
          const newProgress =
            prev + (100 / (currentMovement.estimatedDuration * 60)) * 0.1;
          if (newProgress >= 100) {
            completeTravel();
            return 100;
          }
            ...
            <Grid item xs={6}>
              <Box sx={{ textAlign: 'center' }}>
                <Typography variant="h6" color="primary">
                  {Math.ceil(
                    ((100 - travelProgress) / 100) *
                      currentMovement.estimatedDuration
                  )}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  Minutes Left
  ...
  return (
    <Card sx={{ maxWidth: 500, width: '100%' }}>
      <CardContent>
        ...
        <List>
          {nearbyDojos.map((dojo) => (
            ...
            >
              <ListItemIcon>{getStatusIcon(dojo.status)}</ListItemIcon>
              <ListItemText
                primary={dojo.name}
                secondary={
        ...
        <Alert severity="info" sx={{ mt: 2 }}>
          <Typography variant="body2">
            Click on a dojo to start traveling. Different travel methods have
            different costs and durations.
          </Typography>
        </Alert>
      </CardContent>
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle>Travel to {selectedDestination?.name}</DialogTitle>
        <DialogContent>
          ...
          <Grid container spacing={2} sx={{ mt: 2 }}>
            {travelOptions.map((option) => {
              const duration = calculateTravelTime(
                selectedDestination?.distance || 0,
                option.method
              );
              return (
                <Grid item xs={12} key={option.method}>
                  <Card
                    ...
                    sx={{
                      ...
                      borderColor:
                        selectedTravelMethod === option.method
                          ? option.color
                          : 'divider',
                      '&:hover': {
          ...
          <Alert severity="warning" sx={{ mt: 2 }}>
            <Typography variant="body2">
              Travel time is based on real-world distances. You'll be unable to
              perform other actions while traveling.
            </Typography>
          </Alert>
        </DialogContent>
        <DialogActions>
