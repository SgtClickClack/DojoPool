...
import {
  ...
  type Participant,
  TournamentStatus,
} from '@/types/tournament'; // Reverted: No Round/Match
...
const TournamentDetail: React.FC = () => {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // TODO: Show success feedback (Snackbar ideally)
      alert('Successfully registered!');
      // TODO: Optionally refresh tournament data to reflect new participant count/status
    } catch (err) {
      console.error('Registration error:', err);
      const message =
        err instanceof Error ? err.message : 'Failed to register.';
      setRegisterError(message);
      // TODO: Show error feedback (Snackbar ideally)
    ...
    <Paper sx={{ p: 3, m: 2 }}>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={6}>
          ...
          {/* TODO: Display Venue Info (requires fetching venue details based on venueId) */}
          {tournament.venueId && (
            <Typography>
              <strong>Venue ID:</strong> {tournament.venueId} (Details TBD)
            </Typography>
          )}
        </Grid>
