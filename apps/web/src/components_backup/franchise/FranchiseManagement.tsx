...
const FranchiseCard = styled(Card)(({
  ...
  const getStatusColor = (status: FranchiseStatus) => {
    switch (status) {
      case FranchiseStatus.ACTIVE:
        return '#00ff88';
      case FranchiseStatus.PENDING:
        return '#ffcc00';
      case FranchiseStatus.SUSPENDED:
        return '#ff0044';
      case FranchiseStatus.TERMINATED:
        return '#666666';
      case FranchiseStatus.EXPANSION:
        return '#00ccff';
      default:
        return '#666666';
    }
...
const getStatusIcon = (status: FranchiseStatus) => {
  switch (status) {
    ...
    case FranchiseStatus.TERMINATED:
      return <Error sx={{ color: '#666666' }} />;
    case FranchiseStatus.EXPANSION:
      return <TrendingUp sx={{ color: '#00ccff' }} />;
    default:
      return <Warning sx={{ color: '#ffcc00' }} />;
  }
};
...
const getTierColor = (tier: FranchiseTier) => {
  switch (tier) {
    case FranchiseTier.STARTER:
      return '#666666';
    case FranchiseTier.PROFESSIONAL:
      return '#00ccff';
    case FranchiseTier.ELITE:
      return '#00ff88';
    case FranchiseTier.MASTER:
      return '#ffcc00';
    case FranchiseTier.LEGENDARY:
      return '#ff0099';
    default:
      return '#666666';
  }
...
export const FranchiseManagement: React.FC<FranchiseManagementProps> = ({
  ...
  useEffect(() => {
    ...
    const unsubscribe = franchiseService.subscribe(
      'franchise_updated',
      (updatedFranchise: Franchise) => {
  ...
  if (!franchise) {
    return (
      <Box sx={{ p: 2 }}>
        <Alert
          severity="error"
          sx={{ background: neonColors.error, color: '#fff' }}
        >
          Franchise not found
        </Alert>
      </Box>
      ...
      {network && (
        <CyberpunkCard sx={{ mt: 3 }}>
          <CardContent>
            <Typography
              variant="h6"
              sx={{ color: neonColors.secondary, mb: 2, fontWeight: 'bold' }}
            >
              Venue Network
            </Typography>
            ...
            >
              <Typography
                variant="subtitle2"
                sx={{ color: neonColors.info, mb: 1, fontWeight: 'bold' }}
              >
                Network Performance Summary
              </Typography>
              <Grid container spacing={2}>
