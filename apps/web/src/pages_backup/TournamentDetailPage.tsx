...
const TournamentDetailPage: React.FC = () => {
  ...
  const [reportingScoreFor, setReportingScoreFor] = useState<number | null>(
    null
  ); // Track which match score is being reported
  const [scoreReportMessage, setScoreReportMessage] = useState<string | null>(
    null
  );
  ...
  const handleReportScore = async (matchId: number) => {
    ...
    if (!match || !match.player1_id || !match.player2_id) {
      setScoreReportError(
        'Cannot report score for this match (missing players).'
      );
      return;
    }
    ...
    const winnerInput = prompt(
      `Report winner for Match ${matchId} (P${match.player1_id} vs P${match.player2_id}): Enter winner ID (${match.player1_id} or ${match.player2_id})`
    );
    ...
    if (
      isNaN(winnerId) ||
      ![match.player1_id, match.player2_id].includes(winnerId)
    ) {
    ...
    const scoreInput = prompt(
      `Enter final score (e.g., 7-5) for Match ${matchId}:`
    );
    ...
    try {
      ...
      setScoreReportMessage(
        `Score for Match ${matchId} reported successfully!`
      );
      fetchTournamentDetails(); // Refresh data to show updated match status/bracket
    } catch (err) {
      setScoreReportError(
        err instanceof Error ? err.message : 'Score reporting error'
      );
    } finally {
  ...
  if (!tournament)
    return (
      <Layout>
        <div>Tournament data not available.</div>
      </Layout>
    );
    ...
    <Layout>
      <div>
        ...
        {tournament.max_participants && (
          <p>
            <strong>Max Players:</strong> {tournament.max_participants}
          </p>
        )}
        {tournament.entry_fee && (
          <p>
            <strong>Entry Fee:</strong> ${tournament.entry_fee.toFixed(2)}
          </p>
        )}
        {tournament.prize_pool && (
          <p>
            <strong>Prize Pool:</strong> ${tournament.prize_pool.toFixed(2)}
          </p>
        )}
