...
import { getWallet, type Wallet } from '../../api/walletApi';
...
const TournamentList: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchWallet = async () => {
      ...
      try {
        ...
        setWalletError(
          err.response?.data?.error || err.message || 'Failed to load wallet.'
        );
      } finally {
  ...
  const handleConfirmJoin = async () => {
    ...
    try {
      await tournamentApi.joinTournament(String(selectedTournament.id));
      setJoinSuccess(
        `Joined tournament ${selectedTournament.id} successfully!`
      );
      // Optionally refetch tournaments
      fetchTournaments();
    } catch (err: any) {
      setError(
        err.response?.data?.error || err.message || 'Failed to join tournament.'
      );
    } finally {
      setJoiningId(null);
      ...
      {!loading && !error && (
        <List>
          {tournaments.length === 0 ? (
            <ListItem>
              <ListItemText primary="No tournaments found." />
            </ListItem>
          ) : (
            tournaments.map((t) => (
              <ListItem
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            {selectedTournament ? (
              <>
                Are you sure you want to join <b>{selectedTournament.name}</b>?
                <br />
                Entry Fee:{' '}
                <b>
                  {'entry_fee' in selectedTournament
