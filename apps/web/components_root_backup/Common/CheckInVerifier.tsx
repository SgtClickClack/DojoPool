...
const CheckInVerifier: React.FC<CheckInVerifierProps> = ({ tournamentId }) => {
  ...
  const verifyCheckIn = async () => {
    if (!tournament || !user || !scannedCode) return;

    setVerifying(true);
    setVerificationError(null);

    try {
      // For now, skip geolocation verification since tournament type doesn't have venue coordinates
      // TODO: Update tournament type to include venue coordinates
      ...
      await fetch(
        `/api/users/${user.id}/tournaments/${tournament.id}/checkin`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          <Typography variant="body1">Type: {tournament.type}</Typography>
      ...
      {locationError ? (
        <Alert severity="error" sx={{ mb: 3 }}>
          {locationError?.message || 'Location access error'}
        </Alert>
      ) : (
        <Card sx={{ mb: 3 }}>
          <CardContent>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)}>
        ...
        <DialogContent>
          <Typography gutterBottom>Scanned Code: {scannedCode}</Typography>
