...
import {
  ...
  ListItemText,
  IconButton,
  type SelectChangeEvent, // Added for Select onChange type
} from '@mui/material';
import { useNavigate, useParams } from 'react-router-dom';
import { useTournament } from '../hooks/useTournament'; // Assuming this fetches tournament details
import { useUser } from '../hooks/useUser'; // Fixed import path
...
const TournamentRegister: React.FC<TournamentRegisterProps> = ({
  tournamentId,
}) => {
  ...
  useEffect(() => {
    if (tournament?.divisions && tournament.divisions.length > 0) {
      ...
      // Tournament loaded but no divisions
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.teamMembers.length < MIN_TEAM_MEMBERS) {
      setRegistrationError(
        `At least ${MIN_TEAM_MEMBERS} team member(s) are required.`
      );
      return false;
    }
    if (!tournament?.divisions?.includes(formData.division)) {
      setRegistrationError(
        'Invalid division selected. Please choose a valid division.'
      );
      return false;
    }
  ...
  const handleRegister = async () => {
    if (!tournament || !user) {
      // Ensure user context is also loaded/available if needed for registration logic
  ...
  if (tournamentError) {
    return (
      <Container
        maxWidth="lg"
        sx={{ mt: 4, mb: 4, display: 'flex', justifyContent: 'center', py: 4 }}
      >
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={5}>
          {' '}
          {/* Adjusted grid size for potentially better layout */}
          <Card sx={{ p: { xs: 1, sm: 2 } }}>
            <CardContent>
              ...
              <Typography variant="body1" gutterBottom>
                <strong>Entry Fee:</strong> {tournament.entryFee ?? 'N/A'} Dojo
                Coins
              </Typography>
              {tournament.divisions && tournament.divisions.length > 0 && (
                <Typography variant="body1" gutterBottom>
                  <strong>Available Divisions:</strong>{' '}
                ...
                <Grid item xs={12}>
                  ...
                  <Box display="flex" alignItems="center" gap={1}>
                    ...
                    <Button
                      ...
                      sx={{ height: '56px' /* Align with TextField */ }}
                    >
                      Add Member
                    </Button>
                  </Box>
                    ...
                    >
                      ...
                      tournament.divisions.length > 0 ? (
                        ...
                        <MenuItem value={DEFAULT_DIVISION} disabled>
                          {DEFAULT_DIVISION.charAt(0).toUpperCase() +
                            DEFAULT_DIVISION.slice(1)}{' '}
                          (default)
                        </MenuItem>
                      )}
                    </Select>
    ...
    return (
      <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
        <Alert severity="error">
          Tournament ID is missing. Please check the URL.
        </Alert>
      </Container>
    );
  }
