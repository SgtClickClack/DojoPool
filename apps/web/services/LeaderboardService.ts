...
export async function getLeaderboard(
  ...
  const response = await apiClient.get('/leaderboard', { params: filters });
...
export async function getStyleLeaders(
  ...
  const response = await apiClient.get(`/leaderboard/style/${style}`);
  return response.data;
}
...
export async function getRegionalLeaderboard(
  ...
  const response = await apiClient.get(`/leaderboard/region/${region}`);
  return response.data;
}
...
export async function getLeaderboardHistory(userId: string): Promise<{
  ...
  const response = await apiClient.get(`/leaderboard/history/${userId}`);
  return response.data;
}
...
export async function subscribeToLeaderboardUpdates(
  ...
  await apiClient.post('/leaderboard/subscribe', { userId });
}

export async function unsubscribeFromLeaderboardUpdates(
  userId: string
): Promise<void> {
  await apiClient.post('/leaderboard/unsubscribe', { userId });
}
...
export async function getChallengeLeaderboard(challengeId: string): Promise<{
  ...
  const response = await apiClient.get(`/leaderboard/challenge/${challengeId}`);
  return response.data;
}
