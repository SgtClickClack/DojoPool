...
export class PerformanceMonitor {
  ...
  public getMetrics(startTime: number, endTime: number): MetricsSnapshot {
    ...
    const toMetricData = (values: number[], labels: string[]): MetricData[] => {
      return values.map((value, index) => ({
        timestamp: filteredMetrics[index].timestamp,
        value,
        label: labels[index],
    ...
    return {
      ...
      historical: {
        ...
        memoryAvailable: toMetricData(
          filteredMetrics.map((m) => m.memoryAvailable),
          filteredMetrics.map(
            (m) => `${(m.memoryAvailable / 1024).toFixed(1)} GB`
          )
        ),
      },
      ...
      updateTimes: toMetricData(
        ...
        filteredMetrics.map((m) => new Date(m.timestamp).toISOString())
      ),
      successRate: filteredMetrics.map(() => 100),
      timestamp: Date.now(),
    };
  }
}
