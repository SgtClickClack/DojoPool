...
const getDeviceIcon = (type: HardwareDeviceType) => {
  switch (type) {
    case HardwareDeviceType.CAMERA:
      return <Videocam />;
    case HardwareDeviceType.SENSOR:
      return <Sensors />;
    case HardwareDeviceType.PROCESSOR:
      return <Memory />;
    case HardwareDeviceType.DISPLAY:
      return <DisplaySettings />;
    case HardwareDeviceType.NETWORK:
      return <NetworkCheck />;
    case HardwareDeviceType.AUDIO:
      return <VolumeUp />;
    case HardwareDeviceType.LIGHTING:
      return <Lightbulb />;
    default:
      return <Settings />;
  }
...
const getStatusIcon = (status: DeviceStatus) => {
  switch (status) {
    ...
    case DeviceStatus.OFFLINE:
      return <Error sx={{ color: '#ff0044' }} />;
    case DeviceStatus.MAINTENANCE:
      return <Build sx={{ color: '#ffcc00' }} />;
    case DeviceStatus.ERROR:
      return <Error sx={{ color: '#ff0044' }} />;
    case DeviceStatus.CALIBRATING:
      return <Refresh sx={{ color: '#00ccff' }} />;
    default:
      return <Warning sx={{ color: '#ffcc00' }} />;
  }
};

export const VenueHardwareIntegration: React.FC<
  VenueHardwareIntegrationProps
> = ({ venueId = 'venue_001' }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeDeviceStatus = venueHardwareService.subscribe(
    ...
    const unsubscribeSensorData = venueHardwareService.subscribe(
      'sensor_data',
      (data: SensorData) => {
    ...
    const unsubscribeCameraData = venueHardwareService.subscribe(
      'camera_data',
      (data: CameraData) => {
        if (data.deviceId === selectedDevice?.id) {
  ...
  const handleUpdateConfig = async (config: any) => {
    ...
    try {
      await venueHardwareService.updateDeviceConfiguration(
        selectedDevice.id,
        config
      );
    } catch (error) {
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={6} md={3}>
                  <Box>
                    ...
                    <Typography variant="h6" sx={{ color: neonColors.primary }}>
                      {(status.performance.calibrationAccuracy * 100).toFixed(
                        1
                      )}
                      %
                    </Typography>
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 1, mb: 1 }}>
                      <Tooltip title="Temperature">
                        <Thermostat
                          sx={{
                            color: getHealthColor(device.health.temperature, {
                              low: 40,
                              medium: 60,
                              high: 70,
                            }),
                      ...
                      <Tooltip title="CPU Usage">
                        <Memory
                          sx={{
                            color: getHealthColor(device.health.cpuUsage, {
                              low: 30,
                              medium: 60,
                              high: 80,
                            }),
                      ...
                      <Tooltip title="Memory Usage">
                        <Storage
                          sx={{
                            color: getHealthColor(device.health.memoryUsage, {
                              low: 50,
                              medium: 70,
                              high: 90,
                            }),
        ...
        <CyberpunkCard>
          <CardContent>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
              ...
              <Typography
                variant="h6"
                sx={{ color: neonColors.primary, ml: 1, flex: 1 }}
              >
                  ...
                  <Box>
                    ...
                    >
                      ...
                      <Typography
                        ...
                        sx={{
                          color: getHealthColor(
                            selectedDevice.health.temperature,
                            { low: 40, medium: 60, high: 70 }
                          ),
                        }}
                      >
                    ...
                    <TableBody>
                      {cameraData.slice(-5).map((data, index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell sx={{ color: 'text.primary' }}>
                            {data.analysis.processing.processingTime.toFixed(1)}
                            ms
                          </TableCell>
                        </TableRow>
                      ))}
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body2" sx={{ color: 'text.secondary', mb: 2 }}>
            This will calibrate the {selectedDevice?.model} device. The process
