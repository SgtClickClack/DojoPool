...
import {
  ...
  FaArrowUp,
  FaInfoCircle,
} from 'react-icons/fa'; // Added FaInfoCircle for default
import { type IconType } from 'react-icons'; // For typing the icon map
...
const PerformanceInsights: React.FC<PerformanceInsightsProps> = ({
  ...
  if (!insights || insights.length === 0) {
    return (
      <Box mt={4}>
        {' '}
        {/* Added mt if this component might stand alone */}
    ...
    <Box>
      ...
      <VStack align="stretch">
        {insights.map((insight, index) => {
          const visuals =
            insightVisuals[insight.type] || insightVisuals.neutral; // Fallback to neutral
          ...
          return (
            ...
            >
              <Flex alignItems="flex-start">
                {' '}
                {/* Align icon to the top with text */}
