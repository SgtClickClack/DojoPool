...
const POCKET_POSITIONS = [
  { x: 0, y: 0 },
  { x: TABLE_VIS_WIDTH / 2, y: 0 },
  { x: TABLE_VIS_WIDTH, y: 0 },
  { x: 0, y: TABLE_VIS_HEIGHT },
  { x: TABLE_VIS_WIDTH / 2, y: TABLE_VIS_HEIGHT },
  { x: TABLE_VIS_WIDTH, y: TABLE_VIS_HEIGHT },
];
    ...
    const fetchGameData = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.message ||
              `Failed to fetch game analysis (HTTP ${response.status})`
          );
        }
          ...
          <Layer>
            ...
            {POCKET_POSITIONS.map((pocket, i) => (
              <KonvaCircle
                ...
                fill={useColorModeValue('gray.700', 'black')}
              />
            ))}
  ...
  const renderPerformanceStats = () => {
    ...
    const statItems = [
      ...
      },
      {
        ...
        icon: FaChartLine,
        change: calculateChangeDisplay(
          stats.avg_difficulty,
          stats.previous_avg_difficulty
        ),
      },
      {
        label: 'Position Play',
        ...
        icon: FaRegDotCircle,
        change: calculateChangeDisplay(
          stats.position_score,
          stats.previous_position_score
        ),
      },
    ];
      ...
      <Box p={5} bg={bgColor}>
        <Alert status="info" borderRadius="md">
          ...
          <Box>
            ...
            <Text>
              There is no analysis data available for game ID: {gameId}.
            </Text>
          </Box>
        </Alert>
      </Box>
        ...
        <Box bg={cardBgColor} p={5} borderRadius="lg" boxShadow="md">
          ...
          >
            ...
            <Text mx={3} color={textColor} fontSize="sm" whiteSpace="nowrap">
              Shot {shots.length > 0 ? currentShotIndex + 1 : 0} of{' '}
              {shots.length}
            </Text>
            <Tooltip label="Next Shot" placement="top">
              <IconButton
                ...
                isDisabled={
                  currentShotIndex >= shots.length - 1 || isPlayingReplay
                }
              />
            </Tooltip>
            {!isPlayingReplay ? (
          ...
          {(!insights || insights.length === 0) && !isLoading && (
            <Box bg={cardBgColor} p={5} borderRadius="lg" boxShadow="md">
              <Text color={textColor}>
                No specific performance insights generated for this game.
              </Text>
            </Box>
          )}
        </Box>
