...
const RealTimeMatchTracker: React.FC<RealTimeMatchTrackerProps> = ({
  ...
  const simulateMatchProgress = () => {
    ...
    const randomPlayer =
      Math.random() > 0.5 ? matchState.player1.id : matchState.player2.id;
  ...
  const getEventDescription = (type: string, playerId: string): string => {
    const playerName =
      playerId === matchState?.player1.id
  ...
  const endMatch = () => {
    ...
    const matchWinner =
      finalScore.player1 > finalScore.player2
        ? matchState.player1.id
        : matchState.player2.id;
    const winnerName =
      matchWinner === matchState.player1.id
        ? matchState.player1.name
        : matchState.player2.name;
    ...
    <Card sx={{ maxWidth: 800, width: '100%' }}>
      ...
      <Dialog open={showEndDialog} onClose={() => setShowEndDialog(false)}>
        ...
        <DialogContent>
          ...
          <Alert severity="warning" sx={{ mt: 2 }}>
            <Typography variant="body2">
              This action cannot be undone. The match will be recorded with the
              current scores.
            </Typography>
          </Alert>
        </DialogContent>
        <DialogActions>
          ...
          <Button variant="contained" color="error" onClick={endMatch}>
            End Match
          </Button>
        </DialogActions>
      </Dialog>
      ...
      {/* Winner Dialog */}
      <Dialog
        open={showEndDialog && !!winner}
        onClose={() => setShowEndDialog(false)}
      >
        <DialogTitle sx={{ textAlign: 'center' }}>
          <EmojiEvents color="primary" sx={{ fontSize: 40, mb: 1 }} />
          <Typography variant="h5">Match Complete!</Typography>
        </DialogTitle>
        <DialogContent>
          <Box sx={{ textAlign: 'center' }}>
            ...
            <Typography variant="body1" color="text.secondary">
              Final Score: {matchState.player1.score} -{' '}
              {matchState.player2.score}
            </Typography>
        ...
        <DialogActions>
          <Button onClick={() => setShowEndDialog(false)}>Close</Button>
        </DialogActions>
      </Dialog>
    </Card>
  );
