  ...
  useEffect(() => {
    ...
    gameState.players.forEach((player) => {
      ...
      if (playerShots.length > 1) {
        const timeDiffs = playerShots
          .slice(1)
          .map(
            (shot, i) =>
              new Date(shot.timestamp).getTime() -
              new Date(playerShots[i].timestamp).getTime()
          );
        avgShotTime =
      ...
      const avgDifficulty =
        playerShots.length > 0
          ? playerShots.reduce(
              (sum, shot) => sum + (shot.analysis?.difficulty || 0),
              0
            ) / playerShots.length
          : 0;
      ...
      const aiRating = calculateAIRating({
        ...
        avgDifficulty,
        avgShotTime,
        fouls: gameState.rules.fouls.filter((f) => f.includes(player.id))
          .length,
      });
      ...
      stats[player.id] = {
        ...
        accuracy:
          playerShots.length > 0
            ? (successfulShots.length / playerShots.length) * 100
            : 0,
        avgShotTime,
        bestStreak,
        currentStreak,
        fouls: gameState.rules.fouls.filter((f) => f.includes(player.id))
          .length,
        safetyShots: playerShots.filter(
          (s) => s.analysis?.aiScore && s.analysis.aiScore < 0.3
        ).length,
        avgDifficulty,
        aiRating,
      };
  ...
  useEffect(() => {
    ...
    if (recentShots.length >= 3) {
      const recentAccuracy =
        recentShots.filter((s) => s.success).length / recentShots.length;
    ...
    if (lastShot && !lastShot.success) {
      insights.push({
        ...
        type: 'tip',
        message:
          'Consider a safety shot to leave your opponent in a difficult position',
        confidence: 0.7,
        timestamp: new Date(),
      });
    }
    ...
    if (recentFouls.length >= 2) {
      insights.push({
        ...
        type: 'warning',
        message:
          'Multiple fouls detected. Focus on clean shots to avoid penalties',
        confidence: 0.9,
        timestamp: new Date(),
      });
    }
          ...
          <Grid item xs={12} md={6} key={stats.playerId}>
            ...
            >
              <CardContent>
                ...
                <Divider
                  sx={{ my: 2, borderColor: alpha(neonColors.info, 0.3) }}
                />
