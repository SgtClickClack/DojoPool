...
const EarnedNftsDisplay: React.FC<EarnedNftsDisplayProps> = ({
  nfts,
  onNftClick,
  onActionClick,
}) => {
    ...
    <Paper sx={{ p: 2, mt: 2 }}>
      ...
      <List dense>
        {nfts.length === 0 ? (
          ...
          nfts.map((nft) => (
            <ListItem key={nft.id} button onClick={() => onNftClick(nft)}>
              {nft.imageUrl && (
                <ListItemAvatar>
