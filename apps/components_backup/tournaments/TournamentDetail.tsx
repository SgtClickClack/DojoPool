...
import {
  ...
  TournamentStatus,
  type Match,
} from '@/types/tournament'; // Import Match
import { type Venue } from '@/dojopool/frontend/types/venue'; // Adjusted path
// Auth Hook
...
const ParticipantsList: React.FC<ParticipantsListProps> = ({
  ...
  return (
    <List
      dense
      sx={{ background: 'rgba(10, 10, 10, 0.8)', borderRadius: 1, p: 1 }}
    >
      {participants.map((p) => (
        <ListItem
          key={p.id}
              ...
              <Card sx={cyberCardStyle}>
                <Typography
                  ...
                  sx={{
                    ...
                    mb: 2,
                    color:
                      roundIdx === rounds.length - 1 ? '#ffff00' : '#00a8ff',
                  }}
                >
                ...
                {roundMatches.map((match) => (
                  ...
                  >
                    ...
                    >
                      {match.participant1?.username || 'TBD'} vs{' '}
...
const TournamentDetail: React.FC = () => {
  ...
  const renderActionButtons = () => {
    ...
    const isRegistered = tournament.participantsList?.some(
      (p) => p.id === user.uid
    );
        ...
        <Box sx={{ position: 'relative', zIndex: 1 }}>
          <Card sx={cyberCardStyle}>
            ...
            >
              ...
              {!loading &&
                registrationStep === RegistrationStep.IDLE &&
                renderActionButtons()}
            </Box>
            ...
            {registrationStep === RegistrationStep.CONFIRM_DETAILS && (
              ...
              >
                <Typography
                  variant="h5"
                  sx={{ ...neonTextStyle, mb: 2, color: '#00a8ff' }}
                >
                ...
                <Box sx={{ display: 'flex', gap: 2 }}>
                  <Button
                    sx={cyberButtonStyle}
                    onClick={() =>
                      setRegistrationStep(RegistrationStep.CONFIRM_RULES)
                    }
                  >
                    Next
                  </Button>
              ...
              >
                ...
                <Box sx={{ display: 'flex', gap: 2 }}>
                  <Button
                    sx={cyberButtonStyle}
                    onClick={() =>
                      setRegistrationStep(RegistrationStep.PAYMENT)
                    }
                  >
                    Accept & Continue
                  </Button>
                  <Button
                    variant="outlined"
                    onClick={() =>
                      setRegistrationStep(RegistrationStep.CONFIRM_DETAILS)
                    }
                    sx={{
                      borderColor: '#888',
                      color: '#888',
              ...
              >
                ...
                {tournament.entryFee !== undefined ? (
                  ...
                  >
                    Entry Fee:{' '}
                    <strong>
                      {tournament.entryFee} {tournament.currency || 'coins'}
                    </strong>
                  </Typography>
                ) : (
            ...
            <Card sx={{ ...cyberCardStyle }}>
              <Typography
                variant="h5"
                sx={{ ...neonTextStyle, mb: 3, color: '#00ff9d' }}
              >
                Video Highlights
              </Typography>
        ...
        <DialogContent>
          ...
          >
            ...
            {reportingMatch?.participant1 && (
              <MenuItem
                value={reportingMatch.participant1.id}
                sx={{ color: '#fff' }}
              >
            ...
            {reportingMatch?.participant2 && (
              <MenuItem
                value={reportingMatch.participant2.id}
                sx={{ color: '#fff' }}
              >
