...
export const TournamentMobileIntegration: React.FC<
  TournamentMobileIntegrationProps
> = ({ tournamentId }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeSettings = tournamentMobileService.subscribe(
      'settings_updated',
      (newSettings) => {
    ...
    const unsubscribeNotifications = tournamentMobileService.subscribe(
    ...
    const unsubscribeSync = tournamentMobileService.subscribe(
      'sync_status_changed',
      (status) => {
    ...
    const unsubscribeOnline = tournamentMobileService.subscribe(
      ...
      <CyberpunkCard sx={{ mb: 3 }}>
        <CardContent>
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
            ...
            <Typography
              variant="h6"
              sx={{ color: isOnline ? neonColors.primary : neonColors.error }}
            >
          ...
          <Box sx={{ display: 'flex', gap: 1 }}>
            ...
            <NeonButton
              startIcon={<Download />}
              onClick={() =>
                tournamentMobileService.cacheTournamentData([], [])
              }
              disabled={!isOnline}
              size="small"
              sx={{
          ...
          {syncStatus?.error && (
            <Alert
              severity="error"
              sx={{ mt: 2, background: neonColors.error, color: '#fff' }}
            >
              {syncStatus.error}
            </Alert>
          )}
      ...
      <CyberpunkCard sx={{ mb: 3 }}>
        <CardContent>
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
            ...
            <Typography
              variant="h6"
              sx={{ color: neonColors.secondary, flex: 1 }}
            >
              Push Notifications
            </Typography>
            <IconButton
