...
const BracketVisualization: React.FC = () => {
  ...
  const handleMatchUpdate = () => {
    if (selectedNode && matchUpdate) {
      ...
      if (bracket) {
        ...
        const nodeIndex = updatedBracket.nodes.findIndex(
              ...
              >
                <ListItemIcon>
                  {node.status === 'completed' && (
                    <CheckCircle sx={{ color: '#00ff9d' }} />
                  )}
                  {node.status === 'in-progress' && (
                    <SportsEsports sx={{ color: '#feca57' }} />
                  )}
                  {node.status === 'pending' && (
                    <Schedule sx={{ color: '#00a8ff' }} />
                  )}
                </ListItemIcon>
                <ListItemText
                  primary={
  ...
  return (
    <Box sx={{ p: 4 }}>
              ...
              <Grid item xs={12} md={6}>
                ...
                <Slider
                  ...
                  onChange={(_, value) =>
                    setTempConfig({
                      ...tempConfig,
                      cameraDistance: value as number,
                    })
                  }
                  min={10}
                  max={30}
      ...
      >
        <DialogTitle
          sx={{ color: '#feca57', fontFamily: 'Orbitron, monospace' }}
        >
          Update Match
        </DialogTitle>
        <DialogContent>
                    ...
                    onChange={(e) =>
                      setMatchUpdate({
                        ...
                        player2Score: parseInt(
                          matchUpdate?.player2Score?.toString() ||
                            selectedNode.score?.split('-')[1] ||
                            '0'
                        ),
                    ...
                    onChange={(e) =>
                      setMatchUpdate({
                        ...
                        player1Score: parseInt(
                          matchUpdate?.player1Score?.toString() ||
                            selectedNode.score?.split('-')[0] ||
                            '0'
                        ),
