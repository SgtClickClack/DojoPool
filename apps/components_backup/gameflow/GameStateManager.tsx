...
type GameAction =
  ...
  | {
      type: 'UPDATE_PLAYER';
      payload: { playerId: string; updates: Partial<Player> };
    }
...
export const GameStateProvider: React.FC<GameStateProviderProps> = ({
  ...
  useEffect(() => {
    ...
    newSocket.on(
      'aiTrackingUpdate',
      (tracking: Partial<GameState['aiTracking']>) => {
  ...
  const switchTurn = useCallback(() => {
    const nextPlayerId =
      gameState.players.find((p) => p.id !== gameState.currentPlayerId)?.id ||
      '';
    dispatch({ type: 'SWITCH_TURN', payload: { nextPlayerId } });
  ...
  const reportFoul = useCallback(
    (playerId: string, foulType: string) => {
...
// Utility functions
export const getPlayerBallType = (
  playerId: string,
  gameState: GameState
): 'solid' | 'stripe' | null => {
  ...
  const playerShots = gameState.shots.filter(
    (s) => s.playerId === playerId && s.success
  );
...
export const isPlayerTurn = (
  ...
  return (
    gameState.currentPlayerId === playerId && gameState.status === 'in-progress'
  );
};

export const canShoot8Ball = (
  playerId: string,
  gameState: GameState
): boolean => {
  ...
  const remainingBalls = gameState.balls.filter(
    (b) => b.type === ballType && !b.isPocketed
  );
...
export const calculateScore = (
  ...
  return gameState.shots.filter((s) => s.playerId === playerId && s.success)
    .length;
};
