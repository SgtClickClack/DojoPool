...
const DebugPanel: React.FC = () => {
  ...
  const [networkStatus] = useState(() => networkStatusService);
  ...
  const StatusIndicator = useMemo(() => {
    ...
    const networkQuality = networkStatus.getConnectionQuality();
    const connectionInfo = networkStatus.getStatus();
    const latestMetrics = {
      latency: 0,
      bandwidth: 0,
      packetLoss: 0,
    ...
    return (
      <Box display="flex" alignItems="center" gap={2}>
        ...
        <Tooltip title={`Network: ${networkQuality} (${connectionInfo.effectiveType || 'unknown'})`}>
  ...
  // No cleanup needed for NetworkStatus service
