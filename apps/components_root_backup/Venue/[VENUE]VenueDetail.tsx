...
import { type Venue, TableStatus, TableType } from '../../types/venue';
        ...
        <Grid item xs={12} md={4}>
          ...
          <Paper sx={{ p: 3, mb: 3 }}>
            ...
            {venue.businessHours.map((hours) => (
              ...
              >
                <Typography variant="body2">{getDayName(hours.day)}</Typography>
                <Typography variant="body2">
            ...
            {venue.pricing.map((price, index) => (
              <Box key={index} sx={{ mb: 2 }}>
                ...
                <Typography variant="h6" color="primary">
                  ...
                  {price.type.toLowerCase().replace('_', ' ')}
                </Typography>
                {price.peakHours && (
                  <Typography variant="body2" color="text.secondary">
                    Peak hours ({price.peakHours.start} - {price.peakHours.end}
                    ): {price.amount * price.peakHours.multiplier}{' '}
                    {price.currency}
                  </Typography>
                )}
              </Box>
