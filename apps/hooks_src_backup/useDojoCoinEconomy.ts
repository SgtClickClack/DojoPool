...
export const useDojoCoinEconomy = (userId: string) => {
  ...
  const loadEconomyData = useCallback(async () => {
    try {
      ...
      const transactions = dojoCoinEconomyService.getUserTransactions(
        userId,
        50
      );
    ...
    async (
      ...
      try {
        ...
        setState((prev) => ({
          ...prev,
          error:
            error instanceof Error ? error.message : 'Failed to earn coins',
        }));
        throw error;
      }
    },
    ...
    async (
      ...
      try {
        ...
        setState((prev) => ({
          ...prev,
          error:
            error instanceof Error ? error.message : 'Failed to spend coins',
        }));
        throw error;
      }
    },
