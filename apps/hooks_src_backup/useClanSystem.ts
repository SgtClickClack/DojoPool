...
export const useClanSystem = () => {
  ...
  const promoteMember = useCallback(
    async (memberId: string, newRole: ClanMember['role']) => {
      setError(null);
      try {
        const success = await clanSystemService.promoteMember(
          memberId,
          newRole
        );
        if (success) {
  ...
  const submitWarMatch = useCallback(
    async (
      warId: string,
      matchData: {
        player1Id: string;
      ...
      setError(null);
      try {
        const success = await clanSystemService.submitWarMatch(
          warId,
          matchData
        );
        if (!success) {
  ...
  const getTopClans = useCallback(
    async (limit: number = 10): Promise<Clan[]> => {
      try {
