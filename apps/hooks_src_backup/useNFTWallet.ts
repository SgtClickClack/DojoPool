...
import {
  ...
  type TrophyInfo,
} from '../services/wallet/NFTWalletService';
...
export const useNFTWallet = (): UseNFTWalletReturn => {
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
      await Promise.all([refreshBalance(), refreshTrophies()]);
    } catch (err) {
  ...
  const refreshTrophies = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to refresh trophies'
      );
    }
  }, [isConnected]);
  ...
  const mintTerritoryTrophy = useCallback(
    async (
      territoryId: string,
      territoryName: string,
      rarity: number
      ...
      try {
        const tokenId = await nftWalletService.mintTerritoryTrophy(
          territoryId,
          territoryName,
          rarity
        );
  ...
  const onConnected = useCallback(
    (callback: (connection: WalletConnection) => void) => {
