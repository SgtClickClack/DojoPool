...
import {
  ...
  type TerritoryIncome,
  type IncomePayout,
  type PassiveIncomeConfig,
} from '../services/economy/PassiveIncomeService';
...
export const usePassiveIncome = (): UsePassiveIncomeReturn => {
  ...
  const loadTerritoryIncomes = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'Failed to load territory incomes';
  ...
  const registerTerritory = useCallback(
    async (
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to register territory';
        setError(errorMessage);
        throw err;
      }
    },
  ...
  const updateVenueActivity = useCallback(
    async (territoryId: string, activity: any): Promise<void> => {
      setError(null);

      try {
        const response = await fetch(
          `/api/passive-income/territories/${territoryId}/activity`,
          {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : 'Failed to update venue activity';
        setError(errorMessage);
        throw err;
      }
    },
  ...
  const getTerritoryIncome = useCallback(
    (territoryId: string): TerritoryIncome | null => {
      return (
        territoryIncomes.find((t) => t.territoryId === territoryId) || null
      );
    },
    [territoryIncomes]
  );
  ...
  const getPayoutHistory = useCallback(
    (territoryId: string): IncomePayout[] => {
      return payoutHistory.get(territoryId) || [];
    },
    [payoutHistory]
  );

  const getUserTotalEarnings = useCallback(
  ...
  const updateConfig = useCallback(
    async (newConfig: Partial<PassiveIncomeConfig>): Promise<void> => {
      setError(null);
  ...
  const startSystem = useCallback(async (): Promise<void> => {
    ...
    try {
      const response = await fetch('/api/passive-income/start', {
        method: 'POST',
      });
      if (!response.ok)
        throw new Error('Failed to start passive income system');

      const data = await response.json();
      if (!data.success) {
        throw new Error(
          data.message || 'Failed to start passive income system'
        );
      }
    } catch (err) {
      const errorMessage =
        err instanceof Error
          ? err.message
          : 'Failed to start passive income system';
      setError(errorMessage);
      throw err;
    }
  }, []);
  ...
  const stopSystem = useCallback(async (): Promise<void> => {
    ...
    try {
      const response = await fetch('/api/passive-income/stop', {
        method: 'POST',
      });
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : 'Failed to stop passive income system';
      setError(errorMessage);
      throw err;
    }
  }, []);
  ...
  const refreshData = useCallback(async (): Promise<void> => {
    await Promise.all([loadTerritoryIncomes(), loadConfig(), loadStats()]);
