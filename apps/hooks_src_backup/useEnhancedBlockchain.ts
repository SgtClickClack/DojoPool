...
import {
  ...
  type DojoCoinBalance,
  type CrossChainTransaction,
  type NFTMarketplaceItem,
} from '../services/blockchain/EnhancedBlockchainService';
...
export const useEnhancedBlockchain = (): UseEnhancedBlockchainReturn => {
  const [balances, setBalances] = useState<Map<string, DojoCoinBalance>>(
    new Map()
  );
  const [crossChainTransactions, setCrossChainTransactions] = useState<
    Map<string, CrossChainTransaction>
  >(new Map());
  ...
  const getBalance = useCallback(
    ...
    ): Promise<DojoCoinBalance> => {
      setLoadingBalances(true);
      setError(null);

      try {
        const balance = await enhancedBlockchainService.getDojoCoinBalance(
          userId,
          network
        );
        setBalances((prev) =>
          new Map(prev).set(`${userId}-${network}`, balance)
        );
        return balance;
      } catch (err) {
        const errorMessage =
  ...
  // Cross-Chain Operations
  const initiateCrossChainTransfer = useCallback(
    async (
      fromUserId: string,
      toUserId: string,
      amount: string,
      ...
      try {
        const transaction =
          await enhancedBlockchainService.initiateCrossChainTransfer(
            fromUserId,
            toUserId,
            amount,
            fromNetwork,
        ...
        setCrossChainTransactions((prev) =>
          new Map(prev).set(transaction.id, transaction)
        );
        return transaction;
      } catch (err) {
        const errorMessage =
          err instanceof Error ? err.message : 'Cross-chain transfer failed';
        setError(errorMessage);
        throw err;
      } finally {
  ...
  const getCrossChainTransaction = useCallback(
    async (transactionId: string): Promise<CrossChainTransaction | null> => {
      try {
        const transaction =
          await enhancedBlockchainService.getCrossChainTransaction(
            transactionId
          );
        if (transaction) {
          setCrossChainTransactions((prev) =>
            new Map(prev).set(transactionId, transaction)
          );
        }
        return transaction;
      } catch (err) {
  ...
  const buyNFT = useCallback(
    async (
      ...
      try {
        const result = await enhancedBlockchainService.buyNFT(
          itemId,
          buyer,
          paymentMethod
        );
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : 'Failed to get marketplace items';
        setError(errorMessage);
        return [];
      } finally {
        setLoadingMarketplace(false);
  ...
  const mintNFT = useCallback(
    async (
      ...
      try {
        return await enhancedBlockchainService.mintNFT(
          to,
          tokenId,
          metadata,
          network
  ...
  const refreshAnalytics = useCallback(async () => {
    ...
    try {
      const analyticsData =
        await enhancedBlockchainService.getBlockchainAnalytics();
