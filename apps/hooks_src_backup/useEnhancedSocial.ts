...
interface UseEnhancedSocialReturn {
  ...
  createChallenge: (
    challenge: Omit<CommunityChallenge, 'id' | 'participants' | 'status'>
  ...
  submitChallengeEntry: (
    ...
    submission: Omit<
      ChallengeSubmission,
      'id' | 'submittedAt' | 'verified' | 'comments'
    >
  ...
  createGroup: (
    group: Omit<
      SocialGroup,
      'id' | 'members' | 'stats' | 'createdAt' | 'isActive'
    >
  ) => Promise<SocialGroup>;
  ...
  createEvent: (
    event: Omit<SocialEvent, 'id' | 'attendees' | 'status'>
  ) => Promise<SocialEvent>;
  registerForEvent: (
    eventId: string,
    userId: string,
...
export const useEnhancedSocial = (): UseEnhancedSocialReturn => {
    ...
    async (userId: string) => {
      try {
        ...
        const userReputation =
          await enhancedSocialService.getReputation(userId);
  ...
  const addReputationEvent = useCallback(
    async (
      userId: string,
      event: Omit<ReputationEvent, 'id' | 'timestamp'>
    ) => {
      try {
        ...
        setError(
          err instanceof Error ? err.message : 'Failed to add reputation event'
        );
      }
    },
  ...
  const createChallenge = useCallback(
    async (
      ...
      try {
        setError(null);
        const newChallenge =
          await enhancedSocialService.createChallenge(challenge);
  ...
  const joinChallenge = useCallback(
    async (challengeId: string, userId: string) => {
      try {
        setError(null);
        const participant = await enhancedSocialService.joinChallenge(
          challengeId,
          userId
        );
        return participant;
  ...
  const submitChallengeEntry = useCallback(
    async (
      challengeId: string,
      userId: string,
      submission: Omit<
      ...
      try {
        ...
        const challengeSubmission =
          await enhancedSocialService.submitChallengeEntry(
            challengeId,
            userId,
            submission
          );
        ...
        setError(
          err instanceof Error
            ? err.message
            : 'Failed to submit challenge entry'
        );
        throw err;
      }
    },
  ...
  const getActiveChallenges = useCallback(() => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to fetch active challenges'
      );
    } finally {
  ...
  const createEvent = useCallback(
    async (event: Omit<SocialEvent, 'id' | 'attendees' | 'status'>) => {
      try {
        setError(null);
  ...
  const registerForEvent = useCallback(
    async (
      eventId: string,
      userId: string,
      status: 'registered' | 'attending' | 'declined' | 'maybe' = 'registered'
    ) => {
      try {
        setError(null);
        const attendee = await enhancedSocialService.registerForEvent(
          eventId,
          userId,
          status
        );
  ...
  const getFeaturedEvents = useCallback(() => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to fetch featured events'
      );
    } finally {
      setLoadingEvents(false);
    }
