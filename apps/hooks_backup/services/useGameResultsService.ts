...
import {
  type GameResult,
  type GameSpecificReward,
} from '../../../../packages/types/src/types/game'; // Assuming game types are here
...
const useGameResults =
  (/* initialGameId?: string, initialUserId?: string */): UseGameResultsServiceReturn => {
    const [gameResultsData, setGameResultsData] = useState<GameResultsData>({
      gameResult: null,
      gameSpecificRewards: [],
    });
      ...
      async (gameId: string, userId?: string) => {
        ...
        try {
          ...
          const mockRewards: GameSpecificReward[] = [
            {
              id: `game-reward-${Math.random().toString(36).substring(7)}`,
              name: 'Participation Bonus',
              description: '+10 DOJO',
            },
            {
              id: `game-reward-${Math.random().toString(36).substring(7)}`,
              name: 'Break Master Bonus',
              description: 'Awarded for a clean break',
            },
          ];

          setGameResultsData({
            gameResult: mockGameResult,
            gameSpecificRewards: mockRewards,
          });
        } catch (err: any) {
