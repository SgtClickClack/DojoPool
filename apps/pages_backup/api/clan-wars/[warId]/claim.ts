...
export default async function handler(
  ...
  try {
    ...
    if (clanWar.clan1Score > clanWar.clan2Score) {
      ...
      return res
        .status(400)
        .json({ error: 'War ended in a tie, no territory can be claimed' });
    }
    ...
    if (winningClan.leaderId !== userId) {
      return res
        .status(403)
        .json({ error: 'Only the winning clan leader can claim territory' });
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const losingClanId =
        winningClanId === clanWar.clan1Id ? clanWar.clan2Id : clanWar.clan1Id;
      await tx.clan.update({
