    ...
    it('should load game page efficiently', () => {
      cy.visit(`/games/${games.activeGame.id}`, {
        ...
        onLoad: (win) => {
          ...
          win.performance.measure('game-load', 'start-game-load', 'end-game-load');
        }
      });
    ...
    it('should efficiently load and cache images', () => {
      ...
      cy.get('img').should(($imgs) => {
        ...
        cy.window().then((win) => {
          ...
          win.performance.measure('image-load', 'start-image-load', 'end-image-load');
  ...
  describe('Database Performance', () => {
    it('should handle large game history efficiently', () => {
      ...
      cy.window().then((win) => {
        ...
        cy.findByTestId('game-history-list').within(() => {
          cy.findByRole('listitem').should('have.length.at.least', 100);
        }).then(() => {
          win.performance.mark('end-render');
          win.performance.measure('history-render', 'start-render', 'end-render');
          const measure = win.performance.getEntriesByName('history-render')[0];
