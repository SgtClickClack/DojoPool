describe('Achievement Tracking and Progression', () => {
  ...
  describe('Achievement Unlocks', () => {
    ...
    it('should handle multiple achievements in one session', () => {
      cy.createGame(users.validUser.id, users.existingUser.id).then((gameId) => {
        ...
        const shots = Array(8).fill(null).map((_, i) => ({
          ball: i + 1,
          type: 'shot',
          bankShot: true,
          success: true
        ...
        shots.forEach(shot => {
          cy.window().then((win) => {
            win.dispatchEvent(new CustomEvent('gameAction', {
              detail: shot
            }));
          });
        });
      ...
      venues.forEach((venueId, index) => {
        ...
        cy.findByTestId('venue-master-progress').within(() => {
          ...
          cy.findByRole('progressbar')
            .should('have.attr', 'aria-valuenow', ((index + 1) / 5 * 100).toString());
        });
      });
    });
  ...
  describe('Achievement Rewards', () => {
    ...
    it('should handle rank progression', () => {
      ...
      cy.findByTestId('player-rank').then(($rank) => {
        ...
        cy.window().then((win) => {
          win.dispatchEvent(new CustomEvent('achievementUnlock', {
            detail: {
              id: 'perfect_game',
              points: 1000
            }
        ...
        cy.findByTestId('player-rank')
          .should('not.have.text', initialRank);
  ...
  describe('Seasonal Achievements', () => {
    it('should track seasonal progress correctly', () => {
      ...
      cy.findByTestId('seasonal-progress')
        .should('contain', 'Progress: 1/10');
      ...
      cy.findByText('Winter Champion 2024')
  ...
  describe('Social Features', () => {
    ...
    it('should show achievement notifications to friends', () => {
      ...
      cy.window().then((win) => {
        win.dispatchEvent(new CustomEvent('friendAchievement', {
          detail: {
