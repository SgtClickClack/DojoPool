      ...
      cy.createGame(users.validUser.id, users.existingUser.id).then((gameId) => {
        ...
        cy.findByTestId('shot-history').within(() => {
          shots.forEach((ball, index) => {
            cy.findByRole('listitem').eq(index)
  ...
  describe('State Synchronization', () => {
    it('should resolve conflicting shots', () => {
      cy.createGame(users.validUser.id, users.existingUser.id).then((gameId) => {
        ...
        cy.findByTestId('game-state').should('contain', users.existingUser.username);
      });
    });
    ...
    it('should handle rapid shot sequences', () => {
      cy.createGame(users.validUser.id, users.existingUser.id).then((gameId) => {
        ...
        cy.window().then((win) => {
          ...
          win.performance.measure('shot-sequence', 'start-shots', 'end-shots');
          const measure = win.performance.getEntriesByName('shot-sequence')[0];
