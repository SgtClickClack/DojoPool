...
class ProductionDeployment {
    ...
    async performPreDeploymentChecks() {
        ...
        if (this.compareVersions(nodeVersion, requiredVersion) < 0) {
            this.errors.push(`Node.js version ${requiredVersion} or higher required. Current: ${nodeVersion}`);
        } else {
    ...
    displayDeploymentSummary() {
        ...
        console.log(`   Health: http://localhost:${this.config.server.port}/api/health`);
        console.log(`   Metrics: http://localhost:${this.config.server.port}/api/metrics`);
        console.log(`   Game Status: http://localhost:${this.config.server.port}/api/game-status`);
        console.log(`   Features: http://localhost:${this.config.server.port}/api/features`);
