  ...
  try {
    ...
    const territory = await prisma.territory.create({
      data: {
        ...
        coordinates: JSON.stringify([{ lat: 40.7128, lng: -74.006 }]),
    ...
    console.log(
      `🏰 Winning Clan: ${clan1.name} (${clan1.tag}) - Score: ${clanWar.clan1Score}`
    );
    console.log(
      `🏰 Losing Clan: ${clan2.name} (${clan2.tag}) - Score: ${clanWar.clan2Score}`
    );
    console.log(`🏛️ Contested Territory: ${territory.name}`);
    console.log(`⚔️ Clan War: ${clanWar.name} (ID: ${clanWar.id})`);
    console.log(
      `📅 War Status: ${clanWar.status} (ended ${pastDate.toLocaleString()})`
    );
