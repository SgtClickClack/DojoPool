...
const DYNAMIC_CACHE: any = 'dojopool-dynamic-v1';

// Assets to cache on install
const STATIC_ASSETS: any = [
  '/',
  '/index.html',
  '/manifest.json',
  '/favicon.ico',
...
const API_ROUTES: any = ['/api/tournaments', '/api/venues', '/api/leaderboard', '/api/user/profile'];
      ...
      return fetch(request)
        .then((response) => {
          ...
          if (response.ok && response.type === 'basic') {
            const responseToCache: any = response.clone();
...
async function handleApiRequest(request: any): any {
  try {
    const response: any = await fetch(request);
    if (response.ok) {
      const cache: any = await caches.open(DYNAMIC_CACHE);
...
async function syncGameData(): any {
  const cache: any = await caches.open(DYNAMIC_CACHE);
  const requests: any = await cache.keys();
...
async function updateLeaderboard(): any {
  try {
    const response: any = await fetch('/api/leaderboard');
    if (response.ok) {
      const cache: any = await caches.open(DYNAMIC_CACHE);
