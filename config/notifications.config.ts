export const notificationConfig = {
  email: {
    ...
    recipients: (process.env.EMAIL_NOTIFICATION_RECIPIENTS || '').split(',').filter(Boolean),
    smtpConfig: {
  ...
  alertThresholds: {
    ...
    consecutiveFailures: parseInt(process.env.CONSECUTIVE_FAILURES_THRESHOLD || '3', 10),
    minTimeBetweenAlerts: parseInt(process.env.MIN_TIME_BETWEEN_ALERTS || '3600000', 10), // 1 hour in milliseconds
  },
  batchingConfig: {
