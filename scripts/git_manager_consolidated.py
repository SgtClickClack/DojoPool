...
def generate_ssh_key():
    ...
    try:
        ...
        if agent_output.returncode == 0:
            # Extract SSH_AUTH_SOCK and SSH_AGENT_PID from output
            for line in agent_output.stdout.splitlines():
                if 'SSH_AUTH_SOCK' in line or 'SSH_AGENT_PID' in line:
                    var, value = line.split(';')[0].split('=')
                    os.environ[var] = value

            subprocess.run(['ssh-add', str(key_file)], check=True)
            logger.info("SSH key added to agent")
        ...
        logger.error(f"Error setting up SSH key: {str(e)}", exc_info=True)
        return False

def configure_git_repository():
    """Configure Git settings, authentication, and clean up repository issues."""
    try:
        ...
        if not Path('.git').exists():
            ...
            if run_git_command("git init") is None:
                return False
...
"""
        ...
        logger.error(f"Error during git setup: {str(e)}", exc_info=True)
        return False

if __name__ == "__main__":
    success = configure_git_repository()
