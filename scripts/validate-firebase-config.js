...
function validateKeys(env) {
  ...
  for (const key of REQUIRED_KEYS) {
    if (!env[key]) {
      ...
      console.error(`Typo detected in ${key}: 'firebasestorage.app' should be 'appspot.com'`);
      valid = false;
...
function fixEnv(env) {
  ...
  for (const key of REQUIRED_KEYS) {
    if (env[key] && /firebasestorage\.app/i.test(env[key])) {
      ...
      changed = true;
      console.log(`Fixed typo in ${key}: replaced 'firebasestorage.app' with 'appspot.com'`);
    }
...
async function fetchFirebaseConfig() {
  const credPath = process.env.FIREBASE_SERVICE_ACCOUNT_KEY || 'firebase-credentials.json';
  if (!fs.existsSync(credPath)) {
    console.error('No Firebase service account key found. Set FIREBASE_SERVICE_ACCOUNT_KEY or place firebase-credentials.json in project root.');
    process.exit(1);
  }
  ...
  try {
    const result = execSync(`gcloud firebase web apps list --project ${projectId} --format=json`, { encoding: 'utf8' });
    const apps = JSON.parse(result);
    if (!apps.length) throw new Error('No web apps found for this project.');
    const appId = apps[0].appId;
    const configResult = execSync(`gcloud firebase web apps describe ${appId} --project ${projectId} --format=json`, { encoding: 'utf8' });
...
async function main() {
  ...
  if (args.includes('--fix')) {
    ...
    if (fixed) {
      const confirmed = await promptConfirm('Apply fixes and overwrite .env.local?');
      if (confirmed) {
        writeEnv(fixed);
  ...
  if (args.includes('--fetch')) {
    try {
      ...
      const confirmed = await promptConfirm('Overwrite .env.local with config from Firebase Console?');
      if (confirmed) {
