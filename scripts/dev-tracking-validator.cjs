#!/usr/bin/env node
/**
 * Development Tracking Validator (CommonJS)
  ...
  if (args.length === 0) {
    ...
    switch (command) {
      ...
      case 'template':
        if (args.length < 3) {
          console.log(
            'Usage: node dev-tracking-validator.cjs template "Feature Name" "Description"'
          );
          process.exit(1);
        }
      ...
      case 'check':
        if (args.length < 2) {
          console.log(
            'Usage: node dev-tracking-validator.cjs check <file-path>'
          );
          process.exit(1);
        }
