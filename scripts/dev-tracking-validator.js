...
class DevTrackingValidator {
    constructor(projectRoot = '.') {
        ...
        this.correctTrackingDir = path.join(this.projectRoot, 'docs', 'planning', 'tracking');
        this.legacyFiles = [
    ...
    getCorrectTrackingFiles() {
        ...
        if (fs.existsSync(this.correctTrackingDir)) {
            ...
            items.forEach(item => {
                if (item.endsWith('.md') && (item.startsWith('part-') || item === 'index.md')) {
    ...
    validateTrackingStructure() {
        ...
        const partFiles = Object.keys(result.correctFiles).filter(name => name.startsWith('part-'));
        ...
        if (result.legacyFiles.length > 0) {
            result.warnings.push(`Found ${result.legacyFiles.length} legacy files that should NOT be updated`);
        }
...
if (require.main === module) {
    ...
    if (args.length === 0) {
        ...
        switch (command) {
            ...
            case 'template':
                if (args.length < 3) {
                    console.log('Usage: node dev-tracking-validator.js template "Feature Name" "Description"');
                    process.exit(1);
                }
            ...
            case 'check':
                if (args.length < 2) {
                    console.log('Usage: node dev-tracking-validator.js check <file-path>');
                    process.exit(1);
                }
                validator.validateUpdate(args[1]);
                break;
            default:
                console.log('Unknown command. Use: status, template, next, validate, or check');
                process.exit(1);
        }
    }
}
