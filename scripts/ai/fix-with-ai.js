...
async function main() {
  ...
  if (!filePath) {
    console.error(
      'Usage: node scripts/ai/fix-with-ai.js --file <path> [--error "..."] [--apply]'
    );
    process.exit(2);
  }
  ...
  const instruction = [
    ...
    errorText
      ? 'Error/context:\n' + errorText
      : '(no explicit error context provided)',
  ].join('\n');
  ...
  if (apply) {
    ...
    console.log(
      `Applied AI changes to ${filePath}. Backup saved at ${path.relative(process.cwd(), backupPath)}.`
    );
  } else {
