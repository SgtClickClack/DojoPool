...
async function main() {
  ...
  if (!filePath) {
    console.error(
      'Usage: node scripts/ai/refactor.js --file <path> [--mode simplify|optimize|readability] [--apply]'
    );
    process.exit(2);
  }
  ...
  const goals = {
    simplify:
      'Refactor to simplify control flow, reduce nesting, and extract clear helpers. No behavior changes.',
    optimize:
      'Optimize for performance and memory without altering behavior. Prefer O(n) over O(n^2), efficient data structures, and early-exit guards.',
    readability:
      'Improve readability: clearer names, small functions, remove dead code, and consistent style. No behavior changes.',
  };
  ...
  if (apply) {
    ...
    console.log(
      `Applied refactor to ${filePath}. Backup at ${path.relative(process.cwd(), backupPath)}.`
    );
  } else {
