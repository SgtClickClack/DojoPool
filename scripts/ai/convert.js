...
import { parseArgs, proposeFullFileRewrite, writeText, safeBasename, ensureDir, timestampSlug } from './_util.js';
...
async function main() {
  ...
  if (!filePath || !to) {
    console.error('Usage: node scripts/ai/convert.js --file <path> [--from ts|js|py|java] --to py|ts|js|java');
    process.exit(2);
  }
  ...
  const outDir = path.resolve(process.cwd(), 'reports', 'ai-conversions', timestampSlug());
  await ensureDir(outDir);
  const baseNoExt = safeBasename(filePath).replace(/\.[^.]+$/, '');
  const extMap = { js: '.js', ts: '.ts', py: '.py', java: '.java', jsx: '.jsx', tsx: '.tsx' };
  const outExt = extMap[to] || '.txt';
  const outPath = path.join(outDir, `${baseNoExt}${outExt}`);
  await writeText(outPath, updated);
  console.log('Converted file written to ' + path.relative(process.cwd(), outPath));
}
