...
export function ensureApiKey() {
  ...
  if (!apiKey) {
    console.error('Missing OPENAI_API_KEY env var. Set it before running this command.');
    process.exit(1);
  }
  return apiKey;
}
...
export function getGitDiff({ staged = false } = {}) {
  const cmd = staged ? 'diff --staged --patch --minimal' : 'diff --patch --minimal';
  return runGit(cmd) || '';
}
...
export async function chatComplete({ system, user, model = getModel(), temperature = 0.2, maxTokens = 2048 }) {
...
export async function proposeFullFileRewrite({ filePath, code, instruction, model = getModel(), temperature = 0.1, maxTokens = 4096 }) {
  const system = [
...
export async function explainText({ title = 'Issue', text, model = getModel(), style = 'plain' }) {
  const system = [
  ...
  return chatComplete({ system, user, model, temperature: 0.2, maxTokens: 2048 });
}

export async function unifiedDiff(oldPath, newPath) {
  try {
    const out = execSync(`git --no-pager diff --no-index --minimal -- ${oldPath} ${newPath}`, {
      encoding: 'utf8',
