...
async function main() {
  ...
  let log = runGit(`log --pretty=format:%h\\ %s ${baseRef}..HEAD`) || runGit('log -5 --pretty=format:%h %s');
  let stat = runGit(`diff --stat ${baseRef}..HEAD`) || runGit('diff --stat HEAD~5..HEAD') || getGitDiff();
  ...
  const desc = await chatComplete({ system, user, maxTokens: 1024, temperature: 0.2 });
