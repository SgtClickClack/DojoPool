...
const { PerformanceNotificationService } = require('../src/services/notifications/PerformanceNotificationService');
...
class PerformanceChecker {
  constructor(config) {
    ...
    this.notificationService = new PerformanceNotificationService(notificationConfig, this.alertHistoryService);
  ...
  async shouldSendAlert(metric, type) {
    ...
    if (timeSinceLastAlert < notificationConfig.alertThresholds.minTimeBetweenAlerts) {
      return false;
    }
  ...
  async detectRegressions(currentResults, history) {
    ...
    const { maxDegradation, significantChange } = this.config.regressionThresholds;
  ...
  async generateReport(results, budgetResults, regressions) {
    ...
    try {
      const analytics = await this.alertHistoryService.getAlertAnalytics(startDate, endDate);
  ...
  async check(results) {
    ...
    const report = await this.generateReport(results, budgetResults, regressions);

    const hasErrors = budgetResults.violations.length > 0 || 
                     regressions.filter(r => r.severity === 'error').length > 0;

    if (hasErrors) {
      console.error('❌ Performance budget violations or regressions detected');
      process.exit(1);
    } else if (budgetResults.warnings.length > 0 || 
...
// Execute checks
const results = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'performance-results/latest.json'), 'utf8'));
