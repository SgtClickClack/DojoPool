...
export const taskList: Task[] = [
  // --- PHASE 1: TOURNAMENT SYSTEM COMPLETION ---
  {
    ...
    prompt: "Enhance the existing `TournamentRegistration.tsx` and create the new components `TournamentDiscovery.tsx` and `TournamentPayment.tsx`. Also, create a new stylesheet `src/styles/tournament.scss`. The goal is a complete tournament registration flow with the following features:\n1. A discovery page with filters for venue, date, and format.\n2. A registration workflow that integrates with a user's wallet.\n3. An entry fee payment system using 'Dojo Coins'.\n4. Real-time registration status updates.\n5. Apply cyberpunk styling with neon effects.",
    relevantFiles: [
  ...
  },
  {
    ...
    prompt: "Enhance the existing `TournamentBracket.tsx` and create a new `BracketVisualization.tsx` component. The objective is to build an interactive bracket visualization that features real-time match updates and tracks player progression. Style the bracket with a cyberpunk grid aesthetic.",
    relevantFiles: [
  ...
  },
  {
    ...
    prompt: "Enhance `GamePlay.tsx` and `LiveGameDisplay.tsx`, and create a new `GameAnalytics.tsx` component. Integrate the AI ball tracking system, enable real-time score updates, implement a live commentary system, and develop a post-game analytics display.",
    relevantFiles: [
  ...
  // --- PHASE 4: CSS STYLING COMPLETION ---
  {
    ...
    prompt: "Create new SCSS partials for tournament, venue, and gameflow components, and enhance the existing game styles. Apply a consistent cyberpunk theme across the entire application, featuring neon effects, animations, a dark theme with accent colors, and ensure a responsive design for all screen sizes.",
    relevantFiles: [
  ...
  },
  {
    ...
    prompt: "Create new SCSS files dedicated to animations and effects: `_cyberpunk.scss`, `_neon.scss`, and `_glow.scss`. Implement smooth transitions between game states, neon text and border effects, loading animations, and engaging hover effects and interactions.",
    relevantFiles: [
  ...
  // --- PHASE 5: INTEGRATION & TESTING ---
  {
    ...
    prompt: "Integrate all newly created components. Update `App.tsx` with the complete routing setup. Update `Dashboard.tsx` to integrate the new components. Enhance `useAuth.ts` to support the full game flow. Implement component state management and error boundaries.",
    relevantFiles: [
      "src/App.tsx",
  ...
  },
  {
    ...
    prompt: "Create a comprehensive test suite. Write end-to-end (E2E) tests using Cypress for the tournament, check-in, and game flows. Implement component unit tests and integration tests for API calls in the `src/components/__tests__/` directory, aiming for over 80% test coverage. Conduct performance testing.",
    relevantFiles: [
  ...
  // --- PHASE 6: MOBILE OPTIMIZATION ---
  {
    ...
    prompt: "Implement a mobile-first responsive design. Create new SCSS files for mobile and tablet breakpoints. Update all relevant component CSS files to be fully responsive, ensuring touch-friendly interactions, optimized layouts for small screens, and high performance on mobile devices.",
    relevantFiles: [
  ...
  // --- PHASE 7: DEPLOYMENT PREPARATION ---
  {
    ...
    prompt: "Prepare the application for production. Optimize the build configuration in `vite.config.ts`. Review production dependencies in `package.json`. Implement code splitting, lazy loading, and asset optimization. Set up performance monitoring and error tracking integrations via environment configuration files.",
    relevantFiles: [
      "vite.config.ts",
      "package.json",
      ".env.production"
  ...
  },
  {
    ...
    prompt: "Create comprehensive documentation for the project. Write a `GAME_FLOW_IMPLEMENTATION.md` and a `CSS_STYLING_GUIDE.md` in the `docs/` directory. The documentation should include component usage examples, styling guidelines, and clear deployment instructions.",
    relevantFiles: [
