...
def test_reward_and_transfer_flow(db, users, wallets, wallet_service):
    ...
    assert transfer.metadata["recipient_user_id"] == users[2].id

    # Check final balances
    wallet0 = wallet_service.get_user_wallet(users[0].id)
    wallet1 = wallet_service.get_user_wallet(users[1].id)
    wallet2 = wallet_service.get_user_wallet(users[2].id)
...
def test_transaction_history_and_stats(db, users, wallets, wallet_service):
    ...
    wallet_service.award_coins(users[0].id, RewardType.MATCH_WIN)
    wallet_service.award_coins(users[0].id, RewardType.TOURNAMENT_WIN)
    wallet_service.award_coins(users[0].id, RewardType.TRICK_SHOT)
    
    # Transfer some coins
    wallet_service.transfer_coins(users[0].id, users[1].id, 20.0, "Test transfer")

    # Get transaction history
    wallet = wallet_service.get_user_wallet(users[0].id)
...
def test_error_handling(db, users, wallets, wallet_service):
    ...
    with pytest.raises(WalletError) as exc:
        wallet_service.create_wallet(users[0].id)
    ...
    with pytest.raises(InsufficientFundsError) as exc:
        wallet_service.transfer_coins(users[0].id, users[1].id, 1000.0)
    assert "insufficient" in str(exc.value).lower()

    # Try to transfer to non-existent wallet
    with pytest.raises(WalletError) as exc:
        wallet_service.transfer_coins(users[0].id, 999999, 10.0)
    assert "not found" in str(exc.value)
