...
def test_get_rankings_in_range() -> None:
    # Setup global cache with dummy data
    gr._game_cache = {
        "player1": {"player_id": "player1", "score": 50, "last_update": datetime.utcnow()},
        "player2": {"player_id": "player2", "score": 75, "last_update": datetime.utcnow()},
        "player3": {"player_id": "player3", "score": 25, "last_update": datetime.utcnow()},
    }
    rankings = gr.get_rankings_in_range(0, 2)
    assert len(rankings) == 2

def test_get_global_ranking() -> None:
    gr._game_cache = {
        "player1": {"player_id": "player1", "score": 90, "last_update": datetime.utcnow()}
    }
    rank = gr.get_global_ranking("player1")
    assert rank == 90.0

@pytest.mark.asyncio
async def test_async_get_leading_player() -> None:
    gr._game_cache = {
        "player1": {"player_id": "player1", "score": 30, "last_update": datetime.utcnow()},
        "player2": {"player_id": "player2", "score": 60, "last_update": datetime.utcnow()},
        "player3": {"player_id": "player3", "score": 45, "last_update": datetime.utcnow()},
    }
