...
@Module({
  providers: [
    ...
    // Rate limiting configurations for different endpoints
    {
      ...
      useFactory: () => new RateLimitMiddleware({
    ...
    // Circuit breaker configurations
    {
      ...
      useFactory: () => new CircuitBreakerMiddleware({
        failureThreshold: 5,
    ...
    // Request logging configurations
    {
      ...
      useFactory: () => new RequestLoggingMiddleware({
...
export class MiddlewareModule implements NestModule {
  configure(consumer: MiddlewareConsumer) {
    ...
    consumer
      ...
      .forRoutes(
        { path: 'auth/*', method: 'POST' }, // Strict rate limiting for auth
        { path: 'api/*', method: 'ALL' },    // General API rate limiting
      );
  }
}
