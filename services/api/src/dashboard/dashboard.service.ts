...
export class DashboardService {
  ...
  async getStats(userId: string): Promise<DashboardStats> {
    try {
      const [user, wins, losses, tournamentsJoined] = await Promise.all([
        ...
        this.prisma.match.count({ where: { loserId: userId, status: 'COMPLETED' } }),
      ...
      this.logger.warn(`Falling back to zeroed dashboard stats due to error: ${err instanceof Error ? err.message : String(err)}`);
      return {
        dojoCoinBalance: 0,
  ...
  async getCdnCost(): Promise<CdnCostResponse> {
    ...
    const breakdown: CdnCostBreakdownDay[] = Array.from({ length: 30 }, (_, i) => {
      ...
      const costPerGb = 0.10; // $0.10 per GB as a simple model
    ...
    const totalCostUSD = Math.round(breakdown.reduce((sum, d) => sum + d.cost, 0) * 100) / 100;
